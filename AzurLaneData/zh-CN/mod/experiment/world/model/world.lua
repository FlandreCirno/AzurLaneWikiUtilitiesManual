...
slot0.Dispose = function (slot0, slot1)
	...
	if slot1 == slot0.TypeBase then
		slot2 = {
			worldBossProxy = slot0.worldBossProxy
		}
	elseif slot1 == slot0.TypeFull then
		slot2 = {
			worldBossProxy = slot0.worldBossProxy
		}
	elseif slot1 == slot0.TypeReset then
		slot2 = {
			realm = slot0.realm,
	...
	for slot6 in pairs(slot0.InheritNameList) do
...
slot0.InheritReset = function (slot0, slot1)
	...
	for slot5, slot6 in pairs(slot0.InheritNameList) do
...
slot0.NewAtlas = function (slot0, slot1)
...
slot0.OnSwitchMap = function (slot0)
	...
	slot0:DispatchEvent(slot0.EventSwitchMap)
...
slot0.UpdateProgress = function (slot0, slot1)
	if slot0.progress < slot1 then
		...
		slot0.atlas:UpdateProgress(slot2, slot1)
		slot0:DispatchEvent(slot0.EventUpdateProgress)
	end
end
...
slot0.UpdateSubmarineSupport = function (slot0, slot1)
	...
	slot0:DispatchEvent(slot0.EventUpdateSubmarineSupport)
end
...
slot0.GetFleet = function (slot0, slot1)
	...
	end)) or slot0.GetActiveMap(slot0):GetFleet()
end
...
slot0.GetShips = function (slot0)
	...
	return table.mergeArray(slot1, slot0:GetPortShips())
end
...
slot0.GetShipVOs = function (slot0)
	if slot0.type == World.TypeBase then
		...
		return _.map(slot0.GetShips(slot0), function (slot0)
...
slot0.SetPortShips = function (slot0, slot1)
	slot0.portShips = slot1

	pg.ShipFlagMgr.GetInstance():UpdateFlagShips("inWorld")
end

slot0.GetPortShips = function (slot0)
	return slot0.portShips
end

slot0.GetPortShipVOs = function (slot0)
	return _.map(slot0:GetPortShips(), function (slot0)
		return WorldConst.FetchShipVO(slot0.id)
	end)
end

slot0.AddPortShip = function (slot0, slot1)
	slot1.fleetId = nil

	table.insert(slot0.portShips, slot1)
	pg.ShipFlagMgr.GetInstance():UpdateFlagShips("inWorld")
	slot0:DispatchEvent(slot0.EventAddPortShip, slot1)
end

slot0.RemovePortShip = function (slot0, slot1)
	for slot5 = #slot0.portShips, 1, -1 do
		if slot0.portShips[slot5].id == slot1 then
			pg.ShipFlagMgr.GetInstance():UpdateFlagShips("inWorld")
			slot0:DispatchEvent(slot0.EventRemovePortShip, table.remove(slot0.portShips, slot5))

			return table.remove(slot0.portShips, slot5)
		end
	end
end
...
slot0.GetWorldPower = function (slot0)
	underscore.each(slot0.fleets, function (slot0)
		slot0 = slot0 + slot0:GetGearScoreSum()
	end)

	return math.floor(0 * (1 + slot0.GetWorldMapBuffAverageLevel(slot0) / pg.gameset.world_strength_correct.key_value))
end

slot0.GetWorldRank = function (slot0)
	slot1 = 0
	slot2 = underscore.map(slot0:GetNormalFleets(), function (slot0)
...
slot0.VerifyFormation = function (slot0)
	...
	_.each(slot0:GetShips(), function (slot0)
		slot0[slot0.id] = (slot0[slot0.id] or 0) + 1
	end)
end
...
slot0.CheckFleetMovable = function (slot0)
	return slot0:GetRound() == WorldConst.RoundPlayer and slot1:CheckFleetMovable(slot0:GetActiveMap().GetFleet(slot1)) and not slot1:CheckInteractive()
end
...
slot0.GetAchievements = function (slot0, slot1)
	_.each(slot1.config.normal_target, function (slot0)
		table.insert(slot0, slot1:GetAchievement(slot0))
	end)
...
slot0.AnyUnachievedAchievement = function (slot0, slot1)
	...
	if _.detect(slot1:GetAchievementAwards(), function (slot0)
		return not slot0[slot0.star]
	end) then
		slot4, slot5 = slot0.CountAchievements(slot0, slot1)
...
slot0.BuildFormationIds = function (slot0)
	...
	for slot7, slot8 in slot4(slot5) do
		if slot3[slot8:GetFleetType()] <= slot2[slot8.GetFleetType()] then
	...
	if slot4 then
		for slot9 = #slot1[FleetType.Normal] + 1, slot4, 1 do
			slot1[FleetType.Normal][slot9] = slot0()
		end
	end
...
slot0.GetNowEntrance = function (slot0)
	return slot0.nowEntrance
end

slot0.SetNowEntrance = function (slot0, slot1)
...
slot0.UpdateWorldShopGoods = function (slot0, slot1)
	_.each(slot1, function (slot0)
		slot0.goodDic[slot0.goods_id] = slot0.goodDic[slot0.goods_id] + slot0.count
	end)
	slot0.DispatchEvent(slot0, slot0.EventUpdateShopGoods, slot0.goodDic)
end
...
slot0.InitWorldColorDictionary = function (slot0)
	...
	_.each(pg.world_chapter_colormask.all, function (slot0)
		slot0.colorDic[Color.New(pg.world_chapter_colormask[slot0].color[1] / 255, pg.world_chapter_colormask[slot0].color[2] / 255, pg.world_chapter_colormask[slot0].color[3] / 255).ToHex(slot2)] = pg.world_chapter_colormask[slot0].id
	end)
end
...
slot0.AddGlobalBuff = function (slot0, slot1, slot2)
	...
	slot0:DispatchEvent(slot0.EventUpdateGlobalBuff)
end
...
slot0.GetWorldMapBuffLevel = function (slot0)
	return _.map(pg.gameset.world_mapbuff_list.description, function (slot0)
		return slot0:GetGlobalBuff(slot0).floor
	end)
end

slot0.GetWorldMapBuffAverageLevel = function (slot0)
	underscore.each(slot0:GetWorldMapBuffLevel(), function (slot0)
		slot0 = slot0 + slot0
	end)

	return 0 / #slot0.GetWorldMapBuffLevel()
end

slot0.GetWorldMapBuffs = function (slot0)
	return _.map(pg.gameset.world_mapbuff_list.description, function (slot0)
		return slot0:GetGlobalBuff(slot0)
	end)
end
	...
	elseif slot1 == WorldConst.OpReqSub then
		if slot0:GetSubmarineFleet() then
			underscore.each(slot3:GetShips(true), function (slot0)
				slot0 = slot0 + slot0:GetImportWorldShipVO():GetStaminaDiscount(WorldConst.OpReqSub)
			end)
		end

		return World.CalcCDTimeCost(pg.gameset.world_instruction_submarine.description, slot0.GetReqCDTime(slot0, WorldConst.OpReqSub), slot2)
...
slot0.CheckCommanderInFleet = function (slot0, slot1)
	if slot0.type == World.TypeBase then
		...
		for slot5, slot6 in ipairs(slot0.fleets) do
			if slot6.HasCommander(slot6, slot1) then
				return true
			end
		end
...
slot0.IsMapVisioned = function (slot0, slot1)
	if slot0:GetActiveMap().id == slot1 and slot0.ReplacementMapType(slot0:GetActiveEntrance(), slot2) == "base_chapter" and slot2.isPressing then
		return true
	end
