...
slot0.GetConfig = function (slot0, slot1)
	return (pg.world_chapter_random[slot0.id] and slot2[slot1]) or (pg.world_chapter_template[slot0.gid] and pg.world_chapter_template[slot0.gid][slot1]) or nil
end
...
slot0.UpdateGridId = function (slot0, slot1)
	...
	for slot5, slot6 in ipairs(slot0.config.world_chapter_buff) do
		slot12, slot13, slot14 = unpack(slot6)
...
slot0.SetupGrid = function (slot0, slot1)
	_.each(slot0.config.grids, function (slot0)
		WPool:Get(WorldMapCell).Setup(slot1, slot0)

		if slot0:AlwaysInFOV() then
			slot1.infov = bit.bor(slot1.infov, WorldConst.FOVMapSight)
		end

		slot0.cells[WorldMapCell.GetName(slot1.row, slot1.column)] = slot1

		if not slot1 then
			slot1:AddListener(WorldMapCell.EventAddAttachment, slot0.onUpdateAttachmentExist)
			slot1:AddListener(WorldMapCell.EventRemoveAttachment, slot0.onUpdateAttachmentExist)
		end
	end)
	...
	for slot5 = 0, WorldConst.MaxRow, 1 do
		...
		for slot11 = 0, WorldConst.MaxColumn, 1 do
			if slot0.GetCell(slot0, slot5, slot11) then
				if not slot6 then
					slot6 = slot11
		...
		if slot7 then
			slot0:GetCell(slot5, slot7).dir = bit.bor(slot0.GetCell(slot5, slot7).dir, bit.lshift(1, WorldConst.DirRight))
		end
	...
	for slot5 = 0, WorldConst.MaxColumn, 1 do
		slot6, slot7 = nil

		for slot11 = 0, WorldConst.MaxRow, 1 do
		...
		if slot7 then
			slot0:GetCell(slot7, slot5).dir = bit.bor(slot0.GetCell(slot7, slot5).dir, bit.lshift(1, WorldConst.DirDown))
		end
...
slot0.SetupPort = function (slot0)
	if #slot0.config.port_id > 0 then
		WPool:Get(WorldMapPort).Setup(slot1, slot0.config.port_id[1])

		slot2, slot3 = unpack(slot0.config.port_id[2])

		for slot7 = slot2 - 1, slot2 + 1, 1 do
			for slot11 = slot3 - 1, slot3 + 1, 1 do
		...
		for slot5, slot6 in ipairs(slot0:GetNormalFleets()) do
			...
			if slot0:GetFleetTerrain(slot6) ~= WorldMapCell.TerrainFog then
				WorldConst.RangeCheck(slot6, slot0:GetFOVRange(slot6), function (slot0, slot1)
					if slot0.cells[WorldMapCell.GetName(slot0, slot1)] then
			...
			elseif slot0.findex == slot5 then
				...
				slot8 = slot0.IsFleetTerrainSairenFog(slot0, slot6)
...
slot0.UpdateFleetIndex = function (slot0, slot1)
	if slot0.findex ~= slot1 then
		...
		slot0.DispatchEvent(slot0, slot0.EventUpdateFIndex)
	end
end
...
slot0.UpdateActive = function (slot0, slot1)
	...
	if slot0.active ~= slot1 then
		...
		slot0:DispatchEvent(slot0.EventUpdateActive)
	end
end
...
slot0.InPort = function (slot0, slot1, slot2)
	...
	if slot0:GetFleet(slot1).GetFleetType(slot4) == FleetType.Submarine then
		return slot3.id
...
slot0.CalcTransportPos = function (slot0, slot1, slot2)
	...
	for slot12 = slot0.left + 1, slot0.right - 1, 1 do
	...
	for slot12 = slot0.top + 1, slot0.bottom - 1, 1 do
...
slot0.AnyFleetInEdge = function (slot0)
	return slot0.active and _.any(slot0:GetNormalFleets(), function (slot0)
		return slot0.row == slot0.top or slot0.row == slot0.bottom or slot0.column == slot0.left or slot0.column == slot0.right
	end)
end
...
slot0.CheckInteractive = function (slot0, slot1)
	...
	if slot0:GetCell(slot1 or slot0:GetFleet().row, slot1 or slot0.GetFleet().column).discovered then
...
slot0.GetFOVRange = function (slot0, slot1, slot2, slot3)
	return (slot0:GetCell(slot2 or slot1.row, slot3 or slot1.column).GetTerrain(slot4) == WorldMapCell.TerrainFog and slot4.terrainStrong) or slot1:GetFOVRange()
end
...
slot0.CalcFleetSpeed = function (slot0, slot1)
	...
	if slot0:GetCell(slot1.row, slot1.column).GetTerrain(slot3) == WorldMapCell.TerrainFog then
...
slot0.FindPath = function (slot0, slot1, slot2, slot3)
	if not slot0.pathFinder then
		slot0.pathFinder = PathFinding.New({}, WorldConst.MaxRow, WorldConst.MaxColumn)
	end

	slot5 = {}

	for slot9 = 0, WorldConst.MaxRow - 1, 1 do
		if not slot5[slot9] then
			slot5[slot9] = {}
		end

		for slot13 = 0, WorldConst.MaxColumn - 1, 1 do
...
slot0.FindAIPath = function (slot0, slot1, slot2)
	...
	for slot8 = 0, WorldConst.MaxRow - 1, 1 do
		if not slot4[slot8] then
			slot4[slot8] = {}
		end

		for slot12 = 0, WorldConst.MaxColumn - 1, 1 do
...
slot0.GetMoveRange = function (slot0, slot1)
	...
	while #slot7 > 0 do
		...
		_.each({
			...
			if slot0.row >= 0 and slot0.row < WorldConst.MaxRow and slot0.column >= 0 and slot0.column < WorldConst.MaxColumn and slot0.step <=  and slot2[slot0.row][slot0.column] then
				slot2[slot0.row][slot0.column] = false

				if slot3:IsObstacle(slot0.row, slot0.column) then
	...
	slot8 = _.filter(slot6, function (slot0)
		return slot0:IsStayPoint(slot0.row, slot0.column)
	end)
...
slot0.BuildLongMoveInfos = function (slot0)
	...
	for slot5 = 0, WorldConst.MaxRow - 1, 1 do
		slot1[slot5] = slot1[slot5] or {}

		for slot9 = 0, WorldConst.MaxColumn - 1, 1 do
...
slot0.GetLongMoveRange = function (slot0, slot1)
	...
	slot0:BuildLongMoveInfos()[slot1.row][slot1.column].dp[0] = 0
	slot0.BuildLongMoveInfos()[slot1.row][slot1.column].isMark = true

	slot10(slot0.BuildLongMoveInfos()[slot1.row][slot1.column])

	while slot11 do
		_.each(slot8, function (slot0)
			if slot0(slot1.row + slot0.row, 0, WorldConst.MaxRow - 1) or slot0(slot1.column + slot0.column, 0, WorldConst.MaxColumn - 1) then
				return
			end

			if slot2[slot1.row + slot0.row][slot1.column + slot0.column] and not slot1.isFinish then
				for slot5 = 1, slot3, 1 do
					if slot1.dp[slot5 - 1] and (not slot1.dp[slot5] or slot1.dp[slot5 - 1] < slot1.dp[slot5]) then
						slot1.dp[slot5] = slot1.dp[slot5 - 1]
						slot1.last[slot5] = {
							slot1,
							slot5 - 1
						}
		...
		until table.remove(slot7, 1) and not slot11.isObstacle
	end
...
slot0.FindNearestBlankPoint = function (slot0, slot1, slot2)
	...
	for slot7 = 0, WorldConst.MaxRow - 1, 1 do
		if not slot3[slot7] then
			slot3[slot7] = {}
		end

		for slot11 = 0, WorldConst.MaxColumn - 1, 1 do
	...
	while #slot4 > 0 do
		...
		_.each({
			...
			if slot0.row >= 0 and slot0.row < WorldConst.MaxRow and slot0.column >= 0 and slot0.column < WorldConst.MaxColumn and not (_.any(_.any, function (slot0)
				return slot0.row == slot0.row and slot0.column == slot0.column
			end) or _.any(slot2, function (slot0)
				return slot0.row == slot0.row and slot0.column == slot0.column
			end)) and slot3[slot0.row][slot0.column] then
				if slot4.ExistAny(slot2, slot0.row, slot0.column) then
...
slot0.ConstructFormationData = function (slot0)
	_.each(slot1, function (slot0)
		_.each(slot0:GetShipVOs(true), function (slot0)
			table.insert(slot0, slot0)
		end)
	end)
	_.each(nowWorld.GetPortShipVOs(slot4), function (slot0)
		table.insert(slot0, slot0)
	end)

	return {
		fleets = _.map(slot0:GetPortFleets(), function (slot0)
			return slot0:Clone()
		end),
		shipVOs = {}
	}
...
slot0.WriteBack = function (slot0, slot1, slot2)
	...
	for slot8, slot9 in ipairs(slot0:GetFleet().GetShips(slot3, true)) do
	...
	slot6 = slot0.GetCell(slot0, slot3.row, slot3.column):GetStageEnemy()
	...
	if slot1 then
		...
		_.each(slot0:GetFleets(), function (slot0)
			slot0 or slot0:HasDamageLevel():ClearDamageLevel()
		end)

		if false then
			table.insert(slot0.phaseDisplayList, 1, {
				story = "W1500",
				hp = slot6.GetMaxHP(slot6)
			})
		end
	else
		slot0.isLoss = true
	...
	_.each(slot2.hpDropInfo, function (slot0)
		slot1 = #slot0.phaseDisplayList + 1

		for slot5, slot6 in ipairs(slot0.phaseDisplayList) do
...
slot0.GetMapMinMax = function (slot0)
	...
	for slot6 = 0, WorldConst.MaxRow - 1, 1 do
		for slot10 = 0, WorldConst.MaxColumn - 1, 1 do
...
slot0.EventEffectOpenFOV = function (slot0, slot1)
	...
	_.each(slot0:FindAttachments(WorldMapAttachment.TypeEvent, slot2), function (slot0)
		...
		for slot4 = math.max(slot0.row - slot1, 0), math.min(slot0.row + slot1, WorldConst.MaxRow - 1), 1 do
			for slot8 = math.max(slot0.column - slot1, 0), math.min(slot0.column + slot1, WorldConst.MaxColumn - 1), 1 do
				if WorldConst.InFOVRange(slot0.row, slot0.column, slot4, slot8, slot1) and slot0:GetCell(slot4, slot8) then
					if slot2 then
...
slot0.OrderAROpenFOV = function (slot0, slot1)
	if slot1 then
		slot0.centerCellFOV = {
			row = slot0:GetFleet().row,
			column = slot0.GetFleet().column
		}
	end
...
slot0.CheckFleetSalvage = function (slot0, slot1)
	if underscore.detect(slot0:GetFleets(), function (slot0)
		return slot0:IsCatSalvage() and (slot0 or slot0:IsSalvageFinish() or slot1.salvageAutoResult or slot0.catSalvageFrom ~= slot1.id)
	end) then
		return slot2.id
	else
...
slot0.getFleetBattleBuffs = function (slot0, slot1, slot2)
	...
	if slot2 and slot1.IsCatSalvage(slot1) then
	else
...
slot0.triggerCheck = function (slot0, slot1, slot2, slot3)
	...
	elseif slot4 == FleetSkill.TriggerDDHead then
		return #slot1.GetTeamShipVOs(slot1, TeamType.Vanguard, false) > 0 and slot5[1]:getShipType() == ShipType.QuZhu
	...
	elseif slot4 == FleetSkill.TriggerAroundEnemy then
		...
		for slot11 = -slot3[2], slot7, 1 do
			for slot16 = -(slot7 - math.abs(slot11)), slot7 - math.abs(slot11), 1 do
				table.insert(slot6, slot0:GetCell(slot5.row + slot11, slot5.column + slot16))
			end
		end

		return underscore.any(slot6, function (slot0)
			return (type(slot0[3]) == "number" and slot0[3] == ((slot0:ExistEnemy() and slot0:GetStageEnemy().config.type) or nil)) or (type(slot0[3]) == "table" and table.contains(slot0[3], (slot0.ExistEnemy() and slot0.GetStageEnemy().config.type) or nil))
		end)
	...
	elseif slot4 == FleetSkill.TriggerAroundLand then
		...
		for slot10 = -slot3[2], slot3[2], 1 do
			for slot15 = -(slot6 - math.abs(slot10)), slot6 - math.abs(slot10), 1 do
		...
		if slot4 == FleetSkill.TriggerAroundCombatAlly then
			slot5 = {
				row = slot1.row,
				column = slot1.column
			}
			...
			return _.any(slot0.fleets, function (slot0)
				return slot0.id ~= slot0.id and slot0:GetFleetType() == FleetType.Normal and slot1:GetCell(slot0.line.row, slot0.line.column):ExistEnemy() and ManhattonDist(slot2, {
					row = slot0.line.row,
		...
		if slot4 == FleetSkill.TriggerInSubTeam then
			return true
		end
	end
end
...
slot0.OnUpdateAttachmentExist = function (slot0, slot1, slot2, slot3)
	...
	if slot3:GetVisionRadius() > 0 then
		...
		for slot10 = slot2.row - slot5, slot2.row + slot5, 1 do
			for slot14 = slot2.column - slot5, slot2.column + slot5, 1 do
...
slot0.GetBuffList = function (slot0, slot1, slot2)
	if slot1 == slot0.FactionSelf then
		return underscore.filter(underscore.values(slot0.factionBuffs[slot1]), function (slot0)
			return slot0:GetFloor() > 0
		end)
	elseif slot1 == slot0.FactionEnemy then
		if WorldMapAttachment.IsEnemyType(slot2.type) or (slot2.type == WorldMapAttachment.TypeEvent and slot2.GetSpEventType(slot2) == WorldMapAttachment.SpEventEnemy) then
			return underscore.filter(underscore.values(slot0.factionBuffs[slot1]), function (slot0)
				return slot0:GetFloor() > 0
			end)
		else
...
slot0.FlushFaction = function (slot0, slot1)
	...
	elseif slot1 == slot0.FactionEnemy then
		...
		underscore.each(slot0.FindEnemys(slot0), function (slot0)
			slot0[WorldMapCell.GetName(slot0.row, slot0.column)] = true
		end)
		underscore.each(slot0.FindAttachments(slot0, WorldMapAttachment.TypeEvent), function (slot0)
			if slot0:GetSpEventType() == WorldMapAttachment.SpEventEnemy then
				slot0[WorldMapCell.GetName(slot0.row, slot0.column)] = true
			end
		end)

		for slot6 in pairs(slot2) do
			slot0.cells[slot6]:DispatchEvent(slot0.EventUpdateMapBuff)
		end
	end
end
...
slot0.GetBattleLuaBuffs = function (slot0, slot1, slot2)
	underscore.each(slot0:GetBuffList(slot1, slot2), function (slot0)
		if slot0.config.lua_id > 0 then
			table.insert(slot0, slot0.config.lua_id)
		end
	end)
...
slot0.UpdateFleetLocation = function (slot0, slot1, slot2, slot3)
	if slot0:GetFleet(slot1).row ~= slot2 or slot4.column ~= slot3 then
		...
		slot4.DispatchEvent(slot4, WorldMapFleet.EventUpdateLocation)
	end
end
...
slot0.GetRangeDic = function (slot0, slot1)
	WorldConst.RangeCheck(slot1, slot0:GetFOVRange(slot1), function (slot0, slot1)
		if slot0.cells[WorldMapCell.GetName(slot0, slot1)] then
			slot1[slot2] = defaultValue(slot1[slot2], 0) + 1
		end
	end)
...
slot0.CheckFleetUpdateFOV = function (slot0, slot1, slot2)
	...
	slot4 = slot0:GetFleetTerrain(slot1) == WorldMapCell.TerrainFog
		...
		if slot4 ~= slot8 or slot5 ~= slot9 then
			for slot17, slot18 in pairs(slot0.cells) do
				...
				if slot13[slot17] and slot13[slot17] ~= 0 then
					slot18:UpdateFog(slot8, slot13[slot17] > 0, slot9)
				end
			end
	...
	if slot11 then
		slot0:DispatchEvent(slot0.EventUpdateFleetFOV)
	end

	if slot12 then
		slot0:DispatchEvent(slot0.EventUpdateMoveSpeed)
	end
end
...
slot0.CheckSelectFleetUpdateFog = function (slot0, slot1)
	...
	slot4 = slot0:GetFleetTerrain(slot0:GetFleet()) == WorldMapCell.TerrainFog
	slot5 = slot0:IsFleetTerrainSairenFog(slot2)

	slot1()

	slot6 = slot0:GetRangeDic(slot2)
	slot7 = slot0:GetFleetTerrain(slot0:GetFleet()) == WorldMapCell.TerrainFog
	...
	if slot4 ~= slot7 or slot5 ~= slot8 then
		for slot13, slot14 in pairs(slot0.cells) do
			...
			if slot9[slot13] and slot9[slot13] ~= 0 then
				slot14:UpdateFog(slot7, slot9[slot13] > 0, slot8)
			end
		end
	else
...
slot0.CheckEventAutoTrigger = function (slot0, slot1)
	...
	if slot1:GetEventEffect() then
		...
		elseif slot4 == WorldMapAttachment.EffectEventCatSalvage then
			return slot3.GetDisplayCommander(slot3) and not slot3:IsCatSalvage()
		end
	end
...
slot0.CanAutoFight = function (slot0)
	if slot0.config.is_auto > 0 then
		for slot4 = 1, slot0.config.is_auto, 1 do
