...
slot0.IsEnemyType = function (slot0)
	return slot0 == slot0.TypeEnemy or slot0 == slot0.TypeEnemyAI or slot0 == slot0.TypeBoss
end

slot0.IsFakeType = function (slot0)
	return slot0 == slot0.TypePort
end

slot0.IsInteractiveType = function (slot0)
	return slot0:IsEnemyType() or slot0 == slot0.TypeEvent or slot0 == slot0.TypeBox
end
...
slot0.DebugPrint = function (slot0)
	if slot0.type == slot0.TypeEvent then
		slot1 = {}
		slot3 = ""
...
事件  [id: %d]  [%s]  [位置: %d, %d]  [flag: %s]  [data: %d]  [感染值：%s]  [自动优先级：%s] 
	...
	elseif slot0.IsEnemyType(slot0.type) then
	...
	elseif slot0.type == slot0.TypeTrap then
	...
	elseif slot0.type == slot0.TypeFleet then
	...
	elseif slot0.type == slot0.TypeArtifact then
...
slot0.IsVisible = function (slot0)
	...
	elseif slot0.IsEnemyType(slot0.type) then
		return slot1 and slot0:IsAlive()
	end
end

slot0.IsFloating = function (slot0)
	return (slot0.type == slot0.TypeEvent and slot0.config.icontype == 1) or slot0.type == slot0.TypeBox
end
...
slot0.UpdateFlag = function (slot0, slot1)
	if slot0.flag ~= slot1 then
		...
		slot0:DispatchEvent(slot0.EventUpdateFlag)
	end
end
...
slot0.UpdateData = function (slot0, slot1, slot2)
	...
	slot0:DispatchEvent(slot0.EventUpdateData)
end
...
slot0.UpdateLurk = function (slot0, slot1)
	if slot0.lurk ~= slot1 then
		...
		slot0:DispatchEvent(slot0.EventUpdateLurk)
	end
end
...
slot0.GetOpEffect = function (slot0)
	return pg.world_effect_data[slot0.config.event_op[#slot0.config.event_op - slot0.dataop + 1]]
end
...
slot0.ShouldMarkAsLurk = function (slot0)
	return slot0.type == slot0.TypeEvent and slot0.config.visuality == 1 and slot0.config.discover_type == 2
end

slot0.CanLeave = function (slot0)
	if slot0.IsEnemyType(slot0.type) then
		return false
	elseif slot0.type == slot0.TypeEvent or slot0.type == slot0.TypeTrap then
...
slot0.CanArrive = function (slot0)
	if slot0.type == slot0.TypeEvent or slot0.type == slot0.TypeTrap then
...
slot0.IsBoss = function (slot0)
	return slot0.IsEnemyType(slot0.type) and WorldConst.EnemySize[slot0.config.type] == 99
end
...
slot0.GetWeaknessBuffId = function (slot0)
	if not slot0.IsEnemyType(slot0.type) then
		return
	end

	slot1 = {}

	underscore.each(underscore.flatten(pg.world_expedition_data[slot0:GetBattleStageId()].weak_list), function (slot0)
		slot0[slot0] = true
	end)
...
slot0.GetCompassType = function (slot0)
	...
	elseif slot0.IsEnemyType(slot0.type) then
		if slot0:IsBoss() then
			return slot0.CompassTypeBoss
		else
			return slot0.CompassTypeBattle
		end
	elseif slot0.type == slot0.TypeBox then
		return slot0.CompassTypeExploration
	elseif slot0.type == slot0.TypePort then
		return slot0.CompassTypePort
	end
end
...
slot0.GetDeviation = function (slot0)
	if slot0.type == slot0.TypeEvent or slot0.type == slot0.TypeArtifact then
		return Vector2(slot0.config.deviation[1], slot0.config.deviation[2])
	end
...
slot0.GetScale = function (slot0, slot1)
	...
	elseif slot0.type == slot0.TypeTrap and slot0.id == 200 then
		slot2 = slot2 * (((slot1 or slot0.data) + ()) - 1)
	end
...
slot0.GetModelOrder = function (slot0)
	if slot0.type == slot0.TypeTrap then
...
slot0.GetDirType = function (slot0)
	if slot0:GetSpEventType() == slot0.SpEventFufen then
...
slot0.IsScannerAttachment = function (slot0)
	return (slot0.IsEnemyType(slot0.type) and 4) or (slot0.type == slot0.TypeTrap and 3) or (slot0.type == slot0.TypeEvent and slot0.config.is_scanevent > 0 and 2) or (slot0.type == slot0.TypePort and 1)
end

slot0.SetHP = function (slot0, slot1)
	if slot0.type == slot0.TypeEnemyAI or slot0.type == slot0.TypeBoss then
		slot2 = {}

		for slot6, slot7 in ipairs(pg.world_expedition_data[slot0:GetBattleStageId()].phase_story) do
...
slot0.GetHP = function (slot0)
	if slot0.type == slot0.TypeTransportFleet then
		return slot0.data
	elseif slot0.type == slot0.TypeEnemyAI or slot0.type == slot0.TypeBoss then
		return slot0.hp
	end
end
...
slot0.GetMapBuffs = function (slot0)
	if slot0.type == slot0.TypeEvent then
...
slot0.IsGuideFinish = function (slot0)
