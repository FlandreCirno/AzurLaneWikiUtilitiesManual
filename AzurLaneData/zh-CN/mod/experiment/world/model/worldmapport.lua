...
slot0.IsValid = function (slot0)
	return slot0.expiredTime and pg.TimeMgr.GetInstance():GetServerTime() <= slot0.expiredTime and slot0.zeroHourTime and pg.TimeMgr.GetInstance().GetServerTime() <= slot0.zeroHourTime
end
...
slot0.UpdateTaskIds = function (slot0, slot1)
	if slot0.taskIds ~= slot1 then
		...
		slot0:DispatchEvent(slot0.EventUpdateTaskIds)
	end
end
...
slot0.UpdateGoods = function (slot0, slot1)
	if slot0.goods ~= slot1 then
		...
		slot0:DispatchEvent(slot0.EventUpdateGoods)
	end
end
