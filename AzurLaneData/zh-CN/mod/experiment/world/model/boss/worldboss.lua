...
slot0.Setup = function (slot0, slot1, slot2)
	...
	slot0.type = slot0.SetBossType(slot0)
end
...
slot0.GetOilConsume = function (slot0)
	...
	return pg.gameset.joint_boss_oil_consume.description[math.min(slot0.fightCount + 1, #pg.gameset.joint_boss_oil_consume.description)]
end
...
slot0.SetBossType = function (slot0)
	...
	if slot0.owner == getProxy(PlayerProxy):getData().id then
		return slot0.BOSS_TYPE_SELF
	else
		if slot3 and slot3:getMemberById(slot0.owner) then
			return slot0.BOSS_TYPE_GUILD
		end

		if slot2:getFriend(slot0.owner) then
			return slot0.BOSS_TYPE_FRIEND
		end
	end

	return slot0.BOSS_TYPE_WORLD
end

slot0.IsSelf = function (slot0)
	return slot0.type == slot0.BOSS_TYPE_SELF
end
...
slot0.UpdateKillTime = function (slot0)
	if nowWorld.GetBossProxy(slot1):GetRank(slot0.id) and #slot2 > 1 then
...
slot0.BuildTipText = function (slot0)
	...
	if slot0.type == slot0.BOSS_TYPE_FRIEND then
		return i18n("world_joint_call_friend_support_txt", slot1, slot2, slot3)
	elseif slot0.type == slot0.BOSS_TYPE_GUILD then
