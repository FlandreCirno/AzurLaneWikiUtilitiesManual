...
slot0.DirToLine = function (slot0)
	if slot0 == slot0.DirNone then
		return {
			row = 0,
			column = 0
		}
	elseif slot0 == slot0.DirUp then
		return {
			row = -1,
			column = 0
		}
	elseif slot0 == slot0.DirRight then
		return {
			row = 0,
			column = 1
		}
	elseif slot0 == slot0.DirDown then
		return {
			row = 1,
			column = 0
		}
	elseif slot0 == slot0.DirLeft then
		return {
			row = 0,
			column = -1
		}
...
slot0.InFOVRange = function (slot0, slot1, slot2, slot3, slot4)
	return (slot0 - slot2) * (slot0 - slot2) + (slot1 - slot3) * (slot1 - slot3) <= (slot4 or slot0.GetFOVRadius()) * ()
end
...
slot0.GetTerrainEffectRes = function (slot0, slot1, slot2)
	if slot0 == WorldMapCell.TerrainStream then
		return "world/object/" .. slot3, slot0.TerrainStreamRes[slot1]
	elseif slot0 == WorldMapCell.TerrainWind then
		return "world/object/" .. slot3, slot0.TerrainWindRes[slot1]
	elseif slot0 == WorldMapCell.TerrainIce then
		return "world/object/ice", "ice"
	elseif slot0 == WorldMapCell.TerrainPoison then
		return "world/object/" .. slot3, slot0.TerrainPoisonRes[slot2]
	end
end
...
slot0.ArrayEffectOrder = function (slot0, slot1)
	...
	for slot7 = 0, slot0:GetComponentsInChildren(typeof(Renderer), true).Length - 1, 1 do
		table.insert(slot2, slot3[slot7])
	end

	for slot8 = 0, slot0:GetComponentsInChildren(typeof(Canvas), true).Length - 1, 1 do
...
slot0.GetWindScale = function (slot0)
	return Vector3((slot0 and slot0.WindScale[slot0]) or 1, , )
end

slot0.BaseMoveDuration = 0.35

slot0.GetTerrainMoveStepDuration = function (slot0)
	slot0.MoveStepDuration = slot0.MoveStepDuration or {
		[WorldMapCell.TerrainNone] = slot0.BaseMoveDuration,
		[WorldMapCell.TerrainWind] = slot0.BaseMoveDuration / 2,
		[WorldMapCell.TerrainStream] = slot0.BaseMoveDuration / 2,
		[WorldMapCell.TerrainIce] = slot0.BaseMoveDuration / 2,
		[WorldMapCell.TerrainFog] = slot0.BaseMoveDuration,
		[WorldMapCell.TerrainFire] = slot0.BaseMoveDuration,
		[WorldMapCell.TerrainPoison] = slot0.BaseMoveDuration
	}

	return slot0.MoveStepDuration[slot0]
end
...
slot0.CalcRelativeRectPos = function (slot0, slot1, slot2, slot3)
	...
	function slot8(slot0)
		return slot0 <= slot0.x and slot0.x <=  and slot2 <= slot0.y and slot0.y <= slot3
	end
	...
	return _.min({
		...
		return Vector2.Distance(slot0, slot0)
	end)
end
...
slot0.GetAreaFocusPos = function (slot0)
	return Vector2(pg.world_regions_data[slot0].regions_pos[1], pg.world_regions_data[slot0].regions_pos[2])
end

slot0.GetTransportBlockEvent = function ()
	if not slot0.blockEventDic then
		slot0.blockEventDic = {}

		for slot3, slot4 in ipairs(pg.gameset.world_movelimit_event.description) do
			slot0.blockEventDic[slot4] = true
		end
	end

	return slot0.blockEventDic
end

slot0.GetTransportStoryEvent = function ()
	if not slot0.blockStoryDic then
		slot0.blockStoryDic = {}

		for slot3, slot4 in ipairs(pg.gameset.world_transfer_eventlist.description) do
			slot0.blockStoryDic[slot4] = true
		end
	end

	return slot0.blockStoryDic
end
...
slot0.GetObstacleKey = function (slot0)
	return bit.lshift(1, #slot0.ObstacleType - table.indexof(slot0.ObstacleType, slot0))
end

slot0.GetObstacleConfig = function (slot0, slot1)
	return bit.band(slot0.ObstacleConfig[slot0], slot0.GetObstacleKey(slot1)) > 0
end

slot0.RangeCheck = function (slot0, slot1, slot2)
	for slot6 = slot0.row - slot1, slot0.row + slot1, 1 do
		for slot10 = slot0.column - slot1, slot0.column + slot1, 1 do
			if slot0.InFOVRange(slot0.row, slot0.column, slot6, slot10, slot1) then
				slot2(slot6, slot10)
			end
		end
	end
