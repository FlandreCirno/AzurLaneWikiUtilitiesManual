...
slot0.Build = function (slot0)
	...
	PoolMgr.GetInstance():GetUI("WorldStaminaRecoverUI", true, function (slot0)
		if not slot0.UIMain then
			...
			onButton(slot0, slot0.transform.Find(slot3, "window/top/btnBack"), function ()
				slot0:Hide()
			end, SFX_CANCEL)
			onButton(slot0, slot0.transform.Find(slot3, "window/button_container/custom_button_2"), function ()
				slot0:Hide()
			end, SFX_CANCEL)
		end
	end)
...
slot0.Show = function (slot0)
	...
	for slot11, slot12 in ipairs(slot7) do
		...
		onToggle(slot0, slot13, function (slot0)
			if slot0 then
				...
				elseif slot1 == 1 then
					setText(slot2.Find(slot2, "content/Text"), i18n("world_stamina_text", slot3.cost, slot3.stamina, slot3.times, slot3.limit))
	...
	if slot0.preSelectIndex then
		triggerToggle(slot4.GetChild(slot4, slot0.preSelectIndex - 1), true)
	else
		slot8 = 1
