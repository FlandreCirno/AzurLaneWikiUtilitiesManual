...
slot0.Init = function (slot0)
	...
	slot0.transform.anchoredPosition = slot0.map.theme.GetLinePosition(slot1, slot0.cell.row, slot0.cell.column)
	slot0.transform.sizeDelta = slot0.map.theme.cellSize
	slot0.rtAttachments = slot0.transform.Find(slot3, "attachments")
	slot0.rtAttachments.localEulerAngles = Vector3(-slot0.map.theme.angle, 0, 0)
	slot0.rtMask = slot0.transform.Find(slot3, "mask")
	slot0.rtMask.sizeDelta = slot0.map.theme.cellSize + Vector2(WorldConst.LineCross * 2, WorldConst.LineCross * 2)
	slot0.rtFog = slot0.transform.Find(slot3, "fog")

	slot1(slot0.rtFog:Find("dark_fog"), slot0.wsMapResource.rtDarkFog:Find(WorldConst.Pos2FogRes(slot0.cell.row, slot0.cell.column)), slot2, slot5)
	slot1(slot0.rtFog:Find("sairen_fog"), slot0.wsMapResource.rtSairenFog:Find(WorldConst.Pos2FogRes(slot0.cell.row, slot0.cell.column)), slot0.cell, slot0.map.theme.cellSize + slot0.map.theme.cellSpace)
	slot0:Update()
...
slot0.Update = function (slot0, slot1)
	...
	if slot1 == nil or slot1 == WorldMapCell.EventUpdateFog then
		...
		if slot2:InFog() then
			...
			if slot1 and slot5 > 0 then
				...
				slot0.fogTimer = slot0.wsTimer:AddInMapTimer(function ()
					slot0.fogUid = LeanTween.alphaCanvas(GetComponent(slot0.rtFog, typeof(CanvasGroup)), 1, LeanTween.alphaCanvas).uniqueId

					slot0.wsTimer:AddInMapTween(slot0.fogUid)
				end, slot5)
		...
		elseif slot1 and slot5 > 0 then
			slot0.fogTimer = slot0.wsTimer:AddInMapTimer(function ()
				slot0.fogUid = LeanTween.alphaCanvas(GetComponent(slot0.rtFog, typeof(CanvasGroup)), 0, LeanTween.alphaCanvas):setOnComplete(System.Action(function ()
					setActive(slot0.rtFog, false)
				end)).uniqueId

				slot0.wsTimer.AddInMapTween(slot0, slot0.fogUid)
			end, slot5)
		...
		if slot2:GetInFOV() then
			if slot1 and slot5 > 0 then
				slot0.maskTimer = slot0.wsTimer:AddInMapTimer(function ()
					slot0.maskUid = LeanTween.alpha(slot0.rtMask, 0, LeanTween.alpha).uniqueId

					slot0.wsTimer:AddInMapTween(slot0.maskUid)
				end, slot5)
			...
			if slot1 and slot5 > 0 then
				slot0.maskTimer = slot0.wsTimer:AddInMapTimer(function ()
					slot0.maskUid = LeanTween.alpha(slot0.rtMask, LeanTween.alpha, ).uniqueId
...
slot0.UpdateFogImage = function (slot0)
	setActive(slot0.rtFog:Find("dark_fog"), not slot0.cell.LookSairenFog(slot1))
	setActive(slot0.rtFog:Find("sairen_fog"), slot0.cell.LookSairenFog(slot1))
end
