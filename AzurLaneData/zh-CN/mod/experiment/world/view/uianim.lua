slot0 = class("UIAnim", import("...BaseEntity"))
slot0.Fields = {
	transform = "userdata",
	aniEvent = "userdata",
	onEnd = "function",
	playing = "boolean",
	prefab = "string",
	onTrigger = "function",
	onStart = "function"
}
slot0.EventLoaded = "UIAnim.EventLoaded"

slot0.Setup = function (slot0, slot1)
	slot0.prefab = slot1
end

slot0.Dispose = function (slot0)
	slot0:Unload()
	slot0:Clear()
end

slot0.Load = function (slot0)
	PoolMgr.GetInstance():GetUI(slot0.prefab, true, function (slot0)
		if slot0 == slot1.prefab then
			slot1.transform = slot0.transform

			slot1:Init()
			slot1.Init:DispatchEvent(slot2.EventLoaded)
		else
			slot3:ReturnUI(slot0, slot0)
		end
	end)
end

slot0.Unload = function (slot0)
	if slot0.prefab and slot0.transform then
		PoolMgr.GetInstance():ReturnUI(slot0.prefab, slot0.transform.gameObject)
	end

	slot0.prefab = nil
	slot0.transform = nil
end

slot0.Play = function (slot0, slot1)
	slot0.playing = true
	slot0.onStart = nil
	slot0.onTrigger = nil
	slot0.onEnd = slot1

	slot0:Update()
end

slot0.Stop = function (slot0)
	slot0.playing = false

	slot0:Update()
end

slot0.Init = function (slot0)
	setActive(slot0.transform, false)

	slot0.aniEvent = slot0.transform:GetComponent("DftAniEvent")

	slot0:Update()
end

slot0.Update = function (slot0)
	if slot0.aniEvent then
		setActive(slot0.transform, slot0.playing)

		if slot0.playing then
			slot0.aniEvent:SetStartEvent(function ()
				if slot0.onStart then
					slot0.onStart()
				end
			end)
			slot0.aniEvent.SetTriggerEvent(slot1, function ()
				if slot0.onTrigger then
					slot0.onTrigger()
				end
			end)
			slot0.aniEvent.SetEndEvent(slot1, function (slot0)
				slot0:Stop()

				if slot0.onEnd then
					slot0.onEnd()
				end
			end)
		end
	end
end

return slot0
