...
slot0.Init = function (slot0)
	slot0.transform.name = "carry_item_" .. slot0.carryItem.id
	...
	if slot1 == nil then
		...
		if slot7 then
			slot0:LoadModel(WorldConst.ModelSpine, slot4.config.icon, nil, true, function ()
				slot0.model:SetParent(slot1:Find("ship"), false)
			end)
		else
			slot0.LoadModel(slot0, WorldConst.ModelPrefab, WorldConst.ResBoxPrefab .. slot4.config.icon, slot4.config.icon, true, function ()
				slot0.model:SetParent(slot0.model, false)
			end)
		end

		setActive(slot2.Find(slot2, "buffs"), false)
...
slot0.FollowPath = function (slot0, slot1)
	...
	table.insert(slot0.followList, function ()
		slot2.row, slot2.column = slot0:GetLocation()

		slot0.wsMapPath:UpdateObject(slot0)
		slot0.wsMapPath:UpdateAction(WorldConst.ActionMove)
		slot0.wsMapPath:UpdateDirType(WorldConst.DirType2)
		slot0.wsMapPath:StartMove({
			row = slot0,
			column = slot1
		}, slot1)
	end)
