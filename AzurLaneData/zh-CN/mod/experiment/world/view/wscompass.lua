...
slot0.InitCells = function (slot0, slot1)
	...
	for slot6 = 0, slot1.x, 1 do
		for slot10 = 0, slot1.y, 1 do
...
slot0.UpdateMarks = function (slot0, slot1, slot2, slot3)
	...
	_.each(slot0.cells, function (slot0)
		if WorldConst.InFOVRange(slot0, WorldConst.InFOVRange, slot0.row, slot0.column, slot2.fov) then
			slot2:UpdateInnerMark(slot0, slot0, slot2.UpdateInnerMark)
		else
			slot2:UpdateOutsideMark(slot0, slot0, slot2.UpdateOutsideMark)
		end
	end)
end
...
slot0.AddCellListener = function (slot0, slot1)
	...
	_.each(slot1.attachments, function (slot0)
		slot0:AddListener(WorldMapAttachment.EventUpdateLurk, slot0.onUpdateAttachment)
		slot0:AddListener(WorldMapAttachment.EventUpdateData, slot0.onUpdateAttachment)
		slot0:AddListener(WorldMapAttachment.EventUpdateFlag, slot0.onUpdateAttachment)
	end)
end

slot0.RemoveCellsListener = function (slot0)
	_.each(slot0.cells or {}, function (slot0)
		slot0:RemoveListener(WorldMapCell.EventAddAttachment, slot0.onAdd)
		slot0:RemoveListener(WorldMapCell.EventRemoveAttachment, slot0.onRemove)
		_.each(slot0.attachments, function (slot0)
			slot0:RemoveListener(WorldMapAttachment.EventUpdateLurk, slot0.onUpdateAttachment)
			slot0:RemoveListener(WorldMapAttachment.EventUpdateData, slot0.onUpdateAttachment)
			slot0:RemoveListener(WorldMapAttachment.EventUpdateFlag, slot0.onUpdateAttachment)
		end)
	end)
end
...
slot0.UpdateBorder = function (slot0, slot1, slot2, slot3)
	slot0.border.sizeDelta = Vector2(slot6, slot7)
...
slot0.CalcTaskMarkPos = function (slot0, slot1)
	return Vector3(math.sin(math.rad(slot2)) * slot0.normal * (slot0.fov + 1), math.cos(math.rad(slot2)) * slot0.normal * (slot0.fov + 1), 0)
end
...
slot0.CalcInnerPos = function (slot0, slot1, slot2, slot3, slot4)
	return Vector3(slot0.normal * Vector2(slot2 - slot4, -1 * (slot1 - slot3)).x, slot0.normal * Vector2(slot2 - slot4, -1 * (slot1 - slot3)).y, 0)
end
...
slot0.CalcOutsidePos = function (slot0, slot1, slot2, slot3, slot4)
	return Vector3(math.sin(math.rad(slot5)) * slot0.normal * (slot0.fov + 0.5) * Mathf.Sign(Vector2(slot2 - slot4, slot3 - slot1).x), math.cos(math.rad(slot5)) * slot0.normal * (slot0.fov + 0.5) * Mathf.Sign(Vector2(slot2 - slot4, slot3 - slot1).y), 0)
end

slot0.NewMark = function (slot0, slot1, slot2, slot3)
	slot0.pool:Get(slot4).transform.localScale = slot0.scaleRate
...
slot0.ClearMarks = function (slot0)
	_.each(slot0.marks, function (slot0)
		...
		slot0.pool:Return(slot0.name, slot0.go)
	end)
...
slot0.GetMarkPosition = function (slot0, slot1, slot2)
	...
	return slot0.rangeTF:TransformPoint((not WorldConst.InFOVRange(slot0.map.GetFleet(slot3).row, slot0.map.GetFleet(slot3).column, slot1, slot2, slot0.fov) or slot0:CalcInnerPos(slot1, slot2, slot4.row, slot4.column)) and slot0:CalcOutsidePos(slot1, slot2, slot4.row, slot4.column))
end

slot0.GetEntranceTrackMark = function (slot0, slot1)
	slot9, slot10 = slot0:getVector(slot2)
	slot11, slot12 = slot0:getVector(slot0.entrance)
