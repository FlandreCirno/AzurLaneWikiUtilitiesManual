ys = ys or {}
slot1 = ys.Battle.BattleTargetChoise
slot2 = ys.Battle.BattleFormulas
slot3 = class("BattleSpaceLaserUnit", ys.Battle.BattleColumnAreaBulletUnit)
slot3.__name = "BattleSpaceLaserUnit"
ys.Battle.BattleSpaceLaserUnit = slot3
...
slot3.DoTrack = function (slot0)
	...
	if slot2:GetPosition() - slot1:GetPosition().Magnitude(slot3) <= 1e-05 then
	...
	slot10 = math.sin(slot1._sinAngularSpeed * slot1:GetSpeedRatio())
	slot11 = slot3.x * slot0._speedNormal.x + slot3.z * slot0._speedNormal.z
	slot12 = slot3.z * slot0._speedNormal.x - slot3.x * slot0._speedNormal.z

	if slot3.x * slot0._speedNormal.x + slot3.z * slot0._speedNormal.z < math.cos(slot1._cosAngularSpeed * slot1.GetSpeedRatio()) then
		slot11 = slot9
		slot12 = slot10 * ((slot12 > 0 and 1) or -1)
	end

	slot1._speed:Set(slot13, 0, slot14)
	slot1._speed:Mul(slot15)
	slot0._speedNormal:Set(slot13, 0, slot5.z * slot11 + slot5.x * slot12)
	slot0._speedNormal:SetNormalize()

	slot0._yAngle = math.rad2Deg * math.atan2(slot5.x * slot11 - slot5.z * slot12, slot5.z * slot11 + slot5.x * slot12)
end

slot3.InitSpeed = function (slot0, ...)
	slot0.super.InitSpeed(slot0, ...)

	if slot0:IsTracker() then
		slot0._speedNormal = Vector3(math.cos(slot1), 0, math.sin(math.deg2Rad * slot0._yAngle))
