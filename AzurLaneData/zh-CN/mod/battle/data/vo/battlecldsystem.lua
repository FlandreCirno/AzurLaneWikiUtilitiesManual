...
slot6.UpdateBulletCld = function (slot0, slot1)
	...
	if slot2 == slot0.BulletField.SURFACE then
		...
		if slot1:GetType() == slot0.BulletType.SCALE then
			slot8, slot12, slot10 = slot1:GetRadian()
			...
			if math.abs(slot9) ~= 1 then
				...
				slot7 = slot6:GetCldListGradient(slot8, slot11.z * 2, slot11.x * 2, Vector3(slot1:GetPosition().x - slot11.x * slot9, 1, slot1.GetPosition().z - slot11.x * slot10))
			else
				slot7 = slot6:GetCldList(slot3, slot1)
			end
		else
			slot7 = slot6:GetCldList(slot3, slot1)
		end
...
slot6.HandleBulletCldWithShip = function (slot0, slot1, slot2)
	...
	for slot8 = 1, #slot1, 1 do
		if slot1[slot8].data.type == slot0.CldType.SHIP and slot9.Active == true then
			...
			if slot10:GetCurrentOxyState() == slot1.DIVE and slot2:GetCldData().Surface ~= slot0.OXY_STATE.DIVE then
			elseif slot12 then
...
slot6.UpdateAOECld = function (slot0, slot1)
	...
	if slot1:GetFieldType() == slot0.BulletField.SURFACE then
		...
		slot1:ClearCLDList()
		slot0:HandleAreaCldWithVehicle(slot1, ((slot1:GetAreaType() ~= slot0.AreaType.COLUMN and slot1:GetAnchorPointAlignment() ~= Vector3.zero) or slot8:GetCldList(slot2, slot1)) and slot8:GetCldListGradient(slot4, slot1:GetHeight(), slot1:GetWidth(), slot1:GetPosition()) and slot0._aircraftTree:GetCldList(slot2, slot1))
	...
	for slot9 = 1, #slot2, 1 do
		...
		if (slot4 and slot10.IFF ~= slot3.IFF) or (not slot4 and slot10.IFF == slot3.IFF) then
			...
			if slot1:GetDiveFilter() then
				slot14 = slot0:GetShip(slot10.UID).GetCurrentOxyState(slot13)
...
slot6.UpdateWallCld = function (slot0, slot1)
	...
	if slot1:GetCldObjType() == slot1.CLD_OBJ_TYPE_BULLET then
		...
		slot0:HandleWallCldWithBullet(slot1, (slot1:GetIFF() ~= slot0._friendlyCode or slot0._foeSurafceBulletTree:GetCldList(slot2, slot0)) and slot0._surfaceBulletTree:GetCldList(slot2, slot0))
	elseif slot3 == slot1.CLD_OBJ_TYPE_SHIP then
		slot4 = nil

		slot0:HandleWllCldWithShip(slot1, (slot1:GetIFF() ~= slot0._friendlyCode or slot0._foeShipTree:GetCldList(slot2, slot0)) and slot0._shipTree:GetCldList(slot2, slot0))
	end
end
...
slot6.HandleWllCldWithShip = function (slot0, slot1, slot2)
	slot4 = {}

	for slot8 = 1, #slot2, 1 do
		if slot2[slot8].data.type == slot0.CldType.SHIP and slot9.Active == true then
			if slot0:GetShip(slot9.UID).GetCurrentOxyState(slot10) == slot1.DIVE then
			else
