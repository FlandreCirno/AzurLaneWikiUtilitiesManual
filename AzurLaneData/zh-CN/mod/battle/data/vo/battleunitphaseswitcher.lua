...
ys.Battle.BattleUnitPhaseSwitcher.Update = function (slot0)
	...
	for slot6, slot7 in ipairs(slot0._currentPhaseSwitchParam) do
		...
		if slot7.type == slot0.DURATION then
			if slot9 < pg.TimeMgr.GetInstance():GetCombatTime() - slot0._phaseStartTime then
				slot2 = slot7.to
				slot7.andFlag = false
			end
		elseif slot8 == slot0.POSITION_X_GREATER then
			if slot9 < slot0._client:GetPosition().x then
				slot2 = slot7.to
				slot7.andFlag = false
			end
		elseif slot8 == slot0.POSITION_X_LESS then
			if slot0._client:GetPosition().x < slot9 then
				slot2 = slot7.to
				slot7.andFlag = false
			end
		elseif slot8 == slot0.OXYGEN and slot0._client:GetCuurentOxygen() <= slot9 then
			slot2 = slot7.to
			slot7.andFlag = false
		end
...
ys.Battle.BattleUnitPhaseSwitcher.UpdateHP = function (slot0, slot1)
	...
	for slot7, slot8 in ipairs(slot0._currentPhaseSwitchParam) do
		...
		if slot8.type == slot0.HP and slot1 < slot10 then
			slot3 = slot11
			slot8.andFlag = false
		end
...
ys.Battle.BattleUnitPhaseSwitcher.switch = function (slot0, slot1)
	...
	if slot2.addBuff then
		for slot8, slot9 in ipairs(slot2.addBuff) do
			slot0._client:AddBuff(slot0.Battle.BattleBuffUnit.New(slot9, 1, slot0._client))
		end
	end
...
ys.Battle.BattleUnitPhaseSwitcher.packagePhaseSwitchParam = function (slot0, slot1)
