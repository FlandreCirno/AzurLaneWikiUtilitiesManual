...
slot8 = ys.Battle.BattleAttr
slot9 = class("BattleFleetVO")
ys.Battle.BattleFleetVO = slot9
...
slot9.CalcSubmarineBaseLine = function (slot0, slot1)
	...
	elseif slot0._IFF == slot0.FOE_CODE and slot1 == SYSTEM_DUEL then
	end
end
...
slot9.init = function (slot0)
	...
	slot0._fleetStaticSonar = slot0.Battle.BattleFleetStaticSonar.New(slot0)
...
slot9.FleetUnitSpwanFinish = function (slot0)
	...
	for slot5, slot6 in ipairs(slot0._unitList) do
		slot0.SetCurrent(slot6, "fleetGS", slot1)
	end
end
...
slot9.fireChargeWeapon = function (slot0, slot1, slot2, slot3)
	...
	function slot5()
		slot0.DispatchBlink(slot1, function ()
			slot0:Fire(slot0)
		end)
	end
	...
	if slot2 then
		if slot0._IFF == slot0.FRIENDLY_CODE then
			slot0._chargeWeaponVO.PlayCutIn(slot6, slot4, 1 / slot0.FOCUS_MAP_RATE)
		end
...
slot9.CoupleEncourage = function (slot0)
	...
	for slot6, slot7 in ipairs(slot0._unitList) do
		if #slot0.GetWords(slot7:GetSkinID(), "couple_encourage", slot7:GetIntimacy()) > 0 then
			slot1[slot7] = slot9
		end
	end

	slot3 = slot1.CPChatType
	slot4 = slot1.CPChatTargetFunc

	function slot5(slot0, slot1)
		slot2 = {}

		if slot0 == slot0.GROUP_ID then
			slot2.groupIDList = slot1
		elseif slot0 == slot0.SHIP_TYPE then
			slot2.ship_type_list = slot1
		elseif slot0 == slot0.RARE then
			slot2.rarity = slot1[1]
		elseif slot0 == slot0.NATIONALITY then
			slot2.nationality = slot1[1]
		elseif slot0 == slot0.ILLUSTRATOR then
			slot2.illustrator = slot1[1]
		elseif slot0 == slot0.TEAM then
	...
	for slot9, slot10 in pairs(slot1) do
		for slot14, slot15 in ipairs(slot10) do
			slot16 = slot15[1]
			slot17 = slot15[2]
			slot18 = slot15[4] or slot3.GROUP_ID
			slot19 = nil

			if type(slot18) == "table" then
				for slot23, slot24 in ipairs(slot18) do
					slot19 = slot2.Battle.BattleTargetChoise[slot4[slot24]](slot9, slot5(slot24, slot16[slot23]), slot19)
				end
			elseif type(slot18) == "number" then
				slot19 = slot2.Battle.BattleTargetChoise[slot4[slot18]](slot9, slot5(slot18, slot16))
			end
	...
	if #slot2 > 0 then
		slot2[math.random(#slot2)].cp:DispatchVoice(slot8)
		slot2[math.random(#slot2)].cp:DispatchChat(slot2[math.random(#slot2)].content, 3, "link" .. slot2[math.random(#slot2)].linkIndex)
	end
end
...
slot9.AutoBotUpdated = function (slot0, slot1)
	slot2 = (slot1 and slot0.BuffEffectType.ON_AUTOBOT) or slot0.BuffEffectType.ON_MANUAL
