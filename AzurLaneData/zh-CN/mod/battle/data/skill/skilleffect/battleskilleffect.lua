...
ys.Battle.BattleSkillEffect.AniEffect = function (slot0, slot1, slot2)
	...
	if slot0._casterAniEffect and slot0._casterAniEffect ~= "" then
		...
		slot1.DispatchEvent(slot1, slot0.Event.New(slot1.ADD_EFFECT, {
			effect = slot5.effect,
			offset = slot5.offset,
			posFun = slot6
		}))
	...
	if slot0._targetAniEffect and slot0._targetAniEffect ~= "" then
		...
		slot2.DispatchEvent(slot2, slot0.Event.New(slot1.ADD_EFFECT, {
			effect = slot5.effect,
			offset = slot5.offset,
			posFun = slot6
		}))
...
ys.Battle.BattleSkillEffect.DataEffect = function (slot0, slot1, slot2, slot3)
	if slot0._delay > 0 then
		...
		slot0._timerList[slot0._timerIndex + 1] = pg.TimeMgr.GetInstance().AddBattleTimer(slot7, "BattleSkill", -1, slot0._delay, function ()
...
ys.Battle.BattleSkillEffect.DataEffectWithoutTarget = function (slot0, slot1, slot2)
	if slot0._delay > 0 then
		slot3 = nil
		slot0._timerIndex = slot0._timerIndex + 1
		slot0._timerList[slot0._timerIndex + 1] = pg.TimeMgr.GetInstance().AddBattleTimer(slot6, "BattleSkill", -1, slot0._delay, function ()
			if slot0 and slot0:IsAlive() then
...
ys.Battle.BattleSkillEffect.Interrupt = function (slot0)
	return
end
...
ys.Battle.BattleSkillEffect.calcCorrdinate = function (slot0, slot1, slot2)
	slot3 = nil
	...
	elseif slot0.absoulteRandom then
		slot3 = slot0.RandomPos(slot0.absoulteRandom)
	elseif slot0.casterRelativeCorrdinate then
		slot3 = Vector3(slot1:GetIFF() * slot0.casterRelativeCorrdinate.hrz + slot1:GetPosition().x, 0, slot1.GetIFF() * slot0.casterRelativeCorrdinate.vrt + slot1.GetPosition().z)
	elseif slot0.casterRelativeRandom then
		slot5 = slot1:GetPosition()
		slot3 = slot0.RandomPos({
			X1 = slot1:GetIFF() * slot0.casterRelativeRandom.front + slot5.x,
			X2 = slot1.GetIFF() * slot0.casterRelativeRandom.rear + slot5.x,
	...
	elseif slot0.targetRelativeCorrdinate then
		if slot2 then
			slot3 = Vector3(slot2:GetIFF() * slot0.targetRelativeCorrdinate.hrz + slot2:GetPosition().x, 0, slot2.GetIFF() * slot0.targetRelativeCorrdinate.vrt + slot2.GetPosition().z)
		end
	elseif slot0.targetRelativeRandom and slot2 then
		slot5 = slot2:GetPosition()
		slot3 = slot0.RandomPos({
			X1 = slot2:GetIFF() * slot0.targetRelativeRandom.front + slot5.x,
			X2 = slot2.GetIFF() * slot0.targetRelativeRandom.rear + slot5.x,
...
ys.Battle.BattleSkillEffect.GetDamageSum = function (slot0)
	return 0
end
