...
ys.Battle.BattleWaveInfo.IsFlagsPass = function (slot0)
	if not slot0._blockFlags or not next(slot0._blockFlags) then
		return true
	end

	if not slot0.Battle.BattleDataProxy.GetInstance():GetWaveFlags() or not next(slot2) then
		return false
	end
...
ys.Battle.BattleWaveInfo.IsPreWavesFinished = function (slot0)
...
ys.Battle.BattleWaveInfo.DoBranch = function (slot0)
	for slot4, slot5 in ipairs(slot0._branchWaves) do
		if not slot0._branchWaveIDs[slot5:GetIndex()] or slot5:GetState() ~= slot0.STATE_PASS then
			if not slot6 and slot5:GetState() == slot0.STATE_FAIL then
			else
				slot0:doFail()
