...
ys or .Battle.BattleDataFunction.GetBuffBulletRes = function (slot0, slot1, slot2, slot3)
	...
	if slot0:GetShipMetaFromDataTemplate() then
...
ys or .Battle.BattleDataFunction.GetResFromBuff = function (slot0, slot1, slot2, slot3)
	...
	if slot6.last_effect and slot6.last_effect ~= "" then
		...
		for slot11, slot12 in ipairs(slot7) do
			slot4[#slot4 + 1] = slot1.Battle.BattleResourceManager.GetFXPath(slot12)
		end
	end
	...
	for slot10, slot11 in ipairs(slot6.effect_list) do
		...
		if slot11.arg_list.effect then
			slot4[#slot4 + 1] = slot1.Battle.BattleResourceManager.GetFXPath(slot18)
		end
	end
...
ys or .Battle.BattleDataFunction.GetBuffListRes = function (slot0, slot1, slot2)
	slot3 = {}
	slot4 = {}

	for slot8, slot9 in ipairs(slot0) do
		for slot15, slot16 in ipairs(slot0.GetResFromBuff(slot10, slot11, slot4, slot2)) do
...
ys or .Battle.BattleDataFunction.GetBulletResFromSkill = function (slot0, slot1, slot2)
	...
	if type(slot4.castCV) == "table" then
		slot1.Battle.BattleResourceManager.GetInstance().AddPreloadCV(slot6, slot4.castCV.skinID)
	end
...
ys or .Battle.BattleDataFunction.GetLimitAttributeRange = function (slot0, slot1)
