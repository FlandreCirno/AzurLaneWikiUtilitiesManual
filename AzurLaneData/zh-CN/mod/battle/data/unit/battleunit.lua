...
slot9.SummonSickness = function (slot0, slot1)
	...
	slot0._sicknessTimer = pg.TimeMgr.GetInstance().AddBattleTimer(slot3, "summonSickness", 0, slot1, function ()
...
slot9.UpdateHP = function (slot0, slot1, slot2)
	...
	if not slot2.isHeal then
		...
		if slot0._currentHP <= ()["damage"] then
			slot0:TriggerBuff(slot0.BuffEffectType.ON_BEFORE_FATAL_DAMAGE, {})
		end
	...
	if slot11 and not slot11:EqualZero() then
		...
		slot20 = slot11:Clone()
		slot20.x = Mathf.Clamp(slot20.x, slot16.x - slot0:GetBoxSize().x, slot16.x + slot0.GetBoxSize().x)
...
slot9.UpdateHPAction = function (slot0, slot1)
	slot0:DispatchEvent(slot0.Event.New(slot1.UPDATE_HP, slot1))
end

slot9.DeadAction = function (slot0)
	slot0:TriggerBuff(slot0.BuffEffectType.ON_SINK, {})
...
slot9.GetCLDZCenterPosition = function (slot0)
	if slot0._zCenterFrame ~= slot0._battleProxy.FrameIndex then
		...
		slot0._cldZCenterCache = (slot0:GetCldBox().min + slot0.GetCldBox().max) * 0.5
	end
...
slot9.EnterGCD = function (slot0, slot1, slot2)
	...
	slot0._weaponQueue.QueueEnterGCD(slot4, slot2, slot1)
...
slot9.AddBuff = function (slot0, slot1)
	...
	if slot0:GetBuff(slot1.GetID()) then
		slot3.buff_level = math.max(slot4:GetLv(), slot1:GetLv())
	...
	slot0:TriggerBuff(slot2.BuffEffectType.ON_BUFF_ADDED, {
		buffID = slot2
	})
end
...
slot9.RemoveBuff = function (slot0, slot1)
	...
	slot0:TriggerBuff(slot0.BuffEffectType.ON_BUFF_REMOVED, {
		buffID = slot1
	})
end
...
slot9.Undetected = function (slot0)
	...
	slot0:updateExposeTimeStamp(slot0.SUB_EXPOSE_LASTING_DURATION)
end
...
slot9.updateExposeTimeStamp = function (slot0, slot1)
	...
	slot0._exposedOverTimeStamp = (pg.TimeMgr.GetInstance():GetCombatTime() + slot1 < slot0._exposedOverTimeStamp and slot0._exposedOverTimeStamp) or pg.TimeMgr.GetInstance().GetCombatTime() + slot1
end
...
slot9.dispatchDetectedTrigger = function (slot0)
	if slot0:GetDiveDetected() then
		slot0:TriggerBuff(slot0.BuffEffectType.ON_SUB_DETECTED, {})
	else
		slot0:TriggerBuff(slot0.BuffEffectType.ON_SUB_UNDETECTED, {})
	end
end
...
slot9.InitCloak = function (slot0)
	slot0._cloak = slot0.Battle.BattleUnitCloakComponent.New(slot0)
...
slot9.UpdateCloakConfig = function (slot0)
	...
	slot0:DispatchEvent(slot0.Event.New(slot1.UPDATE_CLOAK_CONFIG))
end
...
slot9.ActiveWeaponSectorView = function (slot0, slot1, slot2)
	slot0:DispatchEvent(slot0.Event.New(slot1.WEAPON_SECTOR, {
		weapon = slot1,
		isActive = slot2
	}))
end
