...
slot0.Exit = function (slot0, slot1)
	...
	slot3 = ys.Battle.BattleConst.BattleScore.B <= slot0.statistics._battleScore

	if slot3 and slot2 and not slot2:isEnd() then
		slot4 = 0

		for slot9 = 1, slot2:getConfig("config_client")[1], 1 do
		...
		if slot4 < math.min((slot6:DiffDay(slot2.data1, slot6:GetServerTime()) + 1) * 2, slot5 * 3) then
			slot12 = slot2:getKVPList(1, math.floor((table.indexof(slot10, slot9) - 1) / (#slot2:getConfig("config_client")[2] / slot5)) + 1) or 0
			slot13 = slot2:getKVPList(2, slot11) == 1

			if slot12 < 3 and not slot13 then
