ys = ys or {}
slot1 = singletonClass("BattleMissileFactory", ys.Battle.BattleBombBulletFactory)
slot1.__name = "BattleMissileFactory"
ys.Battle.BattleMissileFactory = slot1
...
slot1.CreateBulletAlert = function (slot0)
	...
	if slot0:GetIFF() == slot0.GetDataProxy():GetFriendlyCode() then
		return
	end
	...
	slot5 = slot1.Battle.BattleFXPool.GetInstance():GetFX(slot3).transform
	slot6 = 0

	if pg.effect_offset[slot1.alert_fx] and slot7[slot3].y_scale == true then
		slot6 = slot2
	end
