...
slot0.execute = function (slot0, slot1)
	...
	if slot2.name == BACKYARD.FURNITURE_ADD then
		if not slot4:getData():canPutFurniture(slot0.DormFurniture2HouseFurniture(slot2.furniture)) then
		...
		if slot3 == BACKYARD.FURNITURE_POS_CHANGE then
			...
			function slot11(slot0)
				for slot4, slot5 in pairs(slot0.child) do
					pg.backyard:sendNotification(BACKYARD.REMOVE_ITEM, Clone(slot0:getFurnitureById(slot4)))
					slot0:changeFurniturePos(slot4, slot0:getChildPosById(slot4))
					slot1(slot0.getFurnitureById(slot4))
				end
			end
...
slot0.DormFurniture2HouseFurniture = function (slot0)
	slot2 = getBackYardProxy(BackYardHouseProxy).getData(slot1)
	slot5 = StartUpBackYardCommand.GetHouseByDorm({
		furnitures = getProxy(DormProxy):getData().getOtherFloorFurnitrues(slot3, getProxy(DormProxy).floor)
	})
	slot6 = Clone(slot0)

	if not slot0:isPaper() and (slot2:ContainsFurnitrue(slot0.id) or slot5:ContainsFurnitrue(slot0.id)) then
		for slot10 = 1, slot0.count - 1, 1 do
