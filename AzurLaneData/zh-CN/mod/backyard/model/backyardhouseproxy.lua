...
slot0.InterActionTransportAgain = function (slot0, slot1, slot2)
	...
	if #slot5 == 1 then
		...
		slot8 = _.detect(slot5, function (slot0)
			return slot0 ~= slot0
		end)
		slot7 = slot8
	end
...
slot0.addMoveForFurniture = function (slot0, slot1, slot2)
	slot2 = slot0:getFurnitureById(slot1):getSpineSpeed() * slot2
...
slot0.clearSpineInterAction = function (slot0, slot1)
	...
	if slot3 then
		...
		if slot0.data.furnitures[slot3].hasSpineExtra(slot4) then
			slot6 = {}
...
slot0.addMoveOnFurnitrue = function (slot0, slot1, slot2, slot3)
	if not BackYardConst.DEBUG then
		...
		slot0.timer[slot6] = Timer.New(function ()
			...
			function slot2()
				for slot3, slot4 in ipairs(ipairs) do
					if slot1.data:canMoveBoatOnFurniture(slot2, slot3, slot4) then
						table.insert(slot4, slot4)
					end
				end
			end
		...
		if slot0.delayTimer[slot6] then
			slot0.delayTimer[slot6].Stop(slot10)

			slot0.delayTimer[slot6] = nil
		end

		slot10 = (slot3 and 0.0001) or math.random(2, 8)
...
slot0.ResetShipPos = function (slot0, slot1)
	if not slot0.data:GetSurroundGridByPoint(slot0.data.ships[slot1].getPosition(slot2), 1, 1, false) then
		return
	end
...
slot0.shipSeriesMove = function (slot0, slot1, slot2, slot3)
	...
	slot0.timer[slot4] = Timer.New(function ()
		...
		slot3[slot2].sendNotification(slot2, slot7.BACKYARD_SHIP_MOVE, {
			id = slot4 <= slot2 + 1,
			pos = slot3[slot2],
	...
	slot0.timer[slot4].Start(slot8)
	slot0.timer[slot4].func()
end

slot0.GetCanWalkGrid = function (slot0, slot1)
	slot3 = {}
...
slot0.shipRomdonMove = function (slot0, slot1, slot2)
	...
	slot0.timer[slot3] = Timer.New(function ()
		...
		if slot0:RandomSpineInterAction(slot0.RandomSpineInterAction) and slot1:IsFollower() then
			slot0:StartFollowerInteraction(slot1, slot1.id)
		end

		if not slot0:GetCanWalkGrid(slot0.GetCanWalkGrid) then
		...
		slot3(slot4 + 1, slot3.BACKYARD_SHIP_MOVE, slot6)
		...
		if slot6 <= slot4 then
			...
			slot0:CheckFollowerFurniture(slot1)
		end
	end, slot6 + 0.01, -1)

	if slot0.delayTimer[slot3] then
		slot0.delayTimer[slot3].Stop(slot7)
...
slot0.restoreFurnitures = function (slot0)
	...
	if slot0.data.hasChangeFloorPaper(slot5) then
	...
	if slot0.data.hasChangeWallPaper(slot5) then
	...
	if #slot2 > 0 then
		...
		slot0.sendNotification(slot0, slot0.BACKYARD_RESTORED)
	end
end
...
slot0.addFurniture = function (slot0, slot1, slot2)
	...
	if slot1:isFloor() then
		...
		slot0.initWallFurnitruePos(slot0, slot1, function ()
			slot0:updateHouse(slot0.data)
...
slot0.RandomSpineInterAction = function (slot0, slot1)
	if slot0.isHappen(100 / (table.getCount(slot0.data.ships) + 1)) then
...
slot0.StartFollowerInteraction = function (slot0, slot1, slot2)
	slot0:removeFurniture(slot2)
	slot0.data.ships[slot1].SetFollower(slot4, slot2)
	slot0.data.ships[slot1].SetFollowTime(slot4, slot5)
	slot0:sendNotification(slot0.ON_START_FOLLOWER_INTERACTION, {
		id = slot1,
		furnitureId = slot2
	})
end

slot0.CancelFollowerInteraction = function (slot0, slot1)
	slot0.data.ships[slot1].ClearFollower(slot3, nil)

	slot11, slot12 = BackyardFurnitureVO.New({
		id = slot0.data.ships[slot1].GetFollower(slot3),
		position = position
	}).getSize(slot5)

	if slot0.data:GetSurroundGridByPoint(slot0.data.ships[slot1].getPosition(slot3), slot6, slot7, false) then
		slot5:updatePosition(slot8)
		slot0:addFurniture(slot5)
		slot0:sendNotification(slot0.ON_CANCEL_FOLLOWER_INTERACTION, {
			id = slot1,
			furnitureId = slot4
		})
	end
...
slot0.CheckFollowerFurniture = function (slot0, slot1, slot2)
