...
slot0.GetFurnituresAndPapers = function (slot0)
	slot1 = {}
	...
	for slot6 = slot0.MAX_SIZE_X, 0, -1 do
		...
		for slot11 = slot0.MAX_SIZE_Y, 0, -1 do
			...
			elseif slot11 < slot0.startY or slot6 < slot0.startX then
				...
				table.insert(slot7, (not slot0:canMoveBoat(nil, Vector2(slot6, slot11)) or 0) and 1)
			end
		end
...
slot0.ContainsFurnitrue = function (slot0, slot1)
...
slot0.getMoveableFurnitureNextDir = function (slot0, slot1, slot2, slot3)
...
slot0.canMoveFurniture = function (slot0, slot1, slot2, slot3)
...
slot0.checkFloorOccupyGrids = function (slot0, slot1, slot2, slot3)
	slot4 = true

	return slot0:checkOccupyGrids(slot1, slot2, slot3) and slot0:checkChildOccupyGrids(slot1, slot2, slot3)
end

slot0.checkOccupyGrids = function (slot0, slot1, slot2, slot3)
	slot4 = true
...
slot0.checkChildOccupyGrids = function (slot0, slot1, slot2, slot3)
	slot4 = true
...
slot0.isOccupyFurnPos = function (slot0, slot1, slot2, slot3)
	...
	if slot3 then
		slot4, locatedFurnVO = slot0.isLocatedOnFurn(slot0, slot3, slot1, slot2)
	end
	...
	if slot0.furnitures[slot2]:isStageFurniture() then
		...
		for slot9, slot10 in pairs(slot0.ships) do
			if slot1 ~= slot10.id then
				...
				if slot10.getPosition(slot10) and slot12.x == slot3.x and slot12.y == slot3.y then
					return false
				end
...
slot0.getCanMoveNearerPosOnFurnitrue = function (slot0, slot1, slot2)
	slot4 = slot0.furnitures[slot2].getCanPutOnGrid(slot3, slot0.furnitures[slot2].position)
...
slot0.getTransportPoint = function (slot0, slot1, slot2)
	if slot0.furnitures[slot2]:isTransPort() and slot0:hasEmptyGrid() then
		...
		end(slot3.getTransportPoint(slot3))
	else
...
slot0.getGridForMoveableFurniture = function (slot0, slot1)
...
slot0.GetFurnitruesOccupyPoints = function (slot0)
	slot1 = {}
...
slot0.GetInteractionableSpineFurnitureId = function (slot0, slot1)
	...
	slot3 = slot0.GetFurnitruesOccupyPoints(slot0)
...
slot0.GetAllOccupyPoints = function (slot0)
	slot1 = {}
...
slot0.GetSurroundGridByPoint = function (slot0, slot1, slot2, slot3, slot4)
	slot6 = {}
	...
	function slot8(slot0)
		if not table.contains(slot0, slot0) and slot1:IsLegalPoint(slot0) and not table.contains(table.contains, slot0) then
	...
	function slot9(slot0, slot1, slot2, slot3)
		if not slot0 and slot1 == slot1 then
			return false
		end

		for slot7 = slot1.x, (slot1.x + slot2) - 1, 1 do
			for slot11 = slot1.y, (slot1.y + slot3) - 1, 1 do
				if table.contains(slot0, Vector2(slot7, slot11)) or not slot2:IsLegalPoint(Vector2(slot7, slot11)) then
					return false
				end
			end
		end
	...
	while #slot5 > 0 do
		...
		if slot9(slot7, table.remove(slot5, 1), slot2, slot3) then
			return slot10
		else
...
slot0.IsLegalPoint = function (slot0, slot1)
