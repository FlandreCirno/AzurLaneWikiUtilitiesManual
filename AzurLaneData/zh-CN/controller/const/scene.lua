...
SCENE.SetSceneInfo = function (slot0, slot1)
	...
	elseif slot1 == SCENE.INVITATION then
		slot0.mediator = SCENE.GetInvitationPage(slot0.data.itemVO).mediator
		slot0.viewComponent = SCENE.GetInvitationPage(slot0.data.itemVO).viewComponent
...
SCENE.GetInvitationPage = function (slot0)
	slot2 = slot0[slot0:getTempCfgTable().open_ui[1]]

	class("TempAssignedShipScene", import("view.activity.AssignedShipScene")).getUIName = function (slot0)
		return slot0
	end

	if slot0.getTempCfgTable().open_ui[1] == "greeting_year" then
		slot3.init = function (slot0)
			slot0.super.init(slot0)

			slot0.backBtn = slot0._tf:Find("layer/top/back")
			slot0.rtTitle = slot0._tf:Find("layer/top/title")

			slot0.itemList:make(function (slot0, slot1, slot2)
				slot3 = slot0.shipIdList[slot1 + 1]

				if slot0 == UIItemList.EventUpdate then
					GetImageSpriteFromAtlasAsync("extra_page/" .. slot0.style .. "/i_" .. slot3, "", slot2:Find("unselected/icon"))
					GetImageSpriteFromAtlasAsync("extra_page/" .. slot0.style .. "/i_" .. slot3, "", slot2:Find("selected/icon"))
