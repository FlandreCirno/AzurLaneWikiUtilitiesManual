	...
	pg.ConnectionMgr.GetInstance():Send(26103, {
		...
		if slot0.result == 0 then
			...
			if slot0.data.id > 0 then
				MiniGameDataCreator.DataCreateFunc(slot0, slot1, slot0.data.datas, slot0.data.date1_key_value_list)
			end

			if slot0 == slot2.CMD_COMPLETE and slot1:GetHubByHubId(slot1.GetHubByHubId):getConfig("reward_target") ~= "" and slot4 ~= 0 then
				table.insert(slot2, {
					count = 1,
			...
			slot4:sendNotification(GAME.SEND_MINI_GAME_OP_DONE, {
				awards = slot2,
				hubid = slot3,
				cmd = slot0,
				argList = slot1
