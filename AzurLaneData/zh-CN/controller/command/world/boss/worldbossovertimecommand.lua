class("WorldBossOverTimeCommand", pm.SimpleCommand).execute = function (slot0, slot1)
	slot2 = slot1:getBody()
	...
	if nowWorld.worldBossProxy:GetSelfBoss() and slot5:IsExpired() then
		...
		slot4.ClearRank(slot4, slot5.id)
		slot4:RemoveSelfBoss()
		slot0:sendNotification(GAME.WORLD_SELF_BOSS_OVERTIME_DONE)
	end
end

return class("WorldBossOverTimeCommand", pm.SimpleCommand)
