class("CheckHotfixCommand", pm.SimpleCommand).execute = function (slot0, slot1)
	...
	if BundleWizard.Inst:GetGroupMgr("DEFAULT_RES").CurrentVersion.Major > 0 and (not slot3.lastRequestVersionTime or Time.realtimeSinceStartup - slot3.lastRequestVersionTime > 1800) then
		...
		VersionMgr.Inst:FetchVersion(function (slot0)
			...
			if slot1.CurrentVersion.Major < false.Major or (slot0.Major == slot1.CurrentVersion.Major and slot1.CurrentVersion.Minor < slot0.Minor) or (slot0.Major == slot1.CurrentVersion.Major and slot0.Minor == slot1.CurrentVersion.Minor and slot1.CurrentVersion.Build < slot0.Build) then
...
return class("CheckHotfixCommand", pm.SimpleCommand)
