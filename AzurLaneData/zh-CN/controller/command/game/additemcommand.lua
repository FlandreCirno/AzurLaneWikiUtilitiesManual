class("AddItemCommand", pm.SimpleCommand).execute = function (slot0, slot1)
	...
	elseif slot2.dropType == DROP_TYPE_VITEM then
		...
		elseif slot4 == 1 then
			...
			slot7 = nil
			slot7 = (slot3:getConfig("link_id") <= 0 or slot5:getActivityById(slot6)) and slot5:getActivityByType(ActivityConst.ACTIVITY_TYPE_PUZZLA)
		...
		elseif slot4 == 2 or slot4 == 3 then
			...
			if slot4 == 2 then
				getProxy(ActivityProxy).GetVoteActivity(slot6).data1 = getProxy(ActivityProxy).GetVoteActivity(slot6).data1 + slot3.count
				slot5.votes = slot5.votes + slot3.count
			end
		elseif slot4 == 4 then
			slot6[slot3.id] = (getProxy(ColoringProxy).getColorItems(slot5)[slot3.id] or 0) + slot3.count
		elseif slot4 == 6 then
		...
		elseif slot4 == 20 and math.min(pg.gameset.urpt_chapter_max.description[2] - getProxy(BagProxy).GetLimitCntById(slot5, slot7), slot2.count) > 0 then
	...
	elseif slot2.dropType == DROP_TYPE_WORLD_ITEM then
		nowWorld.GetInventoryProxy(slot3):AddItem(slot2.id, slot2.count)
