...
slot0.mainHandler = function (slot0, slot1)
	...
	for slot17, slot18 in ipairs(getProxy(ActivityProxy).getPanelActivities(slot13)) do
		function slot19()
			...
			if slot0:getConfig("type") == ActivityConst.ACTIVITY_TYPE_BB then
				slot0.data2 = 0
			elseif slot0:getConfig("type") == ActivityConst.ACTIVITY_TYPE_LOTTERY_AWARD then
				slot0.data2 = 0
			end

			return table.contains(slot0, slot0:getConfig("type"))
		end
	...
	slot23 = nowWorld
	slot24 = pg.TimeMgr.GetInstance():GetServerWeek() == 1
	...
	if slot23 then
		slot23:GetBossProxy().increasePt(slot25)

		if slot13:getActivityByType(ActivityConst.ACTIVITY_TYPE_WORLD_WORLDBOSS) and not slot26:isEnd() then
			slot26.data1 = math.floor(pg.gameset.joint_boss_ticket.description[1] * slot26.data1 / pg.gameset.joint_boss_ticket.description[math.min(#pg.gameset.joint_boss_ticket.description, slot26.data2 + 1)])
			slot26.data2 = 0
	...
	if slot28:GetPublicGuild() then
		slot31 = math.random(2, 5)
		slot32 = nil

		Timer.New(function ()
			slot0:Stop()
			slot0:sendNotification(GAME.GET_PUBLIC_GUILD_USER_DATA, {
				flag = true
			})
		end, slot31, 1):Start()
	end
