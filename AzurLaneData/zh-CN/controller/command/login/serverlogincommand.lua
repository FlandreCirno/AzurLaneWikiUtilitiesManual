...
slot0.execute = function (slot0, slot1)
		...
		slot1(pg.ConnectionMgr.GetInstance(), 10022, {
			...
			if slot0.result == 0 then
				...
				if slot0.status == Server.STATUS.REGISTER_FULL and slot0.user_id == 0 then
					...
					slot0.id:SaveCacheGatewayFlag(slot1.arg2)
			...
			elseif slot0.result == 17 then
				slot3:sendNotification(GAME.SERVER_LOGIN_FAILED_USER_BANNED, slot0.user_id)
	...
	if math.abs(os.time() - slot0.LoginLastTime) > 1 or slot0.LoginSafeLock >= 5 then
		...
		if pg.ConnectionMgr.GetInstance().getConnection(slot10) and pg.ConnectionMgr.GetInstance():isConnected() then
			slot8()
		else
		...
		pg.TipsMgr.GetInstance().ShowTips(slot10, i18n("login_game_frequence"))
	end
end
