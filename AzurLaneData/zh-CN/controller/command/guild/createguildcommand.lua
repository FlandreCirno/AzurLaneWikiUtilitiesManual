class("CreateGuildCommand", pm.SimpleCommand).execute = function (slot0, slot1)
	...
	pg.ConnectionMgr.GetInstance():Send(60001, {
		...
		if slot0.result == 0 then
			...
			slot1:StartTech(slot4)
			slot2:addGuild(slot1)
			slot1:consume({
				gem = slot2
			})
			slot3:updatePlayer(slot1)
			slot4:sendNotification(GAME.HANDLE_GUILD_AND_PUBLIC_GUILD_TECH)
			slot4:sendNotification(GAME.CREATE_GUILD_DONE)
			slot4:sendNotification(GAME.GUILD_GET_USER_INFO)
