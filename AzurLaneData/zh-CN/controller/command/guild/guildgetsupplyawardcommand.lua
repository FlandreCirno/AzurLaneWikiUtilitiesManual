class("GuildGetSupplyAwardCommand", pm.SimpleCommand).execute = function (slot0, slot1)
	slot2 = slot1:getBody()
	...
	pg.ConnectionMgr.GetInstance():Send(62009, {
		...
		if slot0.result == 0 then
			...
			if not slot0:getData():ExistSupply() then
				slot1 = slot2:GetSupplyEndTime()
			end

			slot2:updateSupplyTime(slot1)
			slot0:updateGuild(slot2)
			slot1:sendNotification(GAME.GUILD_GET_SUPPLY_AWARD_DONE, {
...
return class("GuildGetSupplyAwardCommand", pm.SimpleCommand)
