class("GuildRefreshMissionCommand", import(".GuildEventBaseCommand")).execute = function (slot0, slot1)
	slot2 = slot1:getBody()
	...
	pg.ConnectionMgr.GetInstance():Send(61023, {
		...
		if slot0.result == 0 then
			slot4 = getProxy(GuildProxy).getData(slot1).GetActiveEvent(slot2).GetMissionById(slot3, slot0)
			...
			slot1:sendNotification(GAME.GUILD_REFRESH_MISSION_DONE, {
				id = slot4.id
			})
...
return class("GuildRefreshMissionCommand", import(".GuildEventBaseCommand"))
