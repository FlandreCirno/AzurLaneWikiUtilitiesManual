class("GetGuildBossInfoCommand", import(".GuildEventBaseCommand")).execute = function (slot0, slot1)
	slot2 = slot1:getBody()
	...
	pg.ConnectionMgr.GetInstance():Send(61027, {
		...
		if slot0.result == 0 then
			...
			slot2:GetActiveEvent().GetBossMission(slot3).Flush(slot4, slot0.boss_event)
			slot1:updateGuild(slot2)
			slot1:SetRefreshBossTime(pg.TimeMgr.GetInstance():GetServerTime())
			slot0:sendNotification(GAME.GUILD_GET_BOSS_INFO_DONE)
		elseif slot0.result == 20 then
			slot3 = false

			if getProxy(GuildProxy):getData().GetActiveEvent(slot1) then
				slot2:Deactivate()
			...
			slot0:sendNotification(GAME.GUILD_END_BATTLE)
		else
...
return class("GetGuildBossInfoCommand", import(".GuildEventBaseCommand"))
