...
slot0.GetMissionById = function (slot0, slot1)
	if slot0:ExistMission(slot1) then
		return getProxy(GuildProxy):getRawData().GetActiveEvent(slot2):GetMissionById(slot1)
	end
end
...
slot0.CanFormationMission = function (slot0, slot1)
	...
	if getProxy(GuildProxy):getRawData().GetActiveEvent(slot2).GetMissionById(slot3, slot1):GetCanFormationIndex() == -1 then
...
slot0.ExistBoss = function (slot0)
	...
	if not getProxy(GuildProxy):getRawData().GetActiveEvent(slot1):GetBossMission() or not slot3:IsActive() then
...
slot0.CheckCapital = function (slot0, slot1, slot2)
	if getProxy(GuildProxy):getRawData().getCapital(slot3) < slot1:GetConsume() then
