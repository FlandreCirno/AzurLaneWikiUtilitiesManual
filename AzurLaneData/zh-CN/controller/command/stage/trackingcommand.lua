...
slot0.execute = function (slot0, slot1)
	...
	slot13 = getProxy(BayProxy).getRawData(slot12)

	if not getProxy(PlayerProxy).getData(slot15):isEnough({
		oil = slot10:getConfig("oil") * slot0.CalculateSpItemMoreCostRate(slot5)
	}) then
	...
	for slot25, slot26 in ipairs(slot19) do
		if slot10:singleEliteFleetVertify(slot25) then
			...
			for slot34, slot35 in pairs(slot30) do
				table.insert(slot29, {
					pos = slot34,
					id = slot35
				})
	...
	pg.ConnectionMgr.GetInstance():Send(13101, {
		...
		if slot0.result == 0 then
			slot0:update(slot0.current_chapter)
			slot0.update:consume({
				oil = slot2 * slot0:GetExtraCostRate()
			})
			slot3:updatePlayer(slot3.updatePlayer)

			if slot2 * slot0.GetExtraCostRate() ~= 0 then
				getProxy(BagProxy):removeItemById(slot4, 1)
			end
			...
			slot5:updateChapter(slot0)

			if slot6:isEscort() then
				slot5.escortChallengeTimes = slot5.escortChallengeTimes + 1
			end

			if slot6:isRemaster() then
				slot5.remasterTickets = slot5.remasterTickets - 1
			end

			slot7:sendNotification(GAME.TRACKING_DONE, slot0)
			getProxy(ChapterProxy):updateExtraFlag(slot0, slot0.extraFlagList, {}, true)

			slot1 = slot8.autoFightFlag

			if slot9 ~= 0 and slot1 then
				getProxy(ChapterProxy):SetChapterAutoFlag(slot10, true)
			end
		elseif slot0.result == 1 then
			pg.TipsMgr.GetInstance():ShowTips(i18n("levelScene_tracking_error_retry"))
			slot7:sendNotification(GAME.CHAPTER_OP, {
...
slot0.CalculateSpItemMoreCostRate = function (slot0)
	...
	slot4 = _.map(string.split(string.sub(slot3, 2, -2), ","), function (slot0)
