class("TaskGoCommand", pm.SimpleCommand).execute = function (slot0, slot1)
	...
	slot10 = math.fmod(slot3:getConfig("sub_type"), 10)
	...
	elseif slot9 == 2 then
		...
		elseif slot10 > 7 or (type(slot11) == "string" and tonumber(slot11) == 0) then
			...
			if type(slot11) == "table" and _.all(slot11, function (slot0)
				return not slot0:getChapterById(slot0):isUnlock()
			end) then
				pg.TipsMgr.GetInstance().ShowTips(slot12, i18n("battle_levelScene_lock_1"))
