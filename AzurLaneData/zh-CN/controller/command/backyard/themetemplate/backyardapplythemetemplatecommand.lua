...
slot0.execute = function (slot0, slot1)
	...
	function slot6(slot0, slot1)
		...
		pg.m02:sendNotification(GAME.PUT_FURNITURE, {
			...
			callback = function (slot0, slot1)
				if slot0 then
					slot0:sendNotification(GAME.BACKYARD_APPLY_THEME_TEMPLATE_DONE)
				else
	...
	slot7 = 1
	slot9 = slot1.getBody().template.GetAllFurniture(slot3)
	slot11 = {}

	if slot1.getBody().template.IsOccupyed(slot3, slot0.GetAllFloorFurnitures(), 1) then
...
slot0.WarpList = function (slot0)
	...
	function slot6(slot0)
		return not slot0:isPaper() and (slot0.position.x < slot0 or slot0.position.y < )
	end

	slot7 = slot1.level
	slot8 = slot1.GetAllFurniture(slot1)
