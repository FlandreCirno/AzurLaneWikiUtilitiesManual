return {
	...
	},
	{
		...
		isTip = function ()
			...
			return _.any(getProxy(ShipSkinProxy).GetAllSkins(slot3), function (slot0)
				return slot0:getConfig("genre") == ShopArgs.SkinShopTimeLimit and not slot0:hasSkin(slot0:getSkinId())
			end)
		end
	},
	{
	...
	},
	{
		...
		isTip = function ()
			if getProxy(ActivityProxy):getActivityByType(ActivityConst.ACTIVITY_TYPE_AIRFIGHT_BATTLE) and not slot0:isEnd() then
				...
				for slot6 = 1, slot0:getConfig("config_client")[1], 1 do
	...
	},
	{
		...
		isShow = function ()
			return getProxy(ActivityProxy):getActivityById(4100) and not slot1:isEnd()
		end,
		isTip = function ()
			slot0 = 970703
			slot1 = getProxy(MetaCharacterProxy):getMetaProgressVOByID(970703)
			...
			if slot2 == false then
				slot2 = getProxy(MetaCharacterProxy):getRedTag(970703)
			end
	...
	},
	{
		...
		isTip = function ()
			...
			if pg.activity_event_worldboss[slot0:getConfig("config_id")] then
				slot2 = slot4.time ~= "stop" and pg.TimeMgr.GetInstance():parseTimeFromConfig(slot4.time[2]) and pg.TimeMgr.GetInstance():GetServerTime() <= (slot4.time ~= "stop" and pg.TimeMgr.GetInstance().parseTimeFromConfig(slot4.time[2]))
				slot2 = not slot2
			end
