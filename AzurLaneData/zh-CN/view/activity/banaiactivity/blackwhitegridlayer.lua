	...
	return setmetatable(slot1, {
		__index = slot11({
			...
			Primary = function (slot0, slot1)
				...
				if slot0.primaryCells and slot0.primaryCell and slot1.IsSame(slot1, slot0.primaryCell) then
			...
			Serialize = function (slot0)
				...
				return slot1 .. "}#" .. slot0.calcStep .. "#" .. ((slot0.started and "1") or "0")
			end,
			Dispose = function (slot0)
				_.each(slot0.cells, function (slot0)
					slot0:Dispose()
...
slot0.didEnter = function (slot0)
	...
	if slot0.GetLastestUnlockMap(slot0) then
		triggerButton(slot4)
	end
...
slot0.isUnlock = function (slot0, slot1)
	return getProxy(ChapterProxy):getChapterById(slot1.unlock[2]) and slot4:isUnlock() and slot4:isAllAchieve() and (slot1.unlock[1] == 0 or table.contains(slot0.passIds, slot1.unlock[1]))
end
...
slot0.playStory = function (slot0, slot1)
	if slot0[slot0.mapView.map.id].story and slot2 ~= "" then
