...
slot0.InitInteractable = function (slot0)
	...
	onToggle(slot0, slot0.commanderToggle, function (slot0)
		if slot0 then
			slot0.viewParent.contextData.showCommander = slot0
	...
	onToggle(slot0, slot0.formationToggle, function (slot0)
		if slot0 then
			slot0.viewParent.contextData.showCommander = not slot0
...
slot0.SetSettings = function (slot0, slot1, slot2, slot3, slot4)
...
slot0.UpdateView = function (slot0)
	slot0:clearFleets()
	slot0:UpdateFleets()
	triggerToggle((slot0.viewParent.contextData.showCommander and not LOCK_COMMANDER and pg.SystemOpenMgr.GetInstance():isOpenSystem(getProxy(PlayerProxy):getRawData().level, "CommandRoomMediator") and slot0.useCMD and slot0.commanderToggle) or slot0.formationToggle, true)
...
slot0.updateFleet = function (slot0, slot1, slot2)
	...
	if slot3 then
		...
		onButton(slot0, slot10, function ()
			slot0:emit(slot0.viewParent.contextData.mediatorClass.ON_FLEET_RECOMMEND, slot1.id)
		end)
		onButton(slot0, slot11, function ()
			slot0:emit(slot0.viewParent.contextData.mediatorClass.ON_FLEET_CLEAR, slot1.id)
		end, SFX_UI_CLICK)
	end
end
...
slot0.updateShips = function (slot0, slot1, slot2, slot3, slot4)
	...
	for slot9 = 1, 3, 1 do
		...
		onButton(slot0, slot14, function ()
			slot0:emit(slot0.viewParent.contextData.mediatorClass.ON_OPEN_DOCK, {
				fleet = slot1,
				shipVO = slot2,
				fleetIndex = slot3,
				teamType = slot3
...
slot0.updateCommanderBtn = function (slot0, slot1, slot2)
	...
	setActive(slot0:findTF("btn_clear", slot5), slot2 <= slot0:getLimitNums(slot1) and not slot0.viewParent.contextData.showCommander)
	setActive(slot0:findTF("btn_recom", slot5), slot2 <= slot0.getLimitNums(slot1) and not slot0.viewParent.contextData.showCommander)
	setActive(slot0:findTF("commander", slot5), slot2 <= slot0.getLimitNums(slot1) and slot2 <= slot0.getLimitNums(slot1) and slot0.fleets[slot1][slot2] and slot0.viewParent.contextData.showCommander)
	setActive(slot0:findTF("blank", slot5), not (slot2 <= slot0.getLimitNums(slot1)) or (slot2 <= slot0.getLimitNums(slot1) and not (slot2 <= slot0.getLimitNums(slot1) and slot0.fleets[slot1][slot2]) and slot0.viewParent.contextData.showCommander))
end
