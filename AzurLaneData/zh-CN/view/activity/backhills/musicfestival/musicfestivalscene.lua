...
slot0.didEnter = function (slot0)
	...
	slot0.InitFacility(slot0, slot0.stage, function ()
		pg.m02:sendNotification(GAME.GO_MINI_GAME, 6)
	end)
	slot0.InitFacility(slot0, slot0.screen, function ()
		slot0:emit(MusicFestivalMediator.GO_SCENE, SCENE.ACTIVITY, {
			id = ActivityConst.IDOL_PT_ID
		})
	end)
	slot0.InitFacility(slot0, slot0.shop, function ()
		slot0:emit(MusicFestivalMediator.GO_SCENE, SCENE.SHOP, {
			warp = NewShopsScene.TYPE_ACTIVITY
		})
	end)
	slot0.InitFacility(slot0, slot0.painting, function ()
		slot0:emit(MusicFestivalMediator.GO_SCENE, SCENE.SKINSHOP)
	end)
	slot0.InitFacility(slot0, slot0.cube, function ()
		slot0:emit(MusicFestivalMediator.GO_SCENE, SCENE.GETBOAT, {
			projectName = "new",
			page = 1
		})
	end)
	slot0.InitFacility(slot0, slot0.foutain, function ()
		slot0.emit(slot1, MusicFestivalMediator.GO_SUBLAYER, Context.New({
	...
	slot0.InitFacility(slot0, slot0.door, function ()
...
slot0.updateStudents = function (slot0)
	for slot5, slot6 in pairs(slot1) do
		if not slot0.academyStudents[slot5] then
			...
			slot8.updateStudent(slot8, slot6)
