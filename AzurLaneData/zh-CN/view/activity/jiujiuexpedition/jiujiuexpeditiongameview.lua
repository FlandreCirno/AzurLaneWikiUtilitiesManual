...
slot0.didEnter = function (slot0)
	...
	slot0.stgDatas = pg.activity_template[slot0.activityId].config_data
	slot0.stgAmount = #pg.activity_template[slot0.activityId].config_data
	slot0.uiAtlasName = slot0:getUIName()
	slot2 = findTF(slot0._tf, "ad")

	onButton(slot0, findTF(slot2, "back"), function ()
		if slot0.isTweening > 0 then
			return
		end
	...
	for slot6 = 1, slot0.stgAmount, 1 do
	...
	pg.m02.sendNotification(slot3, GAME.ACTIVITY_OPERATION, {
		cmd = 0,
...
slot0.activityUpdate = function (slot0)
	...
	if #slot0.gridTypes == 0 then
		...
		for slot5 = 1, slot0.chequerMap[1] * slot0.chequerMap[2], 1 do
...
slot0.showBookUnLock = function (slot0)
	...
	LeanTween.delayedCall(go(slot0.bookUnLock), 3, System.Action(function ()
		LeanTween.alpha(rtf(slot0.bookUnLock), 0, 2)
	end))
end
...
slot0.showBaoxiang = function (slot0, slot1, slot2, slot3, slot4)
	...
	LeanTween.delayedCall(go(slot4), 0.5, System.Action(function ()
		slot4, slot1 = slot0:getPosition(slot0, )
		slot0.localPosition = Vector3(slot0, slot1 + 50, -1)

		setActive(Vector3(slot0, slot1 + 50, -1), true)
		setActive(findTF(findTF, "baoxiang_guan"), true)
		LeanTween.moveLocal(go(go), Vector3(slot0, slot1, -1), 0.2)

		slot0.isTweening = slot0.isTweening - 1

		onButton(slot0, , function ()
			if not slot0.isMoveChar and not slot0.isOpenBaoxiang then
				slot0.isOpenBaoxiang = true

				slot0:openBaoxiang(slot0, )
			end
		end)
	end))
end
...
slot0.openBaoxiang = function (slot0, slot1, slot2)
	...
	LeanTween.delayedCall(go(slot1), 1, System.Action(function ()
		...
		slot0:getGridReward(slot0)

		for slot3 = #slot0.getGridReward.baoxiangList, 1, -1 do
			if slot0.baoxiangList[slot3].tf == slot2 then
				table.remove(slot0.baoxiangList, slot3)
			end
		end

		slot0:returnBaoxiang(slot2)

		slot0.returnBaoxiang.isOpenBaoxiang = false
	end))
end
	...
	if not slot0.complete and slot0.curSelectLevel == slot0.level and slot0.charPos > 0 then
		if slot0.charPos ~= slot0.curCharPos then
			...
			if slot0:getMapByIndex(slot0.charPos) then
				...
				slot6, slot7 = slot0:getPosition(slot1.v, slot1.h)

				slot0:moveChar(slot2, slot3, function ()
					slot0.isMoveChar = false

					slot0:checkExpeditionMap()
				end)
			end
		else
			slot0.checkExpeditionMap(slot0)
		end
	else
	...
	for slot4 = 1, #slot0.mapDic, 1 do
		...
		if slot0.curSelectLevel < slot0.level then
			...
			if (slot0.charPos <= 0 or not slot0.charPos) and slot0.tickets > 0 then
				slot0:setMapGridType(slot5, slot0)
...
slot0.updateConfig = function (slot0)
	...
	for slot5 = 1, #Clone(slot0.chequerConfig.empty_grid), 1 do
...
slot0.getGridSideOpen = function (slot0, slot1)
	...
	slot3 = (slot1.h % 2 ~= 1 or {
		slot1.posNum - 1,
		slot1.posNum + 1,
		slot1.posNum - slot0.chequerMap[2],
		slot1.posNum + slot0.chequerMap[2],
		(slot1.posNum + slot0.chequerMap[2]) - 1,
		slot1.posNum + slot0.chequerMap[2] + 1
	}) and {
		slot1.posNum - 1,
		slot1.posNum + 1,
	...
	for slot9 = #slot3, 1, -1 do
		slot12 = (slot3[slot9] - 1) % slot0.chequerMap[2] + 1

		if math.abs(math.ceil(slot3[slot9] / slot0.chequerMap[2]) - slot4) > 1 or math.abs(slot12 - slot5) > 1 then
...
slot0.updateMap = function (slot0)
	...
	for slot8 = 1, slot0.chequerMap[1], 1 do
		for slot12 = 1, slot3, 1 do
			...
			if not table.contains(slot0.emptyPosNums, slot0:getPosNum(slot8, slot12)) then
				...
				slot17 = {
					mask = slot0:getMask(),
					bottomGrid = slot0:getBottomGrid(),
					upGrid = slot16,
					v = slot8,
					h = slot12,
					posNum = slot13,
				...
				onButton(slot0, slot16, function ()
					slot0:onClickGrid(slot0)
				end, SFX_CONFIRM)
				table.insert(slot0.mapDic, slot17)
			end
		end
	end
end
...
slot0.moveChar = function (slot0, slot1, slot2, slot3)
	...
	if isActive(slot0.charactor) then
		slot0:hideChar(function ()
			slot0:showChar(slot0, , )
		end)
	else
		slot0.showChar(slot0, slot1, slot2, slot3)
	end
end

slot0.showChar = function (slot0, slot1, slot2, slot3)
	slot0.charactor.localPosition = Vector3(slot1, slot2 + slot0)

	setActive(slot0.charactor, true)
	LeanTween.value(go(slot0.charactor), 0, 1, 0.2):setOnUpdate(System.Action_float(function (slot0)
		GetComponent(slot0.charactor, typeof(CanvasGroup)).alpha = slot0
	end))
...
slot0.hideChar = function (slot0, slot1)
	...
	LeanTween.moveLocal(go(slot0.charactor), Vector3(slot0.charactor.localPosition.x, slot0.charactor.localPosition.y + slot0, 0), 0.2).setOnComplete(slot3, System.Action(function ()
		setActive(slot0.charactor, false)

		if slot0.charactor then
			slot1()
		end
	end))
end
...
slot0.clear = function (slot0, slot1)
	for slot5 = 1, #slot0.mapDic, 1 do
		slot0:returnBottomGrid(slot0.mapDic[slot5].bottomGrid)
		slot0:returnMask(slot0.mapDic[slot5].mask)
		slot0:returnUpGrid(slot0.mapDic[slot5].upGrid)
	end
...
slot0.willExit = function (slot0)
	...
	for slot4 = 1, #slot0.baoxiangList, 1 do
