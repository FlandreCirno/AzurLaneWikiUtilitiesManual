...
slot0.Init = function (slot0, slot1, slot2)
	...
	seriesAsync({
		function (slot0)
			slot0:InitBlock(slot0)
		end,
		function (slot0)
			slot0:InitPlayer(slot0.InitPlayer, slot0)
		end,
		function (slot0)
			slot0:InitGround(slot0.InitGround, slot0)
		end,
		function (slot0)
			slot0.player:SetPosition(slot0.blocks[1].position)
			slot0:SendMapEvent("OnPlayerLifeUpdate", slot0.player.life)
			slot0()
		end
	}, slot2)
end
...
slot0.InitBlock = function (slot0, slot1)
	...
	for slot7 = 1, TowerClimbingGameSettings.GetBlockInitCnt(slot0.mapHeight), 1 do
		table.insert(slot2, function (slot0)
			table.insert(slot0.blocks, slot1)
			slot0:SendMapEvent("OnCreateBlock", slot0:CreateBlock(), slot0)
		end)
	end
...
slot0.CreateBlock = function (slot0)
	...
	slot2 = slot0(slot0.nextBlockIndex, slot0.id)

	return {
		id = i,
		type = slot2[1],
		width = slot2[2],
		position = slot1(slot2[2], slot1),
		isActive = not slot0.blocks[#slot0.blocks],
...
slot0.AddNewBlock = function (slot0, slot1)
	...
	slot0:SendMapEvent("OnCreateBlock", slot0:CreateBlock(), slot1)
end

slot0.DoSink = function (slot0, slot1, slot2, slot3)
	table.insert(slot4, function (slot0)
		slot0:SendMapEvent("SinkHandler", TowerClimbingGameSettings.SINK_DISTANCE * slot0.SendMapEvent)
	end)
	table.insert(slot4, 1, function (slot0)
		if not slot0.ground.IsRuning then
			slot0()

			return
		end

		slot0.ground.position = Vector2(slot0.ground.position.x, slot2)
...
slot0.OverHigestScore = function (slot0)
	function slot1(slot0)
		for slot4, slot5 in ipairs(slot0.awards) do
			if slot0 == slot5 then
				return true
			end
		end
	...
	if slot0.player.IsOverHigestScore(slot2) and slot1(slot0.player.score) then
...
slot0.DoCheck = function (slot0, slot1)
	...
	seriesAsync({
		function (slot0)
			slot0:AddNewBlock(slot0)
		end,
		function (slot0)
			parallelAsync({
				function (slot0)
					slot0:DoSink(slot0.higestLevel, slot0.DoSink, slot0)
				end,
				function (slot0)
					slot1:SendMapEvent("OnSink", TowerClimbingGameSettings.SINK_DISTANCE * slot0, slot0)
				end
			}, slot0)
		end,
		function (slot0)
			slot0:CheckCorssBoundBlocks(slot0)
		end,
		function (slot0)
			slot0:CheckGroundState()
			slot0()
		end
	})
end
...
slot0.ReBornPlayer = function (slot0)
	...
	if not _.detect(slot1, function (slot0)
		return slot0.level == slot0.higestLevel
	end) then
	...
	slot0.player.SetPosition(slot4, slot3.position + Vector2(0, 10))
end
...
function slot3(slot0)
	slot2 = TowerClimbingGameSettings.GROUND_RISE_UP_SPEED[#TowerClimbingGameSettings.GROUND_RISE_UP_SPEED][2]
...
slot0.Update = function (slot0)
	...
	if slot0.ground.IsRuning and slot0.ground.sleepTime <= 0 then
		slot0.ground.position = Vector2(slot0.ground.position.x, slot0.ground.position.y + slot0(slot0.higestLevel) * Time.deltaTime)
...
slot0.SendMapEvent = function (slot0, slot1, ...)
	slot0.view.map[slot1](slot0.view.map, unpack({
		...
	}))
end
