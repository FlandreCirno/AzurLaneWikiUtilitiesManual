...
slot0.OnLoaded = function (slot0, slot1)
	...
	slot0.physics2DItem.CollisionEnter:AddListener(function (slot0)
		if slot0.map:GetHitBlock(slot0.collider.gameObject) and slot0.collider.name == TowerClimbingGameSettings.BLOCK_NAME and slot0.contacts.Length > 0 then
			slot0.map:SendEvent("EnterBlock", slot0.contacts[0], slot1.block.level)
		end

		if slot0.collider.name == TowerClimbingGameSettings.FIRE_NAME then
			slot0.map:SendEvent("EnterAttacker")
		end
		...
		if slot0.collider.name == TowerClimbingGameSettings.GROUND_NAME then
			slot0.map:SendEvent("EnterGround")
		end
	end)
	slot0.physics2DItem.CollisionStay.AddListener(slot2, function (slot0)
		slot1 = {}

		for slot5 = 1, slot0.contacts.Length, 1 do
			table.insert(slot1, slot0.contacts[slot5 - 1])
		end

		if slot0.collider.name == TowerClimbingGameSettings.BLOCK_NAME then
			slot0.map:SendEvent("StayBlock", slot1, slot0.rigbody.velocity)
		end
	end)
	slot0.physics2DItem.CollisionExit.AddListener(slot2, function (slot0)
		slot1 = slot0.map:GetHitBlock(slot0.collider.gameObject)

		if slot0.collider.name == TowerClimbingGameSettings.BLOCK_NAME then
			slot0.map:SendEvent("ExitBlock", slot1.block.level)
		end
	end)

	slot0.spineAnim = slot0._go.GetComponent(slot2, "SpineAnimUI")

	SetParent(slot1, slot0.map._tf:Find("game/block_play_con"))

	slot1.name = "player"
	slot0._tf.localScale = Vector3(slot0, slot0, 1)
...
slot0.BeFatalInjured = function (slot0, slot1)
	slot0.spineAnim:SetActionCallBack(function (slot0)
		if slot0 == "finish" then
			...
			slot0.spineAnim.SetActionCallBack()
		end
	end)

	slot0.action = "dead"

	slot0.spineAnim.SetAction(slot2, slot0.action, 0)
end
