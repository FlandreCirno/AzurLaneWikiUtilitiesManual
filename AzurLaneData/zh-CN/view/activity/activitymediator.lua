...
slot0.register = function (slot0)
	...
	slot0.bind(slot0, slot0.OPEN_VOTEBOOK, function (slot0)
		slot0:addSubLayers(Context.New({
			mediator = VoteOrderBookMediator,
			viewComponent = VoteOrderBookLayer
		}))
	end)
	slot0.bind(slot0, slot0.GO_SUBMARINE_RUN, function (slot0, slot1)
		slot0:sendNotification(GAME.BEGIN_STAGE, {
			system = SYSTEM_SUBMARINE_RUN,
			stageId = slot1
		})
	end)
	slot0.bind(slot0, slot0.GO_DODGEM, function (slot0)
		slot0:sendNotification(GAME.BEGIN_STAGE, {
	...
	slot0.bind(slot0, slot0.ON_SIMULATION_COMBAT, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.BEGIN_STAGE, {
	...
	slot0.bind(slot0, slot0.ON_AIRFIGHT_COMBAT, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.BEGIN_STAGE, {
	...
	slot0.bind(slot0, slot0.RETURN_AWARD_OP, function (slot0, slot1)
	...
	slot0.bind(slot0, slot0.SHOW_AWARD_WINDOW, function (slot0, slot1, slot2)
		slot0.viewComponent:ShowWindow(slot1, slot2)
	end)
	slot0.bind(slot0, slot0.EVENT_PT_OPERATION, function (slot0, slot1)
		slot0:sendNotification(GAME.ACT_NEW_PT, slot1)
	end)
	slot0.bind(slot0, slot0.OPEN_LAYER, function (slot0, slot1)
		slot0:addSubLayers(slot1)
	end)
	slot0.bind(slot0, slot0.OPEN_RED_PACKET_LAYER, function (slot0)
		slot0:addSubLayers(Context.New({
			mediator = RedPacketMediator,
			viewComponent = RedPacketLayer
		}))
	end)
	slot0.bind(slot0, slot0.CLOSE_LAYER, function (slot0, slot1)
	...
	slot0.bind(slot0, slot0.EVENT_OPERATION, function (slot0, slot1)
		slot0:sendNotification(GAME.ACTIVITY_OPERATION, slot1)
	end)
	slot0.bind(slot0, slot0.EVENT_GO_SCENE, function (slot0, slot1, slot2)
	...
	slot0.bind(slot0, slot0.BLACKWHITEGRID, function ()
	...
	slot0.bind(slot0, slot0.MEMORYBOOK, function ()
	...
	slot0.bind(slot0, slot0.GO_SHOPS_LAYER, function (slot0, slot1)
	...
	slot0.bind(slot0, slot0.GO_SHOPS_LAYER_STEEET, function (slot0, slot1)
		slot0:sendNotification(GAME.GO_SCENE, SCENE.SHOP, slot1 or {
			warp = NewShopsScene.TYPE_SHOP_STREET
		})
	end)
	slot0.bind(slot0, slot0.BATTLE_OPERA, function ()
	...
	slot0.bind(slot0, slot0.SPECIAL_BATTLE_OPERA, function ()
		slot1, slot2 = getProxy(ChapterProxy).getLastMapForActivity(slot0)
		...
		if not slot1 or not slot0:getMapById(slot1):isUnlock() then
			...
			if not slot4 then
				pg.m02:sendNotification(GAME.GO_SCENE, SCENE.LEVEL, {
					...
					mapIdx = (not (Map.lastMap and slot3:getMapById(Map.lastMap)) or not Map.lastMap and slot3.getMapById(Map.lastMap):isUnlock() or Map.lastMap) and slot3:getLastUnlockMap().id
				})
			end
		else
			pg.m02:sendNotification(GAME.GO_SCENE, SCENE.LEVEL, {
	...
	slot0.bind(slot0, slot0.REQUEST_VOTE_INFO, function (slot0, slot1)
		slot0:sendNotification(GAME.REQUEST_VOTE_INFO, slot1)
	end)
	slot0.bind(slot0, slot0.GO_VOTE_LAYER, function (slot0, slot1)
		if not _.detect(pg.activity_vote.all, function (slot0)
			...
			pg.TipsMgr.GetInstance().ShowTips(slot3, i18n("common_activity_not_start"))
	...
	slot0.bind(slot0, slot0.GO_FISRT_VOTE, function (slot0, slot1)
		slot0:addSubLayers(Context.New({
			mediator = VoteFameHallMediator,
			viewComponent = VoteFameHallLayer
		}))
	end)
	slot0.bind(slot0, slot0.GO_LOTTERY, function (slot0)
		slot0:addSubLayers(Context.New({
	...
	slot0.bind(slot0, slot0.EVENT_COLORING_ACHIEVE, function (slot0, slot1)
	...
	slot0.bind(slot0, slot0.GO_PRAY_POOL, function (slot0)
		slot0:sendNotification(GAME.GO_SCENE, SCENE.GETBOAT, {
			goToPray = true
		})
	end)
	slot0.bind(slot0, slot0.FETCH_INSTARGRAM, function (slot0, ...)
		slot0:sendNotification(GAME.ACT_INSTAGRAM_FETCH, ...)
	end)
	slot0.bind(slot0, slot0.MUSIC_GAME_OPERATOR, function (slot0, ...)
		slot0:sendNotification(GAME.SEND_MINI_GAME_OP, ...)
	end)
	slot0.bind(slot0, slot0.SELECT_ACTIVITY, function (slot0, slot1)
		slot0.viewComponent:verifyTabs(slot1)
	end)
	slot0.bind(slot0, slot0.SHOW_NEXT_ACTIVITY, function (slot0)
		slot0:showNextActivity()
	end)
	slot0.bind(slot0, slot0.ACTIVITY_PERMANENT, function (slot0, slot1)
	...
	slot0.bind(slot0, slot0.FINISH_ACTIVITY_PERMANENT, function (slot0)
		slot0:sendNotification(GAME.ACTIVITY_PERMANENT_FINISH, {
			activity_id = getProxy(ActivityPermanentProxy):getDoingActivity().id
		})
	end)
	slot0.viewComponent.setActivities(slot2, getProxy(ActivityProxy).getPanelActivities(slot1))
	slot0.viewComponent:setPlayer(slot3)
	slot0.viewComponent:setFlagShip(getProxy(BayProxy).getShipById(slot4, getProxy(PlayerProxy).getRawData(slot2).character))
end
...
slot0.handleNotification = function (slot0, slot1)
	...
	elseif slot2 == GAME.ACT_NEW_PT_DONE then
		slot0.viewComponent.emit(slot4, BaseUI.ON_ACHIEVE, slot3.awards, slot3.callback)
	...
	elseif slot2 == GAME.ACTIVITY_PERMANENT_START_DONE then
		slot0.viewComponent.verifyTabs(slot4, slot3.id)
...
slot0.showNextActivity = function (slot0)
	...
	elseif not slot0.viewComponent.activity then
		slot0.viewComponent.verifyTabs(slot5, slot0.contextData.id or (slot0.contextData.type and checkExist(_.detect(slot1:getPanelActivities(), function (slot0)
