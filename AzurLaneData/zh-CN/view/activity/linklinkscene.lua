...
slot0.didEnter = function (slot0)
	...
	slot0.SetState(slot0, slot0.GAME_STATE_BEGIN)
end
...
slot0.GameBegin = function (slot0)
	...
	slot0.list.align(slot3, slot0.MAX_ROW * slot0.MAX_COLUMN)
	...
	slot0.llist.align(slot3, slot0.MAX_ROW * slot0.MAX_COLUMN)
	...
	end)).setRepeat(slot5, 4):setLoopType(LeanTweenType.punch):setOnCompleteOnRepeat(true):setEase(LeanTweenType.easeOutSine)
end
...
slot0.GameLoop = function (slot0)
	function slot1(slot0)
		...
		for slot6 = 1, #slot0 - 1, 1 do
			...
			if slot9 ~= slot1 and slot10 ~= slot2 then
				...
				slot11:Find("joint").localEulerAngles = Vector3(0, 0, (((slot9 ~= -1 or slot2 ~= 1) and (slot1 ~= 1 or slot10 ~= -1)) or 0) and ((((slot10 == -1 and slot1 == -1) or (slot9 == 1 and slot2 == 1)) and 90) or (((slot9 == 1 and slot2 == -1) or (slot1 == -1 and slot10 == 1)) and 180) or 270))
	...
	slot0.list.each(slot6, function (slot0, slot1)
		onButton(slot0, slot1:Find("display/icon"), function ()
			...
			elseif slot3.id ~= slot2.id then
				...
				slot4 = nil
				slot3 = nil
			elseif not slot2:LinkLink(slot2.LinkLink, slot2) then
				setActive(slot4:Find("display/selected"), false)
				...
				if slot6 then
					slot2.SetState(slot7, slot1.GAME_STATE_END)
				end
			end
		end, SFX_PANEL)
	end)
	...
	onButton(slot0, slot0.resetBtn, function ()
		if slot0.state ~= slot1.GAME_STATE_GAMING then
			return
		elseif Time.realtimeSinceStartup - slot2 < slot2.RESET_CD then
...
slot0.DisplayResult = function (slot0, slot1)
	...
	pg.UIMgr.GetInstance().BlurPanel(slot4, slot0.result)
end
