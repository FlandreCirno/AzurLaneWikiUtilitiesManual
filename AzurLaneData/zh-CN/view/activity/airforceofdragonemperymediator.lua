...
slot0.register = function (slot0)
	slot0:bind(slot0.ON_BATTLE, function (slot0, slot1)
		slot0:sendNotification(GAME.BEGIN_STAGE, {
			system = SYSTEM_AIRFIGHT,
			stageId = slot1
		})
	end)
	slot0.bind(slot0, slot0.ON_ACTIVITY_OPREATION, function (slot0, slot1)
		slot0:sendNotification(GAME.ACTIVITY_OPERATION, slot1)
	end)
	slot0.viewComponent:SetActivityData(getProxy(ActivityProxy).getActivityByType(slot1, ActivityConst.ACTIVITY_TYPE_AIRFIGHT_BATTLE))
end
