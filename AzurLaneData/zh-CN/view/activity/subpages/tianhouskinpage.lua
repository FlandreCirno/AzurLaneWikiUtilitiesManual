...
slot0.OnFirstFlush = function (slot0)
	...
	slot0.ishow = getProxy(MiniGameProxy).GetMiniGameData(slot1, 26):GetRuntimeData("elements") and #slot2 >= 4 and slot2[4] == slot0.GetCurrentDay()

	onButton(slot0, slot0.playEffectBtn, function ()
		if not slot0.ishow then
			return
		end

		slot0:PlayFirework(slot0)
		setActive(slot0.playEffectBtn, false)
	end, SFX_PANEL)
	blinkAni(slot0.findTF(slot0, "light", slot0.playEffectBtn), 0.5)
end

slot0.OnUpdateFlush = function (slot0)
	slot2 = getProxy(MiniGameProxy).GetHubByHubId(slot1, slot0.hubID)
...
slot0.PlayFirework = function (slot0, slot1)
	...
	pg.PoolMgr.GetInstance():GetPrefab("ui/firework", "", false, function (slot0)
		...
		setParent(slot0, slot1.effectNode)

		slot0.transform.localPosition = Vector2(0, 0)
		slot1.fireEffect = slot0
	end)
	slot0.PlaySE(slot0)
end
