...
slot0.OnFirstFlush = function (slot0)
	...
	slot0.levelBtn = slot0.findTF(slot0, "level_btn", slot0.bg)
	...
	onButton(slot0, slot0.getBtn, function ()
		...
		slot0:emit(ActivityMediator.EVENT_PT_OPERATION, {
			...
			arg1 = slot1,
			callback = function ()
				slot0:showBubble(i18n("sofmapsd_3"))
			end
		})
	end, SFX_PANEL)
...
slot0.OnUpdateFlush = function (slot0)
	...
	if slot4 <= slot0.SpineActionByStep[1] then
		slot0.shopAnim:SetAction("stand2", 0)
	elseif slot4 <= slot0.SpineActionByStep[2] then
		slot0.shopAnim:SetAction("stand1", 0)
	elseif slot4 <= slot0.SpineActionByStep[3] then
		slot0.shopAnim:SetAction("stand1x2", 0)
	elseif slot4 <= slot0.SpineActionByStep[4] then
		slot0.shopAnim:SetAction("stand1x4", 0)
	elseif slot4 <= slot0.SpineActionByStep[5] then
