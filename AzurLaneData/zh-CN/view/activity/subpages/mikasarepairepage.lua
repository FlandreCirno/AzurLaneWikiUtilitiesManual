...
slot0.update_task_list_mikasa_museum = function (slot0, slot1, slot2, slot3)
	...
	for slot19 = 1, 4, 1 do
		...
		for slot23 = 1, 4, 1 do
			slot0:set_mikasa_btn(slot5[(slot19 - 1) * 4 + slot23], slot0:findTF("repair" .. slot23, slot10), (slot23 ~= 1 or 0) and slot5[((slot19 - 1) * 4 + slot23) - 1], slot23 >= 4)
		...
		setActive(slot10.Find(slot10, "gear"), not slot4.getTaskById(slot5[slot19 * 4]) or slot4.getFinishTaskById(slot14):isFinish())
	...
	onButton(slot0, slot12, function ()
		if not slot0:isFinish() then
			...
			if _.all(_.flatten({
				...
				pg.TipsMgr.GetInstance().ShowTips(slot3, i18n("word_museum_1"))
			else
...
slot0.set_mikasa_btn = function (slot0, slot1, slot2, slot3, slot4, slot5)
	...
	if slot4 then
		...
		setActive(slot8.Find(slot8, "mask"), slot7:isReceive())
