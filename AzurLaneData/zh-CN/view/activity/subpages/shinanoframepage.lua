...
slot0.OnFirstFlush = function (slot0)
	...
	onButton(slot0, slot0.switchBtn, function ()
		setActive(slot0.phaseTF_1, not isActive(slot0.phaseTF_1))
		setActive(slot0.phaseTF_2, not isActive(slot0.phaseTF_2))
	end, SFX_PANEL)

	slot2 = slot0.ptData.dropList[1][2]

	setParent(LoadAndInstantiateSync("IconFrame", slot3), slot0.frameTF, false)
end
...
slot0.OnUpdateFlush = function (slot0)
	if not getProxy(ActivityProxy):getActivityById(ActivityConst.SHINANO_EXP_ACT_ID) or slot1:isEnd() then
		...
		slot7, slot9, slot9 = slot0.ptData:GetResProgress()
