...
slot0.load = function (slot0, slot1, slot2, slot3, slot4)
	...
	slot5.StartPreload(slot5, function ()
		...
		slot5.InstCharacter(slot1, slot2:getPrefab(), function (slot0)
			slot0(slot0)
		end)
	end, nil)
...
slot0.SeaFire = function (slot0)
	...
	if slot0._fireTimer then
		...
		slot0._fireTimer = pg.TimeMgr.GetInstance().AddBattleTimer(slot3, "barrageFireTimer", -1, 1.5, slot2)
		slot0._delayTimer = pg.TimeMgr.GetInstance().AddBattleTimer(slot4, "", -1, 3, function ()
			slot0._delayTimer:Stop()
...
slot0.createEmitterCannon = function (slot0, slot1, slot2)
	slot3 = slot0:getEmitterHost()

	function slot4(slot0, slot1, slot2, slot3, slot4)
		slot6 = ys.Battle.BattleDataFunction.CreateBattleBulletData(slot0, slot0, slot1, slot5, slot2 + Vector3(40, 0, 0))

		slot6:SetOffsetPriority(slot3)
		slot6:SetShiftInfo(slot0, slot1)
		slot6:SetRotateInfo(nil, 0, slot2)

		if slot3.equipSkinId > 0 then
			slot7 = pg.equip_skin_template[slot3.equipSkinId]
			slot8, slot9, slot10, slot11 = ys.Battle.BattleDataFunction.GetEquipSkin(slot3.equipSkinId)
			slot14 = nil
		...
		(slot6:GetType() ~= ys.Battle.BattleConst.BulletType.CANNON or ys.Battle.BattleCannonBullet.New()) and (slot7 ~= slot8.BOMB or ys.Battle.BattleBombBullet.New()) and (slot7 ~= slot8.TORPEDO or ys.Battle.BattleTorpedoBullet.New()) and ys.Battle.BattleBullet.New():SetBulletData(slot6)
		...
		function slot10(slot0)
			...
			slot0:SetSpawn(slot0.SetSpawn)

			if slot2.bulletList then
				table.insert(slot2.bulletList, slot0)
			end
		end

		ys.Battle.BattleResourceManager.GetInstance().InstBullet(slot11, (slot6.GetType() ~= ys.Battle.BattleConst.BulletType.CANNON or ys.Battle.BattleCannonBullet.New()) and (slot7 ~= slot8.BOMB or ys.Battle.BattleBombBullet.New()) and (slot7 ~= slot8.TORPEDO or ys.Battle.BattleTorpedoBullet.New()) and ys.Battle.BattleBullet.New():GetModleID(), function (slot0)
			slot0(slot0)
		end)
	end
...
slot0.createEmitterAir = function (slot0, slot1, slot2, slot3)
	function slot4(slot0, slot1, slot2, slot3, slot4)
		...
		if slot2.equipSkinId > 0 and table.contains(pg.equip_skin_template[slot2.equipSkinId].equip_type, EquipType.AirProtoEquipTypes[slot6.type]) then
			slot12 = ys.Battle.BattleDataFunction.GetEquipSkin(slot2.equipSkinId)
			slot10 = slot12
		end
