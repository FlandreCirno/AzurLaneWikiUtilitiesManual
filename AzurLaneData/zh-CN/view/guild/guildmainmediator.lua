...
slot0.register = function (slot0)
	if getProxy(ContextProxy).getCurrentContext(slot1).mediator == NewGuildMediator then
		slot1:popContext()
	end

	slot3 = getProxy(GuildProxy)

	slot0.viewComponent:setGuildVO(slot4)
	slot0.viewComponent:setChatMsgs(slot5)
	slot0:bind(slot0.OPEN_MAIN, function ()
		slot0:closePage(GuildEventReportMediator)
	end)
	slot0.bind(slot0, slot0.OPEN_EVENT, function (slot0)
		slot0.viewComponent:openPage(GuildMainScene.TOGGLE_TAG[6])
	end)
	slot0.bind(slot0, slot0.OPEN_EVENT_REPORT, function (slot0)
		slot0:sendNotification(GAME.GUILD_OPEN_EVENT_REPORT)
	end)
	slot0.bind(slot0, slot0.ON_FETCH_CAPITAL, function (slot0)
		slot0:sendNotification(GAME.GUILD_REFRESH_CAPITAL)
	end)
	slot0.viewComponent:setPlayerVO(slot7)
	slot0:bind(slot0.ON_BACK, function (slot0)
		slot0:sendNotification(GAME.GO_BACK)
	end)
	slot0.bind(slot0, slot0.REBUILD_ALL, function (slot0)
		slot0.viewComponent:UpdateAllChat(getProxy(GuildProxy).getChatMsgs(slot1))
	end)
	slot0.bind(slot0, slot0.OPEN_MEMBER, function ()
		slot0:closePage(GuildEventReportMediator)
		slot0.closePage:addSubLayers(Context.New({
			viewComponent = GuildMemberLayer,
			mediator = GuildMemberMediator
		}))
	end)
	slot0.bind(slot0, slot0.CLOSE_MEMBER, function ()
		slot0:closePage(GuildMemberMediator)
	end)
	slot0.bind(slot0, slot0.OPEN_APPLY, function ()
		slot0:closePage(GuildEventReportMediator)
		slot0.closePage:addSubLayers(Context.New({
	...
	slot0.bind(slot0, slot0.CLOSE_APPLY, function ()
		slot0:closePage(GuildRequestMediator)
	end)
	slot0.bind(slot0, slot0.MODIFY, function (slot0, slot1, slot2, slot3)
	...
	slot0.bind(slot0, slot0.DISSOLVE, function (slot0, slot1)
		slot0:sendNotification(GAME.GUILD_DISSOLVE, slot1)
	end)
	slot0.bind(slot0, slot0.QUIT, function (slot0, slot1)
		slot0:sendNotification(GAME.GUILD_QUIT, slot1)
	end)
	slot0.bind(slot0, slot0.ON_REBUILD_LOG_ALL, function (slot0)
		slot0.viewComponent:UpdateAllLog(getProxy(GuildProxy).getData(slot1).getLogs(slot2))
	end)
	slot0.bind(slot0, slot0.SEND_MSG, function (slot0, slot1)
		slot0:sendNotification(GAME.GUILD_SEND_MSG, slot1)
	end)
	slot0.bind(slot0, slot0.OPEN_EMOJI, function (slot0, slot1, slot2)
	...
	slot0.bind(slot0, slot0.OPEN_OFFICE, function ()
		slot0:closePage(GuildEventReportMediator)
		slot0.closePage:addSubLayers(Context.New({
	...
	slot0.bind(slot0, slot0.CLOSE_OFFICE, function ()
		slot0:closePage(GuildOfficeMediator)
	end)
	slot0.bind(slot0, slot0.OPEN_TECH, function ()
		slot0:closePage(GuildEventReportMediator)
		slot0.closePage:addSubLayers(Context.New({
	...
	slot0.bind(slot0, slot0.CLOSE_TECH, function ()
		slot0:closePage(GuildTechnologyMediator)
	end)
	slot0.bind(slot0, slot0.ON_FETCH_CAPITAL_LOG, function (slot0)
		if slot0:getData():shouldRequestCapitalLog() then
			slot1:sendNotification(GAME.GUILD_FETCH_CAPITAL_LOG)
		else
			slot1.viewComponent:openResourceLog()
		end
	end)
	slot0.bind(slot0, slot0.OPEN_BATTLE, function ()
		slot0:closePage(GuildEventReportMediator)
		slot0.closePage:addSubLayers(Context.New({
	...
	slot0.bind(slot0, slot0.CLOSE_BATTLE, function ()
		slot0:closePage(GuildEventMediator)
	end)
	slot0.viewComponent:setPlayerVO(getProxy(PlayerProxy).getData(slot8))
end
