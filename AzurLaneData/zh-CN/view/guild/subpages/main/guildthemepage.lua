...
slot0.OnLoaded = function (slot0)
	...
	slot0.chatPanelWidth = slot0.chatPanel.rect.width + 300
	slot0.chatBtnWidth = slot0.chatBtn.rect.width + 300
...
slot0.OnInit = function (slot0)
	onButton(slot0, slot0.battleEvent, function ()
		triggerToggle(slot0.contextData.toggles[GuildMainScene.TOGGLE_TAG[6]], true)
	end, SFX_PANEL)
	onButton(slot0, slot0.battleReport, function ()
		slot0:emit(GuildMainMediator.OPEN_EVENT_REPORT)
	end, SFX_PANEL)
	onButton(slot0, slot0.chatBtn, function ()
		slot0:InitChatWindow()
		slot0.InitChatWindow:ShowOrHideChatWindow(true)
	end, SFX_PANEL)
	onButton(slot0, slot0.chatCloseBtn, function ()
		getProxy(GuildProxy):ClearNewChatMsgCnt()
		getProxy(GuildProxy).ClearNewChatMsgCnt:UpdateChatBtn()
		getProxy(GuildProxy).ClearNewChatMsgCnt.UpdateChatBtn:ShowOrHideChatWindow(false)
	end, SFX_PANEL)
	onButton(slot0, slot0.modifyBtn, function ()
		slot0.modifyPage:ExecuteAction("Show", slot0.guildVO, slot0.playerVO)
	end, SFX_PANEL)
end
...
slot0.UpdateBattleBtn = function (slot0)
	...
	setActive(slot0.battleReport, #_.select(_.values(getProxy(GuildProxy):GetReports()), function (slot0)
		return slot0:CanSubmit()
	end) > 0 and not slot0.guildVO:getMemberById(slot0.playerVO.id).IsRecruit(slot2))
	setActive(slot0.battleReportTip, #_.select(_.values(getProxy(GuildProxy).GetReports()), function (slot0)
		return slot0.CanSubmit()
	end) > 0 and not slot0.guildVO.getMemberById(slot0.playerVO.id).IsRecruit(slot2))
...
slot0.UpdateChatBtn = function (slot0)
	setActive(slot0.chatBtnTip, getProxy(GuildProxy):GetNewChatMsgCnt() > 0)

	if getProxy(GuildProxy).GetNewChatMsgCnt() > 0 then
...
slot0.InitChatWindow = function (slot0)
	...
	GetOrAddComponent(slot0.chatRect, typeof(EventTriggerListener)).AddDragEndFunc(slot1, function (slot0, slot1)
		if GetComponent(slot0.chatRect, typeof(ScrollRect)).normalizedPosition.y <= 0.1 then
			slot0:ClearChatTip()
		end
	end)
	slot0.UpdateChatWindow(slot0)

	if slot0.isAdmin then
		onInputEndEdit(slot0, slot0.noticeTxt.gameObject, function ()
			if getInputText(slot0.guildVO:GetAnnounce() or "".noticeTxt.gameObject) == "" or slot1 == slot0 then
				return
			end

			if wordVer(slot1) > 0 then
				pg.TipsMgr.GetInstance():ShowTips(i18n("playerinfo_mask_word"))
				setInputText(slot0.noticeTxt.gameObject, "")

				return
			end

			slot0.noticeScrollTxt:SetText(slot1)
			slot0:emit(GuildMainMediator.MODIFY, 5, 0, slot1)
			setInputText(slot0.noticeTxt.gameObject, "")
		end)
	end
...
slot0.UpdateAllChat = function (slot0, slot1)
	...
	for slot7 = slot0.index, #slot2, 1 do
		table.insert(slot3, function (slot0)
			slot0:Append(slot1[slot2], -1, true)
			slot0()
		end)
	end
...
slot0.AddWorldBossMsg = function (slot0, slot1, slot2, slot3)
	...
	slot7 = ChatBubbleWorldBoss.New(slot6)
...
slot0.AppendWorld = function (slot0, slot1, slot2, slot3)
	...
	slot8 = GuildChatBubble.New(slot7)
...
slot0.UpdateMainInfo = function (slot0)
	slot0.nameTxt.text = slot0.guildVO.getName(slot1)
	slot0.factionTxt.text = slot0.guildVO.getFactionName(slot1)
	slot0.policyTxt.text = slot0.guildVO.getPolicyName(slot1)
	slot0.idTxt.text = "ID:" .. slot0.guildVO.id
	slot0.numberTxt.text = slot0.guildVO.memberCount .. "/" .. slot0.guildVO.getMaxMember(slot1)

	setFillAmount(slot0.expImg, slot0.guildVO.exp / math.max(slot0.guildVO.getLevelMaxExp(slot1), 1))

	slot0.levelTxt.text = (slot0.guildVO.level <= 9 and "0" .. slot1.level) or slot1.level
	slot2 = ""
	slot3 = ""

	for slot8 = 1, math.floor(slot1.level / 10), 1 do
		slot3 = slot3 .. ":"
	end
...
slot0.ShowOrHideChatWindow = function (slot0, slot1)
	...
	end)).setOnComplete(slot7, System.Action(slot6))
...
slot0.EnterOrExitPreView = function (slot0, slot1)
	...
	LeanTween.value(go(slot0.top), (slot1 and {
		0,
		slot0.topPanelWidth
	}) or {
		slot0.topPanelWidth,
		0
	}[1], (slot1 and ) or [2], 0.3):setOnUpdate(System.Action_float(function (slot0)
		setAnchoredPosition(slot0.top, {
			y = slot0
		})
	end))
	LeanTween.value(go(slot0.bottomPanel), (slot1 and {
		0,
		slot0.bottomPanelWidth
	}) or {
		slot0.bottomPanelWidth,
		0
	}[1], (slot1 and ) or [2], 0.3).setOnUpdate(slot4, System.Action_float(function (slot0)
	...
	if slot0.isShowChatWindow then
		...
		LeanTween.value(go(slot0.chatPanel), slot4[1], slot4[2], 0.3).setOnUpdate(slot5, System.Action_float(function (slot0)
			setAnchoredPosition(slot0.chatPanel, {
				x = slot0
			})
		end))
	else
		LeanTween.value(go(slot0.chatBtn), (slot1 and {
			0,
			slot0.chatBtnWidth
		}) or {
			slot0.chatBtnWidth,
			0
		}[1], (slot1 and ) or [2], 0.3).setOnUpdate(slot5, System.Action_float(function (slot0)
			setAnchoredPosition(slot0.chatBtn, {
				x = slot0
			})
		end))
