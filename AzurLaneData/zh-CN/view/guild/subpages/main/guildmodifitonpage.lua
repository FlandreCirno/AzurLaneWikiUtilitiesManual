...
slot0.OnInit = function (slot0)
	...
	onButton(slot0, slot0.confirmBtn, function ()
		...
		if slot0.getFaction(slot0) ~= slot0.guildVO:getFaction() then
			pg.MsgboxMgr.GetInstance():ShowMsgBox({
	...
	function (slot0)
		onInputChanged(slot0, slot0, function ()
			...
			if getInputText(slot1.nameInput) ~= slot1.guildVO:getName() then
				setText(slot1.costTF, pg.gameset.modify_guild_cost.key_value)
			else
				setText(slot1.costTF, 0)
			end
		end)
	end(slot0.manifestoInput)
end
...
slot0.Show = function (slot0, slot1, slot2)
	...
	slot0.nameInput.interactable = slot0.guildVO:getDutyByMemberId(slot0.playerVO.id) == GuildConst.DUTY_COMMANDER
	slot0.manifestoInput.interactable = slot0.guildVO.getDutyByMemberId(slot0.playerVO.id) == GuildConst.DUTY_COMMANDER

	setActive(slot0.confirmBtn, slot0.guildVO.getDutyByMemberId(slot0.playerVO.id) == GuildConst.DUTY_COMMANDER)
	setActive(slot0.cancelBtn, slot0.guildVO.getDutyByMemberId(slot0.playerVO.id) == GuildConst.DUTY_COMMANDER)
	...
	onToggle(slot0, slot0.factionBLHXToggle, function (slot0)
		if slot0 then
			slot0.faction = GuildConst.FACTION_TYPE_BLHX
		end
	end, SFX_PANEL)
	onToggle(slot0, slot0.factionCSZZToggle, function (slot0)
		if slot0 then
			slot0.faction = GuildConst.FACTION_TYPE_CSZZ
		end
	end, SFX_PANEL)

	slot0.policy = slot0.guildVO.getPolicy(slot5)

	onToggle(slot0, slot0.policyRELAXToggle, function (slot0)
		if slot0 then
			slot0.policy = GuildConst.POLICY_TYPE_RELAXATION
		end
	end, SFX_PANEL)
	onToggle(slot0, slot0.policyPOWERToggle, function (slot0)
		if slot0 then
			slot0.policy = GuildConst.POLICY_TYPE_POWER
		end
	end, SFX_PANEL)
	...
	slot0.policyPOWERToggle.GetComponent(slot5, typeof(Toggle)).interactable = slot3
