...
slot0.OnInit = function (slot0)
	...
	onButton(slot0, slot0.supplyUnOpenAdminTF, function ()
		pg.MsgboxMgr:GetInstance():ShowMsgBox({
			content = i18n("guild_start_supply_consume_tip", slot0.guild:getSupplyConsume()),
			onYes = function ()
				slot0:emit(GuildOfficeMediator.ON_PURCHASE_SUPPLY)
			end
		})
	end, SFX_PANEL)
	onButton(slot0, slot0.supplyOpenGetBtn, function ()
		slot0:emit(GuildOfficeMediator.GET_SUPPLY_AWARD)
	end, SFX_PANEL)
...
slot0.UpdateTaskPanel = function (slot0, slot1)
	if slot0.guild.getWeeklyTask(slot2).getState(slot3) == GuildTask.STATE_EMPTY then
...
slot0.UpdateLockTask = function (slot0)
	...
	if slot0.isAdmin then
		onButton(slot0, slot0.unOpenAdmin, function ()
			slot0.selectTaskPage:ExecuteAction("Show", slot0.guild, slot0.isAdmin)
		end, SFX_PANEL)
	end
end
...
slot0.UpdatePrivateTask = function (slot0, slot1)
	...
	slot5 = getProxy(TaskProxy):getTaskById(slot1:GetPresonTaskId()) or slot4:getFinishTaskById(slot3)

	if not (slot5 ~= nil) then
		slot5 = Task.New({
			id = slot3
		})
	end
	...
	onButton(slot0, slot0.privateTaskAcceptBtn, function ()
		pg.MsgboxMgr:GetInstance():ShowMsgBox({
			content = i18n("guild_task_accept", slot0:getConfig("name"), slot1:getConfig("name"), slot1:getConfig("name")),
			onYes = function ()
				slot0:emit(GuildOfficeMediator.ON_ACCEPT_TASK, slot0)
			end
		})
	end, SFX_PANEL)
	onButton(slot0, slot0.privateTaskGetBtn, function ()
		slot0:emit(GuildOfficeMediator.ON_SUBMIT_TASK, slot0)
	end, SFX_PANEL)

	if slot5.isFinish(slot5) and not slot5:isReceive() and not slot2 then
	...
	setActive(slot0.taskMaskAll, slot1:isFinished() and (not slot6 or not slot2))
	setActive(slot0.taskMaskTop, not (slot1.isFinished() and (not slot6 or not slot2)) and slot1:isFinished())
	setActive(slot0.privateTaskReapeatFlag, not slot2)
	setActive(slot0.privateTaskResTxt.gameObject.transform.parent, not not slot2)
...
slot0.UpdatePubliceTask = function (slot0, slot1)
	...
	slot0.taskProgressTxt.text = slot2 .. "/<size=40>" .. slot1:getMaxProgress() .. "</size>"

	setFillAmount(slot0.taskProgressBar, slot2 / slot1.getMaxProgress())
...
slot0.UpdateContributionPanel = function (slot0)
	slot0.contributionList:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			...
			slot4:update(slot3)
			onButton(slot1, slot4.commitBtn, function ()
				updateDropCfg(slot1)
				pg.MsgboxMgr:GetInstance():ShowMsgBox({
					content = i18n("guild_donate_tip", slot1.cfg.name, slot0[3], slot2, (slot1:GetResCntByAward(slot0:getCommitItem()) < slot0.getCommitItem()[3] and "#FF5C5CFF") or "#92FC63FF"),
					onYes = function ()
						slot0:emit(GuildOfficeMediator.ON_COMMIT, slot1.id)
					end
				})
			end, SFX_PANEL)
			setButtonEnabled(slot4.commitBtn, slot2 > 0)
		end
	end)
	slot0.contributionList.align(slot4, #slot0.guild.getDonateTasks(slot1))

	slot0.contributionCntTxt.text = i18n("guild_left_donate_cnt", slot0.guild.getRemainDonateCnt(slot1) + slot0.guild.GetExtraDonateCnt(slot1))
end

slot0.UpdateSupplyPanel = function (slot0)
	slot2 = slot0.guild.isOpenedSupply(slot1)
