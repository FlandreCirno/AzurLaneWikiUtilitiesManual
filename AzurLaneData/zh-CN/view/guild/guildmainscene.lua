...
slot0.didEnter = function (slot0)
	...
	onButton(slot0, slot0.eyeTF, function ()
		slot0.hideFlag = not slot0.hideFlag

		slot0:EnterOrExitPreView()
	end, SFX_PANEL)
	slot0.guildRes.ExecuteAction(slot1, "Update", slot0.playerVO, slot0.guildVO)
	slot0:initToggles()
	slot0:UpdateRes()
	...
	slot0.dynamicBg:Init(slot1)
	slot0:UpdateNotices(slot0.NOTIFY_TYPE_ALL)
end
...
slot0.EnterOrExitPreView = function (slot0)
	...
	LeanTween.value(go(slot0._topPanel), (slot0.hideFlag and {
		0,
		slot0.topWidth
	}) or {
		slot0.topWidth,
		0
	}[1], (slot0.hideFlag and ) or [2], 0.3):setOnUpdate(System.Action_float(function (slot0)
		setAnchoredPosition(slot0._topPanel, {
			y = slot0
		})
	end))
	LeanTween.value(go(slot0._leftLength), (slot0.hideFlag and {
		0,
		slot0.letfWidth
	}) or {
		slot0.letfWidth,
		0
	}[1], (slot0.hideFlag and ) or [2], 0.3).setOnUpdate(slot3, System.Action_float(function (slot0)
		setAnchoredPosition(slot0._leftLength, {
			x = slot0
		})
	end))
	LeanTween.value(go(slot0.topBg), (slot0.hideFlag and {
		0,
		slot0.topBgWidth
	}) or {
		slot0.topBgWidth,
		0
	}[1], (slot0.hideFlag and ) or [2], 0.3).setOnUpdate(slot4, System.Action_float(function (slot0)
		setAnchoredPosition(slot0.topBg, {
			y = slot0
		})
	end))
end
...
slot0.UpdateNotices = function (slot0, slot1)
	...
	if slot1 == slot0.NOTIFY_TYPE_ALL or slot1 == slot0.NOTIFY_TYPE_MAIN then
		setActive(slot0.mainTip, slot2:ShouldShowMainTip())
	end

	if slot1 == slot0.NOTIFY_TYPE_ALL or slot1 == slot0.NOTIFY_TYPE_APPLY then
		setActive(slot0.applyTip, slot2:ShouldShowApplyTip())
	end

	if slot1 == slot0.NOTIFY_TYPE_ALL or slot1 == slot0.NOTIFY_TYPE_OFFICE then
		setActive(slot0.officeTip, slot3:ShouldShowOfficeTip())
	end

	if slot1 == slot0.NOTIFY_TYPE_ALL or slot1 == slot0.NOTIFY_TYPE_BATTLE then
		setActive(slot0.battleTip, slot2:ShouldShowBattleTip())
	end

	if slot1 == slot0.NOTIFY_TYPE_ALL or slot1 == slot0.NOTIFY_TYPE_TECH then
...
slot0.initToggles = function (slot0)
	...
	for slot4, slot5 in ipairs(slot0.TOGGLE_TAG) do
		...
		onToggle(slot0, slot0.contextData.toggles[slot5], function (slot0)
			if slot0 then
				...
				slot1(slot2, setActive ~= slot0._bg)
			else
	...
	if LOCK_GUILD_BATTLE then
		setActive(slot0.contextData.toggles[], false)
	end

	setActive(slot0.contextData.toggles[], slot0.guildVO.getDutyByMemberId(slot1, slot0.playerVO.id) == GuildConst.DUTY_COMMANDER or slot1 == GuildConst.DUTY_DEPUTY_COMMANDER)

	slot0.contextData.page = nil

	triggerToggle(slot0.contextData.toggles[slot0.contextData.page or slot1], true)
end

slot0.TriggerOfficePage = function (slot0)
	triggerToggle(slot0.contextData.toggles[slot0], true)
end

slot0.openPage = function (slot0, slot1)
	setActive(slot0.eyeTF, slot1 == slot0)

	if slot1 == slot1 or slot1 == slot2 then
		slot0.guildRes:Show()
		setActive(slot0.resPanel._tf, false)
	elseif slot1 == slot3 or slot1 == slot4 or slot1 == slot5 then
		slot0.guildRes:Hide()
		setActive(slot0.resPanel._tf, false)
	else
		slot0.guildRes:Hide()
