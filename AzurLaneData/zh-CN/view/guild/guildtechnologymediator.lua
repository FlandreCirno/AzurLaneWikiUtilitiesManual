...
slot0.register = function (slot0)
	slot0:bind(slot0.ON_OPEN_OFFICE, function ()
		slot0:sendNotification(slot1.ON_OPEN_OFFICE)
	end)
	slot0.bind(slot0, slot0.ON_CANCEL_TECH, function (slot0, slot1)
		slot0:sendNotification(GAME.GUILD_CANCEL_TECH, {
			id = slot1
		})
	end)
	slot0.bind(slot0, slot0.ON_UPGRADE, function (slot0, slot1)
		slot0:sendNotification(GAME.GUILD_START_TECH, {
			id = slot1
		})
	end)
	slot0.bind(slot0, slot0.ON_START, function (slot0, slot1)
		slot0:sendNotification(GAME.GUILD_START_TECH_TASK, {
			id = slot1
		})
	end)
	slot0.viewComponent:setGuild(getProxy(GuildProxy).getData(slot1))
end
