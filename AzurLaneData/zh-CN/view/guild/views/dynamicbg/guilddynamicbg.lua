slot0 = class("GuildDynamicBG")
slot1 = false
slot2 = false
slot3 = require("view.guild.views.DynamicBG.GuildDynamicBGConfig")

slot0.Ctor = function (slot0, slot1)
	slot0.mainScrollrect = slot1:Find("scrollrect")
	slot0.assistScrollrect = slot1:Find("scrollrect1")
	slot0.pathContainer = slot1:Find("scrollrect/content/path")
	slot0.nameTF = slot1:Find("name")
	slot0.commanderTag = slot1:Find("commander_tag")
	slot0.path = {}
	slot0.ships = {}
	slot0.furnitures = {}

	onScroll(nil, slot0.mainScrollrect, function (slot0)
		scrollTo(slot0.assistScrollrect, slot0.x / 2, slot0.y)
	end)
end

slot0.Init = function (slot0, slot1)
	slot0.shipVOs = slot1 or {}

	seriesAsync({
		function (slot0)
			slot0:InitPath()
			slot0()
		end,
		function (slot0)
			slot0:InitFurnitures(slot0)
		end,
		function (slot0)
			if slot0 then
				slot1:AddDebugShip(slot0)
			else
				slot1:InitShips(slot0)
			end
		end
	}, function ()
		if slot0 then
			slot1:AddGridDebugView()
		end

		slot1:SortScene()
	end)
end

slot0.InitPath = function (slot0)
	slot2 = slot0.gridCnt[2]
	slot3 = slot0.cantWalkPos
	slot4 = Vector2(slot0.gridSize[1], slot0.gridSize[2])
	slot5 = Vector2(slot0.gridStartPos[1], slot0.gridStartPos[2])

	for slot9 = 0, slot0.gridCnt[1] - 1, 1 do
		slot0.path[slot9] = {}

		for slot13 = 0, slot2 - 1, 1 do
			slot0.path[slot9][slot13] = GuildDynamicBgPathGrid.New({
				position = Vector2(slot9, slot13),
				canWalk = not _.any(slot3, function (slot0)
					return slot0[1] == slot0 and slot0[2] == 
				end),
				sizeDelta = slot4,
				startPosOffset = slot5
			})
		end
	end
end

slot0.GetRandomGrid = function (slot0)
	slot1 = {}

	for slot5, slot6 in pairs(slot0.path) do
		for slot10, slot11 in pairs(slot6) do
			if slot11:CanWalk() then
				table.insert(slot1, slot11)
			end
		end
	end

	return slot1[math.random(1, #slot1)]
end

slot0.GetGrid = function (slot0, slot1, slot2)
	return slot0.path[slot1][slot2]
end

slot0.InitFurnitures = function (slot0, slot1)
	function slot2(slot0, slot1, slot2)
		slot3 = GameObject.New()
		slot3.name = slot1.name

		SetParent(slot3, slot0.pathContainer)

		slot4 = GetOrAddComponent(slot3, typeof(Image))
		GetOrAddComponent(slot3, typeof(RectTransform)).pivot = Vector2(0, 0)
		slot4.sprite = slot1

		slot4:SetNativeSize()
		table.insert(slot0.furnitures, GuildDynamicFurniture.New({
			go = slot3,
			grid = slot0:GetGrid(slot0.position[1], slot0.position[2]),
			path = slot0.path,
			size = Vector2(slot0.size[1], slot0.size[2]),
			offset = Vector2(slot0.offset[1], slot0.offset[2]),
			mode = slot0.mode,
			interactionPosition = Vector2(slot0.interactionPosition[1], slot0.interactionPosition[2]),
			interactionDir = slot0.interactionDir
		}))
		slot2()
	end

	slot3 = {}
	slot4 = slot0.furnitures

	for slot8, slot9 in ipairs(slot4) do
		table.insert(slot3, function (slot0)
			GetSpriteFromAtlasAsync("furniture/guild/" .. slot0.name, "", function (slot0)
				slot0(slot0, slot0, )
			end)
		end)
	end

	seriesAsync(slot3, slot1)
end

slot0.InitShips = function (slot0, slot1)
	slot2 = {}

	for slot6, slot7 in ipairs(slot0.shipVOs) do
		table.insert(slot2, function (slot0)
			slot0:AddShip(slot0.AddShip, slot0)
		end)
	end

	seriesAsync(slot2, slot1)
end

slot0.AddShip = function (slot0, slot1, slot2)
	function slot3(slot0, slot1, slot2, slot3)
		tf(slot2):SetParent(slot0.pathContainer)

		tf(slot2).localScale = Vector3(0.5, 0.5, 1)

		GuildDynamicBgShip.New({
			stand = slot0.stand,
			auto = slot0.auto,
			go = slot2,
			grid = slot1,
			path = slot0.path,
			furnitures = slot0.furnitures,
			name = slot0.name,
			isCommander = slot0.isCommander
		}).SetOnMoveCallBack(slot4, function ()
			slot0:SortScene()
		end)
		table.insert(slot0.ships, slot4)
		slot3()
	end

	slot4 = slot1.getPrefab(slot1)

	if not slot0:GetRandomGrid() then
		slot2()
	end

	slot5:Lock()
	PoolMgr.GetInstance():GetSpineChar(slot4, true, function (slot0)
		if IsNil(slot0.nameTF) then
			return
		end

		slot0.name = slot1

		cloneTplTo(slot0.nameTF, slot0.transform, "name")

		if slot2.isCommander then
			cloneTplTo(slot0.commanderTag, slot0.transform, "tag")
		end

		slot3(slot2, slot4, slot0, slot5)
	end)
end

slot0.ExitShip = function (slot0, slot1)
	for slot5, slot6 in pairs(slot0.ships) do
		if slot6.name == slot1 then
			PoolMgr.GetInstance():ReturnSpineChar(slot6._go.name, slot6._go)
			slot6:Dispose()

			slot0.ships[slot5] = nil

			break
		end
	end
end

slot0.SortScene = function (slot0)
	slot1 = {}

	for slot5, slot6 in pairs(slot0.ships) do
		table.insert(slot1, {
			obj = slot6,
			position = slot6.grid.position
		})
	end

	for slot5, slot6 in pairs(slot0.furnitures) do
		table.insert(slot1, {
			obj = slot6,
			position = slot6.grid.position
		})
	end

	table.sort(slot1, function (slot0, slot1)
		if slot0.position.y == slot1.position.y then
			return slot0.position.x < slot1.position.x
		else
			return slot1.position.y < slot0.position.y
		end
	end)

	for slot5, slot6 in ipairs(slot1) do
		slot6.obj:SetAsLastSibling()
	end
end

slot0.Dispose = function (slot0)
	for slot4, slot5 in pairs(slot0.ships) do
		PoolMgr.GetInstance():ReturnSpineChar(slot5._go.name, slot5._go)
		slot5:Dispose()
	end

	for slot4, slot5 in pairs(slot0.furnitures) do
		slot5:Dispose()
	end

	if slot0 then
		if slot0.timer then
			slot0.timer:Stop()

			slot0.timer = nil
		end

		if slot0.timer1 then
			slot0.timer1:Stop()

			slot0.timer1 = nil
		end
	end
end

slot0.AddGridDebugView = function (slot0)
	slot1 = {}

	for slot5, slot6 in pairs(slot0.path) do
		slot1[slot5] = {}

		for slot10, slot11 in pairs(slot6) do
			slot12 = GameObject.New()

			SetParent(slot12, slot0.pathContainer)

			slot13 = GetOrAddComponent(slot12, typeof(RectTransform))
			slot13.sizeDelta = slot11.sizeDelta
			slot13.pivot = Vector2(0, 0)
			slot13.localPosition = slot11.localPosition
			slot1[slot5][slot10] = GetOrAddComponent(slot12, typeof(Image))
			slot12.name = slot11.position.x .. "-" .. slot11.position.y

			setActive(slot12, true)
		end
	end

	slot0.timer = Timer.New(function ()
		for slot3, slot4 in pairs(slot0.path) do
			for slot8, slot9 in pairs(slot4) do
				slot10 = nil
				slot1[slot3][slot8].color = (not slot9:IsLock() or Color.New(1, 0, 0, 0.3)) and (slot9:CanWalk() or Color.New(0.5, 0.5, 0.5, 0.3)) and Color.New(1, 1, 1, 0.3)
			end
		end
	end, 1, -1)

	slot0.timer.Start(slot2)
	slot0.timer.func()
end

slot0.AddDebugShip = function (slot0, slot1)
	Ship.New({
		id = 0,
		configId = 301284,
		name = "001"
	}).stand = true
	slot0.shipVOs = {
		Ship.New()
	}

	slot0:InitShips(function ()
		slot0.timer1 = Timer.New(function ()
			if Input.GetKeyDown(KeyCode.A) then
				slot0.ships[1]:MoveLeft()
			end

			if Input.GetKeyDown(KeyCode.S) then
				slot0.ships[1]:MoveDown()
			end

			if Input.GetKeyDown(KeyCode.W) then
				slot0.ships[1]:MoveUp()
			end

			if Input.GetKeyDown(KeyCode.D) then
				slot0.ships[1]:MoveRight()
			end
		end, Time.deltaTime, -1)

		slot0.timer1.Start(slot0)
		slot0.timer1.Start.timer1.func()
		slot0()
	end)
end

return slot0
