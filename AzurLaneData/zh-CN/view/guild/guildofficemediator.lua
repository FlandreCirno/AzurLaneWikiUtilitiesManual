...
slot0.register = function (slot0)
	slot0:bind(slot0.REFRES_DONATE_LIST, function (slot0, slot1)
		return
	end)
	slot0.bind(slot0, slot0.UPDATE_WEEKLY_TASK, function (slot0)
		slot0:sendNotification(GAME.GUILD_WEEKLY_TASK_PROGREE_UPDATE)
	end)
	slot0.bind(slot0, slot0.ON_SUBMIT_TASK, function (slot0, slot1)
		slot0:sendNotification(GAME.SUBMIT_TASK, slot1)
	end)
	slot0.bind(slot0, slot0.ON_SELECT_TASK, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.GUILD_SELECT_TASK, {
			taskId = slot1,
			num = slot2
		})
	end)
	slot0.bind(slot0, slot0.ON_ACCEPT_TASK, function (slot0, slot1)
		slot0:sendNotification(GAME.TRIGGER_TASK, slot1)
	end)
	slot0.bind(slot0, slot0.ON_COMMIT, function (slot0, slot1)
		slot0:sendNotification(GAME.GUILD_COMMIT_DONATE, {
			taskId = slot1
		})
	end)
	slot0.bind(slot0, slot0.ON_PURCHASE_SUPPLY, function (slot0)
		slot0:sendNotification(GAME.GUILD_BUY_SUPPLY)
	end)
	slot0.bind(slot0, slot0.GET_SUPPLY_AWARD, function (slot0)
		slot0:sendNotification(GAME.GUILD_GET_SUPPLY_AWARD)
	end)
	slot0.viewComponent:SetGuild(slot2)
