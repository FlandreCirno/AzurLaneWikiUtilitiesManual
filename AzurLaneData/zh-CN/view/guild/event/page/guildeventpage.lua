...
slot0.OnInit = function (slot0)
	...
	onButton(slot0, slot0.formationBtn, function ()
		slot0:emit(GuildEventLayer.ON_OPEN_FORMATION)
	end, SFX_PANEL)
end
...
slot0.UpdateReportBtn = function (slot0)
	setActive(slot0.reportTip, #_.select(_.values(slot0.reports), function (slot0)
		return slot0:CanSubmit()
	end) > 0 and not slot0.guildVO.getMemberById(slot2, slot0.player.id):IsRecruit())
...
slot0.InitEvents = function (slot0)
	...
	slot0.eventList:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			slot0:UpdateEvent(slot2, slot0.events[slot1 + 1])

			if slot0.events[slot1 + 1] then
				slot1[slot3.id] = slot2
			end
		end
	end)
	slot0.eventList.align(slot2, #slot0.displays)
...
slot0.UpdateEvent = function (slot0, slot1, slot2)
	...
	if slot2 then
		...
		slot8.sprite = GetSpriteFromAtlas("ui/GuildEventUI_atlas", "tag_" .. slot6)
	...
	setActive(slot1:Find("consume"), slot2 and not (slot3 and slot2 and slot3.id == slot2.id))
	setActive(slot1:Find("timer"), slot3 and slot2 and slot3.id == slot2.id)
	...
	onButton(slot0, slot1, function ()
		...
		if not slot0:IsUnlock(slot1.guildVO.level) then
			pg.TipsMgr:GetInstance():ShowTips(i18n("guild_level_no_enough"))

			return
		end

		if slot2 and slot2.id ~= slot0.id then
			pg.TipsMgr:GetInstance():ShowTips(i18n("guild_open_event_info_when_exist_active", slot2:getConfig("name")))

			return
		end

		slot1:emit(GuildEventLayer.OPEN_EVENT_INFO, slot1.emit)
	end, SFX_PANEL)
end
...
slot0.BuildTree = function (slot0, slot1)
	slot0.gevent = slot1
	slot0.missions = {}
	slot0.bossPosition = slot0.gevent:GetBossMission().GetPosition(slot3)
	...
	for slot8, slot9 in ipairs(slot4) do
		if not slot9:IsBoss() then
			table.insert(slot3, function (slot0)
				slot0:emit(GuildEventMediator.REFRESH_MISSION, slot1.id, slot0)
			end)
		end
	end

	seriesAsync(slot3, function ()
		if slot0 ~= slot1.lastPosition then
			slot1:RefreshLatelyNode()
		end
	end)
end
...
slot0.AddRefreshTime = function (slot0)
	...
	slot0.timer = Timer.New(function ()
		slot0:RefreshLatelyNode()
		slot0.RefreshLatelyNode:AddRefreshTime()
	end, GuildConst.FORCE_REFRESH_MISSION_TREE_TIME, 1)

	slot0.timer.Start(slot1)
end
...
slot0.GenTree = function (slot0)
	...
	for slot4, slot5 in pairs(slot0.missions) do
		...
		for slot9, slot10 in ipairs(slot5) do
			table.insert(slot0.nodes, slot0:CreateNode(slot11, slot4, slot10))
		end
	end
end
...
slot0.CreateNode = function (slot0, slot1, slot2, slot3)
	...
	onButton(slot0, slot1, function ()
		if slot0.prevSelected == slot1 then
			return
		end
		...
		if slot0.prevSelected then
			slot0:HideDesc(slot0.prevSelected)
		end

		slot0:ShowDesc(slot0)

		slot0.ShowDesc.prevSelected = slot0
	end, SFX_PANEL)
...
slot0.InitTree = function (slot0)
	...
	for slot6, slot7 in ipairs(slot0.nodes) do
		...
		slot10 = math.abs(slot7._tf.anchoredPosition.y)

		if slot1[1] < math.abs(slot7._tf.anchoredPosition.x) then
...
slot0.CreateLinkLine = function (slot0, slot1)
	function slot2(slot0, slot1)
		Instantiate(slot0).name = slot1

		return Instantiate(slot0)
	end

	if slot1.HasChild(slot1) then
...
slot0.ShowDesc = function (slot0, slot1)
	...
	slot0.descPanelTag.sprite = GetSpriteFromAtlas("ui/GuildMissionUI_atlas", "tag" .. slot5)
	...
	onButton(slot0, slot0.descPanel, function ()
		if slot0.data:IsBoss() then
			if not slot1(slot1) then
				return
			end

			slot2:emit(GuildEventLayer.ON_OPEN_BOSS, slot0.data)
		else
			slot2:emit(GuildEventMediator.REFRESH_MISSION, slot0.data.id, function ()
				if not slot0(slot1) then
					return
				end

				slot2.contextData.mission = slot1.data

				slot2:emit(GuildEventLayer.ON_OPEN_MISSION, slot1.data)
			end)
		end
	end, SFX_PANEL)
end
