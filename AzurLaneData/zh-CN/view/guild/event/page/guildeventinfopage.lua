...
slot0.OnInit = function (slot0)
	...
	onButton(slot0, slot0.goBtn, function ()
		if not GuildMember.IsAdministrator(slot0.guild:getSelfDuty()) then
		...
		pg.MsgboxMgr:GetInstance():ShowMsgBox({
			content = i18n("guild_start_event_consume_tip", slot0.gevent:GetConsume(), slot0.gevent:GetName()),
			onYes = function ()
				slot0:emit(GuildEventMediator.ON_ACTIVE_EVENT, slot0.gevent.id)
			end
		})
	end, SFX_PANEL)
	onButton(slot0, slot0.joinBtn, function ()
		if not slot0.activeEvent then
			return
		end

		if slot0.activeEvent:IsLimitedJoin() then
...
slot0.JoinEvent = function (slot0)
	function slot1()
		slot0, slot1 = slot0.activeEvent:GetMainMissionCntAndFinishCnt()

		if slot1 ~= 0 then
			pg.MsgboxMgr:GetInstance():ShowMsgBox({
				content = i18n("guild_join_event_exist_finished_mission_tip"),
				onYes = function ()
					slot0:emit(GuildEventMediator.ON_JOIN_EVENT)
				end
			})
		else
			slot0.emit(slot2, GuildEventMediator.ON_JOIN_EVENT)
		end
	end

	if slot0.activeEvent.GetLeftTime(slot2) <= 604800 then
		pg.MsgboxMgr:GetInstance():ShowMsgBox({
...
slot0.OnShow = function (slot0)
	...
	slot0.icon.sprite = GetSpriteFromAtlas("guildevent/i_" .. slot0.gevent.id, "")

	setActive(slot0.icon.gameObject, true)

	slot0.descTxt.text = slot0.gevent.GetDesc(slot1)
	slot0.consumeTxt.text = "<color=" .. ((slot0.guild:getCapital() < slot0.gevent:GetConsume() and COLOR_RED) or COLOR_GREEN) .. ">" .. slot2 .. "</color>/" .. slot3
...
slot0.UpdateBtnState = function (slot0)
	...
	if slot0.activeEvent then
		...
		slot6, slot9 = slot0.activeEvent:GetMainMissionCntAndFinishCnt()
...
slot0.UpdateAwards = function (slot0, slot1)
	...
	slot0.awardList.align(slot3, #slot1:GetDisplayAward())
end

slot0.UpdateMissions = function (slot0, slot1)
	slot0.missionList:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			slot2:GetComponent(typeof(Image)).sprite = GetSpriteFromAtlas("GuildEventIcon", slot0[slot1 + 1])
		end
	end)
	slot0.missionList.align(slot3, #slot1:GetDisplayMission())
end
