...
slot0.didEnter = function (slot0)
	...
	onButton(slot0, slot0.getAll, function ()
		...
		slot0:emit(GuildEventReportMediator.ON_SUBMIT_REPORTS, slot0)
	end, SFX_PANEL)
...
slot0.UpdateGetAllBtn = function (slot0)
	slot1 = #slot0.displays == 0 or _.all(slot0.displays, function (slot0)
		return not slot0:CanSubmit()
	end)

	setActive(slot0.gotAll, slot1)
end
...
slot0.OnInitItem = function (slot0, slot1)
	...
	onButton(slot0, slot2.getBtn, function ()
		if slot0.report:IsLock() then
			pg.TipsMgr:GetInstance():ShowTips(i18n("guild_can_not_get_tip"))

			return
		end

		slot1:emit(GuildEventReportMediator.ON_SUBMIT_REPORTS, {
			slot0.report.id
		})
	end, SFX_PANEL)
