...
slot0.register = function (slot0)
	slot0:bind(slot0.COMMANDER_FORMATION_OP, function (slot0, slot1)
		slot0:OnComanderOP(slot1)
	end)
	slot0.bind(slot0, slot0.ON_CMD_SKILL, function (slot0, slot1)
	...
	slot0.bind(slot0, slot0.REFRESH_RECOMMAND_SHIPS, function (slot0, slot1)
		slot0:sendNotification(GAME.REFRESH_ALL_ASSULT_SHIP_RECOMMAND_STATE, {
			callback = slot1
		})
	end)
	slot0.bind(slot0, slot0.ON_RECOMM_ASSULT_SHIP, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.GUILD_RECOMMAND_ASSULT_SHIP, {
			shipId = slot1,
			cmd = slot2
		})
	end)
	slot0.bind(slot0, slot0.ON_JOIN_EVENT, function ()
		slot0:sendNotification(GAME.ON_GUILD_JOIN_EVENT)
	end)
	slot0.bind(slot0, slot0.ON_SELECT_COMMANDER, function (slot0, slot1, slot2, slot3)
		slot0:SelectBossBattleCommander(slot1, slot2, slot3)
	end)
	slot0.bind(slot0, slot0.ON_GET_ALL_ASSULT_FLEET, function (slot0, slot1)
		slot0:sendNotification(GAME.GUILD_GET_ASSAULT_FLEET, {
			callback = slot1
		})
	end)
	slot0.bind(slot0, slot0.ON_RECOMM_BOSS_BATTLE_SHIPS, function (slot0, slot1)
		slot0:sendNotification(GAME.GUILD_GET_ASSAULT_FLEET, {
			callback = function ()
				slot0:RecommShipsForBossBattle(slot0)
			end
		})
	end)
	slot0.bind(slot0, slot0.ON_SAVE_FORMATION, function (slot0, slot1)
		slot0:sendNotification(GAME.GUILD_UPDATE_BOSS_FORMATION, {
			editFleet = slot0.contextData.editBossFleet,
			callback = slot1
		})
	end)
	slot0.bind(slot0, slot0.ON_CLEAR_BOSS_FLEET_INVAILD_SHIP, function (slot0)
		slot0:sendNotification(GAME.GUILD_UPDATE_BOSS_FORMATION, {
			force = true,
			editFleet = slot0.contextData.editBossFleet,
			callback = callback
	...
	slot0.bind(slot0, slot0.ON_UPDATE_BOSS_FLEET, function (slot0)
		if not slot0.contextData.editBossFleet then
			slot0:StartBossBattle()
		else
			slot0.viewComponent:emit(slot1.ON_SAVE_FORMATION, function ()
				slot0:StartBossBattle()
			end)
		end
	end)
	slot0.bind(slot0, slot0.ON_SELECT_BOSS_SHIP, function (slot0, slot1, slot2, slot3)
		slot0:sendNotification(GAME.GUILD_GET_ASSAULT_FLEET, {
			callback = function ()
				slot0:SelectBossBattleShip(slot0, , )
			end
		})
	end)
	slot0.bind(slot0, slot0.ON_UPDATE_NODE_ANIM_FLAG, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.GUILD_UPDATE_NODE_ANIM_FLAG, {
			id = slot1,
			position = slot2
		})
	end)
	slot0.bind(slot0, slot0.ON_REFRESH_BOSS_RANK, function (slot0)
		slot0:sendNotification(GAME.GET_GUILD_BOSS_RANK, {})
	end)
	slot0.bind(slot0, slot0.ON_GET_BOSS_INFO, function (slot0)
		slot0:sendNotification(GAME.GUILD_GET_BOSS_INFO)
	end)
	slot0.bind(slot0, slot0.JOIN_MISSION, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.GUILD_JOIN_MISSION, {
			id = slot1,
			shipIds = slot2
		})
	end)
	slot0.bind(slot0, slot0.ON_SELECT_MISSION_SHIP, function (slot0, slot1, slot2, slot3, slot4)
		slot0.viewComponent:emit(slot1.ON_GET_FORMATION, function ()
			slot0:OnSelectMissionShips(slot0, , , )
		end)
	end)
	slot0.bind(slot0, slot0.REFRESH_MISSION, function (slot0, slot1, slot2)
	...
	slot0.bind(slot0, slot0.FORCE_REFRESH_MISSION, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.GUILD_REFRESH_MISSION, {
			force = true,
			id = slot1,
			callback = slot2
	...
	slot0.bind(slot0, slot0.ON_SELECT_SHIP, function (slot0, slot1, slot2, slot3)
		slot0:OnSelectShips(slot1, slot2, slot3)
	end)
	slot0.bind(slot0, slot0.ON_GET_FORMATION, function (slot0, slot1)
		slot4 = {}

		if getProxy(GuildProxy):getRawData().GetActiveEvent(slot2) then
			table.insert(slot4, function (slot0)
				slot0.viewComponent:emit(slot1.ON_GET_ALL_ASSULT_FLEET, slot0)
			end)
		end
	...
	slot0.bind(slot0, slot0.UPDATE_FORMATION, function (slot0)
		if not slot0.contextData.editFleet then
			return
		end

		slot0:sendNotification(GAME.GUILD_UPDATE_MY_ASSAULT_FLEET, {
			fleet = slot0.contextData.editFleet
		})
	end)
	slot0.bind(slot0, slot0.ON_ACTIVE_EVENT, function (slot0, slot1)
		slot0:sendNotification(GAME.GUILD_ACTIVE_EVENT, {
			eventId = slot1
		})
	end)
	slot0.bind(slot0, slot0.ON_OPEN_REPORT, function (slot0)
		slot0:sendNotification(GAME.GUILD_OPEN_EVENT_REPORT)
	end)
...
slot0.StartBossBattle = function (slot0)
	...
	slot5, slot6 = slot3:GetMainFleet().IsLegal(slot4)
...
slot0.SelectBossBattleCommander = function (slot0, slot1, slot2, slot3)
	...
	slot8 = slot0.contextData.editBossFleet[slot1].getCommanders(slot7)
	slot9 = {}
	...
	pg.m02:sendNotification(GAME.GO_SCENE, SCENE.COMMANDROOM, {
		...
		onSelected = function (slot0, slot1)
			slot0:OnDockSelectCommander(true, slot1, slot0.OnDockSelectCommander, slot0, slot0, slot1)
		end,
		onQuit = function (slot0)
			slot0:RemoveCommander(slot0.RemoveCommander)
			slot0()
		end
	})
end
...
slot0.SwopCommanderForBossBattle = function (slot0, slot1, slot2, slot3, slot4, slot5, slot6, slot7)
	...
	pg.MsgboxMgr.GetInstance():ShowMsgBox({
		...
		onYes = function ()
			slot0:RemoveCommander(slot0)
			slot2:AddCommander(slot3, slot4)
...
slot0.RecommShipsForBossBattle = function (slot0, slot1)
	...
	slot0.contextData.editBossFleet[slot1].RemoveAll(slot9)
	...
	function slot15(slot0, slot1)
		if not slot1:ExistSameKindShip(slot0[GuildAssaultFleet.GetRealId(slot0)]) then
			slot1:AddUserShip(GuildAssaultFleet.GetUserId(slot0), slot2)
	...
	if slot9.IsMainFleet(slot9) then
...
slot0.SelectBossBattleShip = function (slot0, slot1, slot2, slot3)
	...
	slot9 = nil
	slot9 = (slot0.contextData.editBossFleet[slot2] or Clone(slot6:GetBossMission().GetFleetByIndex(slot7, slot2))) and Clone(slot0.contextData.editBossFleet[slot2])
	slot10 = nil
	...
	if slot9:IsMainFleet() then
		slot12 = slot0.contextData.editBossFleet[GuildBossMission.SUB_FLEET_ID] or slot7:GetFleetByIndex(GuildBossMission.SUB_FLEET_ID):GetOtherMemberShipCnt(slot11.id)
	else
		slot12 = slot0.contextData.editBossFleet[GuildBossMission.MAIN_FLEET_ID] or slot7:GetFleetByIndex(GuildBossMission.MAIN_FLEET_ID):GetOtherMemberShipCnt(slot11.id)
	end
	...
	slot0:sendNotification(GAME.GO_SCENE, SCENE.DOCKYARD, {
		...
		onShip = function (slot0, slot1, slot2)
			if slot0:GetOtherMemberShipCnt(slot1.id) + slot2 >= 3 and slot0.user.id ~= slot1.id then
				return false, i18n("guild_boss_formation_1")
			end

			if slot0.user.id ~= slot1.id and slot0:ExistUserShip(slot0.user.id) then
				return false, i18n("guild_boss_formation_2")
			end

			if _.any(slot3, function (slot0)
				return slot0.ship:isSameKind(slot0)
			end) then
				return false, i18n("guild_boss_formation_3")
			end

			if pg.ShipFlagMgr.GetInstance().GetShipFlag(slot5, GuildAssaultFleet.GetRealId(slot0.id), "inEvent") then
		...
		onSelected = function (slot0, slot1)
			if slot0[1] then
				slot0:AddUserShip(GuildAssaultFleet.GetUserId(slot2), GuildAssaultFleet.GetRealId(slot2), slot1)
			end

			slot2.contextData.editBossFleet[] = slot0
		end
	})
end
...
slot0.OnSelectShips = function (slot0, slot1, slot2, slot3)
	...
	slot6 = getProxy(BayProxy).getData(slot5)
	slot7 = {}
	...
	slot0:sendNotification(GAME.GO_SCENE, SCENE.DOCKYARD, {
		...
		onShip = function (slot0, slot1, slot2)
			for slot6, slot7 in pairs(slot0) do
				...
				if slot6 ~= slot1 and slot8 == slot0.id then
		...
		onSelected = function (slot0, slot1)
			slot1.contextData.editFleet:InsertBayShip(slot0[1], slot0[slot0[1]])
		end
	})
end
...
slot0.OnCheckMissionShip = function (slot0, slot1, slot2)
	...
	slot5 = slot3:getMemberById(slot4)
	slot6 = slot5:GetAssaultFleet()
	slot7 = slot5:GetExternalAssaultFleet()
	slot9 = slot3:GetActiveEvent().GetJoinShips(slot8)
	slot11 = getProxy(BayProxy).getData(slot10)
	slot12 = _.map(slot1, function (slot0)
		return slot0[slot0]
	end)

	if slot2.isActivityNpc(slot2) then
		return false, i18n("common_npc_formation_tip")
	end

	if _.any(slot8:GetMissionById(slot0).GetMyShips(slot13), function (slot0)
		return slot0[slot0] and slot0[slot0]:isSameKind(slot0[slot0].isSameKind)
	end) then
...
slot0.OnSelectMissionShips = function (slot0, slot1, slot2, slot3, slot4)
	...
	slot0.sendNotification(slot0, GAME.GO_SCENE, SCENE.DOCKYARD, {
		...
		ignoredIds = _.map(slot5:getMemberById(slot6).GetAssaultFleet(slot7).GetShipIds(slot8), function (slot0)
			return GuildAssaultFleet.GetRealId(slot0)
		end),
		onShip = function (slot0)
			return slot0.OnCheckMissionShip(slot0.OnCheckMissionShip, , slot0)
		end,
		onSelected = function (slot0, slot1)
			if slot0[slot1] then
				for slot5, slot6 in ipairs(slot2.contextData.missionShips) do
					if slot6 == slot0[slot1] then
						table.remove(slot2.contextData.missionShips, slot5)
					end
				end
			end

			table.insert(slot2.contextData.missionShips, slot0[1])
		end
	})
end
...
slot0.OnComanderOP = function (slot0, slot1)
	...
	elseif slot2.type == LevelUIConst.COMMANDER_OP_RECORD_PREFAB then
		slot0:sendNotification(GAME.SET_COMMANDER_PREFAB, {
			id = slot2.id,
			commanders = Clone(slot2.fleet.getCommanders(slot4))
		})
	else
		slot3 = slot2.id
		slot4 = slot2.fleet
		...
		if slot2.type == LevelUIConst.COMMANDER_OP_USE_PREFAB then
			...
			for slot13, slot14 in pairs(slot8) do
				table.insert(slot9, function (slot0)
					slot0:OnDockSelectCommander(false, slot0.OnDockSelectCommander, slot0, , {
						slot4.id
					}, slot0)
				end)
			end
...
slot0.handleNotification = function (slot0, slot1)
	...
	elseif slot2 == GAME.GUILD_UPDATE_MY_ASSAULT_FLEET_DONE then
		...
		slot0.viewComponent.OnMyAssultFleetFormationDone(slot4)
	...
	elseif slot2 == GAME.ON_GUILD_JOIN_EVENT_DONE then
		slot0.viewComponent.EnterEvent(slot4)
