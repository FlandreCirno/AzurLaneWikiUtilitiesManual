...
slot0.UpdateUrItemEntrance = function (slot0)
	if not LOCK_UR_SHIP then
		slot0.activtyUrExchangeTxt.text = slot4 .. "/" .. pg.gameset.urpt_chapter_max.description[2]
		slot0.activtyUrExchangeCG.alpha = (getProxy(BagProxy):GetLimitCntById(slot2) == pg.gameset.urpt_chapter_max.description[2] and 0.6) or 1

		setActive(slot0.activtyUrExchangeTip, NotifyTipHelper.ShouldShowUrTip())
		onButton(slot0, slot0.activtyUrExchangeBtn, function ()
			slot0:emit(CommissionInfoMediator.ON_UR_ACTIVITY)
		end, SFX_PANEL)
	else
