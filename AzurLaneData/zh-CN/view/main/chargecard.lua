...
slot0.updateCharge = function (slot0, slot1, slot2, slot3)
	...
	if slot1:isMonthCard() then
		if slot2:getCardById(VipCard.MONTH) and not slot13:isExpire() then
			setActive(slot0.mask, (slot1:getConfig("limit_arg") or 0) < math.floor((slot13:getLeftDate() - pg.TimeMgr.GetInstance():GetServerTime()) / 86400))
			setText(slot0.limitText, i18n("charge_month_card_lefttime_tip", math.floor((slot13.getLeftDate() - pg.TimeMgr.GetInstance().GetServerTime()) / 86400)))
		end
	...
	if slot1 then
		...
		function slot4()
			if slot0.updateTimer then
				...
				slot0.updateTimer.Stop.updateTimer = nil
			end
		end
