...
slot0.update = function (slot0, slot1)
	...
	if slot1.emojiId then
		PoolMgr.GetInstance().GetPrefab(slot13, "emoji/" .. pg.emoji_template[slot1.emojiId].pic, pg.emoji_template[slot1.emojiId].pic, true, function (slot0)
			if IsNil(slot0.tf) then
				return
			end
		...
		PoolMgr.GetInstance():GetPrefab("ChatFrame/" .. slot12, AttireFrame.attireFrameRes(slot3, slot2, AttireConst.TYPE_CHAT_FRAME, slot5), true, function (slot0)
			...
			if slot0.tf and slot0.data then
				...
				slot2 = string.gmatch(slot1.content, ChatConst.EmojiIconCodeMatch)
				slot3 = false
				...
				slot5 = SwitchSpecialChar(slot1.content)
				...
				slot0.GetComponent(slot0, typeof(LayoutElement)).preferredWidth = slot0.chatBgWidth
				slot0.name = slot2

				setParent(slot0, slot0.tf, false)
				tf(slot0):SetAsFirstSibling()
				Canvas.ForceUpdateCanvases()
				slot0:OnChatFrameLoaded(slot0)
