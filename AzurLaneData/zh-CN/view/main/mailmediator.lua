...
slot0.register = function (slot0)
	...
	slot0.bind(slot0, slot0.ON_TAKE, function (slot0, slot1)
		slot0:sendNotification(GAME.TAKE_ATTACHMENT, slot1)
	end)
	slot0.bind(slot0, slot0.ON_TAKE_ALL, function (slot0)
		if #slot0:getMailAttachments() > 0 then
			...
			pg.TipsMgr.GetInstance().ShowTips(slot2, i18n("take_nothing"))
		end
	end)
	slot0.bind(slot0, slot0.ON_DELETE, function (slot0, slot1)
		slot0:sendNotification(GAME.DELETE_MAIL, slot1)
	end)
	slot0.bind(slot0, slot0.ON_DELETE_ALL, function (slot0)
		slot0:sendNotification(GAME.DELETE_ALL_MAIL)
	end)
	slot0.bind(slot0, slot0.ON_MORE_NEWER, function (slot0)
		slot1:sendNotification(GAME.GET_MAIL_LIST, {
			splitId = (slot0:getNewestMail() and slot1.id) or 0,
			type = (slot1 and 1) or 0
		})
	end)
	slot0.bind(slot0, slot0.ON_MORE_OLDER, function (slot0)
		slot1:sendNotification(GAME.GET_MAIL_LIST, {
			splitId = (slot0:getOldestMail() and slot1.id) or 0,
			type = (slot1 and 2) or 0
		})
	end)
	slot0.bind(slot0, slot0.ON_CHANGE_IMP, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.CHANGE_MAIL_IMP_FLAG, {
			id = slot1,
			flag = slot2
		})
	end)
	slot0.viewComponent.setUnreadMailCount(slot2, slot1.unread)
