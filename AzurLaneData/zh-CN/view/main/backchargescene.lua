...
slot0.initDiamondList = function (slot0, slot1)
	slot2 = slot0:findTF("content", slot1)

	function slot4(slot0)
		onButton(slot0, BackChargeDiamondCard.New(slot0, slot0).tr, function ()
			slot0:confirm(slot1.goods)
		end, SFX_PANEL)

		slot0.damondItems[slot0] = BackChargeDiamondCard.New(slot0, slot0)
	end
	...
	slot6 = UIItemList.New(slot2, slot3)
...
slot0.updateDamondsData = function (slot0)
	...
	slot0.damondItemVOs = {}
	slot7 = getProxy(ServerProxy):getLastServer(getProxy(UserProxy).getData(slot2).uid)
	slot6 = getProxy(PlayerProxy).getRefundInfo(slot5) or {}
...
slot0.sortDamondItems = function (slot0)
	...
	if page == slot0.TYPE_DIAMOND then
		slot0.diamondUIItemList.align(slot1, #slot0.tempDamondVOs)
	elseif page == slot0.TYPE_GIFT then
