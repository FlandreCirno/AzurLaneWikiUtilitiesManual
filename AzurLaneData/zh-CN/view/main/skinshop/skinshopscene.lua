...
slot0.didEnter = function (slot0)
	slot0:bind(slot0.EVENT_ON_CARD_CLICK, function (slot0, slot1)
		slot0:OnCardClick(slot1)
	end)
	setActive(slot0.mainPanel, true)
	slot0.initShips(slot0)
	slot0:initSkinPage()
	...
	onButton(slot0, slot0.live2dFilter, function ()
		if #slot0.displays == 0 or not _.any(slot0.displays, function (slot0)
			...
			pg.TipsMgr.GetInstance().ShowTips(slot0, i18n("skinshop_live2d_fliter_failed"))
...
slot0.UpdateLive2dBtn = function (slot0, slot1)
...
slot0.initSkinPage = function (slot0)
	...
	for slot7, slot8 in ipairs(slot2) do
		...
		onButton(slot0, slot9, function ()
			for slot4, slot5 in ipairs(slot0.pageTFs) do
			...
			for slot5 = 1, math.abs(slot1), 1 do
				slot0:onSwitch(slot1)
			end

			slot0:onRelease()
		end, SFX_PANEL)
		slot0.UpdateTagStyle(slot0, slot9, slot0, slot8)
	end

	eachChild(slot1, function (slot0)
		if slot0.gameObject.activeSelf then
			table.insert(slot0.pageTFs, 1, slot0)
		end
	end)
	slot0.addVerticalDrag(slot0, slot0.leftPanel, function (slot0)
		slot0:onSwitch(slot0)
	end, function ()
		slot0:onRelease()
	end)
	slot0.UpdateViewMode(slot0, slot1)
end
...
slot0.updateMainView = function (slot0, slot1)
	...
	slot0.setBg(slot0, slot3, slot2, PathMgr.FileExists(PathMgr.getAssetBundle("painting/" .. slot5 .. "_n")))
...
slot0.setBg = function (slot0, slot1, slot2, slot3)
	...
	if slot5 ~= slot4:rarity2bgPrintForGet() then
		...
		setImageSprite(slot0.GetCurBgTransform(slot0), slot0.defaultBg)
		slot0:AnimBg()
	end
end
...
slot0.onBuyDone = function (slot0, slot1)
	if _.detect(slot0.skinGoodsVOs, function (slot0)
		...
		slot0.updateBuyBtn(slot0, slot2)
...
slot0.updateBuyBtn = function (slot0, slot1)
	...
	if slot1:getConfig("genre") == ShopArgs.SkinShopTimeLimit then
		...
		slot3 = slot0[slot1.getSkinId(slot1)]
	else
		slot3 = slot0[slot1:getSkinId()]

		setActive(slot0.itemBtn, slot1:isDisCount() and slot1:IsItemDiscountType() and slot1.buyCount == 0)
		setActive(slot0.buyBtn, not (slot1.type == Goods.TYPE_ACTIVITY or slot5 == Goods.TYPE_ACTIVITY_EXTRA) and slot1.buyCount == 0 and not (slot1.isDisCount() and slot1.IsItemDiscountType()))
		setActive(slot0.gotBtn, not (slot1.type == Goods.TYPE_ACTIVITY or slot5 == Goods.TYPE_ACTIVITY_EXTRA) and not (slot1.buyCount == 0))
		setActive(slot0.activityBtn, (slot1.type == Goods.TYPE_ACTIVITY or slot5 == Goods.TYPE_ACTIVITY_EXTRA) and not (slot1.isDisCount() and slot1.IsItemDiscountType()))
		onButton(slot0, slot0.itemBtn, function ()
			triggerButton(slot0.buyBtn)
		end, SFX_PANEL)
		onButton(slot0, slot0.buyBtn, function ()
			if slot0.type == Goods.TYPE_SKIN then
				if slot1.showCardId == slot0.id then
					slot2 = i18n("charge_scene_buy_confirm", slot1, HXSet.hxLan(slot2.name))
					slot4 = slot0:isDisCount() and slot0:IsItemDiscountType()

					if slot4 then
						slot2 = i18n("discount_coupon_tip", slot1, slot0:GetDiscountItem().name, HXSet.hxLan(slot2.name))
					end
					...
					pg.MsgboxMgr.GetInstance():ShowMsgBox({
						...
						onYes = function ()
							if slot0 then
								slot1:emit(SkinShopMediator.ON_SHOPPING_BY_ACT, slot2.id, 1)
							else
								slot1:emit(SkinShopMediator.ON_SHOPPING, slot2.id, 1)
							end
						end
					})
				...
				pg.TipsMgr.GetInstance().ShowTips(slot1, ERROR_MESSAGE[9999])
...
slot0.setPaintingPrefab = function (slot0, slot1, slot2, slot3, slot4)
	...
	if PathMgr.FileExists(PathMgr.getAssetBundle("painting/" .. slot2)) then
		...
		PoolMgr.GetInstance():GetPainting(slot2, true, function (slot0)
			...
			ShipExpressionHelper.SetExpression(slot0:GetChild(0), slot1)
		end)
	end
end
...
slot0.onNext = function (slot0)
	...
	if slot1 then
		...
		function slot6()
			...
			return getBounds(slot0.card._tf).size.x < math.ceil(getBounds(slot1._tf).GetMax(slot1).x - slot0:GetMax().x)
		end
...
slot0.isLive2DSkin = function (slot0, slot1)
...
slot0.updateShipRect = function (slot0, slot1)
	...
	if slot0.contextData.pageId and slot0.shipRect then
		...
		for slot6, slot7 in ipairs(slot0.skinGoodsVOs) do
			slot10 = (slot0[slot7.getSkinId(slot7)].shop_type_id == 0 and 9999) or slot9
			slot11 = slot7:getConfig("genre") == ShopArgs.SkinShopTimeLimit

			if (slot0.contextData.pageId ~= slot1.PAGE_TIME_LIMIT or not slot11) and not slot11 and (slot0.contextData.pageId == slot1.PAGE_ALL or slot10 == slot0.contextData.pageId) and slot2(slot8) then
		...
		if slot1 then
			slot0.shipRect.SetTotalCount(slot3, #slot0.displays, slot1)
		else
...
slot0.addVerticalDrag = function (slot0, slot1, slot2, slot3)
	...
	slot4.AddDragFunc(slot4, function (slot0, slot1)
		if slot1.position.y < slot0 and slot1 ~= 0 then
			slot2 = slot1.position
			slot1 = 0
		elseif slot0 < slot1.position.y and slot3 ~= 0 then
			slot2 = slot1.position
			slot3 = 0
		end

		slot3 = math.abs(math.floor((slot1.position.y - slot2.y) / slot4))

		if (slot1.position.y - slot2.y) / slot4 and slot3 < slot3 then
			slot3 = slot3

			slot5(slot2)
		end

		if slot5 and slot3 < slot1 then
			slot1 = slot3
