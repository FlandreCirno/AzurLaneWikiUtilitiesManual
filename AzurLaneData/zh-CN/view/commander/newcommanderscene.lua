...
slot0.onUIAnimEnd = function (slot0, slot1)
	...
	slot2.SetEndEvent(slot2, function ()
		slot0.isAnim = false

		setActive(slot0.clickTF, true)
		slot0.clickTF:SetEndEvent(nil)
		nil()
	end)
end
...
slot0.openMsgBox = function (slot0, slot1)
	...
	if slot0.msgbox.GetLoaded(slot3) then
		slot2()
	else
		slot0.msgbox:Load()
...
slot0.didEnter = function (slot0)
	...
	onButton(slot0, slot0.clickTF, function ()
		...
		elseif slot0.commanderVO:isSSR() and not slot0.commanderVO:isLocked() then
			...
			slot0.emit(slot0, slot1.ON_CLOSE)
		end
	end, SFX_CANCEL)
end
...
slot0.updateTalents = function (slot0)
	...
	slot0.talentsList.align(slot3, 3)
end
