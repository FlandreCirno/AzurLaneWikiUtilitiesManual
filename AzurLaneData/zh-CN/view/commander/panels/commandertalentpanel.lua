...
slot0.attach = function (slot0, slot1)
	...
	onButton(slot0, slot0.resetTimeBtn, function ()
		if #slot0.commanderVO:getTalentOrigins() == #slot0.commanderVO:getTalents() and _.all(slot0, function (slot0)
			...
			pg.TipsMgr.GetInstance().ShowTips(slot2, i18n("commander_reset_talent_is_not_need"))
...
slot0.updateTalents = function (slot0)
	...
	slot0.uilist.align(slot3, CommanderConst.MAX_TELENT_COUNT)
end
...
slot0.updateTalentCard = function (slot0, slot1, slot2)
	...
	if slot2 then
		...
		setText(slot3.Find(slot3, "name_bg/Text"), slot2:getConfig("name"))
...
slot0.openUseagePanel = function (slot0)
	...
	if not slot0.commanderVO.getNotLearnedList(slot1) or #slot2 == 0 then
		...
		slot0.usageList.align(slot4, #slot2)
...
slot0.openResetPanel = function (slot0)
	...
	slot0.resetList.align(slot3, #slot0.commanderVO.getTalentOrigins(slot1))
...
slot0.openReplacePanel = function (slot0, slot1)
	function slot3(slot0, slot1)
		slot0:updateTalentCard(slot0.replaceTargetTF, slot0)
		slot0:updateTalentCard(slot0.replaceTalent, slot1)
		onButton(slot0, slot0.replaceconfirmBtn, function ()
			if slot0 and slot1 and slot2 then
				if slot0:getConfig("worth") > 1 then
					pg.MsgboxMgr.GetInstance():ShowMsgBox({
						content = i18n("commander_ability_replace_warning"),
						onYes = function ()
							slot0:emit(CommanderInfoMediator.ON_LEARN_TALENT, slot1.id, slot2.id, slot3.id)
							slot0.emit:closeReplacePanel()
						end
					})
				else
					slot3.emit(slot0, CommanderInfoMediator.ON_LEARN_TALENT, slot2.id, slot1.id, slot0.id)
					slot3:closeReplacePanel()
				end
			end
		end, SFX_PANEL)
	end
	...
	slot0.replacePanel.SetAsLastSibling(slot4)
