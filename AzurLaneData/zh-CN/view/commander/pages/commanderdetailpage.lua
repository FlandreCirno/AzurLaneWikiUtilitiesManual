...
slot0.OnInit = function (slot0)
	...
	onToggle(slot0, slot0.skillBtn, function (slot0)
		...
		if slot0 then
			slot0:emit(CommanderInfoMediator.ON_CLOSE_PANEL_SElF)
		end
	end, SFX_PANEL)
...
slot0.UpdateInfo = function (slot0)
	...
	eachChild(slot0.fleetnums, function (slot0)
		setActive(slot0, go(slot0).name == tostring(slot0.fleetId or ""))
	end)

	slot4 = (slot0.commanderVO.fleetId and not slot1.inBattle and slot1.sub and 260) or 200
	slot0.line.sizeDelta = Vector2(slot4, slot0.line.sizeDelta.y)

	setActive(slot0.subTF, slot0.commanderVO.fleetId and not slot1.inBattle and slot1.sub)
...
slot0.updateTalents = function (slot0)
	...
	slot0.talentList.align(slot4, #slot0.commanderVO.getTalents(slot2))
end
...
slot0.updateTalentAddition = function (slot0, slot1, slot2)
	...
	slot0.talentAdditionList:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			setScrollText(findTF(slot2, "bg/name_mask/name"), slot0[slot1 + 1].name)
			setText(slot2:Find("bg/value"), ((slot3.value > 0 and "+") or "") .. slot3.value .. ((slot0[slot1 + 1].type == CommanderConst.TALENT_ADDITION_RATIO and "%") or ""))
	...
	slot0.talentAdditionList.align(slot6, #_.values(slot0.commanderVO.getTalentsDesc(slot3)))
end
