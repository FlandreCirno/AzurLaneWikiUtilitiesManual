...
slot0.OnInit = function (slot0)
	...
	onButton(slot0, slot0.okBtn, function ()
		if slot0.selectedID then
			slot0:emit(CommanderHomeMediator.ON_CHANGE_STYLE, slot0.cattery.id, slot0.selectedID)
		end
	end, SFX_PANEL)
end

slot0.OnInitItem = function (slot0, slot1)
	onButton(slot0, CatteryStyleCard.New(slot1)._tf, function ()
		if not slot0.style:IsOwn() then
			pg.TipsMgr.GetInstance():ShowTips(i18n("cathome_style_unlock"))

			return
		end

		slot1.selectedID = slot0.style.id

		slot1:emit(CatteryDescPage.CHANGE_STYLE, slot0.style.id)
	end, SFX_PANEL)

	slot0.cards[slot1] = CatteryStyleCard.New(slot1)
end
