...
slot0.OnInit = function (slot0)
	...
	onButton(slot0, slot0.addBtn, function ()
		if slot0.maxCnt == 0 then
			return
		end

		slot0:UpdateValue(math.min(slot0.value + 1, slot0.maxCnt))
	end, SFX_PANEL)
	onButton(slot0, slot0.reduceBtn, function ()
		...
		slot0:UpdateValue(math.max(1, slot0.value - 1))
	end, SFX_PANEL)
	onButton(slot0, slot0.confirmBtn, function ()
		if slot0.value <= 0 then
			pg.TipsMgr.GetInstance():ShowTips(i18n("cat_accelfrate_notenough"))

			return
		end

		if slot0.maxCnt < slot0.value then
			return
		end

		if slot0:CalcMaxUsageCnt() <= 0 then
			pg.TipsMgr.GetInstance():ShowTips(i18n("commander_box_was_finished"))

			return
		end

		if slot0 < slot0.value then
			slot0:UpdateValue(slot0)
			pg.TipsMgr.GetInstance():ShowTips(i18n("comander_tool_cnt_is_reclac"))

			return
		end

		slot0:emit(CommandRoomMediator.ON_USE_QUICKLY_TOOL, slot0.itemId, slot0.value, slot0.boxId)
		slot0:Hide()
	end, SFX_PANEL)
end
...
slot0.Show = function (slot0, slot1, slot2)
	...
	slot0.maxCnt = math.min(slot4, slot3)
	slot0.maxTxt.text = getProxy(BagProxy):getItemCountById(slot2)
...
slot0.UpdateValue = function (slot0, slot1)
	...
	slot0:AddTimer(getProxy(CommanderProxy):getBoxById(slot0.boxId).getFinishTime(slot4) - slot0.costM * slot1 * 60)
end

slot0.CalcMaxUsageCnt = function (slot0)
	if getProxy(CommanderProxy):getBoxById(slot0.boxId).getFinishTime(slot1) - pg.TimeMgr.GetInstance():GetServerTime() > 0 then
...
slot0.AddTimer = function (slot0, slot1)
	...
	slot0.timer = Timer.New(function ()
		if slot0 - pg.TimeMgr.GetInstance():GetServerTime() <= 0 then
			...
			slot1.time1Txt.text = "00:00:00"
		else
			slot1.time1Txt.text = pg.TimeMgr.GetInstance():DescCDTime(slot1)
		end
	end, 1, -1)
