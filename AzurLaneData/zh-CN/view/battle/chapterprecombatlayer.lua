...
slot0.updateView = function (slot0, slot1)
	...
	slot3 = coroutine.wrap(function ()
		...
		if slot0._gridTFs[TeamType.Main][1].Find("flag") then
			...
			slot0.setAllCharacterPos(slot0, true)
		end
...
slot0.updateStageView = function (slot0, slot1)
	...
	slot10:make(function (slot0, slot1, slot2)
		...
		onButton(slot1, slot2, function ()
			if pg.item_data_statistics[slot0[2]] and slot1[slot0.type] then
				slot2.emit(slot2, ChapterPreCombatMediator.GET_CHAPTER_DROP_SHIP_LIST, slot2.chapter.id, function (slot0)
					slot2 = {}
	...
	slot10.align(slot10, math.min(#slot6, 6))
...
slot0.loadAllCharacter = function (slot0, slot1)
	...
	function slot2(slot0, slot1, slot2, slot3)
		...
		slot0.enabledCharacter(slot5, slot0, true, slot1, slot2)
...
slot0.shift = function (slot0, slot1, slot2, slot3)
	...
	slot0.chapter.fleet.switchShip(slot6, slot0.chapter.fleet.getShipsByTeam(slot6, slot3, false)[slot1].id, slot0.chapter.fleet.getShipsByTeam(slot6, slot3, false)[slot2].id)
...
slot0.enabledCharacter = function (slot0, slot1, slot2, slot3, slot4)
	if slot2 then
		...
		if slot5 then
			...
			GetOrAddComponent(slot5, "EventTriggerListener").AddDragEndFunc(slot7, function (slot0, slot1)
				slot0:GetComponent("SpineAnimUI"):SetAction("tuozhuai", 0)
				tf(slot0):SetParent(slot1._heroContainer, false)
				slot1:emit(ChapterPreCombatMediator.ON_SWITCH_SHIP, slot1.chapter.fleet)
...
slot0.displayFleetInfo = function (slot0)
	...
	slot5, slot6 = slot0.chapter.getFleetCost(slot5, slot1)
			...
			onButton(slot0, slot0._extraCostBuffIcon, function ()
				pg.MsgboxMgr.GetInstance():ShowMsgBox({
					...
					drop = {
						count = 1,
						type = DROP_TYPE_ITEM,
						id = tonumber(slot0.benefit_condition)
					},
					intro = pg.strategy_data_template[slot0.id].desc,
	...
	setActive(slot0.findTF(slot0, "middle/gear_score/vanguard").Find(slot10, "SonarActive"), ChapterFleet.StaticTransformChapterFleet2Fleet(slot1).GetFleetSonarRange(slot11) > 0)
...
slot0.updateStrategyIcon = function (slot0)
	...
	GetImageSpriteFromAtlasAsync("strategyicon/" .. pg.strategy_data_template[slot2.id].icon, "", slot0._strategy.Find(slot7, "icon"))
	onButton(slot0, slot0._strategy, function ()
		slot0:displayStrategyInfo(slot0)
	end, SFX_PANEL)
	setText(slot0._strategy.Find(slot5, "nums"), slot2.count)
