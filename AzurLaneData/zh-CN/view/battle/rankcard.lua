...
slot0.update = function (slot0, slot1, slot2)
	...
	slot5 = (slot1.rank > 0 and slot3) or 4
	...
	if slot0._type == slot1.TYPE_OTHER then
		...
		slot0.scoreTF.text = setColorStr(slot1:getPowerTxt(), slot1.COLORS[slot4])
	...
	if PowerRank.getScoreIcon(slot1.type) then
		if slot1.type == PowerRank.TYPE_PT then
			if slot2 then
				setImageSprite(slot0.scoreIconTF, LoadSprite(pg.item_data_statistics[id2ItemId(slot8)].icon))
			end
		else
