...
slot0.init = function (slot0)
	...
	ys.Battle.BattleVariable.initPainting(slot0)

	ys.Battle.BattleVariable._fxContainerUpper = ys.Battle.BattleVariable._tf:Find("FXContainerUpper")
	ys.Battle.BattleVariable._fxContainerBottom = ys.Battle.BattleVariable._tf:Find("FXContainerBottom")
	slot0._canvasOrder = (ys.Battle.BattleVariable._tf:GetComponentInParent(typeof(UnityEngine.Canvas)) and slot3.sortingOrder) or 0
end
...
slot0.appendSkill = function (slot0, slot1, slot2, slot3, slot4)
...
slot0.appendSkillCover = function (slot0, slot1, slot2, slot3)
...
slot0.painting = function (slot0, slot1, slot2, slot3)
	if slot0._currentPainting then
		slot0._paintingAnimator.enabled = false
...
slot0.didEnter = function (slot0)
	...
	onToggle(slot0, slot0:findTF("AutoBtn"), function (slot0)
		slot1 = slot0:GetBattleType()

		slot1:emit(BattleMediator.ON_AUTO, {
			isOn = not slot0,
			toggle = slot1:findTF("AutoBtn"),
			system = slot1
		})
		slot0:ActiveBot(ys.Battle.BattleState.IsAutoBotActive(slot1))
		setActive(setActive, slot0:ChatUseable())
	end, SFX_PANEL, SFX_PANEL)
	slot1.ConfigBattleEndFunc(slot1, function (slot0)
		slot0:clear()
...
slot0.AddUIFX = function (slot0, slot1, slot2)
	...
	slot1:SetParent(((slot2 or 1) > 0 and slot0._fxContainerUpper) or slot0._fxContainerBottom)
	pg.ViewUtils.SetSortingOrder(slot1, slot0._canvasOrder + (slot2 or 1))
	pg.ViewUtils.SetLayer(slot1, Layer.UI)

	return ((slot2 or 1) > 0 and slot0._fxContainerUpper) or slot0._fxContainerBottom.localScale
end
