...
slot0.quickExitFunc = function (slot0)
	...
	slot0.super.quickExitFunc(slot0)
end
		...
		if slot5 then
			...
			GetOrAddComponent(slot5, "ModelDrag").onModelClick:AddListener(function ()
				if slot0.contextData.system ~= SYSTEM_HP_SHARE_ACT_BOSS and slot0.contextData.system ~= SYSTEM_ACT_BOSS and slot0.contextData.system ~= SYSTEM_BOSS_EXPERIMENT then
					...
					pg.CriMgr.GetInstance().PlaySoundEffect_V3:emit(PreCombatMediator.CHANGE_FLEET_SHIP, pg.CriMgr.GetInstance().PlaySoundEffect_V3, slot0._currentFleetVO, )
				end
			end)
