...
slot0.init = function (slot0)
	...
	slot0.paintBtn = slot0.findTF(slot0, "ui/bg/paintBtn")
...
slot0.didEnter = function (slot0)
	...
	onButton(slot0, slot0.takeBtn, function ()
		...
		elseif slot0.state == slot1.STATE_TAKE_VIDEO then
			slot0 = pg.TimeMgr.GetInstance().STimeDescS(slot0, pg.TimeMgr.GetInstance():GetServerTime(), "*t")
			...
			if not PlayerPrefs.GetInt("hadShowForVideoTip") or slot2 <= 0 then
				...
				onButton(slot0, slot0.videoTipPanel, function ()
					...
					if PLATFORM_CODE == PLATFORM_JP and pg.SdkMgr.GetInstance().GetChannelUID(slot0) == "2" then
				...
				setActive(slot0.stopRecBtn, true)
				LeanTween.moveX(slot0.stopRecBtn, 0, 0.15).setOnComplete(slot3, System.Action(function ()
					slot0:SetMute(true)
					YARecorder.Inst:SetVideoFilename(YARecorder.Inst)
					YARecorder.Inst:StartRecording()
				end))
	...
	slot0.updateCameraCanvas(slot0)
...
slot0.updateSkin = function (slot0)
	...
	if slot0.showType == slot0.SHOW_LIVE2D then
		...
		pg.Live2DMgr.GetInstance():GetLive2DModelAsync(slot0.paintSkin, function (slot0)
			...
			slot0.transform.localPosition = BuildVector3(slot0.skin.live2d_offset)
			...
			slot0.live2dAnimator = slot0.GetComponent(slot0, typeof(Animator))
	...
	elseif slot0.showType == slot0.SHOW_SPINE then
		...
		PoolMgr.GetInstance().GetSpineChar(slot4, slot0.spineSkin, true, function (slot0)
			slot0.name = "model"
