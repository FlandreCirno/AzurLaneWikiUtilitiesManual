...
slot0.initUITextTips = function (slot0)
	setText(slot2, i18n("meta_exp_per_day"))
...
slot0.updateShipImg = function (slot0)
	...
	setLocalPosition(slot0.shipImg, {
		x = MetaCharacterConst.UIConfig[slot0.curMetaCharacterVO.id][7],
		y = MetaCharacterConst.UIConfig[slot0.curMetaCharacterVO.id][8]
	})
...
slot0.updateSkillListPanel = function (slot0)
	...
	slot0.skillUIItemList:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate and slot0[slot1 + 1] then
			slot1.skillBtnList[slot3] = slot2

			slot1:updateSkillTF(slot2, slot3)
		end
	end)
	slot0.skillUIItemList.align(slot4, #slot0:getSkillIDListForShow(slot0.curShipVO.configId))
end
...
slot0.updateSkillTF = function (slot0, slot1, slot2)
	...
	setImageSprite(slot5, LoadSprite("skillicon/" .. getSkillConfig(slot2).icon))
	setScrollText(slot0:findTF("Skill/Mask/Name", slot1), getSkillName(getSkillConfig(slot2).id))

	if slot0.curShipVO.getMetaSkillLevelBySkillID(slot3, slot2) > 0 then
		setText(slot6, "LEVEL: " .. slot12)
		setActive(slot9, false)
		onButton(slot0, slot1, function ()
			if not isActive(isActive) then
				eachChild(slot1.skillContainer, function (slot0)
					setActive(slot0:findTF("Skill/Arrow", slot0), false)
				end)
				setActive(setActive, true)
				setActive.updateTaskPanel(slot0, true)
			end
		end, SFX_PANEL)
	else
		setText(slot6, "LEVEL: ??")
		...
		onButton(slot0, slot1, function ()
			slot0:openUnlockSkillPanel(slot0)
		end, SFX_PANEL)
	end
end
...
slot0.updateSkillInfoPanel = function (slot0, slot1)
	setImageSprite(slot0.curSkillIcon, LoadSprite("skillicon/" .. getSkillConfig(slot1).icon))
	setScrollText(slot0.curSkillNameScrollText, getSkillName(getSkillConfig(slot1).id))
	...
	setText(slot0.nextSkillLevelText, slot7)
	...
	if not slot6 then
		...
		setSlider(slot0.curSkillProgressSlider, 0, slot0:getMetaSkillTacticsConfigBySkillID(slot1, slot5).need_exp, slot8)

		if slot8 < slot0.getMetaSkillTacticsConfigBySkillID(slot1, slot5).need_exp then
		end
	else
...
slot0.updateTaskListPanel = function (slot0, slot1)
	slot0.taskUIItemList:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			slot3 = slot0:findTF("Desc", slot2)
			slot5 = slot0:findTF("Text", slot2)
			slot9 = (slot0:getTaskInfoBySkillAndTaskID(slot2, slot1[slot1 + 1][1]) and slot8.finishCount) or 0

			setText(slot0:findTF("AddExp", slot2), "+" .. slot6[3])
	...
	slot0.taskUIItemList.align(slot6, #slot0:sortTaskConfig(slot1, slot0:getMetaSkillTacticsConfigBySkillID(slot1, slot3).skill_levelup_task))
end
...
slot0.updateTaskPanel = function (slot0, slot1)
	...
	elseif slot1 ~= slot0.curSkillID then
		...
		onButton(slot0, slot0.studySkillBtn, function ()
			if slot0.switchCountLeft == 0 then
		...
		setActive(slot0.taskTplContainer, true)
		setActive(slot0.taskScrollBar, true)
		slot0.updateSkillInfoPanel(slot0, slot1)
		slot0:updateTaskListPanel(slot1)
	end
end
...
slot0.updateMain = function (slot0)
	...
	setActive(slot0.taskScrollTF, not slot5)
...
slot0.tryLearnSkillAfterFirstUnlock = function (slot0)
	...
	slot0:updateTaskPanel(slot0.curUnlockSkillID)
	slot0:updateSkillTFLearning()
	slot0:TryPlayGuide()
end

slot0.moveShipImg = function (slot0, slot1)
	slot0:managedTween(LeanTween.moveX, nil, rtf(slot0.shipImg), (slot1 and MetaCharacterConst.UIConfig[slot0.curMetaCharacterVO.id][7]) or -2000, 0.2):setFrom((slot1 and -2000) or MetaCharacterConst.UIConfig[slot0.curMetaCharacterVO.id][7])
end

slot0.moveRightPanel = function (slot0)
	slot0:managedTween(LeanTween.moveX, nil, rtf(slot0.skillPanel), slot2, 0.2):setFrom(slot1)
	slot0:managedTween(LeanTween.moveX, nil, rtf(slot0.taskPanel), 500, 0.2):setFrom(2000)
end
...
slot0.openUnlockSkillPanel = function (slot0, slot1)
	...
	setText(slot0.skillUnlockPanelTipText, i18n("meta_unlock_skill_tip", slot4, slot5))

	slot7 = slot0:getMetaSkillTacticsConfigBySkillID(slot1, 1).skill_unlock
	...
	slot0.materialUIItemList.align(slot8, #{
		slot0.getMetaSkillTacticsConfigBySkillID(slot1, 1).skill_unlock[1]
	})
...
slot0.updateTacticsRedTag = function (slot0)
	slot2 = slot0.curShipVO.getMetaCharacter(slot1)
	slot4 = false
...
slot0.sortTaskConfig = function (slot0, slot1, slot2)
	table.sort(Clone(slot2), function (slot0, slot1)
		slot10 = slot0[2] > 0 and ((slot0:getTaskInfoBySkillAndTaskID(slot1, slot2) and slot6.finishCount) or 0) <= slot0[2]
		slot11 = slot1[2] > 0 and ((slot0:getTaskInfoBySkillAndTaskID(slot1, slot1[1]) and slot0.getTaskInfoBySkillAndTaskID(slot1, slot1[1]).finishCount) or 0) <= slot1[2]
