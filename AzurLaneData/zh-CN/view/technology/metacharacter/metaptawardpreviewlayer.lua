...
slot0.initUITextTips = function (slot0)
	setText(slot1, i18n("meta_pt_point"))
	setText(slot2, i18n("meta_pt_point"))
	setText(slot3, i18n("meta_award_get"))
...
slot0.findUI = function (slot0)
	...
	slot0.preArrow = slot0:findTF("PreBtn", slot0:findTF("NotchAdapt"))
...
slot0.initScrollList = function (slot0)
	...
	slot0.awardUIItemList:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			slot0:updateAwardTpl(slot2, slot1 + 1)
		end
	end)

	slot0.scrollRectSC = slot0.scrollViewTF.GetComponent(slot1, "ScrollRect")

	slot0.scrollRectSC.onValueChanged:AddListener(function (slot0)
		setActive(slot0.preArrow, slot0.x >= 0.01)
		setActive(slot0.nextArrow, slot0.x <= 0.99)
	end)
end

slot0.updateScrollList = function (slot0)
	slot1, slot6, slot3 = slot0.curMetaProgressVO.metaPtData:GetLevelProgress()
...
slot0.updateAwardTpl = function (slot0, slot1, slot2)
	...
	slot8 = slot0:findTF("NumText", slot7)
	...
	updateDrop(slot3, slot19, {
		hideName = true
	})
	onButton(slot0, slot3, function ()
		slot0:emit(BaseUI.ON_DROP, slot0)
	end, SFX_PANEL)
	setText(slot8, slot18)
	setText(slot10, slot18)
	setText(slot0:findTF("LockText", slot1), "PHASE " .. math.floor(slot0.ptData.targets[slot2] / slot0.curMetaProgressVO.unlockPTNum * 100) .. "%")
