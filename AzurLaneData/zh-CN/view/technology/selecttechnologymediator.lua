...
slot0.register = function (slot0)
	...
	slot0.bind(slot0, slot0.ON_BLUEPRINT, function ()
		slot0:sendNotification(GAME.GO_SCENE, SCENE.SHIPBLUEPRINT)
	end)
	slot0.bind(slot0, TechnologyConst.OPEN_TECHNOLOGY_TREE_SCENE, function ()
		slot0:sendNotification(GAME.GO_SCENE, SCENE.TECHNOLOGY_TREE_SCENE)
	end)
	slot0.bind(slot0, slot0.ON_TRANSFORM_EQUIPMENT, function ()
	...
	slot0.viewComponent.setPlayer(slot2, getProxy(PlayerProxy).getData(slot1))
	slot0.viewComponent:notifyTechnology(slot2)
	slot0.viewComponent:notifyBlueprint(slot3)
	slot0.viewComponent:notifyFleet(slot4)
...
slot0.onBlueprintNotify = function ()
	...
	if not slot0:getBuildingBluePrint() then
		...
		if slot2.getState(slot2) == ShipBluePrint.STATE_DEV_FINISHED then
			return true
		end
