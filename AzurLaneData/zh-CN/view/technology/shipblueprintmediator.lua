...
slot0.register = function (slot0)
	...
	slot0:bind(slot0.ON_CLICK_SPEEDUP_BTN, function ()
		slot0:addSubLayers(Context.New({
			viewComponent = TecSpeedUpLayer,
			mediator = TecSpeedUpMediator
		}))
	end)
	slot0.bind(slot0, slot0.ON_MAIN, function ()
	...
	slot0.bind(slot0, slot0.ON_FINISH_TASK, function (slot0, slot1)
		if Task.New({
			id = slot1
		...
		elseif slot2.getConfig(slot2, "sub_type") == TASK_SUB_TYPE_PLAYER_RES then
			pg.MsgboxMgr.GetInstance():ShowMsgBox({
				content = i18n("blueprint_commit_tip", getDropInfo({
					{
						DROP_TYPE_RESOURCE,
			...
			slot0.sendNotification(slot3, GAME.SUBMIT_TASK, slot1)
		end
	end)
	slot0.bind(slot0, slot0.ON_MOD, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.MOD_BLUEPRINT, {
			id = slot1,
			count = slot2
		})
	end)
	slot0.bind(slot0, slot0.ON_PURSUING, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.PURSUING_BLUEPRINT, {
			id = slot1,
			count = slot2
		})
	end)
	...
	slot0.bind(slot0, slot0.ON_CHECK_TAKES, function (slot0, slot1)
		if getProxy(TechnologyProxy).getBluePrintById(slot2, slot1):isFinishedAllTasks() then
			slot3:finish()
			slot2:updateBluePrint(slot3)
		end
	end)
	slot0.bind(slot0, slot0.SHOW_SKILL_INFO, function (slot0, slot1, slot2, slot3)
		slot0:addSubLayers(Context.New({
	...
	slot0.bind(slot0, slot0.SET_TECHNOLOGY_VERSION, function (slot0, slot1)
	...
	slot0.viewComponent:setShipVOs(getProxy(BayProxy).getRawData(slot3))
...
slot0.handleNotification = function (slot0, slot1)
	...
	elseif GAME.STOP_BLUEPRINT_DONE == slot2 then
		slot0.viewComponent.clearTimers(slot4, slot3.id)
