...
slot0.register = function (slot0)
	slot0:bind(slot0.FETCH_RANK_FORMATION, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.WORLD_BOSS_GET_FORMATION, {
			bossId = slot2,
			userId = slot1
		})
	end)
	slot0.bind(slot0, slot0.GO_META, function (slot0, slot1)
		slot0:sendNotification(GAME.GO_SCENE, SCENE.METACHARACTER, {
			autoOpenSyn = true,
			autoOpenShipConfigID = slot1 * 10 + 1
		})
	end)
	slot0.bind(slot0, slot0.ON_ACTIVE_BOSS, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.WORLD_ACTIVE_WORLD_BOSS, {
			cmd = 1,
			activity_id = slot1,
			arg1 = slot2
		})
	end)
	slot0.bind(slot0, slot0.ON_SELF_BOSS_OVERTIME, function (slot0)
		slot0:sendNotification(GAME.WORLD_SELF_BOSS_OVERTIME)
	end)
	slot0.bind(slot0, slot0.ON_SUBMIT_AWARD, function (slot0, slot1)
		slot0:sendNotification(GAME.WORLD_BOSS_SUBMIT_AWARD, {
			bossId = slot1
		})
	end)
	slot0.bind(slot0, slot0.ON_SURPPORT, function (slot0, slot1)
		if slot1[3] == true then
			slot0:sendNotification(GAME.WORLD_BOSS_SUPPORT, {
	...
	slot0.bind(slot0, slot0.ON_FETCH_BOSS, function (slot0)
		slot0:updateBossProxy()
	end)
	slot0.bind(slot0, slot0.ON_BATTLE, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.WORLD_BOSS_START_BATTLE, {
			bossId = slot1,
			isOther = slot2
		})
	end)
	slot0.bind(slot0, slot0.ON_RANK_LIST, function (slot0, slot1)
		slot0:sendNotification(GAME.WORLD_GET_BOSS_RANK, {
			bossId = slot1
		})
	end)
	slot0.bind(slot0, slot0.GET_RANK_CNT, function (slot0, slot1, slot2)
		if not slot0.viewComponent.bossProxy:GetRank(slot1) then
			slot0:sendNotification(GAME.WORLD_GET_BOSS_RANK, {
				bossId = slot1,
				callback = slot2
			})
	...
	slot0.bind(slot0, slot0.UPDATE_CACHE_BOSS_HP, function (slot0, slot1)
		slot0:sendNotification(GAME.GET_CACHE_BOSS_HP, {
			callback = slot1
		})
	end)
end
