...
slot0.Ctor = function (slot0, slot1, slot2)
	...
	slot0.capacity = math.ceil(slot0.container.parent.parent.rect.height / (slot0.tplHeight + slot0.space))

	for slot6 = 1, slot0.capacity, 1 do
...
slot0.Align = function (slot0, slot1, slot2)
	...
	if slot1 < slot0.capacity then
		slot0.ranges[1] = slot1 - (math.floor(slot1 * 0.5) + 1)
		slot0.ranges[2] = math.floor(slot1 * 0.5) + 1
	end
...
slot0.InitList = function (slot0)
	for slot4 = 1, slot0.capacity, 1 do
...
slot0.AddListener = function (slot0)
	...
	function slot6(slot0)
		if slot0 > 0 then
			return slot0.index < slot0.ranges[2] - 1
		else
			return slot0.index > -slot0.ranges[1]
		end
	end

	slot0.trigger.AddBeginDragFunc(slot7, function (slot0, slot1)
	...
	slot0.trigger.AddDragFunc(slot7, function (slot0, slot1)
		if slot0.animFlag then
			return
		end

		if slot1.position.y < slot1 and slot2 ~= 0 then
			slot2 = slot1.position
			slot3 = 0
		end

		if slot1 < slot1.position.y and slot4 ~= 0 then
			slot4 = 0
			slot3 = slot1.position
		end

		if not slot5(slot1.position.y - slot3.y) then
			slot6 = false

			return
		end

		if slot2 / slot0.distance < math.abs(slot2 / slot0.distance) then
			slot4 = slot3
	...
	slot0.trigger.AddDragEndFunc(slot7, function (slot0, slot1)
		if not slot0 then
			return
		end
...
slot0.SliceTo = function (slot0, slot1)
	...
	for slot9 = 1, math.abs(slot3), 1 do
		table.insert(slot5, function (slot0)
			slot0:Switch(slot0.Switch)
			Timer.New(slot0, slot0.animTime * 2, 1):Start()
		end)
	end
