...
slot0.init = function (slot0)
	...
	onButton(slot0, slot0.confirmBtn, function ()
		...
		if #slot0._selectedShipList > 0 then
			...
			if slot0.itemVO.getWorldItemType(slot2) == WorldItem.UsageBuff then
				slot3 = slot0.itemVO:getItemBuffID()
				slot4 = _.any(slot0._selectedShipList, function (slot0)
					return slot0:IsBuffMax()
				end)
				slot1 = slot4
			elseif slot2 == WorldItem.UsageHPRegenerate or slot2 == WorldItem.UsageHPRegenerateValue then
				slot3 = _.any(slot0._selectedShipList, function (slot0)
	...
	onButton(slot0, slot0.fleetToggleMask, function ()
		slot0:showOrHideToggleMask(false)
	end, SFX_CANCEL)
	onButton(slot0, slot0.btnFleet, function ()
		slot0:showOrHideToggleMask(true)
	end, SFX_PANEL)
	onButton(slot0, slot0._tf:Find("item/reset_btn"), function ()
		slot0.contextData.onResetInfo({
			count = 1,
...
slot0.updateFleetButton = function (slot0, slot1)
	...
	setText(slot0.btnFleet:Find("fleet/enFleet"), slot0.TeamNum[slot1] .. " FLEET")
...
slot0.setFleet = function (slot0, slot1)
	...
	slot0._selectedShipList = {}
	slot0._shipTFList = {}
	slot5 = underscore.map(slot4, function (slot0)
	...
	for slot10 = 1, 6, 1 do
		...
		if slot4[slot10] then
			slot0._shipTFList[slot11.id] = cloneTplTo(slot0.shipTpl, slot0.shipsContainer)
...
slot0.OnUpdateShipHP = function (slot0)
	...
	for slot6, slot7 in pairs(slot0._shipTFList) do
		if slot0._preSelectedList[slot6] then
			setImageColor(slot7:Find("hp").Find(slot9, "progress_bg/bar"), (slot1:GetShip(slot6):IsHpSafe() and Color.New(0.615686274509804, 0.9176470588235294, 0.23529411764705882)) or Color.New(0.615686274509804, 0.9176470588235294, 0.23529411764705882))
			...
			setActive(slot9.Find(slot9, "broken"), slot8:IsBroken())
			...
			if IsNil(slot7:Find("buff/bg/levelup(Clone)")) then
				PoolMgr.GetInstance():GetUI("levelup", true, function (slot0)
					if IsNil(slot0._tf) then
						...
						setParent(slot0, slot1:Find("buff/bg"))
...
slot0.initHP = function (slot0, slot1, slot2)
	...
	onButton(slot0, slot1, function ()
		if table.contains(slot0._selectedShipList, ) then
			...
			slot0:updateSelectShipHP(slot0, false)
			...
			while slot0.quota <= #slot0._selectedShipList do
				slot0:updateSelectShipHP(slot1, false)
				table.remove(slot0._selectedShipList, 1)
			end

			slot0:updateSelectShipHP(slot0, true, slot0.updateSelectShipHP)
	...
	return not slot2.IsHpFull(slot2)
end
...
slot0.initBuff = function (slot0, slot1, slot2)
	...
	GetImageSpriteFromAtlasAsync("attricon", slot9, slot5)
	setText(slot4:Find("value"), (not (slot0._shipVOList[slot2.id]:getBaseProperties()[WorldBuff.GetTemplate(slot7).buff_attr[1]] > 0) and "Lv.-") or (slot2:IsBuffMax(slot7) and "Lv.MAX") or "Lv." .. slot2:GetBuff(slot7):GetFloor())
	onButton(slot0, slot1, function ()
		if table.contains(slot0._selectedShipList, ) then
			if #slot0._selectedShipList <= 0 then
				return
			...
			slot0:updateSelectShipBuff(slot0, false)
		else
			if slot0.quota <= #slot0._selectedShipList then
				return
			end

			slot0:updateSelectShipBuff(slot0, true)
			table.insert(slot0._selectedShipList, table.insert)
		end
	...
	return slot0._shipVOList[slot2.id].getBaseProperties()[WorldBuff.GetTemplate(slot7).buff_attr[1]] > 0 and not slot2.IsBuffMax(slot7)
end
