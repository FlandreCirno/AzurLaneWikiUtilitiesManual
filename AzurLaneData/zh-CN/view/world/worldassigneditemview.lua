...
slot0.OnInit = function (slot0)
	...
	onButton(slot0, slot0.confirmBtn, function ()
		...
		slot0:emit(WorldInventoryMediator.OnUseItem, slot0.itemVO.id, slot0.count, slot0.selectedVO)
		slot0.emit:Hide()
	end, SFX_PANEL)
end
...
slot0.update = function (slot0, slot1)
	...
	slot0.ulist:make(function (slot0, slot1, slot2)
		...
		if slot0 == UIItemList.EventUpdate then
			...
			onToggle(slot1, slot2, function (slot0)
				if slot0 then
					slot0.selectedVO = slot1:getConfig("usage_arg")[]

					setText(slot1.getConfig("usage_arg"), slot0.count * "usage_arg"[3])

					slot0.selectedItem = slot5
				end
			end, SFX_PANEL)
			setScrollText(slot2.Find(slot2, "name_bg/Text"), ({
				type = slot0[slot1][1],
				id = slot0[slot1][2],
				count = slot0[slot1][3]
			})["cfg"].name)
	...
	slot0.ulist.align(slot3, #slot1:getConfig("usage_arg"))
	slot0:updateValue()
