...
slot0.init = function (slot0)
	...
	eachChild(slot0.rtBg, function (slot0)
		setActive(slot0, slot0.name == tostring(slot0))
	end)

	slot0.rtPanel = slot0._tf.Find(slot2, "panel")
...
slot0.didEnter = function (slot0)
	...
	onButton(slot0, slot0.btnGo, function ()
		slot0, slot1 = slot0:CheckValid()

		if slot0 then
			slot0:emit(WorldFleetSelectMediator.OnGO)
		else
	...
	function slot1(slot0)
		...
		for slot4, slot5 in pairs(slot0.rtFleets) do
			for slot9 = 1, #slot0.contextData.fleets[slot4], 1 do
				slot0:updateCommanderBtn(slot5:GetChild(slot9 - 1), slot4, slot9)
			end
		end
	end
...
slot0.UpdateFleets = function (slot0)
	for slot5, slot6 in pairs(slot1) do
		slot8 = UIItemList.New(slot7, slot0.rtFleets[slot5].GetChild(slot7, 0))

		slot8:make(function (slot0, slot1, slot2)
			if slot0 == UIItemList.EventUpdate then
				slot0:UpdateFleet(slot2, slot1, slot1 + 1)
			end
		end)
		slot8.align(slot8, #slot1[slot5])
		setActive(slot0.rtFleets[slot5], #slot1[slot5] > 0)
	end
		...
		if slot0:GetTeamShipCount(slot9[TeamType.Main]) ~= 0 then
			if slot0:GetTeamShipCount(slot9[TeamType.Vanguard]) == 0 then
				...
				for slot16, slot17 in ipairs(slot2) do
					slot23, slot19, slot20, slot21 = unpack(slot17)
...
slot0.updateEliteLimit = function (slot0)
	...
	slot4:make(function (slot0, slot1, slot2)
		...
		if slot0 == UIItemList.EventUpdate then
			slot4, slot5, slot6, slot7 = unpack(slot3)

			if slot1[slot1] == 1 then
			...
			setText(slot2:Find("Text"), AttributeType.EliteCondition2Name(slot4, slot7) .. AttributeType.eliteConditionCompareTip[slot5] .. slot6 .. "（" .. slot2[slot4] .. "）")
		end
	end)
	slot4.align(slot4, #slot1)
end

slot0.updateCommanderBtn = function (slot0, slot1)
	setActive(slot2, not slot0.contextData.showCommander)
	setActive(slot3, not slot0.contextData.showCommander)
	setActive(slot1:Find("commander"), slot0.contextData.showCommander)
end

slot0.UpdateFleet = function (slot0, slot1, slot2, slot3)
	slot0:updateCommanders(slot4, slot2, slot3)
	...
	onButton(slot0, slot8, function ()
		slot0:RecommendFormation(slot0, )
		slot0.RecommendFormation:UpdateFleet(slot0, slot0.RecommendFormation, )
		slot0.RecommendFormation.UpdateFleet:updateEliteLimit()
	end, SFX_PANEL)
	onButton(slot0, slot1:Find("btn_clear"), function ()
		if slot0:GetTeamShipCount(slot1[TeamType.Main]) > 0 or slot0:GetTeamShipCount(slot1[TeamType.Vanguard]) > 0 or slot0:GetTeamShipCount(slot1[TeamType.Submarine]) > 0 then
			pg.MsgboxMgr.GetInstance():ShowMsgBox({
				...
				onYes = function ()
					...
					TeamType.Submarine:UpdateFleet(, slot3, slot4)
					TeamType.Submarine:updateEliteLimit()
				end
			})
		end
	end, SFX_CANCEL)
...
slot0.updateCommanders = function (slot0, slot1, slot2, slot3)
	...
	for slot9 = 1, 2, 1 do
		...
		onButton(slot0, slot12, function ()
			slot0:openCommanderPanel(slot0, , )
		end, SFX_PANEL)
...
slot0.UpdateShips = function (slot0, slot1, slot2, slot3)
	...
	for slot10, slot11 in ipairs({
		...
		for slot15 = 1, 3, 1 do
			table.insert(slot6, (slot3[slot11][slot15] and slot4:getShipById(slot3[slot11][slot15])) or nil)

			if not ((slot3[slot11][slot15] and slot4.getShipById(slot3[slot11][slot15])) or nil) then
	...
	for slot10 = 1, 3, 1 do
		...
		if slot5[slot10] then
			...
			setActive(cloneTplTo(slot0.rtEmptyTpl, slot1, "empty").Find(slot11, "ship_type"), false)
		end

		onButton(slot0, slot11:Find("icon_bg"), function ()
			slot0:emit(WorldFleetSelectMediator.OnSelectShip, slot0, , )
		end, SFX_PANEL)

		slot13 = GetOrAddComponent(slot11.Find(slot11, "icon_bg"), typeof(UILongPressTrigger))
		...
		slot13.onLongPressed:AddListener(function ()
			if not slot0 then
				slot1:emit(WorldFleetSelectMediator.OnSelectShip, , , )
			else
				slot1:emit(WorldFleetSelectMediator.OnShipDetail, {
					shipId = slot0.id,
					shipVOs = slot5
				})
			end
...
slot0.openCommanderPanel = function (slot0, slot1, slot2, slot3)
	slot0.levelCMDFormationView:setCallback(function (slot0)
		if slot0.type == LevelUIConst.COMMANDER_OP_SHOW_SKILL then
			slot0:emit(WorldFleetSelectMediator.OnCommanderSkill, slot0.skill)
		elseif slot0.type == LevelUIConst.COMMANDER_OP_ADD then
			...
			slot0:emit(WorldFleetSelectMediator.OnSelectEliteCommander, slot0.emit, slot0, slot0.pos)
			slot0:closeCommanderPanel()
		else
			slot0:emit(WorldFleetSelectMediator.OnCommanderFormationOp, {
...
slot0.CheckValid = function (slot0)
	...
	slot5, slot2 = slot0:IsPropertyLimitationSatisfy()
	slot3 = 1
