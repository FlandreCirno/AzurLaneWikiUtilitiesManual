...
slot0.init = function (slot0)
	...
	if NotchAdapt.CheckNotchRatio == 2 or not slot3:CheckLargeScreen() then
		slot2 = slot0.equipmentView.rect.width > 2000
	else
		...
		setActive(slot0.findTF(slot0, "dispos", slot0.bottomBack), false)
...
slot0.didEnter = function (slot0)
	...
	onButton(slot0, slot0.selectTransformPanel:Find("confirm_button"), function ()
		if slot0.contextData.onConfirm(slot0) then
			slot0:closeView()
		end
	end, SFX_PANEL)
end
...
slot0.initEquipments = function (slot0)
	...
	slot0.equipmentRect.ScrollTo(slot1, 0)
end
...
slot0.initEquipment = function (slot0, slot1)
	onButton(slot0, EquipmentItemTransformVer.New(slot1).go, function ()
		if slot0.sourceVO == nil then
			return
		end

		slot1:SelectTransformEquip(slot0.sourceVO, slot0.sourceVO.count)
	end, SFX_PANEL)

	slot0.equipmetItems[slot1] = EquipmentItemTransformVer.New(slot1)
end
	...
	if slot1 then
		...
		table.sort(slot0.loadEquipmentVOs, function (slot0, slot1)
			...
			if (slot0.template.shipId or -1) ~= (slot1.template.shipId or -1) then
				...
				return slot0.sortFunc(slot4, slot5, slot1, slot2)
			end
		end)
	end
