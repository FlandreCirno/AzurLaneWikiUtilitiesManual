...
slot0.BindEvent = function (slot0)
	slot0:bind(slot0.TRANSFORM_EQUIP, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.TRANSFORM_EQUIPMENT, {
			candicate = slot1,
			formulaIds = {
				slot2
			}
	...
	slot0.bind(slot0, slot0.UPDATE_NEW_FLAG, function (slot0, slot1)
		slot0:sendNotification(slot1.UPDATE_NEW_FLAG, slot1)
	end)
	slot0.bind(slot0, slot0.OPEN_TRANSFORM_TREE, function (slot0, slot1)
		slot0:getViewComponent():closeView()
		slot0:sendNotification(GAME.GO_SCENE, SCENE.EQUIPMENT_TRANSFORM, {
			targetEquipId = slot1,
			mode = EquipmentTransformTreeScene.MODE_HIDESIDE
		})
	end)
	slot0.bind(slot0, slot0.SELECT_TRANSFORM_FROM_STOREHOUSE, function (slot0, slot1)
		slot0:sendNotification(GAME.GO_SCENE, SCENE.SELECT_TRANSFORM_EQUIPMENT, {
			warp = StoreHouseConst.WARP_TO_WEAPON,
			sourceVOs = slot0.env.tracebackHelper:GetEquipmentTransformCandicates(slot1),
			...
			onConfirm = function (slot0)
				slot0.contextData.sourceEquipmentInstance = slot0[1] or slot0.contextData.sourceEquipmentInstance
