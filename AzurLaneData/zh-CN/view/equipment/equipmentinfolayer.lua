...
slot0.checkOverGold = function (slot0, slot1)
	if slot0.player.GoldMax(slot3, _.detect(slot1, function (slot0)
...
slot0.didEnter = function (slot0)
	...
	slot0.initAndSetBtn(slot0, slot1)
...
slot0.initAndSetBtn = function (slot0, slot1)
	...
	elseif slot1 == EquipmentInfoMediator.TYPE_REPLACE then
		slot0.replaceSrcEquipTF = slot0.findTF(slot0, "equipment", slot0.replacePanel) or slot0:cloneSampleTo(slot0.replacePanel, slot0.Left, "equipment")
	...
	elseif slot1 == EquipmentInfoMediator.TYPE_DISPLAY then
		slot0.displayEquipTF = slot0.findTF(slot0, "equipment", slot0.displayPanel) or slot0:cloneSampleTo(slot0.displayPanel, slot0.Middle, "equipment")
	...
	elseif slot1 == slot0.PANEL_DESTROY then
		...
		slot0.destroyEquipTF = slot0.findTF(slot0, "equipment", slot0.destroyPanel) or slot0:cloneSampleTo(slot0.destroyPanel, slot0.Left, "equipment")
	...
	elseif slot1 == slot0.PANEL_REVERT then
		...
		slot0.revertEquipTF = slot0.findTF(slot0, "equipment", slot0.revertPanel) or slot0:cloneSampleTo(slot0.revertPanel, slot0.Left, "equipment")
...
slot0.updateRevertPanel = function (slot0)
	...
	Equipment.InsertAttrsCompare(slot0.equipmentVO:GetPropertiesInfo().attrs, slot1:GetPropertiesInfo().attrs, slot0.shipVO)
...
slot0.updateEquipmentPanel = function (slot0, slot1, slot2, slot3, slot4)
	...
	if slot2 then
		...
		eachChild(setText, function (slot0)
...
slot0.showDestoryMsgbox = function (slot0, slot1)
	...
	pg.UIMgr.GetInstance().BlurPanel(slot2, slot0.destroyMsgBox)
end
	...
	if isActive(slot0.defaultTransformTipBar) then
		...
		UIItemList.StaticAlign(slot0.defaultTransformTipBar:Find("list"), slot0.defaultTransformTipBar:Find("list/transformTarget"), #slot3, function (slot0, slot1, slot2)
			if slot0 == UIItemList.EventUpdate then
				...
				if not (slot0[slot1[slot1 + 1]] and slot3.target_id) then
				...
				onButton(slot2, slot2:Find("item"), function ()
					slot0:emit(EquipmentInfoMediator.OPEN_LAYER, Context.New({
						mediator = EquipmentTransformMediator,
						viewComponent = EquipmentTransformLayer,
						data = {
							fromStoreHouse = true,
							formulaId = slot1[slot2 + 1],
				...
				slot2.Find(slot2, "mask/name"):GetComponent("ScrollText"):SetText(pg.equip_data_statistics[slot4].name)
			end
		end)
	end
end
