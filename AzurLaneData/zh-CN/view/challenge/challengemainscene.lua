...
slot0.findUI = function (slot0)
	...
	slot0.buildCommanderPanel(slot0)
end
...
slot0.addListener = function (slot0)
	...
	onButton(slot0, slot0.startBtn, function ()
		...
		if slot0:isCrossedSeason() == true then
			...
			if not slot0.curModeInfo then
				...
				pg.MsgboxMgr.GetInstance():ShowMsgBox({
					hideNo = false,
					content = i18n((slot1 == ChallengeProxy.MODE_CASUAL and "challenge_season_update_casual_clear") or "challenge_season_update_infinite_clear", (slot1 == ChallengeProxy.MODE_CASUAL and slot0.curModeInfo.getScore(slot3)) or slot0.curModeInfo:getLevel()),
					onNo = function ()
						slot0:emit(ChallengeConst.RESET_DATA_EVENT, slot0)
					end,
					onYes = function ()
	...
	addSlip(SLIP_TYPE_HRZ, slot0.paintingContainer, slot1, function ()
...
slot0.updateSlider = function (slot0, slot1)
	...
	if slot0.curMode == ChallengeProxy.MODE_INFINITE and ChallengeConst.BOSS_NUM < slot3 then
		...
		slot0.squareList.align(slot6, ChallengeConst.BOSS_NUM)
...
slot0.updateSwitchModBtn = function (slot0)
	...
	onButton(slot0, slot0.casualModBtn, function ()
		...
		if (slot0.curModeInfo and slot0.curModeInfo:getLevel()) or 0.isCrossedSeason(slot2) then
			pg.MsgboxMgr.GetInstance():ShowMsgBox({
				hideNo = false,
	...
	onButton(slot0, slot0.infiniteModBtn, function ()
		...
		if (slot0.curModeInfo and slot0.curModeInfo:getScore()) or slot0.challengeInfo:getGradeList().seasonMaxScore.isCrossedSeason(slot2) then
			pg.MsgboxMgr.GetInstance():ShowMsgBox({
				hideNo = false,
...
slot0.openCommanderPanel = function (slot0, slot1, slot2)
	...
	slot0.levelCMDFormationView:setCallback(function (slot0)
		if slot0.type == LevelUIConst.COMMANDER_OP_SHOW_SKILL then
			slot0:emit(ChallengeMainMediator.ON_COMMANDER_SKILL, slot0.skill)
		elseif slot0.type == LevelUIConst.COMMANDER_OP_ADD then
			...
			slot0:emit(ChallengeMainMediator.ON_SELECT_ELITE_COMMANDER, slot0.emit, slot0.pos)
			slot0:closeCommanderPanel()
			slot0:hideFleetEdit()
		else
			slot0:emit(ChallengeMainMediator.COMMANDER_FORMATION_OP, {
				FleetType = LevelUIConst.FLEET_TYPE_ACTIVITY,
				data = slot0,
				fleetId = slot2.id,
				actId = ChallengeMainMediator.COMMANDER_FORMATION_OP
			})
		end
	end)
