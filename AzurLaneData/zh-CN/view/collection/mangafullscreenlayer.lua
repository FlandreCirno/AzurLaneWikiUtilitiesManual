...
slot0.addListener = function (slot0)
	...
	onButton(slot0, slot0.preBtn, function ()
		if slot0.curMangaIndex > 1 then
			slot0.curMangaIndex = slot0.curMangaIndex - 1

			slot0:readManga()
			slot0.readManga:updatePicImg()
			slot0.readManga.updatePicImg:updateLikeBtn()
		end
	end, SFX_PANEL)
	onButton(slot0, slot0.rightBtn, function ()
		if slot0.curMangaIndex < #slot0.mangaIDLIst then
			slot0.curMangaIndex = slot0.curMangaIndex + 1
	...
	onButton(slot0, slot0.likeOnBtn, function ()
		pg.m02:sendNotification(GAME.APPRECIATE_MANGA_LIKE, {
			mangaID = slot0.mangaIDLIst[slot0.curMangaIndex],
	...
	addSlip(SLIP_TYPE_HRZ, slot0.picImg, function ()
		triggerButton(slot0.preBtn)
	end, function ()
		triggerButton(slot0.rightBtn)
	end)
...
slot0.updatePicImg = function (slot0)
	setImageSprite(slot0.picImg, LoadSprite(MangaConst.MANGA_PATH_PREFIX .. slot2, pg.cartoon[slot0.mangaIDLIst[slot0.curMangaIndex]].resource))
...
slot0.updateLikeBtn = function (slot0)
	...
	setActive(slot0.likeOffBtn, not MangaConst.isMangaLikeByID(slot1))
end
