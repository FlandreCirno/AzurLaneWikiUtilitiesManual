...
slot0.SetBGFuncTag = function (slot0)
	if getProxy(PlayerProxy) then
		PlayerPrefs.SetInt(slot0.Set_BG_Func_Save_Tag .. getProxy(PlayerProxy):getRawData().id, (slot0 and 1) or 0)
	end
end

slot0.GetBGFuncTag = function ()
	if getProxy(PlayerProxy) then
		return (PlayerPrefs.GetInt(slot0.Set_BG_Func_Save_Tag .. getProxy(PlayerProxy):getRawData().id) == 1 and true) or false
	end
end
