...
slot0.register = function (slot0)
	...
	slot0.bind(slot0, slot0.ON_SHIP_DETAIL, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.GO_SCENE, SCENE.SHIPINFO, {
			shipId = slot1.id,
			shipVOs = slot2
		})
	end)
	slot0.bind(slot0, slot0.ON_WORLD_FORMATION, function ()
		return
	end)
	slot0.bind(slot0, slot0.ON_SHIP_REPAIR, function (slot0, slot1, slot2)
...
slot0.handleNotification = function (slot0, slot1)
	...
	if slot1:getName() == GAME.DESTROY_SHIP_DONE then
		...
		slot0.viewComponent.emit(slot5, BaseUI.ON_AWARD, {
			items = slot3.bonus,
	...
	elseif slot2 == GAME.WORLD_FLEET_REDEPLOY_DONE then
		slot0.viewComponent.emit(slot4, BaseUI.ON_BACK)
	end
end
