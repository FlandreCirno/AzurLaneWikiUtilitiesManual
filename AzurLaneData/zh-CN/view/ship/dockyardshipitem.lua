	...
	if tobool(slot0.shipVO) then
		...
		if slot6 then
			...
			GetSpriteFromAtlasAsync(slot6[1], slot6[2], function (slot0)
				setImageSprite(slot0.iconStatus, slot0, true)
				setActive(slot0.iconStatus, true)

				if setActive[1] == "shipstatus" then
					slot0.iconStatus.sizeDelta = Vector2(195, 36)
		...
		slot7, slot8 = slot1.getIntimacyIcon(slot1)
...
slot0.UpdateUser = function (slot0, slot1)
	if slot0.userIconFrame.childCount > 0 then
		PoolMgr.GetInstance():ReturnPrefab("IconFrame/" .. slot0.userIconFrame:GetChild(0).gameObject.name, slot0.userIconFrame.GetChild(0).gameObject.name, slot0.userIconFrame.GetChild(0).gameObject)
	end

	setActive(slot0.userTF, tobool(slot1) and slot1.user and tobool(slot1) and slot1.user.id ~= slot0.palyerId and slot0.detailType == slot0.DetailType0)
	setActive(slot0.userNameTF.gameObject.transform.parent, tobool(slot1) and slot1.user and tobool(slot1) and slot1.user.id ~= slot0.palyerId)

	if slot3 and slot2 ~= slot0.user then
		LoadSpriteAsync("qicon/" .. Ship.New({
			configId = slot2.icon
		}).getPrefab(slot4), function (slot0)
			slot0.userIconTF.sprite = slot0
		end)
		PoolMgr.GetInstance():GetPrefab("IconFrame/" .. slot5, AttireFrame.attireFrameRes(slot2, isSelf, AttireConst.TYPE_ICON_FRAME, slot2.propose), true, function (slot0)
			if IsNil(slot0.tr) then
				return
			end

			if slot0.userIconFrame then
				slot0.name = slot1

				setParent(slot0, slot0.userIconFrame, false)
			else
	...
	if tobool(slot0.shipVO) and slot0.DetailType0 < slot0.detailType then
		...
		for slot11 = 1, 6, 1 do
			...
			if slot0.detailType == slot0.DetailType1 then
				if slot11 == 6 then
					slot17, slot20 = slot0.shipVO:getIntimacyDetail()
					slot14.text = AttributeType.Type2Name(slot16)
					slot15.text = setColorStr(slot20, (slot17 <= slot18 and COLOR_GREEN) or COLOR_WHITE)
				else
					slot14.text = AttributeType.Type2Name(slot16)
...
slot0.updateWorld = function (slot0)
	if slot0.shipVO:getFlag("inWorld") then
		...
		setActive(slot3, slot2:IsHpSafe())
		setActive(slot4, not slot2:IsHpSafe())
...
slot0.updateIntimacy = function (slot0, slot1)
	...
	slot4, slot8 = slot2:getIntimacyDetail()
