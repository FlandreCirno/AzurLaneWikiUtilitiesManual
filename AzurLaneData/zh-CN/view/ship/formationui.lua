...
slot0.init = function (slot0)
	...
	slot0.fleetToggleList = slot0:findTF("list", slot0.fleetToggleMask)
	slot0.fleetToggles = {}

	for slot4 = 1, slot0.MAX_FLEET_NUM, 1 do
...
slot0.SetFleets = function (slot0, slot1)
	...
	end).sort(slot2, function (slot0, slot1)
...
slot0.updateFleetButton = function (slot0)
	slot0.index[slot0._currentFleetVO:getFleetType()] = slot0._currentFleetVO:getIndex()
...
slot0.quickExitFunc = function (slot0)
	...
	slot0.emit(slot0, FormationMediator.COMMIT_FLEET, function ()
...
slot0.didEnter = function (slot0)
	...
	onButton(slot0, slot0.backBtn, function ()
		...
		if slot0._attrFrame.gameObject.activeSelf then
			...
			slot0.emit(slot1, FormationMediator.COMMIT_FLEET, function ()
				GetOrAddComponent(slot0._tf, typeof(CanvasGroup)).interactable = false
	...
	setActive(slot0.findTF(slot0, "stamp"), BATTLE_DEBUG or getProxy(TaskProxy):mingshiTouchFlagEnabled())
	...
	onButton(slot0, slot0:findTF("stamp"), function ()
		if BATTLE_DEBUG then
			print(slot0._currentFleetVO:genRobotDataString())
		end
	...
	onButton(slot0, slot0._renameConfirmBtn, function ()
		...
		slot0:emit(FormationMediator.CHANGE_FLEET_NAME, slot0._currentFleetVO.id, slot0)
	end, SFX_CONFIRM)
	...
	onButton(slot0, slot0.fleetToggleMask, function ()
		setActive(slot0.fleetToggleMask, false)
		setActive:tweenTabArrow(true)
	end, SFX_CANCEL)
	onButton(slot0, slot0.btnRegular, function ()
		...
		triggerToggle(slot0.updateToggleList._currentFleetVO.getFleetType(slot0) == FleetType.Normal.fleetToggles[slot0.updateToggleList._currentFleetVO.getFleetType(slot0) == FleetType.Normal.index[FleetType.Normal]], true)

		if slot0.updateToggleList._currentFleetVO.getFleetType(slot0) == FleetType.Normal then
			setActive(slot0.fleetToggleMask, true)
			slot0:tweenTabArrow(false)
			setAnchoredPosition(slot0.fleetToggleList, Vector3.New(209, 129))
		end
	end, SFX_PANEL)
	onButton(slot0, slot0.btnSub, function ()
		...
		triggerToggle(slot0.updateToggleList._currentFleetVO.getFleetType(slot0) == FleetType.Submarine.fleetToggles[slot0.updateToggleList._currentFleetVO.getFleetType(slot0) == FleetType.Submarine.index[FleetType.Submarine]], true)

		if slot0.updateToggleList._currentFleetVO.getFleetType(slot0) == FleetType.Submarine then
			setActive(slot0.fleetToggleMask, true)
			slot0:tweenTabArrow(false)
			setAnchoredPosition(slot0.fleetToggleList, Vector3.New(755, 129))
		end
	end, SFX_PANEL)
	onButton(slot0, slot0._prevPage, function ()
		...
		slot0:emit(FormationMediator.ON_CHANGE_FLEET, slot0)
	end, SFX_PANEL)
	...
	slot0.SetCurrentFleetID(slot0, defaultValue(slot0.contextData.number, 1))
...
slot0.updateToggleList = function (slot0, slot1)
	...
	for slot7 = 1, #slot0.fleetToggles, 1 do
		setActive(slot0.fleetToggles[slot7], slot1[slot7])
			...
			if slot12 then
				...
				onToggle(slot0, slot8, function (slot0)
					if slot0 then
						...
						if slot0.tweenTabArrow.id ~= slot0 then
							if slot0._currentDragDelegate then
								slot0._forceDropCharacter = true
							...
							slot0:emit(FormationMediator.ON_CHANGE_FLEET, slot1.id)
						end
					end
				end, SFX_UI_TAG)
			else
...
slot0.loadAllCharacter = function (slot0)
	...
	function slot2(slot0, slot1, slot2, slot3)
		...
		for slot13 = 1, slot0.shipVOs[slot1].getStar(slot4), 1 do
		...
		tf(slot0).SetParent(slot13, slot5, false)
		...
		slot0.eventTriggers[slot19] = true
		...
		pg.DelegateInfo.Add(slot0, slot18.onLongPressed)
		slot18.onLongPressed:AddListener(function ()
			slot0:emit(FormationMediator.OPEN_SHIP_INFO, slot1.id, slot0._currentFleetVO, slot2.TOGGLE_FORMATION)
			pg.CriMgr.GetInstance():PlaySoundEffect_V3(SFX_PANEL)
		end)

		slot21, slot22, slot23, slot24 = nil

		pg.DelegateInfo.Add(slot0, slot17.onModelClick)
		slot17.onModelClick.AddListener(slot25, function ()
			slot0:emit(FormationMediator.CHANGE_FLEET_SHIP, slot0, slot0._currentFleetVO, slot2.TOGGLE_FORMATION, )
			pg.CriMgr.GetInstance():PlaySoundEffect_V3(SFX_PANEL)
		end)
		slot19.AddBeginDragFunc(slot19, function ()
		...
		slot19.AddDragFunc(slot19, function (slot0, slot1)
		...
		slot19.AddDragEndFunc(slot19, function (slot0, slot1)
			if slot0._modelDrag ~= slot1 then
				return
			end
			...
			function slot3()
				...
				slot0.switchToDisplayMode.sortSiblingIndex:emit(FormationMediator.CHANGE_FLEET_SHIPS_ORDER, slot0._currentFleetVO)
			end
			...
			if slot1.position.x < UnityEngine.Screen.width * 0.15 or slot1.position.x > UnityEngine.Screen.width * 0.87 or slot1.position.y < UnityEngine.Screen.height * 0.18 or slot1.position.y > UnityEngine.Screen.height * 0.7 then
				if not slot0._currentFleetVO:canRemove(slot3) then
					slot4, slot5 = slot0._currentFleetVO:getShipPos(slot3)

					pg.TipsMgr.GetInstance():ShowTips(i18n("ship_formationUI_removeError_onlyShip", slot3:getConfigTable().name, slot0._currentFleetVO.name, Fleet.C_TEAM_NAME[slot5]))
					slot3()
				else
					pg.MsgboxMgr.GetInstance():ShowMsgBox({
						zIndex = -30,
						...
						onYes = function ()
							...
							slot5:emit(FormationMediator.REMOVE_SHIP, slot4, slot5._currentFleetVO)
						end,
						onNo = slot3
					})
				end
		...
		slot0.setCharacterPos(slot25, slot2, slot3, slot5)
	end
	...
	if slot0._currentFleetVO.getFleetType(slot5) == FleetType.Normal then
...
slot0.resetGrid = function (slot0, slot1)
	...
	if #slot2 < 3 then
		...
		onButton(slot0, slot6, function ()
			slot0:emit(FormationMediator.CHANGE_FLEET_SHIP, nil, slot0._currentFleetVO, slot1.TOGGLE_FORMATION, )
...
slot0.displayFleetInfo = function (slot0)
	...
	if slot0._currentFleetVO:getFleetType() == FleetType.Normal then
		...
		if slot7 > 0 then
			setText(slot0._vanguardGS.Find(slot10, "SonarActive/Text"), math.floor(slot7))
		...
		if #slot0._cards[slot6] > 0 then
			...
			for slot13 = 1, 3, 1 do
				if slot13 <= #slot7 then
					...
					slot8[slot13]:updateProps(slot0:getCardAttrProps(slot0.shipVOs[slot7[slot13]]))
				else
	...
	slot3:AddPointClickFunc(function (slot0, slot1)
		if not slot0.carddrag and slot0 == slot1.go then
			if slot1.shipVO then
				slot0:emit(FormationMediator.OPEN_SHIP_INFO, slot1.shipVO.id, slot0._currentFleetVO, slot2.TOGGLE_DETAIL)
			else
				slot0:emit(FormationMediator.CHANGE_FLEET_SHIP, slot1.shipVO, slot0._currentFleetVO, slot2.TOGGLE_DETAIL, slot0)
			end
	...
	if slot1.shipVO then
		...
		slot3.AddDragFunc(slot3, function (slot0, slot1)
			...
			for slot7 = 1, #slot2, 1 do
				if slot2[slot7] ~= slot1 and slot2[slot7].shipVO and slot1.tr.localPosition.x > slot2[slot7].tr.localPosition.x + ((slot3 < slot0._shiftIndex and 1.1) or -1.1) * slot3 then
					slot3 = slot3 + 1
				end
			end

			if slot0._shiftIndex ~= slot3 then
				slot0:shift(slot0._shiftIndex, slot3, slot0.shift)
		...
		slot3.AddDragEndFunc(slot3, function (slot0, slot1)
			...
			function resetCard()
				...
				slot2.enabled = true
				slot3.enabled = true
				slot4._shiftIndex = nil

				slot4:updateUltimateTitle()
				slot4:sortSiblingIndex()
				slot4:emit(FormationMediator.CHANGE_FLEET_SHIPS_ORDER, slot4._currentFleetVO)
			...
			if slot0._forceDropCharacter then
				...
				LeanTween.value(slot1.go, slot1.tr.anchoredPosition.x, slot3[slot0._shiftIndex].x, slot3):setEase(LeanTweenType.easeOutCubic):setOnUpdate(System.Action_float(function (slot0)
...
slot0.tweenTabArrow = function (slot0, slot1)
	...
	if slot1 then
		...
		slot2.localPosition.y = 80
		slot2.localPosition = slot2.localPosition
		slot3.localPosition.y = 80
