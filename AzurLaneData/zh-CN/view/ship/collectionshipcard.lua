...
slot0.flush = function (slot0)
	...
	if slot1 then
		setActive(slot0.ringTF, slot0.propose and not slot1:isMetaGroup())
		setActive(slot0.ringMetaTF, slot0.propose and slot1.isMetaGroup())
	else
...
slot0.loadImage = function (slot0, slot1)
	...
	slot0.loadingPaintingName = slot1:getPainting(slot0.showTrans)

	LoadSpriteAsync("shipYardIcon/" .. slot4, function (slot0)
		if not IsNil(slot0.go) and slot0.loadingPaintingName ==  then
			slot0.iconShip.sprite = slot0
		end
	end)

	slot0.iconType.sprite = GetSpriteFromAtlas("shiptype", shipType2print(slot1.getShipType(slot1, slot0.showTrans)))

	setScrollText(slot0.labelName, slot1:getName(slot0.showTrans))
	setShipCardFrame(slot0.shipFrameImg, shipRarity2bgPrint(slot2, nil, slot1:isBluePrintGroup(), slot1:isMetaGroup()))
end
