...
slot0.register = function (slot0)
	...
	slot2 = slot0.bayProxy.getShipById(slot2, slot0.contextData.shipId)
	slot0.contextData.index = (_.detect(slot0.contextData.shipVOs, function (slot0)
		return slot0.contextData.shipId == slot0.id
	end) and table.indexof(slot0.contextData.shipVOs, slot0.contextData)) or 1
	...
	slot0.viewComponent:setPlayer((_.detect(slot0.contextData.shipVOs, function (slot0)
		return slot0.contextData.shipId == slot0.id
	end) and table.indexof(slot0.contextData.shipVOs, slot0.contextData)) or 1)

	slot5 = getProxy(ContextProxy)

	slot0:bind(slot0.BUY_ITEM_BY_ACT, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.SKIN_COUPON_SHOPPING, {
			shopId = slot1,
			cnt = slot2
		})
	end)
	slot0.bind(slot0, slot0.OPEN_SHIPPROFILE, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.GO_SCENE, SCENE.SHIP_PROFILE, {
			showTrans = slot2,
			groupId = slot1
		})
	end)
	slot0.bind(slot0, slot0.ON_SKIN_INFO, function (slot0, slot1, slot2)
	...
	slot0.bind(slot0, slot0.ON_DESTROY_SHIP, function (slot0)
	...
	slot0.bind(slot0, slot0.ON_RECORD_EQUIPMENT, function (slot0, slot1, slot2, slot3)
	...
	slot0.bind(slot0, slot0.OPEN_EVALUATION, function (slot0, slot1, slot2)
		if slot2 then
	...
	slot0.bind(slot0, slot0.ON_SELECT_EQUIPMENT_SKIN, function (slot0, slot1)
	...
	slot0.bind(slot0, slot0.ON_SELECT_EQUIPMENT, function (slot0, slot1)
		slot6 = getProxy(BayProxy):getEquipsInShips(slot5, slot1)
		...
		slot0.sendNotification(slot7, GAME.GO_SCENE, SCENE.EQUIPSCENE, {
			lock = true,
			equipmentVOs = slot6,
			shipId = slot0.contextData.shipId,
			pos = slot1,
	...
	slot0.bind(slot0, slot0.ON_UPGRADE, function (slot0, slot1)
		slot0:openUpgrade()
	end)
	slot0.bind(slot0, slot0.CLOSE_UPGRADE, function (slot0)
		slot0:closeUpgrade()
	end)
	slot0.bind(slot0, slot0.OPEN_INTENSIFY, function (slot0)
		slot0:openIntensify()
	end)
	slot0.bind(slot0, slot0.CLOSE_INTENSIFY, function (slot0)
		slot0:closeIntensify()
	end)
	slot0.bind(slot0, slot0.ON_LOCK, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.UPDATE_LOCK, {
			ship_id_list = slot1,
			is_locked = slot2
		})
	end)
	slot0.bind(slot0, slot0.ON_TAG, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.UPDATE_PREFERENCE, {
			shipId = slot1,
			tag = slot2
		})
	end)
	slot0.bind(slot0, slot0.ON_SKILL, function (slot0, slot1, slot2, slot3)
		slot0:addSubLayers(Context.New({
	...
	slot0.bind(slot0, slot0.CHANGE_SKIN, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.SET_SHIP_SKIN, {
			shipId = slot1,
			skinId = slot2
		})
	end)
	slot0.bind(slot0, slot0.BUY_ITEM, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.SHOPPING, {
			id = slot1,
			count = slot2
		})
	end)
	slot0.bind(slot0, slot0.UNEQUIP_FROM_SHIP_ALL, function (slot0, slot1)
	...
	slot0.bind(slot0, slot0.OPEN_REMOULD, function (slot0)
		slot0:openRemould()
	end)
	slot0.bind(slot0, slot0.CLOSE_REMOULD, function (slot0)
		slot0:closeRemould()
	end)
	slot0.bind(slot0, slot0.PROPOSE, function (slot0, slot1, slot2)
		slot0:addSubLayers(Context.New({
	...
	slot0.bind(slot0, slot0.ON_SEL_COMMANDER, function (slot0)
		slot0:sendNotification(GAME.GO_SCENE, SCENE.COMMANDPOST, {
			...
			quitTeam = getProxy(BayProxy):getShipById(slot0.contextData.shipId).hasCommander(slot1)
		})
	end)
	slot0.bind(slot0, slot0.ON_UPGRADE_MAX_LEVEL, function (slot0, slot1)
		slot0:sendNotification(GAME.UPGRADE_MAX_LEVEL, {
			shipId = slot1
		})
	end)
	slot0.bind(slot0, slot0.ON_TECHNOLOGY, function (slot0, slot1)
		slot0:sendNotification(GAME.GO_SCENE, SCENE.SHIPBLUEPRINT, {
			shipId = slot1.id
		})
	end)
	slot0.bind(slot0, slot0.OPEN_EQUIP_UPGRADE, function (slot0, slot1)
		slot0:addSubLayers(Context.New({
	...
	slot0.bind(slot0, slot0.ON_META, function ()
		slot0:sendNotification(GAME.GO_SCENE, SCENE.METACHARACTER, {
			autoOpenShipConfigID = slot1.configId
		})
	end)

	if slot0.contextData.selectedId then
		slot0.sendNotification(slot0, GAME.COMMANDER_EQUIP_TO_SHIP, {
			shipId = slot0.contextData.shipId,
...
slot0.getEquipmentSkins = function (slot0, slot1, slot2)
	...
	slot3 = getProxy(EquipmentProxy):getSkinsByTypes(slot1:getSkinTypes(slot2))
...
slot0.handleNotification = function (slot0, slot1)
	...
	elseif slot2 == GAME.REMOVE_LAYERS then
		if slot3.context.mediator == ProposeMediator then
			slot0.viewComponent.SwitchToDefaultBGM(slot4)
		end
