...
slot0.initTranformInfo = function (slot0)
	...
	slot2 = slot0.getShipTranformData(slot0)
...
slot0.updateGridTF = function (slot0, slot1)
	...
	function slot9()
		if slot0 == slot1.max_level then
			return slot2.STATE_FINISHED
		elseif slot3 then
			return slot2.STATE_ACTIVE
		else
			return slot2.STATE_LOCK
		end
	end
	...
	setActive(slot2.Find(slot2, "bgs/finished"), slot9() == slot0.STATE_FINISHED)
...
slot0.initShipModel = function (slot0)
	...
	PoolMgr.GetInstance().GetSpineChar(slot3, slot1, true, function (slot0)
		slot0(slot0)
	end)
end
...
slot0.updateFinished = function (slot0, slot1)
	...
	if nil then
		...
		setText(slot8.Find(slot8, "name"), i18n("common_ship_type"))
...
slot0.updateProgress = function (slot0, slot1)
	...
	if table.getCount(slot6) > 0 then
		...
		if slot3.skill_id ~= 0 then
			slot8 = pg.skill_data_template[slot3.skill_id].name

			setText(slot0.skillDesc, i18n("ship_remould_material_unlock_skill", HXSet.hxLan(slot8)))
		end
	else
		slot0.contextData.materialShipIds = nil
	end
		...
		if slot12 <= #slot7 then
			...
			if slot7[slot12][1] == id2ItemId(1) then
				...
				if slot16 then
					...
					slot13.GetComponent(slot13, typeof(Button)).targetGraphic = slot13:Find("IconTpl/icon_bg/icon"):GetComponent(typeof(Image))
				end
			else
	...
	if nil then
		...
		setText(slot11.Find(slot11, "name"), i18n("common_ship_type"))
	...
	onButton(slot0, slot0.confirmBtn, function ()
		...
		if slot2 then
			...
			slot6.contentText.AddListener(slot7, function (slot0, slot1)
				if slot0 == "clickDetail" then
					slot0:showToolTip(slot1)
				end
			end)
		else
			slot0.emit(slot6, ShipRemouldMediator.REMOULD_SHIP, slot0.shipVO.id, slot1)
		end
	end, SFX_CONFIRM)
...
slot0.showToolTip = function (slot0, slot1)
	...
	pg.UIMgr.GetInstance().OverlayPanel(slot4, slot0.tooltip, {
