...
slot0.init = function (slot0)
	...
	slot0.contextData.mode = defaultValue(slot0.contextData.mode, slot0.MODE_SELECT)
	slot0.contextData.otherSelectedIds = defaultValue(slot0.contextData.otherSelectedIds, {})
	...
	slot0.settingBtn = slot0.findTF(slot0, "blur_panel/adapt/left_length/frame/setting")
	...
	setActive(slot0.selectPanel, slot0.contextData.mode ~= slot0.MODE_WORLD)
	...
	setActive(slot0.worldPanel, slot0.contextData.mode == slot0.MODE_WORLD)
	...
	setActive(slot0.assultBtn, slot0.contextData.mode == slot0.MODE_GUILD_BOSS)

	if slot0.contextData.mode == slot0.MODE_OVERVIEW then
	...
	elseif slot0.contextData.mode == slot0.MODE_MOD then
		slot0.selecteEnabled = true
	...
	slot0.shipLayout = slot0.findTF(slot0, "main/ship_container/ships")
	slot0.scrollItems = {}
...
slot0.isDefaultStatus = function (slot0)
	...
	if slot0:selectAll(slot0.indexFlag, #slot0 - 1) or slot0:selectNone(slot0.indexFlag, #slot0 - 1) then
		slot1 = true
	end

	if slot0:selectAll(slot0.indexFlag2, #slot1 - 1) or slot0:selectNone(slot0.indexFlag2, #slot1 - 1) then
		slot2 = true
	end

	if slot0:selectAll(slot0.indexFlag3, #slot2 - 1) or slot0:selectNone(slot0.indexFlag3, #slot2 - 1) then
		slot3 = true
	end

	if slot0:selectAll(slot0.indexFlag4, #slot3 - 1) or slot0:selectNone(slot0.indexFlag4, #slot3 - 1) then
		slot4 = true
	end
...
slot0.onInitItem = function (slot0, slot1)
	...
	(not slot0.contextData.selectFriend or DockyardFriend.New(slot1)) and (not slot0.contextData.energyDisplay or DockyardShipItemForEnergy.New(slot1, slot0.contextData.hideTagFlags, slot0.contextData.blockTagFlags)) and DockyardShipItem.New(slot1, slot0.contextData.hideTagFlags, slot0.contextData.blockTagFlags):updateDetail(slot0.itemDetailType)

	(not slot0.contextData.selectFriend or DockyardFriend.New(slot1)) and (not slot0.contextData.energyDisplay or DockyardShipItemForEnergy.New(slot1, slot0.contextData.hideTagFlags, slot0.contextData.blockTagFlags)) and DockyardShipItem.New(slot1, slot0.contextData.hideTagFlags, slot0.contextData.blockTagFlags).isLoading = true

	onButton(slot0, (not slot0.contextData.selectFriend or DockyardFriend.New(slot1)) and (not slot0.contextData.energyDisplay or DockyardShipItemForEnergy.New(slot1, slot0.contextData.hideTagFlags, slot0.contextData.blockTagFlags)) and DockyardShipItem.New(slot1, slot0.contextData.hideTagFlags, slot0.contextData.blockTagFlags).go, function ()
		if slot0.shipVO then
			if not slot1.selecteEnabled then
				pg.CriMgr.GetInstance():PlaySoundEffect_V3(SFX_UI_CLICK)

				DockyardScene.value = pg.CriMgr.GetInstance().shipContainer.value

				slot1.onClick(slot0.shipVO, slot1.shipVOs)
			else
				slot0(slot1, (table.contains(slot1.selectedIds, slot0.shipVO.id) and SFX_UI_CANCEL) or SFX_UI_FORMATION_SELECT)
				slot1:selectShip(slot0.shipVO)
			end
		else
			pg.CriMgr.GetInstance():PlaySoundEffect_V3(SFX_UI_CLICK)
	...
	slot3 = GetOrAddComponent((not slot0.contextData.selectFriend or DockyardFriend.New(slot1)) and (not slot0.contextData.energyDisplay or DockyardShipItemForEnergy.New(slot1, slot0.contextData.hideTagFlags, slot0.contextData.blockTagFlags)) and DockyardShipItem.New(slot1, slot0.contextData.hideTagFlags, slot0.contextData.blockTagFlags).go, "UILongPressTrigger").onLongPressed

	if slot0.contextData.preView == BackYardShipInfoLayer.__cname then
		slot3.RemoveAllListeners(slot3)
		slot3:AddListener(function ()
			if slot0.shipVO then
				slot1.contextData.selectedIds = slot1.selectedIds
				slot1.contextData.skipSelect = true

				slot1.onClick(slot0.shipVO, _.select(slot1.shipVOs, function (slot0)
					return slot0
				end))
			end
		end)
...
slot0.initIndexPanel = function (slot0)
	onButton(slot0, slot0.indexBtn, function ()
		if not slot0.indexPanel then
			...
			slot0.greySprite = slot0.findTF(slot1, "resource/grey", slot0.indexPanel):GetComponent(typeof(Image)).sprite
		...
		elseif slot0.indexPanel.gameObject.activeSelf then
			LeanTween.scale(slot0.indexPanel, Vector3(0, 0, 0), 0.2).setOnComplete(slot1, System.Action(function ()
				setActive(slot0.indexPanel, false)
			end))
			slot0.animationIn(slot1)
...
slot0.initIndex = function (slot0)
	...
	for slot4, slot5 in ipairs(slot0) do
		if slot4 == 1 then
			...
			onButton(slot0, slot0.indexTFAll, function ()
				...
				setImageSprite(slot0.indexTFAll, slot0.blueSprite)
			end, SFX_UI_TAG)
		else
			...
			onToggle(slot0, slot0.indexs.Find("index_" .. slot5) or cloneTplTo(slot0.indexTpl, slot0.indexs, slot6), function (slot0)
				slot0.tmpIndexFlag[setImageSprite - 1] = slot0

				slot1(setImageSprite - 1, (slot0 and slot0.blueSprite) or slot0.greySprite)

				if slot0:selectAll(slot0.tmpIndexFlag, #slot3 - 1) or slot0:selectNone(slot0.tmpIndexFlag, #slot3 - 1) then
					triggerButton(slot0.indexTFAll)
				elseif slot0 then
	...
	slot0.indexTpl2 = slot0.findTF(slot0, "tpl", slot0.indexs2)
	...
	for slot4, slot5 in ipairs(ipairs) do
		if slot4 == 1 then
			...
			onToggle(slot0, slot0.indexs2.Find("index_" .. slot5) or cloneTplTo(slot0.indexTpl2, slot0.indexs2, slot6), function (slot0)
				slot0.tmpIndexFlag2[setImageSprite - 1] = slot0

				slot1(setImageSprite - 1, (slot0 and slot0.blueSprite) or slot0.greySprite)

				if slot0:selectAll(slot0.tmpIndexFlag2, #slot3 - 1) or slot0:selectNone(slot0.tmpIndexFlag2, #slot3 - 1) then
					triggerButton(slot0.indexTFAll2)
				elseif slot0 then
	...
	slot0.indexTpl3 = slot0.findTF(slot0, "tpl", slot0.indexs3)
	...
	for slot4, slot5 in ipairs(slot0.indexTpl3) do
		if slot4 == 1 then
			...
			onToggle(slot0, slot0.indexs3.Find("index_" .. slot5) or cloneTplTo(slot0.indexTpl3, slot0.indexs3, slot6), function (slot0)
				slot0.tmpIndexFlag3[setImageSprite - 1] = slot0

				slot1(setImageSprite - 1, (slot0 and slot0.blueSprite) or slot0.greySprite)

				if slot0:selectAll(slot0.tmpIndexFlag3, #slot3 - 1) or slot0:selectNone(slot0.tmpIndexFlag3, #slot3 - 1) then
					triggerButton(slot0.indexTFAll3)
				elseif slot0 then
	...
	slot0.indexTpl4 = slot0.findTF(slot0, "tpl", slot0.indexs4)
	...
	for slot4, slot5 in ipairs(slot3) do
		if slot4 == 1 then
			...
			onToggle(slot0, slot0.indexs4.Find("index_" .. slot5) or cloneTplTo(slot0.indexTpl4, slot0.indexs4, slot6), function (slot0)
				slot0.tmpIndexFlag4[setImageSprite - 1] = slot0

				slot1(setImageSprite - 1, (slot0 and slot0.blueSprite) or slot0.greySprite)

				if slot0:selectNone(slot0.tmpIndexFlag4, #slot3 - 1) then
					triggerButton(slot0.indexTFAll4)
				elseif slot0 then
					setImageSprite(slot0.indexTFAll4, slot0.greySprite)

					for slot4, slot5 in pairs(slot0.indexTFs4) do
						if getToggleState(slot5) and slot1 - 1 ~= slot4 then
							triggerToggle(slot5, false)
							setImageSprite(slot0.indexTFs4[slot4], slot0.greySprite)
						end
					end
				end
			end, SFX_UI_TAG, SFX_UI_TAG)
...
slot0.initSort = function (slot0)
	...
	setActive(slot0.sorts.Find(slot1, slot0.sorts.Find) or cloneTplTo(slot0.sortTpl, slot0.sorts, slot1):Find("dropdown"), true)

	slot0.sorts.Find(slot1, slot0.sorts.Find) or cloneTplTo(slot0.sortTpl, slot0.sorts, slot1).Find("dropdown").localEulerAngles = Vector3.New(0, 0, 90)

	setText(findTF(slot0.sorts.Find(slot1, slot0.sorts.Find) or cloneTplTo(slot0.sortTpl, slot0.sorts, slot1), "Image"), slot0.sorts.Find(slot1, slot0.sorts.Find) or cloneTplTo(slot0.sortTpl, slot0.sorts, slot1))
...
slot0.updateBarInfo = function (slot0)
	...
	if slot0.contextData.mode == slot0.MODE_WORLD or slot0.contextData.mode == slot0.MODE_GUILD_BOSS then
	...
	elseif slot0.contextData.leftTopWithFrameInfo then
		...
		setText(slot0.bottomTipsText, "")
		setActive(slot0.leftTipsText, false)
	elseif slot0.contextData.mode == slot0.MODE_REMOULD then
		setText(slot0.bottomTipsText, "")
		setActive(slot0.leftTipsText, false)
	else
		setText(slot0.bottomTipsText, "")
...
slot0.initWorldPanel = function (slot0)
	...
	setActive(slot0.worldPanel.Find(slot2, "btn_formation"), false)
end
...
slot0.repairWorldShip = function (slot0, slot1)
	...
	elseif not slot2.IsHpFull(slot2) then
		pg.MsgboxMgr.GetInstance():ShowMsgBox({
		...
		pg.TipsMgr.GetInstance().ShowTips(slot4, i18n("world_ship_repair_no_need"))
	end
end
...
slot0.filter = function (slot0)
	...
	if slot0.isRemouldOrUpgradeMode then
		slot0.filterForRemouldAndUpgrade(slot0)
	else
		slot0:filterCommon()
	end
	...
	if slot0.MODE_OVERVIEW == slot0.contextData.mode and DockyardScene.value then
...
slot0.filterCommon = function (slot0)
	...
	function slot2(slot0)
		if slot0.contextData.mode ~= slot1.MODE_GUILD_BOSS then
			return true
		end

		if slot0.isShowAssultShips then
			return true
		end
		...
		if slot0.user.id == slot0.player.id then
			return true
		end
	...
	for slot6, slot7 in pairs(slot0.shipVOsById) do
		if slot0.contextData.blockLock and slot7.GetLockState(slot7) == Ship.LOCK_STATE_LOCK then
		elseif slot0.teamTypeFilter and slot7:getTeamType() ~= slot0.teamTypeFilter then
		elseif (slot0:selectNone(slot0.indexFlag, #slot1 - 1) or slot0.indexFlag[slot2[slot7:getShipType()] + 2] or slot0.indexFlag[slot3[slot7:getTeamType()]]) and (slot0:selectNone(slot0.indexFlag2, #slot4 - 1) or slot0.indexFlag2[slot5[slot7:getConfig("nationality")]] or (slot0.indexFlag2[#slot0.indexFlag2] and slot5[slot7:getConfig("nationality")] == nil)) and (slot0:selectNone(slot0.indexFlag3, #slot6 - 1) or slot0.indexFlag3[slot7[slot7:getRarity()]]) and (not slot0.indexFlag4[1] or slot7:hasAvailiableSkin()) and (not slot0.indexFlag4[2] or (slot7:isRemouldable() and not slot7:isAllRemouldFinish())) and (not slot0.indexFlag4[3] or (not slot7:isMetaShip() and not slot7:isIntensifyMax())) and (not slot0.indexFlag4[4] or slot7:isReachNextMaxLevel()) and (not slot0.indexFlag4[5] or not slot7:isConfigMaxLevel()) and (not slot0.indexFlag4[6] or slot7:isSpecialFilter()) and (not slot0.indexFlag4[7] or slot7:hasProposeSkin()) and (slot0.filterTag == Ship.PREFERENCE_TAG_NONE or slot0.filterTag == slot7:GetPreferenceTag()) and slot2(slot7) then
	...
	if slot1 == 3 then
		...
		slot3 = (slot0.selectAsc and IndexConst.sortByCombatPowerAsc) or IndexConst.sortByCombatPower
		slot4 = true
	elseif slot1 == 1 then
		slot3 = (slot0.selectAsc and IndexConst.sortByCfgAsc("rarity")) or IndexConst.sortByCfg("rarity")
		slot4 = true
	elseif slot1 == 2 or slot1 == 4 then
		slot3 = (slot0.selectAsc and IndexConst.sortByFieldAsc(({
			"",
			"level",
			"",
			"createTime"
		...
		slot4 = true
	elseif slot1 == 5 then
		slot3 = (slot0.selectAsc and IndexConst.sortByIntimacyAsc) or IndexConst.sortByIntimacy
	else
		slot3 = (slot0.selectAsc and IndexConst.sortByPropertyAsc(({
			"cannon",
			"air",
			"dodge",
			"antiaircraft",
			...
			"durability",
			"antisub"
		})[slot1 - #slot8])) or IndexConst.sortByProperty(()[slot1 - #slot8])
		slot4 = true
	end

	if slot4 and not defaultValue(slot0.contextData.hideTagFlags or {}.inFleet, ShipStatus.TAG_HIDE_BASE.inFleet) then
		slot5 = slot3
	...
	if slot3 then
		table.sort(slot0.shipVOs, function (slot0, slot1)
			...
			elseif slot0.contextData.mode == slot2.MODE_OVERVIEW or slot0.contextData.mode == slot2.MODE_SELECT then
				if slot0.activityNpc == slot1.activityNpc then
					return slot1(slot0, slot1)
				else
					return slot1.activityNpc < slot0.activityNpc
				end
			elseif slot0.contextData.mode == slot2.MODE_GUILD_BOSS then
	...
	setText(slot0:findTF("Image", slot0.sortBtn), slot8[slot1] or slot9[slot1 - #slot8])
end

slot0.UpdateGuildViewEquipmentsBtn = function (slot0)
	setActive(slot0.viewEquipmentBtn, slot0.contextData.mode == slot0.MODE_GUILD_BOSS and #slot0.selectedIds > 0)
end

slot0.didEnter = function (slot0)
	go(slot0.shipContainer):SetActive(true)
	pg.UIMgr.GetInstance():OverlayPanel(slot0.blurPanel)
	setActive(slot0.stampBtn, getProxy(TaskProxy):mingshiTouchFlagEnabled() and slot0.contextData.mode ~= slot0.MODE_GUILD_BOSS)
	...
	if slot0.contextData.mode == slot0.MODE_GUILD_BOSS then
		slot0.isShowAssultShips = false

		onToggle(slot0, slot0.assultBtn, function (slot0)
			slot0.isShowAssultShips = slot0
		...
		slot0.guildShipEquipmentsPage.SetCallBack(slot1, function ()
			slot0:TriggerCard(-1)
		end, function ()
			slot0:TriggerCard(1)
		end)
		onButton(slot0, slot0.viewEquipmentBtn, function ()
			if not slot0.selectedIds[#slot0.selectedIds] then
				return
			end

			slot0.guildShipEquipmentsPage:ExecuteAction("Show", slot0.shipVOsById[slot0], slot0.shipVOsById[slot0].user)
		end, SFX_PANEL)
	end

	eachChild(slot0.findTF(slot0, "attr_toggle", slot0.topPanel):GetComponent("Button"), function (slot0)
		setActive(slot0, false)
	end)

	slot0.isFormTactics = slot0.contextData.prevPage == "NavalTacticsMediator"
	slot2 = slot0.findTF(slot0, "off", slot1):GetComponent("Image")
	...
	if slot0.isRemouldOrUpgradeMode then
		...
		slot0.isFilterLockForMod = slot4.GetDockYardLockBtnFlag(slot4)
	...
	onButton(slot0, findTF(slot0.selectPanel, "cancel_button"), function ()
		...
		if slot0.contextData.mode == slot1.MODE_DESTROY then
			if #slot0.selectedIds > 0 then
				slot0:unselecteAllShips()
	...
	onButton(slot0, findTF(slot0.selectPanel, "confirm_button"), function ()
		...
		if slot0.contextData.mode == slot1.MODE_DESTROY then
			...
			if slot0.contextData.destroyCheck then
				slot1 = underscore.map(slot0.selectedIds, function (slot0)
					return slot0.shipVOsById[slot0]
				end)
				slot2, slot3 = ShipCalcHelper.GetEliteAndHightLevelShips(slot1)

				if #slot2 > 0 or #slot3 > 0 then
					table.insert(slot0, function (slot0)
						slot0.destroyConfirmWindow:ExecuteAction("Show", slot0.destroyConfirmWindow.ExecuteAction, slot0.destroyConfirmWindow, false, slot0)
					end)
				end
				...
				if #underscore.filter(slot1, function (slot0)
					...
					table.insert(slot0, function (slot0)
						slot0.destroyConfirmWindow:ExecuteAction("ShowEliteTag", slot0.destroyConfirmWindow.ExecuteAction, slot0)
					end)
				end
			end

			seriesAsync(slot0, function ()
				if slot0.confirmSelect then
				...
				elseif slot0.callbackQuit then
					slot0.onSelected(slot0.selectedIds, function ()
						slot0:back()
					end)
				else
					slot0.onSelected(slot0.selectedIds)
					slot0.onSelected.back(slot0)
				end
			end)
		end
	end, SFX_CONFIRM)
	onButton(slot0, findTF(slot0.selectPanel, "quick_select"), function ()
		...
		slot6 = _.select(slot0.shipVOs, function (slot0)
			return slot0.configId ~= 100001 and slot0.configId ~= 100011 and slot0:GetLockState() == Ship.LOCK_STATE_UNLOCK and table.contains(slot0, slot0:getRarity()) and slot0.level == 1 and not slot1.blacklist[slot0:getGroupId()] and not table.contains(slot1.selectedIds, slot0.id) and not slot0:hasAnyFlag({
				"inFleet",
				"inChapter",
				"inWorld",
				"inEvent",
		...
		if not _.all(slot6, function (slot0)
			return slot0.blacklist[slot0:getGroupId()]
		end) then
			slot7 = _.select(slot6, function (slot0)
		...
		slot9 = {}
		slot10 = _.select(slot6, function (slot0)
		...
		elseif slot11 then
			...
			slot0.displayDestroyPanel(slot13)
		end
	end, SFX_CONFIRM)

	if not slot0.contextData.selectFriend then
		slot0.shipContainer.GetComponentInChildren(slot4, typeof(GridLayoutGroup)).constraintCount = 7
	end
	...
	pg.SystemGuideMgr.GetInstance().Play(slot4, slot0)
end
...
slot0.TriggerCard = function (slot0, slot1)
	...
	function slot5()
		...
		if not slot2.shipVOs[] or slot2.checkShip(slot0) then
			return slot0
		else
			return slot3()
		end
	...
	function slot7()
		for slot4, slot5 in pairs(slot0.scrollItems) do
			if slot5.shipVO and slot5.go.name ~= "-1" and slot5.shipVO.id == slot1.id then
				slot0 = slot5
	...
	if slot7() and not getBounds(slot0:findTF("main/ship_container")):Intersects(getBounds(slot8.tr)) then
	...
	if slot8 then
		...
		slot0.guildShipEquipmentsPage:Refresh(slot0.shipVOsById[slot8.shipVO.id], slot0.shipVOsById[slot8.shipVO.id].user)
	end
end
...
slot0.selectShip = function (slot0, slot1, slot2)
	...
	if not slot3 then
		...
		if not slot5 then
			if slot6 then
				pg.TipsMgr.GetInstance().ShowTips(slot7, slot6)
			end
...
slot0.updateDestroyRes = function (slot0)
	...
	slot1 = _.map(slot0.selectedIds, function (slot0)
		return slot0.shipVOsById[slot0]
	end)
	slot2, slot3, slot4 = ShipCalcHelper.CalcDestoryRes(slot1)
	...
	if slot0.destroyResList then
		...
		slot0.destroyResList.align(slot6, 2 + #slot4)
	end
...
slot0.quickExitFunc = function (slot0)
	seriesAsync({
		function (slot0)
			if slot0.contextData.onQuickHome then
				slot0.contextData.onQuickHome(slot0)
			else
				slot0()
			end
		end,
...
slot0.displayDestroyPanel = function (slot0)
	if not slot0.destroyPage then
		...
		slot0.destroyPage:SetCardClickCallBack(function (slot0)
			slot0.blacklist[slot0.shipVO:getGroupId()] = true

			if table.indexof(slot0.selectedIds, slot0.shipVO.id) and slot1 > 0 then
		...
		slot0.destroyPage.SetConfirmCallBack(slot1, function ()
			...
			slot3 = underscore.map(slot0.selectedIds, function (slot0)
				return slot0.shipVOsById[slot0]
			end)
			slot4, slot5 = ShipCalcHelper.GetEliteAndHightLevelShips(slot3)

			if #slot4 > 0 or #slot5 > 0 then
				table.insert(slot0, function (slot0)
					slot5, slot8, slot9, slot10 = ShipCalcHelper.CalcDestoryRes(slot0)

					slot1.destroyConfirmWindow:ExecuteAction("Show", slot2, slot3, slot4, slot0)
				end)
			end
			...
			seriesAsync(slot0, function ()
				slot0:emit(DockyardMediator.ON_DESTROY_SHIPS, slot0.selectedIds)
			end)
		end)
	end

	if slot0.destroyPage and not slot0.destroyPage.GetLoaded(slot1) then
		pg.UIMgr.GetInstance():UnOverlayPanel(slot0.blurPanel, slot0._tf)
