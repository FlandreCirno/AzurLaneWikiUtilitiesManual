...
slot0.ShowEliteTag = function (slot0, slot1, slot2)
	...
	slot0.shipList:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			...
			setText(slot2:Find("icon_bg/level/Text"), "Lv." .. slot0[slot1 + 1].level)
		end
	end)
	slot0.shipList.align(slot3, #slot1)
	slot0.super.Show(slot0)
	pg.UIMgr.GetInstance():BlurPanel(slot0._tf)
end
...
slot0.Updatelayout = function (slot0)
	...
	if #slot2 > 0 then
		slot0.title.text = i18n("destroy_eliteship_tip", (slot3 == "" and i18n("destroy_high_level_tip", "")) or slot3 .. "、" .. i18n("destroy_high_level_tip", ""))
	end
	...
	slot5 = slot4 and slot0.overflow

	setActive(slot0.urOverflowLabel, slot5)
	...
	elseif slot6 <= 5 and slot4 then
		slot0.bg.sizeDelta = Vector2(slot0.bg.sizeDelta.x, 415 + ((slot5 and 40) or 0))
		slot0.window.sizeDelta = Vector2(slot0.window.sizeDelta.x, 720 + ((slot5 and 40) or 0))
	elseif slot6 > 5 and not slot4 then
		slot0.bg.sizeDelta = Vector2(slot0.bg.sizeDelta.x, 406)
		slot0.window.sizeDelta = Vector2(slot0.window.sizeDelta.x, 670)
	elseif slot6 > 5 and slot4 then
		slot0.bg.sizeDelta = Vector2(slot0.bg.sizeDelta.x, 537 + ((slot5 and 40) or 0))
		slot0.window.sizeDelta = Vector2(slot0.window.sizeDelta.x, 793 + ((slot5 and 40) or 0))
	end
...
slot0.UpdateShips = function (slot0)
	...
	slot0.shipList.align(slot4, #slot3)
end
