...
slot0.setActivity = function (slot0, slot1)
	...
	if slot0.activity and not slot0.activity:isEnd() then
		if not BuildShipScene.projectName then
			...
			elseif slot2 == ActivityConst.ACTIVITY_TYPE_BUILD then
				slot0:switchProject(slot0.POOL_ID_TO_PROJECTS[slot0.activity:getConfig("config_client").id])
			end
		end
...
slot0.RefreshActivityBuildPool = function (slot0, slot1)
	...
	if slot0.curProjectName == slot0.PROJECTS.ACTIVITY then
		slot0:UpdateBuildPoolExchange(slot1[slot0:getCreateId(slot0.curProjectName)])
	end
end
...
slot0.didEnter = function (slot0)
	...
	if getProxy(ActivityProxy).getActivityById(slot2, ActivityConst.ACTIVITY_PRAY_POOL) and not slot2:isEnd() then
...
slot0.initHelpMsg = function (slot0)
...
slot0.initBuildPanel = function (slot0)
	...
	for slot4, slot5 in pairs(slot0.PROJECTS) do
		...
		slot0.projectToggles[slot5] = slot6.Find(slot6, "frame")
	end

	setActive(slot0.projectToggles[slot0.PROJECTS.ACTIVITY].parent, slot0:isExActBuild())
	...
	elseif BuildShipScene.projectName then
		if BuildShipScene.projectName == slot0.PROJECTS.ACTIVITY and not slot0:isExActBuild() then
	...
	elseif slot0:isPickupBuild() then
		slot0.projectName = slot0.POOL_ID_TO_PROJECTS[slot0.activity:getConfig("config_client").id]
	else
...
slot0.updateActivityBuildPage = function (slot0)
	...
	setActive(slot0.projectToggles[slot0.PROJECTS.ACTIVITY], slot0:isExActBuild())
end
...
slot0.switchProject = function (slot0, slot1)
	...
	onButton(slot0, slot0:findTF("gallery/start_btn", slot0.mainTF), function ()
		slot0.msgbox.show(slot2, math.max(1, _.min({
...
slot0.UpdateBuildPoolExchange = function (slot0, slot1)
	...
	slot5 = slot1.exchange_request and slot2 > 0 and slot1.exchange_available_times and slot1.exchange_available_times > 0 and slot0:isExActBuild()

	if slot5 then
		slot0.buildPoolExchangeTxt.text = "<color=#FFDF48>" .. slot0.activity.data1 .. "</color>/" .. math.min(slot3, slot0.activity.data2 + 1) * slot2

		setActive(slot0.buildPoolExchangeGetBtnMark, slot0.activity.data2 < slot3 and slot8 <= slot6)
		...
		onButton(slot0, slot0.buildPoolExchangeTF, function ()
			if slot0 then
				slot1:emit(BuildShipMediator.ON_BUILDPOOL_EXCHANGE, slot1.activity.id)
			end
		end, SFX_PANEL)
		setGray(slot0.buildPoolExchangeGetBtn, not (slot0.activity.data2 < slot3 and slot8 <= slot6), true)
...
slot0.isExActBuild = function (slot0)
...
slot0.isPickupBuild = function (slot0)
