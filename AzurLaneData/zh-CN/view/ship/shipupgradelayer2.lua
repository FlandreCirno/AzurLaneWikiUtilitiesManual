...
slot0.didEnter = function (slot0)
	...
	onButton(slot0, slot0.breakOutBtn, function ()
		...
		seriesAsync(slot0, function ()
			slot0, slot1 = ShipStatus.ShipStatusCheck("onModify", slot0.shipVO)
			...
			if slot0.shipVO.level < slot0.breakCfg.level then
				pg.TipsMgr.GetInstance():ShowTips(i18n("ship_upgradeStar_error_lvLimit"))

				return
			end

			if not slot0.isEnoughItems then
			...
			if slot0.player.gold < slot0.breakCfg.use_gold then
				GoShoppingMsgBox(i18n("switch_to_shop_tip_2", i18n("word_gold")), ChargeScene.TYPE_ITEM, {
					{
						59001,
						slot0.breakCfg.use_gold - slot0.player.gold,
						slot0.breakCfg.use_gold
					}
				})
			...
			if not slot0.contextData.materialShipIds or #slot0.contextData.materialShipIds < slot0.breakCfg.use_char_num then
				pg.TipsMgr.GetInstance():ShowTips(i18n("ship_upgradeStar_select_material_tip"))

				return
			end

			slot0:emit(ShipUpgradeMediator2.UPGRADE_SHIP, slot0.contextData.materialShipIds)
		end)
	end, SFX_CONFIRM)
	onButton(slot0, slot0.recommandBtn, function ()
		if slot0.contextData.materialShipIds and #slot0.contextData.materialShipIds == slot0.breakCfg.use_char_num then
			return
		end

		if #slot0:getUpgradeRecommendShip(slot0.shipVO, slot0.contextData.materialShipIds or {}, slot0.breakCfg.use_char_num) > 0 then
			table.insert(slot2, function (slot0)
				slot1, slot2 = ShipCalcHelper.GetEliteAndHightLevelShips(underscore.map(slot0, function (slot0)
					return slot0:getShipById(slot0)
				end))

				if #slot1 > 0 or #slot2 > 0 then
					slot2.destroyConfirmWindow.ExecuteAction(slot3, "Show", slot1, slot2, false, slot0)
				else
					slot0()
				end
			end)
			seriesAsync(slot2, function ()
				...
				slot0.contextData:updateBreakOutView(slot0.shipVO)
			end)
		else
			pg.TipsMgr.GetInstance().ShowTips(slot2, i18n("without_selected_ship"))
		end
	end, SFX_CONFIRM)
	slot0.initMaterialShips(slot0)
end
...
slot0.setShip = function (slot0, slot1)
	...
	setActive(slot0.tipActive, not (slot0.shipVO.level < slot0.breakCfg.level or slot0.breakCfg.breakout_id == 0))
	setActive(slot0.tipDeactive, slot0.shipVO.level < slot0.breakCfg.level or slot0.breakCfg.breakout_id == 0)
	setButtonEnabled(slot0.breakOutBtn, not (slot0.shipVO.level < slot0.breakCfg.level or slot0.breakCfg.breakout_id == 0))
...
slot0.updateBattleView = function (slot0)
	...
	for slot4 = 1, slot0, 1 do
		...
		onToggle(slot0, slot0:findTF("stage" .. slot4, slot0.stages), function (slot0)
			if slot0 then
				slot1 = slot0.breakout_view

				for slot5, slot6 in ipairs(slot0.ultimate_bonus) do
					slot1 = slot1 .. "/" .. i18n(slot1[slot6])
				end

				changeToScrollText(slot2.breakView, slot1)
				changeToScrollText:switchStage(changeToScrollText)
			end
		end, SFX_PANEL)
	end

	slot0.findTF(slot0, "stage1", slot0.stages).GetComponent(slot1, typeof(Toggle)).group:SetAllTogglesOff()
...
slot0.updateBreakOutView = function (slot0, slot1)
	...
	if slot8 and slot8 ~= slot0.shipTempCfg[slot1.configId].max_level then
		slot11(slot0.findTF(slot0, "attr_1", slot0.attrs), {
