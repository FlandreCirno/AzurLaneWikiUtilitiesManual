...
slot0.Init = function (slot0, slot1, slot2, slot3, slot4)
	...
	if string.find(slot0.voice.key, ShipWordHelper.WORD_TYPE_MAIN) then
		slot9, slot10, slot11 = ShipWordHelper.GetWordAndCV(slot0.skin.id, ShipWordHelper.WORD_TYPE_MAIN, tonumber(slot17), nil, slot16)
...
slot0.Update = function (slot0)
	...
	if not slot0.isLive2d then
		...
		setActive(slot0._tf, not (slot2 or (slot3 and slot1.l2d_action:match("^" .. ShipWordHelper.WORD_TYPE_MAIN .. "_"))))
	end
...
slot0.UpdateCvBtn = function (slot0)
	...
	setActive(slot0.tagDiff, ShipWordHelper.ExistDifferentMainExWord(slot0.skin.id, slot0.voice.key, slot0.wordData.mainIndex, slot0.shipGroup:GetMaxIntimacy()))
end
