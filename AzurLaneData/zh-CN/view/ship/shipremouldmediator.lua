...
slot0.register = function (slot0)
	...
	slot0.bind(slot0, slot0.ON_SELECTE_SHIP, function (slot0, slot1)
		...
		slot1:sendNotification(GAME.GO_SCENE, SCENE.DOCKYARD, {
			...
			destroyCheck = true,
			skipSelect = true,
			shipVOs = slot0:getUpgradeShips(slot1),
			ignoredIds = pg.ShipFlagMgr.GetInstance():FilterShips(ShipStatus.FILTER_SHIPS_FLAGS_3, underscore.map(slot2, function (slot0)
				return slot0.id
			end)),
