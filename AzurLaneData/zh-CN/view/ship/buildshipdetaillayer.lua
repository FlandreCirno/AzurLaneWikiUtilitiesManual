...
slot0.didEnter = function (slot0)
	...
	slot0.singleList.make(slot1, function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			slot2.gameObject.name = "project_" .. slot1 + 1
			slot0.projectTFs[slot1 + 1] = slot2
	...
	onButton(slot0, slot0.allLaunch, function ()
		...
		slot0.emit(slot1, BuildShipDetailMediator.LAUNCH_ALL)
...
slot0.updateProject = function (slot0, slot1, slot2)
	...
	if slot2.state == BuildShip.ACTIVE then
		...
		if slot0.buildTimers[slot1] then
			pg.TimeMgr.GetInstance().RemoveTimer(slot15, slot0.buildTimers[slot1])
	...
	if slot2.state == BuildShip.FINISH then
		slot0.setSpriteTo(slot0, slot0[tonumber(slot8.ship_icon)], slot0:findTF("ship_modal", slot5), false)
...
slot0.willExit = function (slot0)
	...
	slot0.singleList.each(slot1, function (slot0, slot1)
		eachChild(slot0:findTF("frame/buiding/ship_modal", slot1), function (slot0)
			PoolMgr.GetInstance():ReturnUI(slot0.name, slot0)
		end)
	end)
...
slot0.playCV = function (slot0, slot1)
	slot0:stopCV()
	pg.CriMgr.GetInstance():PlaySoundEffect_V3("event:/cv/build/" .. slot1)

	slot0.voiceContent = "event:/cv/build/" .. slot1
end
