...
return {
	...
	__ctype = 2,
	InstanceMap = setmetatable({}, slot1),
	DATA_ADD = 1,
	DATA_UPDATE = 0,
	DATA_DELETE = -1,
	__index = function (slot0, slot1)
		return slot0[slot1] or slot0._properties[slot1]
	end,
	__newindex = function (slot0, slot1, slot2)
		if rawget(slot0._properties, slot1) ~= slot2 and not slot0._quiet then
			slot0._properties[slot1] = slot0.CreateShell(slot2, slot0)
			slot6 = slot0.DATA_UDPATE

			if slot3 == nil then
				slot6 = slot0.DATA_ADD or slot6
			end

			if slot5 == nil then
				slot6 = slot0.DATA_DELETE or slot6
			end

			if type(slot4) == "table" and slot4.class == slot0 then
				slot4._parents[slot0] = nil
			end

			if type(slot5) == "table" and slot5.class == slot0 then
	...
	Response = function (slot0, slot1, slot2, slot3)
		...
		if not not next(slot0 + 1._listeners) then
		...
		for slot9, slot10 in pairs(slot0._parents) do
			table.insert((slot5 - 1 > 0 and Clone(slot2)) or slot2, 1, table.keyof(slot9:GetRawData(), slot0))
			slot9:Response(slot1, (slot5 - 1 > 0 and Clone(slot2)) or slot2, slot3)
		end
	end,
	...
	CreateShell = function (slot0, slot1)
		if type(slot0) ~= "table" or slot0.class == slot0 then
			return slot0
		end

		if slot0.InstanceMap[slot0] then
			if slot1 then
				slot0.InstanceMap[slot0]._parents[slot1] = true
			end
		...
		setmetatable(slot2._parents, slot1)

		slot0.InstanceMap[slot0] = slot2
		...
		for slot7, slot8 in pairs(slot0) do
			if type(slot8) == "table" and slot8.class ~= slot0 then
				rawset(slot2._properties, slot7, slot0.CreateShell(slot8, slot2))
			else
	...
	AddRawListener = function (slot0, slot1, slot2, slot3)
		...
		return function ()
			return table.removebyvalue(slot0._listeners, )
		end
	end,
	...
	insert = function (slot0, ...)
		if select("#", ...) > 1 then
			...
			for slot9 = slot1 + 1, #slot0._properties + 1, 1 do
				slot0[slot9] = slot4
	...
	remove = function (slot0, slot1)
		...
		slot3 = slot0[((slot1 or slot2) > 0 and (slot1 or slot2)) or slot2 + (slot1 or slot2)]
		slot0[((slot1 or slot2) > 0 and (slot1 or slot2)) or slot2 + (slot1 or slot2)] = nil
