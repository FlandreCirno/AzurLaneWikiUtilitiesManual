	...
	({
		...
		selectGift = function (slot0, slot1)
			...
			if slot2 <= #slot0.selectedGifts then
				slot0._event:emit(slot0._event.emit, Clone(slot0.selectedGifts), function (slot0, slot1)
					if not slot0 then
						slot0:changeWorkerAction(slot1, 1)
					else
						slot0:changeWorkerAction(slot0.changeWorkerAction, 0, nil)
		...
		moveJiujiu = function (slot0, slot1, slot2)
			if slot1 == -1 then
				...
				if slot2 and slot2 > 0 then
					Clone(slot0[slot2]).x = -100
					slot0._workerTf.anchoredPosition = Clone(slot0[slot2])
				else
...
function slot40(slot0, slot1, slot2)
	({
		...
		step = function (slot0, slot1)
			...
			for slot5 = #slot0.fans, 1, -1 do
				...
				elseif slot5 == 1 and not slot6.leave then
					if slot6.gifts == nil then
						...
						slot0:changeCharAction(slot4, 1, function ()
							slot0:changeCharAction(slot0, 0, nil)
						end)
					end
				elseif not slot6.leave then
					slot0.changeFansAction(slot0, slot6, slot6, 0, nil)
				end
			end
			...
			if #slot0.fans > 0 then
				if slot0.fans[1].time and slot2.time < slot1 and not slot2.leave then
					...
					slot0:fanLeave(slot2, slot7, function ()
		...
		showFansWanted = function (slot0, slot1)
			...
			setActive(findTF(slot1.tf, "wanted/time2"), ((math.ceil(slot2 - slot0.stepTime) >= 0 or 0) and slot2 - slot0.stepTime) <= 5)
		...
		finishGift = function (slot0)
			if slot0:getWantedGifts() then
				...
				slot0:fanLeave(slot1, slot0, function ()
					table.remove(slot0.fans, 1)
				end)
				slot0.changeCharAction(slot0, slot0.fans[1], 1, function ()
					slot0:changeCharAction(slot0, 0, nil)
				end)
			end
		end,
	...
	({
		Ctor = function (slot0)
			...
			for slot4 = 1, slot5, 1 do
				...
				table.insert(slot0.groups, SetParent(slot5, slot4, slot0._event))
			end
		end,
		createIdols = function (slot0)
			slot1 = slot0:getRandomIdols()

			for slot5 = 1, #slot0.groups, 1 do
		...
		getApearTime = function (slot0)
			if slot0.lastTime and slot0.lastTime > 0 then
				for slot4 = 1, #slot0, 1 do
					if slot0.lastTime < slot0[slot4][1] then
...
slot0.initUI = function (slot0)
	...
	onButton(slot0, findTF(slot0.menuUI, "btnStart"), function ()
		if slot0:getGameUsedTimes() == 0 and not slot1 then
			slot1 = true
	...
	if not slot0.handle then
		slot0.handle = UpdateBeat.CreateListener(slot3, slot0.Update, slot0)
	end
...
slot0.initTextTip = function (slot0)
	slot1 = i18n("idolmaster_game_tip1") or slot0
	slot1 = i18n("idolmaster_game_tip2") or slot1
	slot2 = i18n("idolmaster_game_tip3") or slot2
	slot3 = i18n("idolmaster_game_tip4") or slot3

	setText(findTF(slot1.settlementUI, "ad/currentTextDesc"), )
	setText(findTF(slot1.settlementUI, "ad/highTextDesc"), slot1.settlementUI)
	setText(findTF(slot1.gameUI, "top/scoreImg/socre"), i18n("idolmaster_game_tip5") or slot4)
	setText(findTF(slot1.pauseUI, "ad/tip"), slot1)
	setText(findTF(slot1.leaveUI, "ad/tip"), setText)
end
...
slot0.openMenuUI = function (slot0)
	...
	if slot0.storyIndex and slot0[slot0.storyIndex] ~= "" and slot0:getGameUsedTimes() == slot0.storyIndex then
		pg.NewStoryMgr.GetInstance():Play(slot0[slot0.storyIndex], function ()
			return
		end, true)
