...
slot0.didEnter = function (slot0)
	...
	onButton(slot0, slot0.startBtn, function ()
		if slot0 then
			setActive(slot1.mainPage, false)
			setActive(slot1.mainPage.gamePage, true)
			slot1.mainPage.gamePage:startGame()
		else
			slot1.mainPage:GetComponent("CanvasGroup").blocksRaycasts = false

			false:managedTween(LeanTween.value, function ()
				slot0.mainPage:GetComponent("CanvasGroup").alpha = 1
				slot0.mainPage.GetComponent("CanvasGroup").mainPage:GetComponent("CanvasGroup").blocksRaycasts = true

				setActive(slot0.mainPage, false)
				setActive(slot0.countdownPage, true)
				pg.CriMgr.GetInstance():PlaySoundEffect_V3(pg.CriMgr.GetInstance())
			end, go(slot1.mainPage), 1, 0, ).setOnUpdate(slot0, System.Action_float(function (slot0)
				slot0.mainPage:GetComponent("CanvasGroup").alpha = slot0
			end))
		end
	end)
	...
	slot0.countdownAnim.GetComponent(slot1, typeof(DftAniEvent)).SetEndEvent(slot1, function (slot0)
		setActive(slot0.countdownPage, false)
		setActive(slot0.gamePage, true)
		slot0:startGame()
	end)
	onButton(slot0, slot0.endBtn, function ()
		slot0.mainPage:GetComponent("CanvasGroup").blocksRaycasts = false

		slot0.mainPage.GetComponent("CanvasGroup"):managedTween(LeanTween.value, function ()
			slot0.mainPage:GetComponent("CanvasGroup").alpha = 1
			slot0.mainPage.GetComponent("CanvasGroup").mainPage:GetComponent("CanvasGroup").blocksRaycasts = true
		end, go(slot0.endPage), 0, 1, slot0.mainPage.GetComponent("CanvasGroup")).setOnUpdate(slot0, System.Action_float(function (slot0)
			slot0.mainPage:GetComponent("CanvasGroup").alpha = slot0
		end))
		setActive(slot0.mainPage, true)
		setActive(slot0.countdownPage, false)
		setActive(slot0.gamePage, false)
		setActive(slot0.endPage, false)
	end)
	...
	slot0.gameListener.AddPointClickFunc(slot3, function (slot0, slot1)
		if slot0 then
			slot0 = false
		...
		if slot1.updating then
			return
		end

		if not slot1.inGame then
			return
		end

		slot3, slot4 = slot1:pos2index(slot2)

		if slot1.selected then
			if slot1.selected == slot1.tileTfs[slot3][slot4] then
				slot1:unselect()
			elseif math.abs(slot3 - slot1.selectedIndex.i) + math.abs(slot4 - slot1.selectedIndex.j) == 1 then
				slot1:tryMoveTo({
					i = slot3,
					j = slot4
				})
			else
				slot1:select(slot3, slot4)
			end
		else
			slot1:select(slot3, slot4)
		end
	end)
	slot0.longPressListener.onLongPressed.AddListener(slot3, function ()
		if slot0.updating then
			return
		end

		if not slot0.inGame then
			return
		end

		slot0 = LuaHelper.ScreenToLocal(slot0.tilesRoot, Input.mousePosition, GameObject.Find("UICamera"):GetComponent(typeof(Camera)))
		slot5, slot6 = slot0:pos2index(slot0)

		slot0:unselect()
		slot0:animate(slot1, slot2, true)
	end)
	slot0.gameListener.AddBeginDragFunc(slot3, function (slot0, slot1)
		if slot0.updating then
			return
		end
		...
		slot8, slot9 = slot0:pos2index(slot3)

		slot0:animate(slot4, slot5, false)
		slot0:unselect()

		slot0.selected = slot0.tileTfs[slot4][slot5]
		slot0.selectedIndex = {
			i = slot4,
			j = slot5
		}

		if math.abs(true.delta.y) < math.abs(true.delta.x) then
			slot4 = 0
		...
		slot0:tryMoveTo({
			i = slot0.selectedIndex.i + slot4,
			j = slot0.selectedIndex.j + slot5
		})
	end)
	setActive(slot0.mainPage, true)
...
slot0.updateData = function (slot0)
	...
	slot0.best = (slot0:GetMGData():GetRuntimeData("elements") and slot0.GetMGHubData().count == 0[1]) or 0
end

slot0.index2pos = function (slot0, slot1, slot2)
	return Vector3.New(slot0 + (slot2 - 1) * slot1, slot2 + (slot1 - 1) * Vector3.New)
end

slot0.pos2index = function (slot0, slot1)
	return math.ceil((slot1.y - (slot2 - slot3 / 2)) / slot3), math.ceil((slot1.x - (slot0 - slot1 / 2)) / slot1)
end

slot0.dropTime = function (slot0)
	return math.max(slot0 * slot0, math.max)
end
...
slot0.animate = function (slot0, slot1, slot2, slot3)
	...
	slot0.tileTfs[slot1][slot2]:GetComponent("Animator"):SetBool("selected", slot3)
...
slot0.tryMoveTo = function (slot0, slot1)
	...
	if slot0:moveValid(slot0.selectedIndex, slot1) then
		...
		slot0:managedTween(LeanTween.move, nil, slot2, slot5, slot1):setLoopPingPong(1)
		slot0:managedTween(LeanTween.move, nil, slot3, slot4, slot1):setLoopPingPong(1)
...
slot0.swap = function (slot0, slot1, slot2)
	slot0:moveTile(slot3, slot2, slot0)
	slot0:moveTile(slot4, slot1, slot0)
...
slot0.startGame = function (slot0)
	...
	slot1 = Timer.New(function ()
		slot0:managedTween(LeanTween.value, function ()
			slot0.gamePage:GetComponent("CanvasGroup").alpha = 1

			slot0.gamePage.GetComponent("CanvasGroup"):stopGame()
		end, go(slot0.gamePage), 1, 0, slot0).setOnUpdate(slot0, System.Action_float(function (slot0)
			slot0.gamePage:GetComponent("CanvasGroup").alpha = slot0
		end))
		UpdateBeat.RemoveListener(slot0, slot0.handle)
	end, (slot0.infinite and Timer.New) or function ()
		slot0.managedTween(LeanTween.value, function ()
			slot0.gamePage.GetComponent("CanvasGroup").alpha = 1
	...
	slot0.handle = UpdateBeat.CreateListener(slot2, function ()
		setText(slot0.countdownText, math.floor(slot1.time))

		if slot1.time <= math.floor and not isActive(slot0.warning) then
			setActive(slot0.warning, true)
		end
	end, slot0)
	...
	slot0.hintFunc = function ()
		...
		slot3, slot6, slot2 = slot0:findMove()
		slot4 = rtf(cloneTplTo(slot0.hintEffect, slot0.tilesRoot))
		slot4.anchoredPosition = (slot0:index2pos(slot0, slot1) + slot0:index2pos(slot0 + slot2[1], slot1 + slot2[2])) / 2
		slot4.localEulerAngles = Vector3.New(0, 0, dir2Angle(slot2))
		slot0.hint = slot4
		slot0.hint1 = slot0.tileTfs[slot0][slot1]
		slot0.hint2 = slot0.tileTfs[slot0 + slot2[1]][slot1 + slot2[2]]

		slot0.hint1:GetComponent("Animator"):SetBool("selected", true)
		slot0.hint2:GetComponent("Animator"):SetBool("selected", true)
	end

	slot0.hintTimer = Timer.New(slot0.hintFunc, slot5)

	slot0.hintTimer.Start(slot2)
end
...
slot0.stopGame = function (slot0)
	...
	slot0:managedTween(LeanTween.value, function ()
		slot0(slot0.newSign, slot0.best < slot0.score)
		...
		pg.CriMgr.GetInstance():PlaySoundEffect_V3(pg.CriMgr.GetInstance())
		pg.CriMgr.GetInstance().PlaySoundEffect_V3:managedTween(LeanTween.value, function ()
			slot0.endBtn:GetComponent("Button").enabled = true

			if slot0.endBtn.GetComponent("Button").infinite or (slot0.timer and slot0.timer.time <= 0) then
				if not slot0.infinite then
					slot0:SendSuccess(0)
				end

				if slot0.best < slot0.score then
			...
			slot0.timer = nil
		end, go(slot0.endBtn), 0, 1, ).setOnUpdate(slot0, System.Action_float(function (slot0)
			setImageAlpha(slot0.endBtn, slot0)
		end))
	end, go(slot0.endScore), 0, slot0.score, (slot0.score > 0 and go(slot0.endScore)) or 0).setOnUpdate(slot1, System.Action_float(function (slot0)
...
slot0.update = function (slot0)
	...
	if next(slot0:tryMatch()) ~= nil then
		...
		for slot7, slot8 in pairs(slot2) do
			slot0.score = slot0.score + ((#slot8 == 3 and 30 * slot0.combo) or (#slot8 == 4 and 60 * slot0.combo) or 20 * #slot8 * slot0.combo)

			setText(slot0.scoreText, slot0.score)
			_.each(slot8, function (slot0)
				slot0.tileIndicies[slot0[1]][slot0[2]] = nil

				if slot0.tileTfs[slot0[1]][slot0[2]] then
					slot1 + slot0.tileTfs[slot0[1]][slot0[2]].anchoredPosition.GetComponent(slot1, "Image").material = slot0.whitenMat
					slot1 + slot0.tileTfs[slot0[1]][slot0[2]].anchoredPosition.localPosition.z = -50
					cloneTplTo(slot0.matchEffect, slot0.tilesRoot).localPosition = slot1 + slot0.tileTfs[slot0[1]][slot0[2]].anchoredPosition.localPosition

					slot0:managedTween(LeanTween.value, function ()
						Destroy(Destroy)
						Destroy(Destroy)
					end, go(slot1), 1, 0, slot1 + slot0.tileTfs[slot0[1]][slot0[2]].anchoredPosition.localPosition):setOnUpdate(System.Action_float(function (slot0)
						setImageAlpha(slot0, slot0)
						setLocalScale(slot0, Vector3.one * slot0 * 2.7)
					end))
				end

				slot0.tileTfs[slot0[1]][slot0[2]] = nil
			end)

			rtf(cloneTplTo(slot0.floatText, slot0.tilesRoot)).anchoredPosition = Vector2.zero / #slot8

			slot0.formatScore(slot0, slot10, (#slot8 == 3 and 30 * slot0.combo) or (#slot8 == 4 and 60 * slot0.combo) or 20 * #slot8 * slot0.combo)
			slot0:managedTween(LeanTween.moveY, function ()
				Destroy(Destroy)
			end, slot10, Vector2.zero / #slot8.y + slot2, (#slot8 == 3 and 30 * slot0.combo) or (#slot8 == 4 and 60 * slot0.combo) or 20 * #slot8 * slot0.combo)
		end
		...
		slot0:managedTween(LeanTween.delayedCall, function ()
			...
			for slot4 = 1, , 1 do
				for slot8 = 1, slot2, 1 do
					if slot0.tileIndicies[slot4][slot8] then
						slot9 = slot4

						for slot13 = slot4, 1, -1 do
							if slot0.tileIndicies[slot13 - 1][slot8] or slot13 == 1 then
								slot9 = slot13
						...
						if slot9 ~= slot4 then
							slot0:moveTile(slot0.tileTfs[slot4][slot8], {
								i = slot9,
								j = slot8
							}, slot0.dropTime(slot4 - slot9))

							slot0.tileTfs[slot9][slot8] = slot0.tileTfs[slot4][slot8]
							slot0.tileIndicies[slot9][slot8] = slot0.tileIndicies[slot4][slot8]
							slot0.tileTfs[slot4][slot8] = nil
							slot0.tileIndicies[slot4][slot8] = nil
						end
					end
				end
			end
			...
			slot1 = slot0:tryMatch()

			if slot0.combo > 1 and next(slot1) == nil then
				slot2 = nil
				slot3 = Vector3.New(0, 0, -50)

				if slot0.combo == 2 then
					slot2 = cloneTplTo(slot0.goodEffect, slot0.tilesRoot)

					pg.CriMgr.GetInstance():PlaySoundEffect_V3(slot3)
				elseif slot0.combo == 3 then
					slot2 = cloneTplTo(slot0.greatEffect, slot0.tilesRoot)

					pg.CriMgr.GetInstance():PlaySoundEffect_V3(slot3)
				else
					slot2 = cloneTplTo(slot0.perfectEffect, slot0.tilesRoot)
			...
			slot0:managedTween(LeanTween.delayedCall, function ()
				...
				slot0:update()
			end, math.max(slot5, slot0.dropTime(slot0)), nil)
		end, slot1, nil)
	end

	if slot0.inGame then
		slot0.hintTimer.Reset(slot3, slot0.hintFunc, slot9)
