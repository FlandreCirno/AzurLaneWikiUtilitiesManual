...
function slot14(slot0)
	({
		...
		updateNoteTf = function (slot0)
			...
			if slot0.longFlag then
				setActive(findTF(slot0._tf, "longNote"), true)
				...
				setActive(findTF(slot0._tf, "singleNote"), true)
...
slot0.setStartData = function (slot0, slot1, slot2, slot3)
	slot1[slot2].puList = slot1
	slot1[slot2].speedLevel = slot2
	slot1[slot2].dgree = slot3
	slot1[slot2].speedOffsetX = slot2[slot2]

	slot1[slot2]:clearNote()
end
...
slot0.step = function (slot0, slot1)
	...
	if #slot0.noteList > 0 and slot0:checkScoreType(slot0.noteList[1]) then
		...
		if not slot2.longFlag or slot3 == slot0 then
	...
	for slot5 = #slot0.noteList, 1, -1 do
		if slot0.noteList[slot5]:getRemoveTime() and slot6 < slot0.stepTime then
			if slot0.noteStateCallback then
				if not slot1 then
					slot0:noteStateCallback()
				else
					slot0.noteStateCallback(slot2)
				end
			end
...
slot0.checkScoreType = function (slot0, slot1)
	...
	elseif not slot1.triggerDown then
		...
		slot4 = slot0.stepTime < slot1.endTime - slot0
...
slot0.checkPuShow = function (slot0, slot1)
	if slot1.begin_time - slot0.stepTime <= slot0 then
		return true
	end
