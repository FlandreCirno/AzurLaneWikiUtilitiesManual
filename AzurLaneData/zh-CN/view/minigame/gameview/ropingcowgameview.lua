...
function slot32(slot0, slot1, slot2)
	({
		ctor = function (slot0)
			slot0._tplCows = slot0
			slot0._container = slot0
			slot0._event = slot2
			slot0.cows = {}
			slot0.cowWeights = {}

			for slot4 = 1, #slot3, 1 do
		...
		step = function (slot0, slot1)
			...
			for slot5 = 1, #slot0.cows, 1 do
				slot0.cows[slot5].tf.anchoredPosition.x = slot0.cows[slot5].tf.anchoredPosition.x - slot0.cows[slot5].data.speed * Time.deltaTime
				slot9 = slot0.cows[slot5].tf.anchoredPosition.x

				if slot0.cows[slot5].tf.anchoredPosition.x >= 0 and slot9 <= 0 then
					slot0:setCowAniamtion(slot6, slot0)
				end
			...
			for slot5 = #slot0.cows, 1, -1 do
				slot7 = slot0.cows[slot5].tf.anchoredPosition

				if slot0.cows[slot5].tf.anchoredPosition.x <= -slot1 then
		...
		captureCow = function (slot0, slot1)
			...
			for slot6 = #slot0.cows, 1, -1 do
				slot8 = slot0.cows[slot6].tf.anchoredPosition

				if slot0[1] <= slot0.cows[slot6].tf.anchoredPosition.x and slot7.anchoredPosition.x <= slot0[2] then
					if not slot2 then
						slot2 = slot6
			...
			if slot2 then
				slot0:setCowAniamtion(table.remove(slot0.cows, slot2).tf, slot1)
				...
				slot0._event:emit(slot3, slot3.data.score)
			elseif slot1 then
				slot1(false)
			end
		end,
		clear = function (slot0)
			for slot4 = 1, #slot0.cows, 1 do
		...
		createCow = function (slot0)
			slot3 = math.random(0, slot0.cowWeights[slot0:getCowWeightIndex()][#slot0.cowWeights[slot0.getCowWeightIndex()]])
			slot4 = nil

			for slot8 = 1, #slot0.cowWeights[slot0.getCowWeightIndex()], 1 do
			...
			slot5 = tf(Instantiate(slot0._tplCows[slot4 or math.random(1, #slot0)]))

			SetActive(slot5, true)
			SetParent(slot5, slot0._container)

			slot5.anchoredPosition = Vector3(slot1, 0, 0)

			slot0:setCowAniamtion(slot5, slot2)
			GetOrAddComponent(findTF(slot5, "anim"), typeof(DftAniEvent)).SetEndEvent(slot7, function ()
				slot0:cowLeave(slot0)
			end)
			table.insert(slot0.cows, {
				tf = slot5,
				data = Clone(slot0[slot4 or math.random(1, #slot0)])
			})
		end,
		getCowWeightIndex = function (slot0)
			for slot4 = 1, #slot0, 1 do
				if slot0.lastTime and slot0.lastTime < slot0[slot4][1] then
					return slot4
				end
			end
		...
		getNextCreateCowTime = function (slot0)
			...
			for slot5 = 1, #slot0, 1 do
				if slot0.lastTime < slot0[slot5][1] then
					return 0.3 + slot0[slot5][2][1] + math.random() * (slot0[slot5][2][2] - slot0[slot5][2][1])
				end
			end

			return math.random(slot0[#slot0][2][1], slot0[#slot0][2][2])
		end,
...
function slot33(slot0, slot1)
	({
		ctor = function (slot0)
			...
			slot0:setPlayerAnim(slot0.setPlayerAnim)

			slot0._event = slot2
			slot0.playerDft = GetOrAddComponent(findTF(slot0._playerTf, "img"), typeof(DftAniEvent))

			slot0.playerDft:SetTriggerEvent(function ()
				slot0._event:emit(slot0._event, nil, function (slot0)
					if slot0 then
						slot0:setPlayerAnim(slot0.setPlayerAnim)
						slot0._event:emit(slot0._event)
					else
						slot0:setPlayerAnim(slot3)
					end
				end)
			end)
		...
		step = function (slot0, slot1)
			if slot0.captureCdTime then
				if slot0.captureCdTime < 0 then
					...
					slot0:setPlayerAnim(slot0)
					slot0._event:emit(slot1)
				else
...
function slot34(slot0)
	({
		ctor = function (slot0)
			slot0._backSceneTf = slot0

			if not slot0.sceneItems then
				slot0.sceneItems = {}

				for slot4 = 1, #slot1, 1 do
					table.insert(slot0.sceneItems, {
						tf = findTF(slot0._backSceneTf, slot1[slot4].name),
						data = slot1[slot4]
					})
				end
			end
		...
		onEventHandle = function (slot0, slot1)
			for slot5 = 1, #slot0.sceneItems, 1 do
				slot7 = slot0.sceneItems[slot5].tf

				if slot0.sceneItems[slot5].data.type == slot0 then
					for slot12, slot13 in ipairs(slot8) do
						if slot13 == slot1 and slot6.states then
							slot0:changeSceneItemAnim(slot1, slot6.states[slot12], slot7)
						end
					end
				end
			end
		...
		step = function (slot0, slot1)
			for slot5 = 1, #slot0.sceneItems, 1 do
				slot8 = slot0.sceneItems[slot5].tf
				...
				if slot0.sceneItems[slot5].data.type == slot0 then
					...
					elseif slot6.time > 0 then
						...
						if slot7.states then
							slot0:changeSceneItemAnim(slot1, slot7.states[math.random(1, #slot7.states)], slot8)
						elseif slot7.trigger then
							slot0:changeSceneItemAnim(slot2, nil, slot8)
						end
					end
				end
			end
...
slot0.initEvent = function (slot0)
	slot0:bind(slot0, function (slot0, slot1, slot2)
		slot0:addScore(slot1)
		slot0:onEventHandle(slot1)
	end)
	slot0.bind(slot0, slot0.bind, function (slot0, slot1, slot2)
		if slot0._cowController then
			slot0._cowController:captureCow(slot2)
		end
	...
	slot0.bind(slot0, slot0, function (slot0, slot1, slot2)
		slot0:onEventHandle(slot1)
	end)
	slot0.bind(slot0, function (slot0, slot1, slot2)
		slot0.onEventHandle(slot1)
...
slot0.initUI = function (slot0)
	...
	for slot6 = 1, #slot0, 1 do
		...
		GetSpriteFromAtlasAsync("ui/minigameui/ropingcowgameui", "battleDesc" .. slot8, function (slot0)
...
slot0.initGameUI = function (slot0)
	...
	slot0.captureButton.AddPointDownFunc(slot1, function (slot0, slot1)
	...
	slot0._playerController = slot1(findTF(slot0.sceneTf, "player"), slot0)
	slot0._cowController = slot2(slot2, slot1, slot0)
	slot0._sceneItemController = slot2(slot2, slot1, slot0)(slot0.backSceneTf)
end
...
slot0.updateGameUI = function (slot0)
	...
	if slot0._playerController then
		if slot0._playerController:getThrowTime() and slot1 > 0 then
			slot0.captureCdMaskImg.fillAmount = slot1 / slot0
		else
...
slot0.addScore = function (slot0, slot1)
	...
	for slot5 = 0, slot0.sceneScoreTf.childCount - 1, 1 do
