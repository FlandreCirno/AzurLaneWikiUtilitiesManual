	...
	({
		Ctor = function (slot0)
			...
			slot0.playerDft:SetStartEvent(function ()
				...
				slot0.playerAnimator.ResetTrigger.playerAnimator:SetBool("snowball", true)
			end)
			slot0.playerDft.SetTriggerEvent(slot1, function ()
				slot0:throwSnowball()
			end)
			slot0.playerDft.SetEndEvent(slot1, function ()
				return
			end)
		...
		step = function (slot0)
			...
			if not slot0.playerAnimator:GetBool("snowball") and slot0 < slot0.stepTime - slot0._reloadTime then
				slot0:reload()
			end
			...
			if (slot0.stepTime - slot0._reloadTime) / slot0 > 1 then
				slot1 = 1
			end
		...
		skill = function (slot0)
			if slot0._skillTime and slot0.stepTime - slot0._skillTime < slot0 then
				return
			end
		...
		createHeart = function (slot0)
			GetComponent(slot1, typeof(DftAniEvent)).SetEndEvent(slot2, function ()
				Destroy(Destroy)
			end)
			setParent(slot1, slot0.heartPos)
			setActive(tf(instantiate(slot0.tplHeart)), true)
		end,
...
function slot28(slot0, slot1, slot2, slot3, slot4)
	({
		...
		setPosition = function (slot0, slot1)
			slot0._tf.anchoredPosition = slot1
			slot0._tf.localEulerAngles = Vector3(0, 0, math.atan(slot0.y / slot0.x) * math.rad2Deg)
		end,
		hit = function (slot0)
			slot0._hitFlag = true
		...
		checkOutScene = function (slot0)
			if slot0._tf.anchoredPosition.x < slot0[1] or slot0[3] < slot1.x or slot1.y < slot0[2] or slot0[4] < slot1.y then
				slot0:dispose()
...
function slot29(slot0, slot1)
	return {
		...
		createSnowball = function (slot0, slot1, slot2, slot3, slot4, slot5)
			...
			slot10 = slot1(slot6, slot9, slot2, slot4, slot5)
		...
		clearEnemySnowball = function (slot0)
			for slot4 = #slot0.snowballs, 1, -1 do
				if slot0.snowballs[slot4]:getType() == slot0 then
...
function slot31(slot0, slot1, slot2, slot3, slot4)
	({
		...
		Ctor = function (slot0)
			...
			slot0.enemyDft.SetTriggerEvent(slot1, function ()
				if slot0._throwCallback then
					slot0._throwCallback(slot0.throwPos.position, slot0._index)
				end
			end)
		...
		step = function (slot0)
			...
			if slot0 < slot0._activeTime then
				slot0._activeTime = 0
	...
	({
		...
		step = function (slot0)
			...
			if slot0:getNextApearTime() < slot0.gameStepTime then
				for slot5 = 1, slot0:getApearAmount(), 1 do
			...
			for slot4 = 1, #slot0.charators, 1 do
		...
		apearCharactor = function (slot0)
			...
			slot10, slot11 = slot0:getCharactorName(slot0[slot2.type])
		...
		setSpeed = function (slot0, slot1)
			...
			for slot5 = 1, #slot0.charators, 1 do
		...
		createCharactor = function (slot0, slot1, slot2, slot3, slot4)
			SetParent(slot5, slot6)
			setActive(slot5, true)

			slot7 = nil

			if slot1.type == slot0 then
				slot7 = slot1(slot1, slot5, slot2, slot3, slot4)
			elseif slot1.type == slot2 then
				slot3(slot1, slot5, slot2, slot3, slot4):setThrowCallback(slot0.throwCallback)
			end
		...
		getCharactorRandomIndex = function (slot0, slot1)
			...
			for slot7 = 1, #slot0:getEmptyIndex(), 1 do
		...
		getCharactorName = function (slot0, slot1)
			...
			return slot1.skin_names[math.random(1, #slot1.skin_names)], slot1.score[math.random(1, #slot1.skin_names)]
		end,
			...
			for slot6 = 1, #slot0, 1 do
				...
				for slot12, slot13 in ipairs(slot7) do
					if table.contains(slot2, slot13) and not slot8 then
						table.insert(slot1, slot0[slot6])
		...
		getApearTime = function (slot0)
			...
			for slot5 = #slot0, 1, -1 do
				if slot0[slot5] < slot0.gameStepTime then
					slot1 = slot5
			...
			return math.random() * (slot1[slot1][2] - slot1[slot1][1]) + slot1[slot1][1]
		end,
...
function slot33(slot0, slot1, slot2, slot3)
	({
		...
		step = function (slot0)
			for slot5 = 1, #slot0._snowballCtrl:getSnowballs(), 1 do
				slot8 = slot0:getCharactorByIndex(slot1[slot5]:getIndex())

				if slot1[slot5]:getType() == slot1 then
					if slot8 and slot8 ~= 0 then
						slot13, slot14 = slot8:getColliderBound()
				...
				elseif slot6 == slot2 then
					slot13, slot14 = slot0._player:getColliderBound()

					if slot1[slot5]:checkArrived(slot0._sceneTf:InverseTransformPoint(slot9), slot10) then
						if slot8 and slot8 ~= 0 and slot8:getType() == slot3 then
							slot8:hit()
						end
...
slot0.initUI = function (slot0)
	...
	slot0.player = slot0(findTF(slot0._tf, "scene/luao"))

	slot0.player.throwCallback = function (slot0)
		slot0:onPlayerThrowSnowball(slot0)
	end

	slot0.player.damageCallback = function ()
		slot0:onPlayerDamage()
	end
	...
	slot0.snowballController = findTF(slot0._tf, "tplSnowball")(slot0.snowballContainer, slot0.tplSnowball)
	...
	GetComponent(slot0.specialTf, typeof(DftAniEvent)).SetTriggerEvent(slot1, function ()
		slot0:specialComplete()
	end)

	slot0.charactorContainer = findTF(slot0._tf, "scene/charactorContainer")

	for slot6, slot7 in pairs(slot2) do
		slot8 = slot7.skin_names
	...
	slot0.charactorController = slot3(slot0.charactorContainer, slot2)

	slot0.charactorController.throwCallback = function (slot0, slot1)
		slot0.charactorController.charactorDamageCallback = function (slot0, slot1)
			slot0:onHitEnemy(slot1, slot0)
		end

		slot0:onEnemyThrowSnowball(slot0, slot1, slot1[slot0.getCurrentDiff(slot3)])
	end

	slot0.colliderController = slot2(slot0.sceneTf, slot0.player, slot0.snowballController, slot0.charactorController)

	slot0.colliderController.hitEnemyCallback = function (slot0, slot1, slot2, slot3)
		slot0:onHitEnemy(slot2, slot3)
	end

	slot3 = findTF(slot0._tf, "scene/moveCollider")
	slot0.playerMoveVecs = {}

	for slot7 = 1, slot0.snowballController, 1 do
	...
	for slot8 = 1, slot7, 1 do
		slot10 = slot8

		onButton(slot0, findTF(slot4, slot8), function ()
			if slot0.charactorController:getCharactorByIndex(slot0.charactorController) and slot0 ~= 0 then
				slot0:throwSnowballTo(slot0.sceneTf:InverseTransformPoint(findTF(slot2, "target").position.x, findTF(slot2, "target").position.y, 0), findTF(slot2, "target").position, slot0)
			end
		end)
	end
	...
	slot0.countDft.SetTriggerEvent(slot5, function ()
		return
	end)
	slot0.countDft.SetEndEvent(slot5, function ()
	...
	onButton(slot0, findTF(slot0.leaveUI, "ad/btnOk"), function ()
		slot0:resumeGame()
		slot0.resumeGame.player:settlement(slot0.resumeGame.player)
		slot0.resumeGame.player.settlement:onGameOver()
	end, SFX_CANCEL)
	...
	for slot10 = 1, slot0.totalTimes, 1 do
	...
	onButton(slot0, findTF(slot0.gameUI, "btnMoveUp"), function ()
		if slot0.playerPosIndex > 1 then
	...
	onButton(slot0, findTF(slot0.gameUI, "btnMoveDown"), function ()
		if slot0.playerPosIndex < #slot0.playerMoveVecs then
			slot0.playerPosIndex = slot0.playerPosIndex + 1
	...
	onButton(slot0, slot0.btnSkill, function ()
		if slot0.skilTime == slot1 then
			slot0.skilTime = 0

			slot0:usePlayerSkill()
		end
	end)
	slot0.updateMenuUI(slot0)
	slot0:openMenuUI()
...
slot0.getCurrentDiff = function (slot0)
	for slot4 = #slot0, 1, -1 do
		if slot0[slot4] < slot0.gameStepTime then
			return slot4
		end
	end
end
...
slot0.gameStep = function (slot0)
	...
	if slot0 < slot0.skilTime then
		slot0.skilTime = slot0
	end
	...
	slot0.skillProgress.fillAmount = slot0.skilTime / slot0

	if slot0.skilTime == slot0 then
...
slot0.updateGameUI = function (slot0)
	setSlider(slot0.lifeProgress, 0, 1, slot0.player:getLife() / slot0)
	setText(slot0.textLife, slot0.player:getLife() .. "/" .. slot0)
...
slot0.onPlayerThrowSnowball = function (slot0, slot1)
	if slot0.throwTarget then
		slot0.snowballController:createSnowball(slot0.sceneTf:InverseTransformPoint(slot1.x, slot1.y, 0), slot0.throwTarget, slot0, slot1, slot0.targetIndex)
...
slot0.onEnemyThrowSnowball = function (slot0, slot1, slot2, slot3)
	slot0.snowballController:createSnowball(slot0.sceneTf:InverseTransformPoint(slot1.x, slot1.y, 0), slot0.sceneTf:InverseTransformPoint(slot0.player:getTargetPosition().x, slot0.player.getTargetPosition().y, 0), slot3, slot0, slot2)
end
...
slot0.specialComplete = function (slot0)
	...
	slot0.specialTime = slot0.gameStepTime + slot0
	slot0.specialIndex = 0
end
...
slot0.addScore = function (slot0, slot1, slot2)
	...
	slot5 = GetComponent(slot4, typeof(DftAniEvent))
	tf(instantiate(slot0.tplScore)).anchoredPosition = slot0.snowballContainer:InverseTransformPoint(slot2)
