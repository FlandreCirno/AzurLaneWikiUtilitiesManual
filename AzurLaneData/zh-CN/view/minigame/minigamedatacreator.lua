...
slot0.DataCreateFunc = function (slot0, slot1, slot2, slot3)
	if slot0 == MiniGameOPCommand.CMD_SPECIAL_GAME then
		slot5 = slot1[2]
		slot7 = getProxy(MiniGameProxy).GetMiniGameData(slot6, slot4)
		slot8 = {}

		if slot1[1] == slot0.ShrineGameID or slot4 == slot0.NewYearShrineGameID then
			if slot5 == 1 then
				slot8.count = slot2[1]
		...
		elseif slot4 == slot0.TowerClimbingGameID and slot5 == 1 then
			slot8.isInited = true
		end
