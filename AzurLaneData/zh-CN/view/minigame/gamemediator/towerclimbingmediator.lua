...
slot0.register = function (slot0)
	slot0:bind(slot0.ON_RECORD_MAP_SCORE, function (slot0, slot1, slot2)
	...
	slot0.bind(slot0, slot0.ON_COLLECTION, function (slot0)
		slot0:addSubLayers(Context.New({
			viewComponent = TowerClimbingCollectionLayer,
			mediator = TowerClimbingCollectionMediator
		}))
	end)
	slot0.bind(slot0, slot0.ON_FINISH, function (slot0, slot1, slot2, slot3)
		if slot3 < slot1 then
			slot0:sendNotification(GAME.SEND_MINI_GAME_OP, {
				hubid = 9,
				cmd = MiniGameOPCommand.CMD_SPECIAL_GAME,
		...
		if getProxy(MiniGameProxy).GetHubByGameId(slot4, MiniGameDataCreator.TowerClimbingGameID).count <= 0 then
			return
		end
	...
	slot0.bind(slot0, slot0.ON_MODIFY_DATA, function (slot0, slot1)
	...
	if getProxy(MiniGameProxy).GetMiniGameData(slot1, MiniGameDataCreator.TowerClimbingGameID) and not slot1:GetRuntimeData("isInited") then
		slot0:sendNotification(GAME.SEND_MINI_GAME_OP, {
			hubid = 9,
			cmd = MiniGameOPCommand.CMD_SPECIAL_GAME,
			args1 = {
