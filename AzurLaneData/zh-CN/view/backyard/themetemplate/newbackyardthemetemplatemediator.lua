...
slot0.register = function (slot0)
	slot0:bind(slot0.GO_DECORATION, function (slot0)
		slot0:sendNotification(GAME.GO_SCENE, SCENE.BACKYARD, {
			openDecoration = true
		})
	end)
	slot0.bind(slot0, slot0.ON_GET_SPCAIL_TYPE_TEMPLATE, function (slot0, slot1)
		slot0:sendNotification(GAME.BACKYARD_GET_SPECIFIED_TYPE_TEMPLATE, {
			type = slot1
		})
	end)
	slot0.bind(slot0, slot0.ON_GET_THEMPLATE_DATA, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.BACKYARD_GET_THEME_TEMPLATE_DATA, {
			templateId = slot1,
			callback = slot2
		})
	end)
	slot0.bind(slot0, slot0.ON_REFRESH, function (slot0, slot1, slot2, slot3)
		slot0:sendNotification(GAME.BACKYARD_REFRESH_SHOP_TEMPLATE, {
			type = slot1,
			page = slot2,
			force = slot3
		})
	end)
	slot0.bind(slot0, slot0.ON_SEARCH, function (slot0, slot1, slot2)
		if slot1 == BackYardConst.THEME_TEMPLATE_TYPE_CUSTOM or slot1 == BackYardConst.THEME_TEMPLATE_TYPE_COLLECTION then
			slot0.viewComponent:SearchKeyChange(slot2)
		elseif slot1 == BackYardConst.THEME_TEMPLATE_TYPE_SHOP then
			slot0:sendNotification(GAME.BACKYARD_SEARCH_THEME_TEMPLATE, {
				str = slot2
			})
		end
	end)
	...
	slot0.bind(slot0, slot0.ON_DISPLAY_PLAYER_INFO, function (slot0, slot1, slot2, slot3)
		slot0.contextData.pos = slot2
		slot0.contextData.themeName = slot3
	...
	slot0.bind(slot0, slot0.GET_TEMPLATE_PLAYERINFO, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.BACKYARD_GET_THEME_TEMPLATE_PLAYE_INFO, {
			type = slot1,
			templateId = slot2.id,
			userId = slot2:GetUserId()
		})
	end)
	slot0.bind(slot0, slot0.ON_UPLOAD_TEMPLATE, function (slot0, slot1)
		...
		slot0:sendNotification(GAME.BACKYARD_UPLOAD_THEME_TEMPLATE, {
			templateId = slot1.id
		})
	end)
	slot0.bind(slot0, slot0.ON_CANCEL_UPLOAD_TEMPLATE, function (slot0, slot1)
		slot0.contextData.msgBox:ExecuteAction("SetUp", {
			content = i18n("backyard_theme_cancel_template_upload_tip"),
			onYes = function ()
				slot0:sendNotification(GAME.BACKYARD_UNLOAD_THEME_TEMPLATE, {
					templateId = slot1.id
				})
			end
		})
	end)
	slot0.bind(slot0, slot0.ON_DELETE_TEMPLATE, function (slot0, slot1)
		slot0.contextData.msgBox:ExecuteAction("SetUp", {
	...
	slot0.bind(slot0, slot0.ON_APPLY_TEMPLATE, function (slot0, slot1, slot2)
		function slot5()
			slot0:sendNotification(GAME.BACKYARD_APPLY_THEME_TEMPLATE, {
				template = slot0
			})
		end

		if not getProxy(DormProxy):getData().OwnThemeTemplateFurniture(slot3, slot1) then
			slot0.contextData.msgBox.ExecuteAction(slot6, "SetUp", {
	...
	slot0.bind(slot0, slot0.ON_LIKE_THEME, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.BACKYARD_LIKE_THEME_TEMPLATE, {
			templateId = slot1.id,
			uploadTime = slot2
		})
	end)
	slot0.bind(slot0, slot0.ON_COLECT_THEME, function (slot0, slot1, slot2, slot3)
		slot0:sendNotification(GAME.BACKYARD_COLLECT_THEME_TEMPLATE, {
			templateId = slot1.id,
			isCancel = slot2,
			uploadTime = slot3
		})
	...
	slot0.bind(slot0, slot0.FETCH_ALL_THEME, function (slot0, slot1)
		slot0:sendNotification(GAME.GET_ALL_BACKYARD_THEME_TEMPLATE, {
			callback = function (slot0, slot1, slot2)
				slot0.viewComponent:SetShopThemeTemplate(slot0)
				slot0.viewComponent:SetCustomThemeTemplate(slot1)
				slot0.viewComponent:SetCollectionThemeTemplate(slot2)
				slot1()
			end
		})
	end)
...
slot0.handleNotification = function (slot0, slot1)
	...
	elseif slot2 == GAME.BACKYARD_REFRESH_SHOP_TEMPLATE_DONE then
		...
		slot0.viewComponent:OnShopTemplatesUpdated(getProxy(DormProxy).GetShopThemeTemplates(slot5))
