...
slot0.initAddButton = function (slot0, slot1, slot2, slot3, slot4)
	...
	for slot17 = 1, 3, 1 do
		...
		slot23 = _.map(slot5, function (slot0)
			return slot0.parent.shipVOs[slot0]
		end)
		...
		slot24.onLongPressed.AddListener(slot25, function ()
			if not slot0 then
				slot1.onClick({
					shipType = slot2,
					fleet = slot3,
	...
	for slot17 = 3, 1, -1 do
		slot13[slot17].SetSiblingIndex(slot18, slot12[slot17] - 1)
	end
...
slot0.updateFleets = function (slot0)
	...
	for slot4, slot5 in ipairs(slot0.tfFleets[FleetType.Submarine]) do
		...
		setActive(slot0.findTF(slot0, "btn_clear", slot5), slot4 <= slot0.chapter:getConfig("submarine_num") and not slot0.contextData.EditingCommander)
