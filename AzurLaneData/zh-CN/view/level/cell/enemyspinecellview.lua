...
slot0.Update = function (slot0)
	...
	if slot0.info.attachment == ChapterConst.AttachAmbush and slot1.flag == ChapterConst.CellFlagAmbush then
	elseif slot1.flag == ChapterConst.CellFlagActive then
		if slot0:UpdateGO(slot0._aliveTpl) then
			...
			slot0:GetLoader():GetSpine(slot2.icon, function (slot0)
				slot0.transform.localScale = Vector3(0.4 * slot0.scale * 0.01, 0.4 * slot0.scale * 0.01, 1)
				...
				slot0.transform:SetParent(slot1.tf, false)
				slot0.transform:SetAsFirstSibling()
			end, "LoadedSpine")
			slot0.ExtraUpdate(slot0, slot2)
		end
