...
slot0.Show = function (slot0)
	...
	})[slot0.contextData.tabIndex or slot0.TabIndex.Formation]) then
		slot4 = slot3[slot0.TabIndex.Formation]
	end
	...
	seriesAsync({
		function (slot0)
			...
			for slot6, slot7 in ipairs(slot0) do
				slot1, slot2 = slot1:getFleetById(slot7):GetEnergyStatus()
		...
		function (slot0)
			...
			if PlayerPrefs.GetInt(slot0, 1) ~= 1 or slot1 or not slot1 then
				return slot0()
			end

			PlayerPrefs.SetInt("autoFight_firstUse_sp", 1)
			PlayerPrefs.Save()
			slot2.emit(slot4, LevelUIConst.HANDLE_SHOW_MSG_BOX, {
				hideNo = true,
		...
		function (slot0)
			slot0 = (slot1:isLoop() and slot2:GetOrderedDuties()) or nil

			slot2:onCancel()

			if not slot2.onCancel:isValid() then
				return
			end

			slot2:emit(LevelUIConst.TRACK_CHAPTER, slot2.emit, slot0)
		end,
		function (slot0)
			PlayerPrefs.GetInt(slot0, 1) == 1:emit(LevelMediator2.ON_TRACKING, slot2.id, PlayerPrefs.GetInt(slot0, 1) == 1, slot2.loopFlag, , , )
		end
	})
end
...
slot0.InitUI = function (slot0)
	...
	for slot4 = 1, 2, 1 do
		slot5 = slot0._tf:Find(string.format("panel/fleet/%d/DutySelect", slot4))
	...
	onButton(slot0, slot0.dropDownSide:Find("Click"), function ()
		setActive(slot0.dropDown, not isActive(slot0.dropDown))
	end, SFX_UI_CLICK)
	...
	slot0.btnASHelp = slot0.dropDownSide.Find(slot2, "help")
	...
	setText(slot0.dropDown:Find("Airsupport/Text"), i18n("level_scene_title_word_3"))
	...
	for slot5 = 1, 2, 1 do
		onButton(slot0, slot0.dutyItems[3][slot5], function ()
			slot0.SetAutoSub(slot1, slot0.SetAutoSub == 1)
		end)
	end
end
...
slot0.set = function (slot0, slot1, slot2, slot3)
	...
	onButton(slot0, slot0.btnGo, function ()
		if slot0.getSPItem(slot1) and slot1 ~= 0 then
			if PlayerPrefs.GetInt("SPOPItemReminder") ~= 1 then
				pg.MsgboxMgr.GetInstance().ShowMsgBox(slot7, {
					...
					intro = i18n("levelScene_select_SP_OP_reminder", slot3, slot4),
					onYes = function ()
	...
	onToggle(slot0, slot0.formationToggle, function (slot0)
		if slot0 then
			slot0.contextData.tabIndex = slot1.TabIndex.Formation
	...
	onToggle(slot0, slot0.commanderToggle, function (slot0)
		if slot0 then
			slot0.contextData.tabIndex = slot1.TabIndex.Commander
	...
	onToggle(slot0, slot0.dutyToggle, function (slot0)
		if slot0 then
			slot0.contextData.tabIndex = slot1.TabIndex.Duty
...
slot0.UpdateFleetBar = function (slot0, slot1, slot2)
	setActive(slot0:findTF("btn_select", slot5), slot2 <= slot0:getLimitNums(slot1) and slot0.contextData.tabIndex == slot0.TabIndex.Formation)
	setActive(slot0:findTF("btn_clear", slot5), slot2 <= slot0.getLimitNums(slot1) and slot0.contextData.tabIndex == slot0.TabIndex.Formation)
	setActive(slot0:findTF("commander", slot5), slot2 <= slot0.getLimitNums(slot1) and slot0:getFleetById(slot10) and slot0.contextData.tabIndex == slot0.TabIndex.Commander)
	setActive(slot0:findTF("blank", slot5), not (slot2 <= slot0.getLimitNums(slot1)) or (slot0.contextData.tabIndex == slot0.TabIndex.Commander and not slot0.getFleetById(slot10)) or slot0.contextData.tabIndex == slot0.TabIndex.Duty or slot0.contextData.tabIndex == slot0.TabIndex.Adjustment)
end
...
slot0.updateFleet = function (slot0, slot1, slot2)
	...
	setActive(slot0.tfFleets[slot1][slot2].Find(slot7, "adjustment_flag"), slot0.contextData.tabIndex == slot0.TabIndex.Adjustment)
...
slot0.updateShips = function (slot0, slot1, slot2)
	...
	for slot7, slot8 in ipairs(slot2) do
		...
		GetOrAddComponent(slot9, "UILongPressTrigger").onLongPressed.RemoveAllListeners(slot10)
...
slot0.UpdateInvestigationComparision = function (slot0, slot1, slot2)
	...
	setText(slot3:Find("Value1"), setColorStr(slot1, (slot2 < math.floor(slot1) and "#51FF55") or COLOR_WHITE))
	setText(slot3:Find("Value2"), slot2)
	triggerToggle(slot0.dropDownSide:Find("Layout/Item1/Dot"), slot2 < math.floor(slot1))
end
...
slot0.UpdateASComparision = function (slot0, slot1, slot2)
	setText(slot0.dropDown:Find("Airsupport").Find(slot3, "Text"), i18n("level_scene_title_word_3"))
	setText(slot3:Find("Value1"), setColorStr(slot1, (slot2 < math.floor(slot1) and "#51FF55") or COLOR_WHITE))
	setText(slot3:Find("Value2"), slot2)
	triggerToggle(slot0.dropDownSide:Find("Layout/Item2/Dot"), slot2 < math.floor(slot1))
end
...
slot0.UpdateSonarRangeValues = function (slot0, slot1, slot2)
	setText(slot0.dropDownSide:Find("Layout/Item3/Values").GetChild(slot3, slot1 - 1), slot2)
end
...
slot0.setOnHard = function (slot0, slot1)
	...
	onButton(slot0, slot0.btnGo, function ()
		...
		slot2 = "chapter_autofight_flag_" .. slot1.chapter.id
		...
		seriesAsync({
			function (slot0)
				if slot0 then
					slot1:emit(LevelUIConst.HANDLE_SHOW_MSG_BOX, {
						modal = true,
						content = i18n("elite_fleet_confirm", Fleet.DEFAULT_NAME[slot0]),
						onYes = slot0
					})
				else
					slot0()
			...
			function (slot0)
				...
				if PlayerPrefs.GetInt(slot0, 1) ~= 1 or not slot1:getSPItem() or slot1 then
					return slot0()
				end

				PlayerPrefs.SetInt("autoFight_firstUse_sp", 1)
				PlayerPrefs.Save()
				slot1.emit(slot4, LevelUIConst.HANDLE_SHOW_MSG_BOX, {
					hideNo = true,
					content = i18n("autofight_special_operation_tip"),
					onYes = function ()
						slot0:clearSPBuff()
			...
			function (slot0)
				slot2 = (slot1:getSPItem().chapter:isLoop() and slot1:GetOrderedDuties()) or nil
				slot3 = (slot1.getSPItem().chapter.isLoop() and slot1.GetOrderedDuties()) or nil.chapter

				(slot1.getSPItem().chapter.isLoop() and slot1.GetOrderedDuties()) or nil.chapter:clear()
				(slot1.getSPItem().chapter.isLoop() and slot1.GetOrderedDuties()) or nil.chapter.clear:onCancelHard()
				(slot1.getSPItem().chapter.isLoop() and slot1.GetOrderedDuties()) or nil.chapter.clear.onCancelHard:emit(LevelUIConst.TRACK_CHAPTER, , slot1.getSPItem())
				pg.CriMgr.GetInstance():PlaySoundEffect_V3(SFX_UI_WEIGHANCHOR_BATTLE)
			end,
			function (slot0)
				PlayerPrefs.GetInt(slot0, 1) == 1:emit(LevelMediator2.ON_ELITE_TRACKING, slot2.id, slot2.loopFlag, PlayerPrefs.GetInt(slot0, 1) == 1, , )
			end
		})
	end, SFX_UI_WEIGHANCHOR_GO)
	onButton(slot0, slot0.btnASHelp, function ()
	...
	onToggle(slot0, slot0.adjustmentToggle, function (slot0)
		if slot0 then
			slot0.contextData.tabIndex = slot1.TabIndex.Adjustment
...
slot0.initAddButton = function (slot0, slot1, slot2, slot3, slot4)
	...
	slot8.GetComponent(slot8, "ContentSizeFitter").enabled = true
	slot8:GetComponent("HorizontalLayoutGroup").enabled = true
	slot0.isDraging = false

	for slot17 = 1, 3, 1 do
		slot18, slot19, slot20 = nil
		slot21 = (slot7[slot17] and slot0.shipVOs[slot7[slot17]]) or nil
		...
		slot23 = _.map(slot5, function (slot0)
			return slot0.shipVOs[slot0]
		end)

		table.sort(slot23, function (slot0, slot1)
		...
		if slot19 and slot0.contextData.tabIndex ~= slot1.TabIndex.Adjustment then
			slot24.onLongPressed:AddListener(function ()
				slot0:onCancelHard()
				slot0.onCancelHard:emit(LevelMediator2.ON_FLEET_SHIPINFO, {
					shipId = slot1.id,
					shipVOs = slot2,
					chapter = slot0.chapter
				})
		...
		slot25:AddPointClickFunc(function (slot0, slot1)
			if slot0 ~= slot0.gameObject then
				return
			end

			if slot1.isDraging then
				return
			end

			slot1:onCancelHard()
			slot1:emit(LevelMediator2.ON_ELITE_OEPN_DECK, {
				shipType = slot2,
				fleet = slot3,
		...
		slot25.RemoveBeginDragFunc(slot25)
		...
		if slot19 and slot0.contextData.tabIndex == slot1.TabIndex.Adjustment then
			...
			slot25:AddBeginDragFunc(function (slot0, slot1)
				...
				slot1.isDraging = true
				slot1.enabled = false
				false.enabled = false

				for slot5 = 1, 3, 1 do
					if slot0 == slot4:GetChild(slot5 - 1) then
						slot1.dragIndex = slot5
					end

					slot5[slot5] = slot6.anchoredPosition
					slot6[slot5] = slot6
				end
			end)
			slot25.AddDragFunc(slot25, function (slot0, slot1)
				if slot0 ~= slot0.gameObject then
					return
				end

				if not slot1.isDraging then
					return
				end

				slot0.localPosition.x = slot1:change2ScrPos(slot0.parent, slot1.position).x
				slot0.localPosition.x = math.clamp(slot0.localPosition.x, slot2[1].x, slot2[3].x)
				slot0.localPosition = slot0.localPosition
				slot3 = 1

				for slot7 = 1, 3, 1 do
					if slot0 ~= slot3[slot7] and slot0.localPosition.x > slot3[slot7].localPosition.x + ((slot3 < slot1.dragIndex and 1.1) or -1.1) * slot4 then
						slot3 = slot3 + 1
					end
				end

				if slot1.dragIndex ~= slot3 then
					slot4 = (slot3 < slot1.dragIndex and -1) or 1

					while slot1.dragIndex ~= slot3 do
						slot5[slot1.dragIndex + slot4] = slot5[slot1.dragIndex]
						slot5[slot1.dragIndex] = slot5[slot1.dragIndex + slot4]
						slot3[slot1.dragIndex + slot4] = slot3[slot1.dragIndex]
						slot3[slot1.dragIndex] = slot3[slot1.dragIndex + slot4]
						slot1.dragIndex = slot1.dragIndex + slot4
					end

					for slot8 = 1, 3, 1 do
						if slot0 ~= slot3[slot8] then
							slot3[slot8].anchoredPosition = slot2[slot8]
						end
					end
				end
			end)
			slot25.AddDragEndFunc(slot25, function (slot0, slot1)
				if slot0 ~= slot0.gameObject then
					return
				end
				...
				slot1.isDraging = false

				for slot5 = 1, 3, 1 do
					if not slot2[slot5] then
						for slot9 = slot5 + 1, 3, 1 do
							if slot2[slot9] then
								slot2[slot9] = slot2[slot5]
								slot2[slot5] = slot2[slot9]
								slot3[slot9] = slot3[slot5]
								slot3[slot5] = slot3[slot9]
							end
						end
					end

					if slot2[slot5] then
						table.removebyvalue(slot4, slot2[slot5])
						table.insert(slot4, slot2[slot5])
					else
						break
					end
				end
				...
				for slot5 = 1, 3, 1 do
					slot3[slot5]:SetSiblingIndex(slot5 - 1)
				end
				...
				slot1:emit(LevelMediator2.ON_ELITE_ADJUSTMENT, slot1.chapter)
			end)
		end
	end
...
slot0.updateEliteFleets = function (slot0)
	slot1 = slot0.contextData.tabIndex == slot0.TabIndex.Formation
	slot2 = slot0.contextData.tabIndex == slot0.TabIndex.Commander
	slot3 = slot0.contextData.tabIndex == slot0.TabIndex.Duty
	slot4 = slot0.contextData.tabIndex == slot0.TabIndex.Adjustment

	for slot8, slot9 in ipairs(slot0.tfFleets[FleetType.Normal]) do
		setActive(slot12, false)
		setActive(findTF(slot9, "selected"), false)
		setActive(findTF(slot9, TeamType.Main), slot8 <= slot0.chapter:getConfig("group_num"))
		setActive(findTF(slot9, TeamType.Vanguard), slot8 <= slot0.chapter.getConfig("group_num"))
		setActive(slot0:findTF("btn_clear", slot9), slot8 <= slot0.chapter.getConfig("group_num") and slot1)
		setActive(slot0:findTF("btn_recom", slot9), slot8 <= slot0.chapter.getConfig("group_num") and slot1)
		setActive(slot0:findTF("blank", slot9), not (slot8 <= slot0.chapter.getConfig("group_num")) or slot3 or slot4)
		setActive(slot0:findTF("commander", slot9), slot8 <= slot0.chapter.getConfig("group_num") and slot2)
		setActive(slot9:Find("adjustment_flag"), slot8 <= slot0.chapter.getConfig("group_num") and slot4)
		setText(slot0:findTF("bg/name", slot9), (slot8 <= slot0.chapter.getConfig("group_num") and Fleet.DEFAULT_NAME[slot8]) or "")

		if slot16 then
			slot21 = slot0:initAddButton(slot9, TeamType.Vanguard, slot19, slot8)

			slot0:initCommander(slot8, slot14, slot0.chapter)

			if slot0:initAddButton(slot9, TeamType.Main, slot18, slot8) and slot21 then
			...
			onButton(slot0, slot11, function ()
				if #slot0.eliteFleetList[slot1] ~= 6 then
					if slot0 ~= 0 then
						...
						slot0.emit(slot1, LevelMediator2.ON_ELITE_RECOMMEND, {
							index = slot1,
							chapterVO = slot0.chapter
						})
					end
	...
	for slot8, slot9 in ipairs(slot0.tfFleets[FleetType.Submarine]) do
		...
		setActive(slot0.findTF(slot0, "btn_clear", slot9), slot8 <= slot0.chapter:getConfig("submarine_num") and slot1)
		setActive(slot0:findTF("btn_recom", slot9), slot8 <= slot0.chapter.getConfig("submarine_num") and slot1)
		setActive(slot0:findTF("blank", slot9), not (slot8 <= slot0.chapter.getConfig("submarine_num")) or slot3 or slot4)
		setActive(slot0:findTF("commander", slot9), slot8 <= slot0.chapter.getConfig("submarine_num") and slot2)
		setActive(slot9:Find("adjustment_flag"), slot8 <= slot0.chapter.getConfig("submarine_num") and slot4)
		...
		slot0:initCommander(slot8 + 2, slot0.findTF("commander", slot9), slot0.chapter)
...
slot0.initSPOPView = function (slot0)
	...
	slot3 = PlayerPrefs.GetInt(slot2, 0)
	...
	if #slot0:getLegalSPBuffList() == 0 then
		slot0:clearSPBuff()
	elseif #slot1 == 1 then
		slot0:setTicketInfo(slot0.btnSp, pg.benefit_buff_template[slot1[1].id].benefit_condition)
		setText(slot0.spDesc, pg.benefit_buff_template[slot1[1].id].desc)
		onButton(slot0, slot0.btnSp:Find("item"), function ()
			slot0:emit(BaseUI.ON_ITEM, tonumber(slot1.benefit_condition))
		end)
		onButton(slot0, slot0.btnSp, function ()
			if slot0.spCheckMark.gameObject.activeSelf then
				PlayerPrefs.SetInt(slot1, 0)
				PlayerPrefs.SetInt:clearSPBuff()
			else
				slot0.spItemID = tonumber(slot2.benefit_condition)

				PlayerPrefs.SetInt(tonumber(slot2.benefit_condition), slot0.spItemID)
				pg.TipsMgr.GetInstance():ShowTips(i18n("levelScene_select_sp"))
				setActive(slot0.spCheckMark, true)
			end
		end)
	...
	elseif #slot1 > 1 then
		...
		for slot7, slot8 in ipairs(slot1) do
			...
			onButton(slot0, slot9, function ()
				slot0:setSPBuffSelected(slot1.id)
				setActive(slot0.spPanel, false)
			end)
		end

		onButton(slot0, slot0.btnSp, function ()
			if slot0.spPanel.gameObject.activeSelf then
				slot0:clearSPBuff()
				PlayerPrefs.SetInt(slot0, 0)
				setActive(slot0.spPanel, false)
			else
				setActive(slot0.spPanel, true)
				setActive(slot0.btnSp:Find("item"), false)
				setText(slot0.spDesc, i18n("levelScene_unselect_SP_OP"))
			end
		end)
...
slot0.setSPBuffSelected = function (slot0, slot1)
	slot0.spItemID = tonumber(pg.benefit_buff_template[slot1].benefit_condition)

	slot0:setTicketInfo(slot0.btnSp, slot0.spItemID)
	setText(slot0.spDesc, pg.benefit_buff_template[slot1].desc)
...
slot0.UpdateDutyBar = function (slot0)
	slot1 = slot0.contextData.tabIndex == slot0.TabIndex.Duty
...
slot0.GetValidFleets = function (slot0, slot1)
	if slot0.mode == slot0.SELECT then
		slot2 = {}
		slot3 = (slot1 and {
			slot1
		}) or {
		...
		return slot2
	elseif slot0.mode == slot0.EDIT then
		slot2 = {}
		slot3, slot4 = nil
...
slot0.IsListOfFleetEmpty = function (slot0, slot1)
	...
	if slot0.mode == slot0.SELECT then
		...
		elseif slot1 == 3 then
			return (slot0.selectIds[FleetType.Submarine][slot1 - 2] or 0) == 0
		end
	elseif slot0.mode == slot0.EDIT then
...
slot0.GetListFleets = function (slot0)
	...
	if slot0.mode == slot0.SELECT then
		...
		for slot8 = 1, slot2, 1 do
			slot1[slot8] = ((slot4[slot8] or 0) > 0 and slot0.fleets[slot4[slot8] or 0]) or nil
		end

		slot4 = slot0.selectIds[FleetType.Submarine]

		for slot8 = 1, slot3, 1 do
			slot1[slot8 + slot2] = ((slot4[slot8] or 0) > 0 and slot0.fleets[slot4[slot8] or 0]) or nil
		end
	elseif slot0.mode == slot0.EDIT then
		slot4 = {}
