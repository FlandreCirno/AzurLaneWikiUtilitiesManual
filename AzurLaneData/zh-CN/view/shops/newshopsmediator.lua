...
slot0.register = function (slot0)
	slot0:bind(slot0.ON_GUILD_SHOPPING, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.ON_GUILD_SHOP_PURCHASE, {
			goodsId = slot1,
			selectedId = slot2
		})
	end)
	slot0.bind(slot0, slot0.REFRESH_GUILD_SHOP, function (slot0, slot1)
		slot0:sendNotification(GAME.GET_GUILD_SHOP, {
			type = (slot1 and GuildConst.MANUAL_REFRESH) or GuildConst.AUTO_REFRESH
		})
	end)
	slot0.bind(slot0, slot0.ON_SKIN_SHOP, function (slot0, slot1)
		slot0:sendNotification(GAME.CHANGE_SCENE, SCENE.SKINSHOP)
	end)
	slot0.bind(slot0, slot0.GO_MALL, function (slot0, slot1)
		slot2 = getProxy(ContextProxy)

		if slot0.contextData.fromMediatorName == ChargeMediator.__cname then
	...
	slot0.bind(slot0, slot0.SELL_BLUEPRINT, function (slot0, slot1)
		slot0:sendNotification(GAME.FRAG_SELL, slot1)
	end)
	slot0.bind(slot0, slot0.ON_ACT_SHOPPING, function (slot0, slot1, slot2, slot3, slot4)
	...
	slot0.bind(slot0, slot0.ON_FRAGMENT_SHOPPING, function (slot0, slot1, slot2)
		slot0:sendNotification(GAME.FRAG_SHOPPING, {
			id = slot1,
			count = slot2
		})
	end)
	slot0.bind(slot0, slot0.ON_SHAM_SHOPPING, function (slot0, slot1, slot2)
	...
	slot0.bind(slot0, slot0.REFRESH_MILITARY_SHOP, function (slot0, slot1)
		if not slot1 then
	...
	slot0.bind(slot0, slot0.REFRESH_STREET_SHOP, function (slot0, slot1)
		if not slot1 then
	...
	slot0.bind(slot0, slot0.SET_PLAYER_FLAG, function (slot0, slot1, slot2)
		if slot2 then
			slot0:sendNotification(GAME.COMMON_FLAG, {
				flagID = slot1
			})
		else
			slot0:sendNotification(GAME.CANCEL_COMMON_FLAG, {
				flagID = slot1
			})
		end
	end)
...
slot0.ShowGetShip = function (slot0, slot1)
...
slot0.ShowGetSkin = function (slot0, slot1)
