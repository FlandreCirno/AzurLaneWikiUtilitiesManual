slot0 = type
slot1 = string.byte
slot2 = string.format
slot3 = string.match
slot4 = string.gmatch
slot5 = string.gsub
slot6 = table.concat
slot7 = require("bit")
slot8 = slot7.band
slot9 = slot7.bor
slot10 = slot7.tohex
slot11 = slot7.lshift
slot12 = slot7.rshift
slot13 = slot7.arshift
slot23 = {
	[0] = false,
	false,
	"tdiARI",
	"twiARI",
	{
		[1037.0] = "evmhosmiRRR",
		[1284.0] = "evmhousiaawRRR",
		[1069.0] = "evmhosmiaRRR",
		[564.0] = "evcmpeqYRR",
		[712.0] = "efsmulRRR",
		[556.0] = "evmergehiRRR",
		[1453.0] = "evmhogsmianRRR",
		[720.0] = "efscfuiR-R",
		[518.0] = "evsubiwRAR~",
		[548.0] = "evslwRRR",
		[728.0] = "efsctuizR-R",
		[736.0] = "efdaddRRR",
		[524.0] = "evrndwRR",
		[520.0] = "evabsRR",
		[1421.0] = "evmhosmianwRRR",
		[764.0] = "efdtstgtYRR",
		[1413.0] = "evmhossianwRRR",
		[1323.0] = "evmhegsmfaaRRR",
		[1283.0] = "evmhessfaawRRR",
		[1291.0] = "evmhesmfaawRRR",
		[659.0] = "evfscfsfR-R",
		[563.0] = "evcmpltsYRR",
		[555.0] = "evsplatfiRS",
		[739.0] = "efdcfsidR-R",
		[747.0] = "efdctsidzR-R",
		[523.0] = "evextshRR",
		[755.0] = "efdcfsfR-R",
		[784.0] = "evlwhexRR0R",
		[633.0] = "evselRRRW",
		[1222.0] = "evdivwsRRR",
		[637.0] = "evselRRRW",
		shift = 0,
		[1481.0] = "evmwlsmianwRRR",
		[640.0] = "evfsaddRRR",
		[1473.0] = "evmwlssianwRRR",
		[1033.0] = "evmhesmiRRR",
		[644.0] = "evfsabsRR",
		[1280.0] = "evmheusiaawRRR",
		[648.0] = "evfsmulRRR",
		[1324.0] = "evmhogumiaaRRR",
		[652.0] = "evfscmpgtYRR",
		[718.0] = "efscmpeqYRR",
		[656.0] = "evfscfuiR-R",
		[726.0] = "efsctufR-R",
		[660.0] = "evfsctuiR-R",
		[550.0] = "evslwiRRA",
		[664.0] = "evfsctuizR-R",
		[1292.0] = "evmhoumiaawRRR",
		[668.0] = "evfststgtYRR",
		[734.0] = "efststeqYRR",
		[730.0] = "efsctsizR-R",
		[534.0] = "evxorRRR",
		[750.0] = "efdcmpeqYRR",
		[746.0] = "efdctuidzR-R",
		[758.0] = "efdctufR-R",
		[1217.0] = "evaddssiaawRR",
		[1101.0] = "evmwhsmiRRR",
		[1480.0] = "evmwlumianwRRR",
		[1225.0] = "evaddsmiaawRR",
		[653.0] = "evfscmpltYRR",
		[657.0] = "evfscfsiR-R",
		[1071.0] = "evmhosmfaRRR",
		[561.0] = "evcmpgtsYRR",
		[557.0] = "evmergeloRRR",
		[1133.0] = "evmwhsmiaRRR",
		[708.0] = "efsabsRR",
		[553.0] = "evsplatiRS",
		[704.0] = "efsaddRRR",
		[737.0] = "efdsubRRR",
		[716.0] = "efscmpgtYRR",
		[1452.0] = "evmhogumianRRR",
		[560.0] = "evcmpgtuYRR",
		[745.0] = "efddivRRR",
		[724.0] = "efsctuiR-R",
		[757.0] = "efdctsiR-R",
		[552.0] = "evrlwRRR",
		[753.0] = "efdcfsiR-R",
		[732.0] = "efststgtYRR",
		[1420.0] = "evmhoumianwRRR",
		[544.0] = "evsrwuRRR",
		[1412.0] = "evmhousianwRRR",
		[740.0] = "efdabsRR",
		[1223.0] = "evdivwuRRR",
		[536.0] = "evnor|evnotRRR=",
		[748.0] = "efdcmpgtYRR",
		[744.0] = "efdmulRRR",
		[756.0] = "efdctuiR-R",
		[752.0] = "efdcfuiR-R",
		[516.0] = "evsubwRRR~",
		[512.0] = "evaddwRRR",
		[760.0] = "efdctuizR-R",
		[773.0] = "evldhRR8",
		[769.0] = "evlddRR8",
		[781.0] = "evlhhousplatRR2",
		[1036.0] = "evmhoumiRRR",
		[777.0] = "evlhhesplatRR2",
		[789.0] = "evlwhouRR4",
		[785.0] = "evlwheRR4",
		[797.0] = "evlwhsplatRR4",
		[1068.0] = "evmhoumiaRRR",
		[793.0] = "evlwwsplatRR4",
		[805.0] = "evstdhRR8",
		[801.0] = "evstddRR8",
		[1100.0] = "evmwhumiRRR",
		[821.0] = "evstwhoRR4",
		[817.0] = "evstwheRR4",
		[829.0] = "evstwwoRR4",
		[1132.0] = "evmwhumiaRRR",
		[825.0] = "evstwweRR4",
		[1363.0] = "evmwssfaaRRR",
		[1371.0] = "evmwsmfaaRRR",
		[1451.0] = "evmhegsmfanRRR",
		[1419.0] = "evmhesmfanwRRR",
		[1411.0] = "evmhessfanwRRR",
		[632.0] = "evselRRR",
		[1220.0] = "evmraRR",
		[636.0] = "evselRRRW",
		[1499.0] = "evmwsmfanRRR",
		[1491.0] = "evmwssfanRRR",
		[1035.0] = "evmhesmfRRR",
		[1027.0] = "evmhessfRRR",
		[663.0] = "evfsctsfR-R",
		[1067.0] = "evmhesmfaRRR",
		[1059.0] = "evmhessfaRRR",
		[1115.0] = "evmwsmfRRR",
		[1107.0] = "evmwssfRRR",
		[1147.0] = "evmwsmfaRRR",
		[1139.0] = "evmwssfaRRR",
		[719.0] = "efscfdR-R",
		[559.0] = "evmergelohiRRR",
		[727.0] = "efsctsfR-R",
		[723.0] = "efscfsfR-R",
		[547.0] = "evsrwisRRA",
		[539.0] = "evorcRRR",
		[535.0] = "evor|evmrRRR=",
		[751.0] = "efdcfsR-R",
		[527.0] = "brincRRR",
		[759.0] = "efdctsfR-R",
		[1219.0] = "evsubfssiaawRR",
		[1227.0] = "evsubfsmiaawRR",
		[772.0] = "evldhxRR0R",
		[768.0] = "evlddxRR0R",
		[780.0] = "evlhhousplatxRR0R",
		[1321.0] = "evmhegsmiaaRRR",
		[776.0] = "evlhhesplatxRR0R",
		[788.0] = "evlwhouxRR0R",
		[1281.0] = "evmhessiaawRRR",
		[796.0] = "evlwhsplatxRR0R",
		[1289.0] = "evmhesmiaawRRR",
		[792.0] = "evlwwsplatxRR0R",
		[804.0] = "evstdhxRR0R",
		[800.0] = "evstddxRR0R",
		[820.0] = "evstwhoxRR0R",
		[816.0] = "evstwhexRR0R",
		[1345.0] = "evmwlssiaawRRR",
		[828.0] = "evstwwoxRR0R",
		[1353.0] = "evmwlsmiaawRRR",
		[824.0] = "evstwwexRR0R",
		[1369.0] = "evmwsmiaaRRR",
		[1449.0] = "evmhegsmianRRR",
		[1417.0] = "evmhesmianwRRR",
		[1409.0] = "evmhessianwRRR",
		mask = 2047,
		[1218.0] = "evsubfusiaawRR",
		[635.0] = "evselRRRW",
		[1226.0] = "evsubfumiaawRR",
		[639.0] = "evselRRRW",
		[1497.0] = "evmwsmianRRR",
		[646.0] = "evfsnegRR",
		[654.0] = "evfscmpeqYRR",
		[1320.0] = "evmhegumiaaRRR",
		[658.0] = "evfscfufR-R",
		[662.0] = "evfsctufR-R",
		[666.0] = "evfsctsizR-R",
		[670.0] = "evfststeqYRR",
		[1288.0] = "evmheumiaawRRR",
		[1065.0] = "evmhesmiaRRR",
		[1113.0] = "evmwsmiRRR",
		[1145.0] = "evmwsmiaRRR",
		[1344.0] = "evmwlusiaawRRR",
		[1352.0] = "evmwlumiaawRRR",
		[710.0] = "efsnegRR",
		[1368.0] = "evmwumiaaRRR",
		[1448.0] = "evmhegumianRRR",
		[562.0] = "evcmpltuYRR",
		[558.0] = "evmergehiloRRR",
		[554.0] = "evrlwiRRA",
		[722.0] = "efscfufR-R",
		[1416.0] = "evmheumianwRRR",
		[546.0] = "evsrwiuRRA",
		[1408.0] = "evmheusianwRRR",
		[542.0] = "evnandRRR",
		[742.0] = "efdnegRR",
		[738.0] = "efdcfuidR-R",
		[530.0] = "evandcRRR",
		[526.0] = "evcntlswRR",
		[522.0] = "evextsbRR",
		[754.0] = "efdcfufR-R",
		[514.0] = "evaddiwRAR~",
		[766.0] = "efdtsteqYRR",
		[1472.0] = "evmwlusianwRRR",
		[762.0] = "efdctsizR-R",
		[1496.0] = "evmwumianRRR",
		[771.0] = "evldwRR8",
		[783.0] = "evlhhossplatRR2",
		[1327.0] = "evmhogsmfaaRRR",
		[1032.0] = "evmheumiRRR",
		[791.0] = "evlwhosRR4",
		[1287.0] = "evmhossfaawRRR",
		[1295.0] = "evmhosmfaawRRR",
		[1064.0] = "evmheumiaRRR",
		[803.0] = "evstdwRR8",
		[1112.0] = "evmwumiRRR",
		[1096.0] = "evmwlumiRRR",
		[1144.0] = "evmwumiaRRR",
		[1128.0] = "evmwlumiaRRR",
		[1455.0] = "evmhogsmfanRRR",
		[1423.0] = "evmhosmfanwRRR",
		[1415.0] = "evmhossfanwRRR",
		[1216.0] = "evaddusiaawRR",
		[634.0] = "evselRRRW",
		[1224.0] = "evaddumiaawRR",
		[638.0] = "evselRRRW",
		[641.0] = "evfssubRRR",
		[645.0] = "evfsnabsRR",
		[649.0] = "evfsdivRRR",
		[1039.0] = "evmhosmfRRR",
		[1031.0] = "evmhossfRRR",
		[661.0] = "evfsctsiR-R",
		[669.0] = "evfststltYRR",
		[1063.0] = "evmhossfaRRR",
		[1103.0] = "evmwhsmfRRR",
		[1095.0] = "evmwhssfRRR",
		[1135.0] = "evmwhsmfaRRR",
		[709.0] = "efsnabsRR",
		[1127.0] = "evmwhssfaRRR",
		[705.0] = "efssubRRR",
		[717.0] = "efscmpltYRR",
		[713.0] = "efsdivRRR",
		[725.0] = "efsctsiR-R",
		[721.0] = "efscfsiR-R",
		[733.0] = "efststltYRR",
		[545.0] = "evsrwsRRR",
		[741.0] = "efdnabsRR",
		[537.0] = "eveqvRRR",
		[749.0] = "efdcmpltYRR",
		[529.0] = "evandRRR",
		[525.0] = "evcntlzwRR",
		[521.0] = "evnegRR",
		[765.0] = "efdtstltYRR",
		[770.0] = "evldwxRR0R",
		[782.0] = "evlhhossplatxRR0R",
		[1325.0] = "evmhogsmiaaRRR",
		[790.0] = "evlwhosxRR0R",
		[1285.0] = "evmhossiaawRRR",
		[1293.0] = "evmhosmiaawRRR",
		[802.0] = "evstdwxRR0R"
	},
	false,
	false,
	"mulliRRI",
	"subficRRI",
	false,
	"cmpl_iYLRU",
	"cmp_iYLRI",
	"addicRRI",
	"addic.RRI",
	"addi|liRR0I",
	"addis|lisRR0I",
	"b_KBJ",
	"sc",
	"bKJ",
	{
		[0] = "mcrfXX",
		[129.0] = "crandcCCC",
		[257.0] = "crandCCC",
		[225.0] = "crnandCCC",
		mask = 1023,
		[150.0] = "isync",
		[289.0] = "creqv|crsetCCC%",
		[16.0] = "b_lrKB",
		[33.0] = "crnor|crnotCCC=",
		[528.0] = "b_ctrKB",
		[193.0] = "crxor|crclrCCC%",
		[449.0] = "cror|crmoveCCC=",
		[417.0] = "crorcCCC",
		shift = 1
	},
	"rlwimiRR~AAA.",
	setmetatable({
		shift = 0,
		mask = -1
	}, {
		__index = function (slot0, slot1)
			slot2 = slot0(slot1(slot1, 11), 31)
			slot4 = slot0(slot1(slot1, 1), 31)

			if slot0(slot1(slot1, 6), 31) == 0 and slot4 == 31 - slot2 then
				return "slwiRR~A."
			elseif slot4 == 31 and slot3 == 32 - slot2 then
				return "srwiRR~-A."
			else
				return "rlwinmRR~AAA."
			end
		end
	}),
	false,
	"rlwnmRR~RAA.",
	"oriNRR~U",
	"orisRR~U",
	"xoriRR~U",
	"xorisRR~U",
	"andi.RR~U",
	"andis.RR~U",
	{
		[0] = "rldiclRR~HM.",
		"rldicrRR~HM.",
		"rldicRR~HM.",
		"rldimiRR~HM.",
		{
			[0] = "rldclRR~RM.",
			"rldcrRR~RM.",
			shift = 1,
			mask = 1
		},
		shift = 2,
		mask = 7
	},
	setmetatable({
		[0] = "cmp_YLRR",
		nil,
		nil,
		nil,
		"twARR",
		nil,
		nil,
		nil,
		"subfcRRR.",
		"mulhduRRR.",
		"addcRRR.",
		"mulhwuRRR.",
		nil,
		nil,
		nil,
		"iselltRRR",
		nil,
		nil,
		nil,
		{
			[0] = "mfcrR",
			"mfocrfRZ",
			shift = 20,
			mask = 1
		},
		"lwarxRR0R",
		"ldxRR0R",
		nil,
		"lwzxRR0R",
		"slwRR~R.",
		nil,
		"cntlzwRR~",
		"sldRR~R.",
		"andRR~R.",
		nil,
		nil,
		nil,
		"cmpl_YLRR",
		[232.0] = "subfmeRR.",
		[824.0] = "srawiRR~A.",
		[233.0] = "mulldRRR.",
		[136.0] = "subfeRRR.",
		[457.0] = "divduRRR.",
		[470.0] = "dcbi-RR",
		[86.0] = "dcbf-R0R",
		[58.0] = "cntlzdRR~",
		[119.0] = "lbzuxRRR",
		[341.0] = "lwaxRR0R",
		[87.0] = "lbzxRR0R",
		[343.0] = "lhaxRR0R",
		[316.0] = "xorRR~R.",
		[79.0] = "iseleqRRR",
		[84.0] = "ldarxRR0R",
		[476.0] = "nandRR~R.",
		[444.0] = "or|mrRR~R=.",
		[854.0] = "eieio",
		[214.0] = "stdcxRR0R.",
		mask = 1023,
		[616.0] = "negoRR.",
		[459.0] = "divwuRRR.",
		[597.0] = "lswiRR0A",
		[150.0] = "stwcxRR0R.",
		[75.0] = "mulhwRRR.",
		[661.0] = "stswxRR0R",
		[662.0] = "stwbrxRR0R",
		[631.0] = "lfduxFRR",
		[154.0] = "prtywRR~",
		[533.0] = "lswxRR0R",
		[53.0] = "lduxRRR",
		[278.0] = "dcbt-R0R",
		[73.0] = "mulhdRRR.",
		[648.0] = "subfeoRRR.",
		[104.0] = "negRR.",
		[663.0] = "stfsxFR0R",
		[599.0] = "lfdxFR0R",
		[650.0] = "addeoRRR.",
		[54.0] = "dcbst-R0R",
		[759.0] = "stfduxFR0R",
		[438.0] = "ecowxRR0R",
		[439.0] = "sthuxRRR",
		[310.0] = "eciwxRR0R",
		[311.0] = "lhzuxRRR",
		[660.0] = "stdbrxRR0R",
		[969.0] = "divduoRRR.",
		[55.0] = "lwzuxRRR",
		[712.0] = "subfzeoRR.",
		[407.0] = "sthxRR0R",
		[918.0] = "sthbrxRR0R",
		[149.0] = "stdxRR0R",
		[68.0] = "tdARR",
		[215.0] = "stbxRR0R",
		[855.0] = "lfiwaxFR0R",
		[151.0] = "stwxRR0R",
		[792.0] = "srawRR~R.",
		[982.0] = "icbi-R0R",
		[183.0] = "stwuxRRR",
		[534.0] = "lwbrxRR0R",
		[695.0] = "stfsuxFRR",
		[279.0] = "lhzxRR0R",
		[200.0] = "subfzeRR.",
		[1001.0] = "divdoRRR.",
		[744.0] = "subfmeoRR.",
		[746.0] = "addmeoRR.",
		[202.0] = "addzeRR.",
		[266.0] = "addRRR.",
		[522.0] = "addcoRRR.",
		[567.0] = "lfsuxFRR",
		[412.0] = "orcRR~R.",
		[971.0] = "divwouRRR.",
		[284.0] = "eqvRR~R.",
		[714.0] = "addzeoRR.",
		[725.0] = "stswiRR0A",
		[727.0] = "stfdxFR0R",
		[552.0] = "subfoRRR.",
		[827.0] = "sradiRR~H.",
		[922.0] = "extshRR~.",
		[986.0] = "extswRR~.",
		[954.0] = "extsbRR~.",
		[983.0] = "stfiwxFR0R",
		[536.0] = "srwRR~R.",
		[539.0] = "srdRR~R.",
		[138.0] = "addeRRR.",
		[535.0] = "lfsxFR0R",
		[532.0] = "ldbrxRR0R",
		[1003.0] = "divwoRRR.",
		[745.0] = "mulldoRRR.",
		[747.0] = "mullwoRRR.",
		[60.0] = "andcRR~R.",
		[758.0] = "dcba-RR",
		[520.0] = "subfcoRRR.",
		[826.0] = "sradiRR~H.",
		[122.0] = "popcntbRR~",
		[512.0] = "mcrxrX",
		[186.0] = "prtydRR~",
		[1014.0] = "dcbz-R0R",
		[508.0] = "cmpbRR~R",
		[375.0] = "lhauxRRR",
		[40.0] = "subfRRR.",
		[246.0] = "dcbtst-R0R",
		shift = 1,
		[778.0] = "addoRRR.",
		[790.0] = "lhbrxRR0R",
		[47.0] = "iselgtRRR",
		[373.0] = "lwauxRRR",
		[794.0] = "sradRR~R.",
		[181.0] = "stduxRRR",
		[247.0] = "stbuxRRR",
		[491.0] = "divwRRR.",
		[124.0] = "nor|notRR~R=.",
		[489.0] = "divdRRR.",
		[234.0] = "addmeRR.",
		[235.0] = "mullwRRR.",
		[144] = {
			[0] = "mtcrfRZ~",
			"mtocrfRZ~",
			shift = 20,
			mask = 1
		},
		[371] = {
			shift = 11,
			[392.0] = "mftbR",
			[424.0] = "mftbuR",
			mask = 1023
		},
		[339] = {
			[256.0] = "mflrR",
			shift = 11,
			[16.0] = "mfspefscrR",
			mask = 1023,
			[288.0] = "mfctrR",
			[32.0] = "mferR"
		},
		[467] = {
			[256.0] = "mtlrR",
			shift = 11,
			[16.0] = "mtspefscrR",
			mask = 1023,
			[288.0] = "mtctrR",
			[32.0] = "mtxerR"
		},
		[598] = {
			[0] = "sync",
			"lwsync",
			"ptesync",
			shift = 21,
			mask = 3
		}
	}, {
		__index = function (slot0, slot1)
			if slot0(slot1, 31) == 15 then
				return "iselRRRC"
			end
		end
	}),
	"lwzRRD",
	"lwzuRRD",
	"lbzRRD",
	"lbzuRRD",
	"stwRRD",
	"stwuRRD",
	"stbRRD",
	"stbuRRD",
	"lhzRRD",
	"lhzuRRD",
	"lhaRRD",
	"lhauRRD",
	"sthRRD",
	"sthuRRD",
	"lmwRRD",
	"stmwRRD",
	"lfsFRD",
	"lfsuFRD",
	"lfdFRD",
	"lfduFRD",
	"stfsFRD",
	"stfsuFRD",
	"stfdFRD",
	"stfduFRD",
	false,
	false,
	{
		[0] = "ldRRE",
		"lduRRE",
		"lwaRRE",
		shift = 0,
		mask = 3
	},
	{
		{
			[0] = false,
			false,
			"fdivsFFF.",
			false,
			"fsubsFFF.",
			"faddsFFF.",
			"fsqrtsF-F.",
			false,
			"fresF-F.",
			"fmulsFF-F.",
			"frsqrtesF-F.",
			false,
			"fmsubsFFFF~.",
			"fmaddsFFFF~.",
			"fnmsubsFFFF~.",
			"fnmaddsFFFF~.",
			shift = 1,
			mask = 15
		},
		shift = 5,
		mask = 1
	},
	false,
	false,
	{
		[0] = "stdRRE",
		"stduRRE",
		shift = 0,
		mask = 3
	},
	{
		[0] = {
			[0] = "fcmpuXFF",
			nil,
			nil,
			nil,
			nil,
			nil,
			nil,
			nil,
			"fcpsgnFFF.",
			nil,
			nil,
			nil,
			"frspF-F.",
			nil,
			"fctiwF-F.",
			"fctiwzF-F.",
			[38.0] = "mtfsb1A.",
			[392.0] = "frinF-F.",
			[711.0] = "mtfsfZF.",
			[583.0] = "mffsF.",
			mask = 1023,
			[846.0] = "fcfidF-F.",
			[814.0] = "fctidF-F.",
			[134.0] = "mtfsfiA>>-A>",
			[32.0] = "fcmpoXFF",
			[40.0] = "fnegF-F.",
			[424.0] = "frizF-F.",
			[70.0] = "mtfsb0A.",
			[456.0] = "fripF-F.",
			[488.0] = "frimF-F.",
			[815.0] = "fctidzF-F.",
			[136.0] = "fnabsF-F.",
			[64.0] = "mcrfsXX",
			[264.0] = "fabsF-F.",
			[72.0] = "fmrF-F.",
			shift = 1
		},
		{
			[0] = false,
			false,
			"fdivFFF.",
			false,
			"fsubFFF.",
			"faddFFF.",
			"fsqrtF-F.",
			"fselFFFF~.",
			"freF-F.",
			"fmulFF-F.",
			"frsqrteF-F.",
			false,
			"fmsubFFFF~.",
			"fmaddFFFF~.",
			"fnmsubFFFF~.",
			"fnmaddFFFF~.",
			shift = 1,
			mask = 15
		},
		shift = 5,
		mask = 1
	}
}
slot24 = {
	[0] = "r0",
	"sp",
	"r2",
	"r3",
	"r4",
	"r5",
	"r6",
	"r7",
	"r8",
	"r9",
	"r10",
	"r11",
	"r12",
	"r13",
	"r14",
	"r15",
	"r16",
	"r17",
	"r18",
	"r19",
	"r20",
	"r21",
	"r22",
	"r23",
	"r24",
	"r25",
	"r26",
	"r27",
	"r28",
	"r29",
	"r30",
	"r31"
}
slot25 = {
	[0] = "lt",
	"gt",
	"eq",
	"so",
	"ge",
	"le",
	"ne",
	"ns"
}

function slot26(slot0)
	if slot0 <= 3 then
		return slot0[slot1(slot0, 3)]
	else
		return slot2("4*cr%d+%s", slot3(slot0, 2), slot0[slot1(slot0, 3)])
	end
end

function slot27(slot0, slot1, slot2)
	slot3 = slot0.pos
	slot4 = ""

	if slot0.rel and slot0.symtab[slot0.rel] then
		slot4 = "\t->" .. slot5
	end

	if slot0.hexdump > 0 then
		slot0.out(slot0("%08x  %s  %-7s %s%s\n", slot0.addr + slot3, slot1(slot0.op), slot1, slot2(slot2, ", "), slot4))
	else
		slot0.out(slot0("%08x  %-7s %s%s\n", slot0.addr + slot3, slot1, slot2(slot2, ", "), slot4))
	end

	slot0.pos = slot3 + 4
end

function slot28(slot0)
	return slot0(slot0, ".long", {
		"0x" .. slot1(slot0.op)
	})
end

function slot29(slot0)
	slot7, slot9, slot10, slot10 = slot0(slot0.code, slot0.pos + 1, slot0.pos + 4)
	slot7 = {}
	slot8 = nil
	slot9 = 21
	slot0.op = slot1(slot2(slot2, 24), slot2(slot3, 16), slot2(slot4, 8), slot5)
	slot0.rel = nil
	slot10 = slot3[slot4(slot2, 2)]

	while slot5(slot10) ~= "string" do
		if not slot10 then
			return slot6(slot0)
		end

		slot10 = slot10[slot7(slot4(slot6, slot10.shift), slot10.mask)]
	end

	slot11, slot14 = slot8(slot10, "^([a-z0-9_.]*)(.*)")
	slot13, slot14 = slot8(slot12, "|([a-z0-9_.]*)(.*)")

	if slot13 then
		slot12 = slot14
	end

	for slot18 in slot9(slot12, ".") do
		slot19 = nil

		if slot18 == "R" then
			slot19 = slot10[slot7(slot4(slot6, slot9), 31)]
			slot9 = slot9 - 5
		elseif slot18 == "F" then
			slot19 = "f" .. slot7(slot4(slot6, slot9), 31)
			slot9 = slot9 - 5
		elseif slot18 == "A" then
			slot19 = slot7(slot4(slot6, slot9), 31)
			slot9 = slot9 - 5
		elseif slot18 == "S" then
			slot19 = slot11(slot2(slot6, 27 - slot9), 27)
			slot9 = slot9 - 5
		elseif slot18 == "I" then
			slot19 = slot11(slot2(slot6, 16), 16)
		elseif slot18 == "U" then
			slot19 = slot7(slot6, 65535)
		elseif slot18 == "D" or slot18 == "E" then
			slot20 = slot11(slot2(slot6, 16), 16)

			if slot18 == "E" then
				slot20 = slot7(slot20, -4)
			end

			if slot8 == "r0" then
				slot8 = "0"
			end

			slot7[#slot7] = slot12("%d(%s)", slot20, slot8)
		elseif "2" <= slot18 and slot18 <= "8" then
			slot20 = slot7(slot4(slot6, slot9), 31) * slot18

			if slot8 == "r0" then
				slot8 = "0"
			end

			slot7[#slot7] = slot12("%d(%s)", slot20, slot8)
		elseif slot18 == "H" then
			slot19 = slot7(slot4(slot6, slot9), 31) + slot2(slot7(slot6, 2), 4)
			slot9 = slot9 - 5
		elseif slot18 == "M" then
			slot19 = slot7(slot4(slot6, slot9), 31) + slot7(slot6, 32)
		elseif slot18 == "C" then
			slot19 = slot13(slot7(slot4(slot6, slot9), 31))
			slot9 = slot9 - 5
		elseif slot18 == "B" then
			slot21 = slot7(slot4(slot6, 16), 31)
			slot22 = ""
			slot9 = slot9 - 10

			if slot7(slot4(slot6, 21), 4) == 0 then
				slot22 = (slot7(slot20, 2) == 0 and "dnz") or "dz"

				if slot7(slot20, 16) == 0 then
					slot22 = slot22 .. ((slot7(slot20, 8) == 0 and "f") or "t")
				end

				if slot7(slot20, 16) == 0 then
					slot19 = slot13(slot21)
				end

				slot11 = slot11 .. ((slot7(slot20, 1) == slot7(slot4(slot6, 15), 1) and "-") or "+")
			elseif slot7(slot20, 16) == 0 then
				slot22 = slot14[slot7(slot21, 3) + ((slot7(slot20, 8) == 0 and 4) or 0)]

				if slot21 > 3 then
					slot19 = "cr" .. slot4(slot21, 2)
				end

				slot11 = slot11 .. ((slot7(slot20, 1) == slot7(slot4(slot6, 15), 1) and "-") or "+")
			end

			slot11 = slot15(slot11, "_", slot22)
		elseif slot18 == "J" then
			slot19 = slot11(slot2(slot6, 27 - slot9), 29 - slot9) * 4

			if slot7(slot6, 2) == 0 then
				slot19 = slot0.addr + slot1 + slot19
			end

			slot0.rel = slot19
			slot19 = "0x" .. slot16(slot19)
		elseif slot18 == "K" then
			if slot7(slot6, 1) ~= 0 then
				slot11 = slot11 .. "l"
			end

			if slot7(slot6, 2) ~= 0 then
				slot11 = slot11 .. "a"
			end
		elseif slot18 == "X" or slot18 == "Y" then
			slot19 = (slot7(slot4(slot6, slot9 + 2), 7) ~= 0 or slot18 ~= "Y" or nil) and "cr" .. slot19
			slot9 = slot9 - 5
		elseif slot18 == "W" then
			slot19 = "cr" .. slot7(slot6, 7)
		elseif slot18 == "Z" then
			slot19 = slot7(slot4(slot6, slot9 - 4), 255)
			slot9 = slot9 - 10
		elseif slot18 == ">" then
			slot7[#slot7] = slot4(slot7[#slot7], 1)
		elseif slot18 == "0" then
			if slot8 == "r0" then
				slot7[#slot7] = nil

				if slot13 then
					slot11 = slot13
				end
			end
		elseif slot18 == "L" then
			slot11 = slot15(slot11, "_", (slot7(slot6, 2097152) ~= 0 and "d") or "w")
		elseif slot18 == "." then
			if slot7(slot6, 1) == 1 then
				slot11 = slot11 .. "."
			end
		elseif slot18 == "N" then
			if slot6 == 1610612736 then
				slot11 = "nop"

				break
			end
		elseif slot18 == "~" then
			slot7[#slot7] = slot7[#slot7 - 1]
			slot7[#slot7 - 1] = slot7[#slot7]
		elseif slot18 == "=" then
			if slot8 == slot7[#slot7 - 1] then
				slot7[slot20] = nil
				slot11 = slot13
			end
		elseif slot18 == "%" then
			if slot8 == slot7[#slot7 - 1] and slot8 == slot7[slot20 - 2] then
				slot7[slot20] = nil
				slot7[slot20 - 1] = nil
				slot11 = slot13
			end
		elseif slot18 == "-" then
			slot9 = slot9 - 5
		end

		if slot19 then
			slot7[#slot7 + 1] = slot19
			slot8 = slot19
		end
	end

	return slot17(slot0, slot11, slot7)
end

function slot30(slot0, slot1, slot2)
	slot3 = (slot2 and (slot1 or 0) + slot2) or #slot0.code
	slot3 = slot3 - slot3 % 4
	slot0.pos = slot1 - slot1 % 4
	slot0.rel = nil

	while slot0.pos < slot3 do
		slot0(slot0)
	end
end

return {
	create = function (slot0, slot1, slot2)
		return {
			code = slot0,
			addr = slot1 or 0,
			out = slot2 or io.write,
			symtab = {},
			disass = slot0,
			hexdump = 8
		}
	end,
	disass = function (slot0, slot1, slot2)
		slot0(slot0, slot1, slot2):disass()
	end,
	regname = function (slot0)
		if slot0 < 32 then
			return slot0[slot0]
		end

		return "f" .. slot0 - 32
	end
}
