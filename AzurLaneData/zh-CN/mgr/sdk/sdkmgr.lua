...
pg.SdkMgr.LogoutSDK = function (slot0, slot1)
...
pg.SdkMgr.CheckWorldTest = function (slot0)
	if PLATFORM_CODE == PLATFORM_CH then
...
pg.SdkMgr.ShowLicence = function (slot0)
	slot0:Call("ShowLicence")
end

pg.SdkMgr.ShowPrivate = function (slot0)
...
function InLoginScene()
	...
	if getProxy(ContextProxy).getCurrentContext(slot1) and slot1.mediator == LoginMediator and not slot0() then
		return true
	end
...
pg.SdkMgr.GetPlatformCode = function (slot0, slot1)
...
pg.SdkMgr.IgnorePlatform = function (slot0, slot1)
	if slot0.pcode and slot1 and #slot1 > 0 and _.any(slot1, function (slot0)
		return tostring(slot0) == slot0
	end) then
		return true
	end
