...
slot0.filterEquipByType = function (slot0, slot1)
	if not slot1 or slot1 == slot0.EquipmentTypeAll then
		return true
	end

	for slot5 = 2, #EquipmentSortCfg.index, 1 do
		if bit.band(slot6, slot1) > 0 and table.contains(EquipmentSortCfg.index[slot5].types, slot0.config.type) then
			return true
		end
	end
...
slot0.filterEquipByCamp = function (slot0, slot1)
	...
	for slot5 = 2, #EquipmentSortCfg.campIndex, 1 do
		if bit.band(slot6, slot1) > 0 then
...
slot0.filterEquipByProperty = function (slot0, slot1)
	...
	for slot7, slot8 in ipairs(slot1) do
		if not slot8 or slot8 == slot0.EquipPropertyAll then
			slot3 = slot3 + 1
		else
			for slot12 = 2, #EquipmentSortCfg.propertyIndex, 1 do
				if bit.band(slot13, slot8) > 0 then
...
slot0.filterEquipAmmo1 = function (slot0, slot1)
	if not slot1 or slot1 == slot0.EquipAmmoAll_1 then
		return true
	end

	for slot5 = 2, #EquipmentSortCfg.ammoIndex1, 1 do
		if bit.band(slot6, slot1) > 0 and table.contains(EquipmentSortCfg.ammoIndex1[slot5].types, slot0:getConfig("ammo")) then
			return true
		end
	end
...
slot0.filterEquipAmmo2 = function (slot0, slot1)
	if not slot1 or slot1 == slot0.EquipAmmoAll_2 then
		return true
	end

	for slot5 = 2, #EquipmentSortCfg.ammoIndex2, 1 do
		if bit.band(slot6, slot1) > 0 and table.contains(EquipmentSortCfg.ammoIndex2[slot5].types, slot0:getConfig("ammo")) then
			return true
		end
	end
...
slot0.filterEquipByRarity = function (slot0, slot1)
	if not slot1 or slot1 == slot0.EquipmentRarityAll then
		return true
	end

	return bit.band(slot3, slot1) > 0
end
...
table.insert(slot0.EquipmentExtraIndexs, 1, slot0.EquipmentExtraNone)

slot0.filterEquipByExtra = function (slot0, slot1, slot2)
	slot1 = slot1 or 0
	...
	if bit.band(slot1, slot0.EquipmentExtraEquiping) > 0 and not slot0.shipId then
		return false
	end

	if bit.band(slot1, slot0.EquipmentExtraStrengthen) > 0 and (not pg.equip_data_template[slot0.id] or not slot3.next or slot3.next == 0) then
		return false
	end

	if bit.band(slot1, slot0.EquipmentExtraTransform) > 0 and (not EquipmentProxy.EquipTransformTargetDict[Equipment.GetEquipRootStatic(slot0.id)] or not slot3.targets) then
		return false
	end
...
for slot4, slot5 in ipairs(pg.equip_skin_theme_template.all) do
	slot6 = pg.equip_skin_theme_template[slot5].name

	table.insert(slot0.EquipSkinThemeNames, slot6)
end
...
slot0.filterByExtra = function (slot0, slot1)
	if bit.band(slot1, bit.lshift(1, slot0.ExtraAll)) > 0 then
		return true
	end

	if bit.band(slot1, bit.lshift(1, slot0.ExtraSpecial)) > 0 and slot0:isSpecialFilter() then
		return true
	end

	if bit.band(slot1, bit.lshift(1, slot0.ExtraNotObtained)) > 0 and ShipGroup.getState(slot2, getProxy(CollectionProxy):getShipGroup(slot2), slot0:isRemoulded()) ~= ShipGroup.STATE_UNLOCK then
		return true
	end
...
slot0.sortByCombatPower = function (slot0, slot1)
	...
	return sortCompare({
		slot0.combatPowerCaches[slot1],
		slot0.combatPowerCaches[slot0]
	}, {
		slot0.configId,
		slot1.configId
	})
...
slot0.sortByProperty = function (slot0)
	...
	return function (slot0, slot1)
		...
		return sortCompare({
			slot0.propertyCaches[slot1][slot1],
			slot0.propertyCaches[slot0][slot1]
		}, {
			slot0.configId,
			slot1.configId
		})
...
slot0.sortByCfg = function (slot0)
	return function (slot0, slot1)
		slot2 = slot0:getConfig(slot0)
		slot3 = slot1:getConfig(slot0)
...
slot0.sortByIntimacy = function (slot0, slot1)
...
slot0.sortByIntimacyAsc = function (slot0, slot1)
	if slot0.intimacy ~= slot1.intimacy then
