...
slot0.nationPointFilter = function (slot0)
	...
	for slot4, slot5 in ipairs(slot0.groupListInCount) do
		if slot5:getNation() ~= tonumber(string.sub(tostring(slot7), 1, 1)) then
...
slot0.getLevelByTecID = function (slot0, slot1)
	...
	return (slot0.techList[slot1] or 0) and (table.indexof(pg.fleet_tech_group[slot1].techs, slot0.techList[slot1].completeID, 1) or 0)
end
