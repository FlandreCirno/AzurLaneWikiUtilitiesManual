...
slot0.InjectPublic = function (slot0, slot1, slot2)
	...
	for slot7 = 1, #slot1.args, 1 do
		...
		elseif slot8.type == PublicArg.TypeShipId then
			...
			slot3 = string.gsub(slot3, "$" .. slot7, (slot8.type ~= PublicArg.TypeEquipId or pg.equip_data_statistics[slot8.int].name) and (slot8.type ~= PublicArg.TypeItemId or pg.item_data_statistics[slot8.int].name) and (slot8.type ~= PublicArg.TypeNums or slot8.int) and (slot8.type ~= PublicArg.TypeWorldBoss or slot8.string) and slot8.string)
		end
	end
	...
	slot0:on(50101, function (slot0)
		...
		elseif slot0.type == ChatConst.CODE_ACTOBSS_MSG_WORD then
			...
			slot0:addNewMsg(ChatMsg.New(ChatConst.ChannelWorld, {
				...
				content = slot2,
				emojiId = slot3,
				timestamp = pg.TimeMgr.GetInstance().GetServerTime(slot5)
			}))
		end
	end)
	slot0.on(slot0, 50103, function (slot0)
		slot1 = {}
