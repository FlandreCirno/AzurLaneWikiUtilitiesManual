...
slot0.UpdateSkirmishProgress = function (slot0)
	...
	slot5 = math.min(getProxy(ActivityProxy).getActivityById(slot2, ActivityConst.ACTIVITY_ID_US_SKIRMISH_RE).getDayIndex(slot4), #slot0.data)
	slot6 = false

	for slot10 = #slot0.data, 1, -1 do
		slot13 = slot1:getTaskVO(slot3[slot10].getConfig(slot11, "task_id"))
		slot14 = nil
		...
		elseif slot13 then
			...
			slot11:SetState((not slot6 or SkirmishVO.StateClear) and SkirmishVO.StateActive)
		end
	end
end
