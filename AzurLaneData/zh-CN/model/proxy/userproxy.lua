...
slot0.GetCacheGatewayFlag = function (slot0, slot1)
	if not slot0.cachePlatform then
		slot0.cachePlatform = PlayerPrefs.GetInt(slot0 .. slot1, PLATFORM)
	end
...
slot0.SaveCacheGatewayFlag = function (slot0, slot1)
	...
	if PlayerPrefs.GetInt(slot0 .. slot1, PLATFORM) ~= slot0.cachePlatform then
		PlayerPrefs.SetInt(slot0 .. slot1, slot0.cachePlatform)
		PlayerPrefs.Save()
	end
end
