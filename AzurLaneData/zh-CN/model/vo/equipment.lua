...
slot0.GetInfoTrans = function (slot0, slot1)
	...
	if slot0.type == AttributeType.CD then
		if not slot1 then
			...
			slot3 = slot0.CalcWeanponCD(slot3, slot1) .. "s" .. i18n("word_secondseach")
		end
...
function slot1(slot0)
	...
	while #slot1 > 0 do
		slot1 = {
			string.match(string.gsub(slot0, slot2 .. "x" .. slot3, slot1[1] * slot1[2], 1), "(%d+)x(%d+)")
		}
	end
...
slot0.CompareInfo = function (slot0, slot1, slot2)
	if slot0.type == AttributeType.Damage then
		slot1.compare = slot0(slot1.value) - slot0(slot0.value)
	elseif slot0.type == AttributeType.CD then
		slot1.compare = -(slot1.CalcWeanponCD(slot1.value, slot2) - slot1.CalcWeanponCD(slot0.value, slot2))
	else
		slot1.compare = slot1.value - slot0.value
	end
end

slot0.InsertAttrsUpgrade = function (slot0, slot1)
	slot0:AlignAttrs(slot1)
...
slot0.InsertAttrsCompare = function (slot0, slot1, slot2)
	...
	for slot6 = 1, #slot0, 1 do
		slot0.CompareInfo(slot0[slot6], slot1[slot6], slot2)
	end
end
...
slot0.GetWeaponInfo = function (slot0, slot1, slot2, slot3)
	...
	elseif slot1 == 6 then
		return {
			...
			value = pg.bullet_template[slot4].damage_type[1] * 100 .. "-" .. pg.bullet_template[slot4].damage_type[2] * 100 .. "-" .. pg.bullet_template[slot4].damage_type[3] * 100
		}
	elseif slot1 == 7 then
		return {
...
slot0.GetEquipAttrInfo = function (slot0, slot1, slot2)
	...
	elseif slot1 == 3 then
		return {
			...
			value = pg.bullet_template[slot2].range - pg.bullet_template[slot2].range_offset .. "-" .. pg.bullet_template[slot2].range + pg.bullet_template[slot2].range_offset
		}
	elseif slot1 == 4 then
		return {
	...
	elseif slot1 == 8 then
		return {
			...
			value = {
				pg.aircraft_template[slot0.id].max_hp,
				pg.aircraft_template[slot0.id].hp_growth
			}
		}
	elseif slot1 == 9 then
		return {
