...
slot0.addTranDrop = function (slot0)
	slot1 = {}
	...
	function slot5(slot0)
		if slot0.type == DROP_TYPE_SKIN then
			warning(slot0.number or slot0.count)
			...
			if (slot0.number or slot0.count) == 0 then
				...
				slot6, slot6.count = Player.skin2Res(slot0.id, slot1)

				return Item.New({
					type = DROP_TYPE_RESOURCE,
					id = slot2,
					count = slot3,
					name = slot4 .. "(" .. slot1[slot0.id].name .. ")"
				})
			end
		elseif slot0.type == DROP_TYPE_ICON_FRAME then
			if (slot0.number or slot0.count) == 0 then
				...
				slot6, slot6.count = Player.headFrame2Res(slot0.id, slot1)

				return Item.New({
					type = DROP_TYPE_RESOURCE,
					id = slot2,
					count = slot3,
					name = slot4 .. "(" .. slot2[slot0.id].name .. ")"
				})
			end
		...
		elseif slot0.type == DROP_TYPE_VITEM and Item.IsSkinCoupun(slot0.id) then
			...
			if not getProxy(ActivityProxy):ExistSkinCouponActivity() then
				pg.TipsMgr.GetInstance():ShowTips(i18n("coupon_timeout_tip", slot0[slot0.id].name))

				return
			elseif slot1:ExistSkinCouponActivityAndShopId(slot2) then
				pg.TipsMgr.GetInstance():ShowTips(i18n("coupon_repeat_tip", slot0[slot0.id].name))
			...
			elseif getProxy(ShipSkinProxy):hasSkin(pg.shop_template[slot2].effect_args[1]) then
				...
				return Item.New({
					...
					name = slot3 .. "(" .. slot0[slot0.id].name .. ")"
				}), Item.New({
					count = 1,
					type = slot0.type,
					id = slot0.id
