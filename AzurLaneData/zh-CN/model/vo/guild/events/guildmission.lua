...
slot0.CompleteData2FullData = function (slot0)
	return {
		...
		complete_time = pg.TimeMgr.GetInstance().GetServerTime() - 10,
		shipinevent = {},
		attr_acc_list = {},
		attr_count_list = {},
		eventnodes = {},
...
slot0.GetNextFormationTime = function (slot0)
	...
	slot5 = _.map(slot4, function (slot0)
		return slot0 + slot0 * 3600
	end)

	if _.detect(slot5, function (slot0)
		return slot0 < slot0
	end) then
		if slot1 < slot3 - 86400 + slot4[4] * 3600 then
			return pg.TimeMgr.GetInstance().GetServerTime(slot8)
		else
			return slot6
		end
	else
...
slot0.GetLogs = function (slot0)
	if slot0:GetMyFlagShip() then
		...
		return _.map(slot0.nodeLogs, function (slot0)
			return string.gsub(slot0, "$2", "<color=#92FC63FF>" .. slot0:getName() .. "</color>")
		end)
		return
	end
...
slot0.GetMyEffect = function (slot0)
	return slot0.CalcMyEffect(slot0.myShips)
end
...
slot0.CompareRecommendShip = function (slot0, slot1, slot2)
	slot4 = slot0.GetRecommendShipNation(slot0)
	slot6 = slot0.GetRecommendShipTypes(slot0)

	return slot0.SeriesSort(slot0, {
		function (slot0, slot1)
			if ((table.contains(slot0, slot0.nation) and 1) or 0) == ((table.contains(slot0, slot1.nation) and 1) or 0) then
				return -1
			else
				return slot3 < slot2
			end
		...
		function (slot0, slot1)
			if ((table.contains(slot0, slot0.type) and 1) or 0) == ((table.contains(slot0, slot1.type) and 1) or 0) then
				return -1
			else
				return slot3 < slot2
			end
...
slot0.SeriesSort = function (slot0, slot1, slot2, slot3)
	slot5 = nil

	function slot5(slot0)
		if slot0[slot0](slot0[slot0], ) == -1 then
