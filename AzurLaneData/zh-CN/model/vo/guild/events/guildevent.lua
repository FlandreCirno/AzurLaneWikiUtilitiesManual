...
slot0.Active = function (slot0, slot1)
	...
	function slot5(slot0)
		if slot0 < GuildMission.New(slot0).GetPosition(slot1) then
			slot0 = slot2
		end

		if not slot1.missions[slot2] then
			slot1.missions[slot2] = {}
		end

		if slot2[slot1.id] then
			slot1:UpdateNodeAnimFlagIndex(slot2[slot1.id])
		end

		if slot3[slot1.id] then
			slot1:UpdateFormationTime(slot3[slot1.id])
		end

		table.insert(slot1.missions[slot2], slot1)
	end
...
slot0.GetCanFormationMisstions = function (slot0)
	function slot1(slot0)
		...
		if slot0.missions[slot0:GetPosition() - 1] then
...
slot0.AnyMissionFirstFleetCanFroamtion = function (slot0)
	return _.detect(slot0:GetCanFormationMisstions(), function (slot0)
		if not slot0:FirstFleetCanFormation() and slot0:IsFinish() then
			slot1 = not slot0:IsFinishedByServer()
		end
