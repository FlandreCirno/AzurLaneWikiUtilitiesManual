...
slot0.IsSuccess = function (slot0)
	return slot0.STATE_SUCCESS < slot0.status
end
...
slot0.GetAwards = function (slot0)
	if slot0.status == slot0.STATE_SUCCESS then
		return slot0:getConfig("success_award")
	elseif slot0.status == slot0.STATE_FAILED then
		return slot0:getConfig("fail_award")
	end
end

slot0.GetLog = function (slot0)
	if slot0.status == slot0.STATE_SUCCESS or slot0.status == slot0.STATE_FAILED then
		slot2 = getDropInfo(slot1)
