...
slot0.PrivateBeFinished = function (slot0)
	if slot0.STATE_ONGOING == slot0:getState() then
		return (getProxy(TaskProxy):getTaskById(slot0:GetPresonTaskId()) or slot2:getFinishTaskById(slot1)) and getProxy(TaskProxy).getTaskById(slot0.GetPresonTaskId()) or slot2.getFinishTaskById(slot1):isFinish() and not getProxy(TaskProxy).getTaskById(slot0.GetPresonTaskId()) or slot2.getFinishTaskById(slot1):isReceive()
	end
