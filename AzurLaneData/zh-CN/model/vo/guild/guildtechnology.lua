...
slot0.GetShipAttrAddition = function (slot0, slot1, slot2)
	slot5 = slot0:getConfig("effect_args")[2]

	if slot0.getConfig("effect_args")[1] == slot1 and table.contains(slot5, slot2) then
...
slot0._ReachTargetLiveness_ = function (slot0)
	return slot0:ReachTargetLiveness(getProxy(GuildProxy):getRawData().getMemberById(slot2, slot1))
end
...
slot0.CanUpgradeBySelf = function (slot0)
	return slot0:_ReachTargetLiveness_() and slot0:GetLevel() < slot0:GetMaxLevel(), slot0._ReachTargetLiveness_(), slot0.GetLevel() < slot0.GetMaxLevel()
end

slot0.GetLivenessOffset = function (slot0)
	return slot0:GetTargetLivness() - getProxy(GuildProxy):getRawData().getMemberById(slot2, slot1):GetLiveness()
end

slot0.GetUpgradeType = function (slot0)
	if slot0:CanUpgradeBySelf() then
		return slot0.UPGRADE_TYPE_SELF
	else
		slot2 = slot0:GetMaxLevel()

		if slot0:GetLevel() < slot0.group:GetFakeLevel() then
			return slot0.UPGRADE_TYPE_PUBLIC
		end
	end
...
slot0.GetConsume = function (slot0)
	...
	if slot0:IsRiseInPrice() then
		return slot1 * slot0:getConfig("contribution_multiple"), slot2 * slot0.getConfig("contribution_multiple")
	else
		return slot1, slot2
	end
end
...
slot0.IsRiseInPrice = function (slot0)
	return slot0:GetUpgradeType() == slot0.UPGRADE_TYPE_PUBLIC
end
