...
slot0.buildText = function (slot0)
	...
	elseif slot0.eventType == GuildConst.TYPE_SUPPLY then
		if getProxy(GuildProxy):getRawData() then
			slot11, slot12 = slot4:getSupplyConsume()
	...
	elseif slot0.eventType == GuildConst.TECHNOLOGY then
		slot1 = i18n("guild_tech_log", slot2, slot0.name, pg.guild_technology_template[slot3].contribution_consume, pg.guild_technology_template[slot3].name, level)
	elseif slot0.eventType == GuildConst.TECHNOLOGY_OVER then
		slot5 = pg.guild_technology_template[slot3].contribution_consume
		slot1 = i18n("guild_tech_over_log", slot2, slot0.name, pg.guild_technology_template[slot3].name)
