...
slot0.updateExtraInfo = function (slot0, slot1)
	slot0.capital = slot1.guild_ex or {}.capital or 0

	slot0:updateWeeklyTask(GuildTask.New(slot1.guild_ex or .this_weekly_tasks or {}))

	slot0.benefitFinishTime = slot1.guild_ex or .benefit_finish_time or 0
	slot0.lastBenefitFinishTime = slot1.guild_ex or .last_benefit_finish_time or 0
	slot0.technologyGroups = {}

	for slot7, slot8 in pairs(slot0.get_id_list_by_group) do
		slot0.technologyGroups[GuildTechnologyGroup.New({
			id = slot7
		}).id] = GuildTechnologyGroup.New()
	end
	...
	for slot7, slot8 in slot4(slot5) do
		slot0.technologyGroups[slot0[slot8.id].group]:update(slot8)
	end
...
slot0.updateUserInfo = function (slot0, slot1)
	slot0.donateCount = slot1.user_info or {}.donate_count or 0
	slot0.benefitTime = (slot1.user_info or .benefit_time and slot1.user_info or .benefit_time > 0 and slot1.user_info or .benefit_time) or 0
	slot0.weeklyTaskFlag = slot1.user_info or .weekly_task_flag or 0
	...
	for slot6, slot7 in pairs(slot0.get_id_list_by_group) do
		slot0.technologys[slot6] = GuildTechnology.New(slot8)
	end
	...
	for slot6, slot7 in slot3(slot4) do
		slot0.technologys[slot0[slot7].group]:Update(slot7, slot0.technologyGroups[slot0[slot7].group])
	end
...
slot0.getSupplyCnt = function (slot0)
	...
	slot1 = ((slot0.benefitTime ~= 0 and slot0.benefitTime >= slot0:getSupplyStartTime()) or math.ceil((slot2 - slot0.getSupplyStartTime()) / 86400)) and math.floor(math.max(0, slot2 - slot0.benefitTime) / 86400)
	slot5 = slot0:getMemberById(getProxy(PlayerProxy):getRawData().id).GetJoinZeroTime(slot4)
...
slot0.getMaxMember = function (slot0)
	return pg.guild_data_level[math.min(slot0.level, slot2)].member_num + slot0:GetGuildMemberCntAddition() + (slot0.maxMemberCntAddition or 0)
end
...
slot0.ShouldShowSupplyTip = function (slot0)
	return slot0:isOpenedSupply() and slot0.getSupplyCnt(slot0) > 0 and function ()
		return not slot0:getMemberById(slot0):IsRecruit() and not slot1:isNewMember()
	end()
end
...
slot0.GetMemberShips = function (slot0, slot1)
	...
	for slot9, slot10 in pairs(slot0.member) do
		slot10.GetShip(slot10).isCommander = slot10:IsCommander()

		if slot10.IsCommander() or slot5(slot10) then
...
slot0.GetMissionAndAssultFleetShips = function (slot0)
	...
	slot6 = slot0.member[getProxy(PlayerProxy):getRawData().id].GetExternalAssaultFleet(slot4)
