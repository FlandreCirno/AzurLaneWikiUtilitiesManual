...
slot0.canPurchase = function (slot0)
	...
	elseif slot0.type == Goods.TYPE_GIFT_PACKAGE or slot0.type == Goods.TYPE_SKIN or slot0.type == Goods.TYPE_WORLD then
		...
		return slot0.super.canPurchase(slot0)
	end
end

slot0.isDisCount = function (slot0)
	slot1 = slot0.InCommodityDiscountTime(slot0.id)
...
slot0.IsShowWhenGroupSale = function (slot0, slot1)
	if slot0:IsGroupSale() then
		slot3 = slot0:getConfig("limit_args2")[1][2]

		if slot1 == slot0.getConfig("limit_args2")[1][3] and slot4 == slot0:getConfig("group_limit") then
			return true
		end
...
slot0.GetPrice = function (slot0)
	...
	if slot0:isDisCount() and slot0:IsItemDiscountType() then
		slot1 = (slot2 - pg.shop_discount_coupon_template[slot0.id].discounted_price) / slot2 * 100
		slot2 = pg.shop_discount_coupon_template[slot0.id].discounted_price
	elseif slot3 then
...
slot0.IsItemDiscountType = function (slot0)
	return slot0.getConfig(slot0, "genre") == ShopArgs.SkinShop and pg.shop_discount_coupon_template[slot0.id] ~= nil and function ()
		return getProxy(ActivityProxy):ExistSkinCouponActivityAndShopId(slot0.id)
	end()
end
